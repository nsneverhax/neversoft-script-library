...
script destroy_global_peds_in_zone 
	if NOT ($disable_global_pedestrians = 1)
		if NOT innetgame
			if iscoiminited
				printf "destroy_global_peds_in_zone called - %a - %b" a = <nodearray> b = <zone>
				formattext textname = path 'zones/%a/%b.qb' a = <nodearray> b = <nodearray>
				if gotparam context
					pushassetcontext context = <context>
				endif
				cleanupglobalpednodearray name = <path> zone = <zone>
				if gotparam context
					popassetcontext
				endif
			endif
...
script spawn_global_peds_in_zone 
	if NOT ($disable_global_pedestrians = 1)
		if ($video_editor_in_video_editor = 0)
			if NOT innetgame
				if iscoiminited
					printf "spawn_global_peds_in_zone called - %a - %b" a = <nodearray> b = <zone>
					formattext textname = path 'zones/%a/%b.qb' a = <nodearray> b = <nodearray>
					if gotparam context
						pushassetcontext context = <context>
					endif
					parseglobalpednodearray name = <path> zone = <zone>
					if gotparam context
						popassetcontext
					endif
				endif
