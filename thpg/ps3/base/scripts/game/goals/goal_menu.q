...
script goal_generic_view_goal_pausemenu z_priority = 0 parent = root_window skate9_arcade_hack = 0
	requireparams [goal] all
	if NOT goal_get_title goal = <goal>
		<view_goals_title> = "GOAL STATUS"
	endif
	if goal_get_descrip_list goal = <goal>
		<view_goals_dialog> = ""
	elseif NOT goal_get_dialog goal = <goal>
		<view_goals_dialog> = ""
	endif
...
script goal_generic_view_goal_pausemenu_display z_priority = 0
	requireparams [title dialog_text am_text pro_text sick_text] all
	...
	goal_intro_screen_grade_descrip {
		parent = goal_view_goal_anchor
		grade = 2
		pos = ((-310.0, 0.0) + <descrip_height> + (0.0, 1.0) * <text_height>)
		text = <pro_text>
		best_grade = <grade>
		z_priority = (<z_priority> + 10)
		<arcade>
...
script goal_get_title 
	...
	if gman_getconstant goal = <goal> name = view_goals_title
		return true title = <view_goals_title> view_goals_title = <view_goals_title>
	endif
	return \{false}
endscript

script goal_get_dialog 
	requireparams \{[
			goal
		]
		all}
	...
	if goal_get_checkpoint_ui goal = <goal> checkpoint = <checkpoint>
		if structurecontains structure = <checkpoint_ui> view_goals_dialog
			return true view_goals_dialog = (<checkpoint_ui>.view_goals_dialog)
		endif
	endif
	if gman_getconstant goal = <goal> name = view_goals_dialog
		return true view_goals_dialog = <view_goals_dialog>
	endif
	return \{false}
endscript
...
script goal_generic_exit_menu do_pause = 1
	...
	wait 2 gameframes
	repeat
	if NOT goal_is_permanent goal_id = <goal>
...
script goal_failed_retry_options txt = "What do you want to do?"
	requireparams [goal] all
	...
	create_dialog_box {
		...
		buttons = [
			...
			{font = text_a1 text = "QUIT" pad_choose_script = goal_failed_quit pad_choose_params = {goal = <goal>}}
		]
	}
	ui_kill_message_hack id = ui_alert_uber
	startrendering
...
script goal_failed_retry_current 
	...
	unpausegame
	goal_retry_current_goal \{retry_from_fail = true}
endscript
...
script goal_show_descrip_list parent = root_window animate = 0
	printf '-- goal_show_descrip_list'
	if NOT gotparam goal
		if NOT gman_getactivatedgoalid
			softassert "no goal specified and no activated goal"
			return
		endif
	...
	if NOT gotparam list
		if NOT goal_get_descrip_list goal = <goal>
			softassert "no view_goals_descrip_list for checkpoint %c" c = <checkpoint>
			return
		endif
	...
	repeat
	setscreenelementlock id = view_goals_descrip_list_anchor on
	setscreenelementlock id = view_goals_descrip_list_anchor off
	if (<animate> = 1)
...
script goal_show_list_item has_rgba = [128 128 128 255] not_rgba = [200 200 200 255]
...
script goal_cross_out_list_item 
	...
	if gman_flagfunc func = was_set_this_try goal = <goal> checkpoint = <checkpoint> tool = <flag_tool> params = {flag = <flag>}
		...
		if gotparam resolved_id
			...
			<strike_id> = <id>
			<strike_id> :domorph scale = (<width> * (0.34, 0.0) + (0.0, 2.0)) time = 0.25
			gman_getgoaltype goal = <goal>
			if checksumequals a = <goal_type> b = classic
				...
				if screenelementexists id = <reward_id>
					<reward_id> :domorph scale = 1.4 time = 0.25
				endif
				if screenelementexists id = <reward_id>
					<reward_id> :domorph scale = 0.9 time = 0.1
				endif
				if screenelementexists id = <reward_id>
