...
script create_gig_posters_window \{created_gignum = 1}
	...
	if (<created_gignum> > 9)
		formatText checksumName = texture_id 'tex/zones/Z_Board_Room/Gig_Tex_Board_Room_Large_%d_.png' d = <created_gignum>
		formatText checksumName = posternamelarge 'Z_Board_Room_G_RM_Gig_poster_%d' d = <created_gignum>
	else
		formatText checksumName = texture_id 'tex/zones/Z_Board_Room/Gig_Tex_Board_Room_Large_0%d_.png' d = <created_gignum>
...
script show_setlist_on_gigboard progression_flag = ($current_progression_flag)
	...
	get_gigboard_section
	if (<section> = 1)
		texture_id = 0x9e117420
	elseif (<section> = 5)
		texture_id = 0xc1f6e45e
	else
		formatText checksumName = texture_id 'tex/zones/Z_Board_Room/RM_Board_Room_setlist0%d.png' d = (<section> - 1)
	endif
	destroy_gig_band_money_display
	create_viewport_ui {
	...
	if (<section> = 1)
		pos_offset = (-495.0, 0.0)
	elseif (<section> = 2)
		pos_offset = (-520.0, 10.0)
	elseif (<section> = 3)
		pos_offset = (-520.0, 60.0)
	elseif (<section> = 4)
		pos_offset = (-520.0, 10.0)
	else
		pos_offset = (-500.0, 10.0)
	endif
	...
	if (<poster_gig_num> = 13)
		Scale = (1.5, 0.98499995)
		pos_offset = (-510.0, 10.0)
	else
		Scale = (1.5, 1.0)
	endif
	CreateScreenElement {
	...
	if StructureContains structure = ($<tier_global>.<tiername>) createagig
		...
		if (<show_song> = 1)
			get_song_title song = <song>
			0x92cf1447 song = <song>
			0x0bc645fd song = <song>
			if (<0xcb78de9e> != qs(0x03ac90f0))
				<song_title> = <0xcb78de9e>
			endif
			<completed> = 0
			CreateScreenElement {
				Type = descinterface
			...
			repeat 5
			if (<0x52718f24> != qs(0x03ac90f0))
				CreateScreenElement {
					Type = descinterface
					parent = gigboard_songlist_menu
					desc = 'gig_board_setlistB_item'
					autosizedims = true
					gig_item_song_text = <0x52718f24>
				}
				if (<completed> = 1)
					<id> :se_setprops gig_item_song_rgba = [192 , 255 , 192 , 255]
				endif
			endif
...
script replace_texture_on_gig_posters progression_flag = ($current_progression_flag)
	...
	if StructureContains structure = ($<tier_global>) <tiername>
		if StructureContains structure = ($<tier_global>.<tiername>) 0x946acc2e
			0x946acc2e = ($<tier_global>.<tiername>.0x946acc2e) AddToStringLookup = true

			if (<gig_num> > 9)
				formatText checksumName = difftexname 'tex\\zones\\Z_Board_Room\\Gig_poster_small_%d.png' d = <gig_num> AddToStringLookup = true
				formatText checksumName = 0x77e185dc 'images\\menuScreens\\gigposters/%d_small.png' d = <0x946acc2e> AddToStringLookup = true
			else
				formatText checksumName = difftexname 'tex\\zones\\Z_Board_Room\\Gig_poster_small_0%d.png' d = <gig_num> AddToStringLookup = true
				formatText checksumName = 0x77e185dc 'images\\menuScreens\\gigposters/%d_small.png' d = <0x946acc2e> AddToStringLookup = true
			endif
