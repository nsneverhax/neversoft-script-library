...
script show_change_bpm 
	...
	jam_studio_element :SetProps bpm_number_text = <curr_bpm_text>
	if 0x1396dc9b <choose_button> <controller>
		break
	endif
...
script jam_highway_loop 
	...
	if ArrayContains array = <loop_controls> contains = cancel
		if 0x1396dc9b <break_button> <controller>
	...
	if ArrayContains array = <loop_controls> contains = clear_loop
		if ($jam_loop_bound_low > -1 && $jam_loop_bound_high > -1)
			if 0x1396dc9b <clear_button> <controller>
				<loop_bound_low> = -1
				<loop_bound_high> = -1
				break
			endif
		...
		if ($blade_active = 0)
			if GuitarGetAnalogueInfo controller = <controller>
				if (<rightx> > 0.3)
					if (<whammy_hold_count> > 3)
						...
						if ScreenElementExists \{id = jam_studio_element}
							if NOT ScriptIsRunning \{show_warning_message}
								jam_studio_element :SetProps \{dialog_text = $0xd91222ef}
							endif
						endif
					endif
					...
					if ScreenElementExists \{id = jam_studio_element}
						if NOT ScriptIsRunning \{show_warning_message}
							jam_studio_element :SetProps \{dialog_text = $0xf97267ce}
						endif
					endif
				endif
			endif
...
script show_jam_helper_box \{title = $0x6d24fcb2
		text = qs(0x5449d8c0)}
	if ScreenElementExists \{id = jam_studio_element}
...
script jam_highway_delete_section 
	...
	if ArrayContains array = <delete_controls> contains = delete_toggle
		if 0x1396dc9b <delete_button> <controller>
			ui_menu_select_sfx
			clean_up_user_control_helpers
			if (<delete_all> = 1)
				<delete_all> = 0
...
script jam_highway_copy 
	...
	if ArrayContains array = <copy_controls> contains = copy_toggle
		if 0x1396dc9b <copy_button> <controller>
	...
	if 0x1396dc9b <loop_copy_button> <controller>
		if ($jam_loop_bound_low > -1 && $jam_loop_bound_high > -1)
...
script jam_highway_paste_control 
	...
	if ArrayContains array = <paste_controls> contains = paste_multi
		if 0x1396dc9b <paste_multiple_button> <controller>
			SoundEvent \{event = ghmix_select}
			show_paste_multiple
			return
		endif
...
script jam_highway_note_nudge 
	...
	if NOT ($jam_current_quantize = 0)
		...
		if ArrayContains array = <nudge_controls> contains = nudge_all
			if 0x1396dc9b <nudge_all_button> <controller>
				SoundEvent \{event = ghmix_scroll_up_down}
...
script jam_highway_select_quantize 
	...
	if NOT (($is_arpeggiator [$jam_current_track] = 1 || $is_drum_machine = 1) && ($jam_highway_recording = 1 && $jam_highway_step_recording != 1))
		if (<disallow_snap> = 0)
			...
			if NOT controllerhasdpad controller = <controller>
				if ControllerPressed select <controller>
					if guitarcontrollermake up <controller>
						if (<lefty> = 1)
							change_quantize_right
						else
							change_quantize_left
			...
			if NOT controllerhasdpad controller = <controller>
				if ControllerPressed select <controller>
					if guitarcontrollermake down <controller>
						if (<lefty> = 1)
							change_quantize_left
						else
							change_quantize_right
	...
	switch <state>
		...
		if NOT controllerhasdpad controller = ($primary_controller)
			if NOT ($game_mode = training)
				add_user_control_helper \{text = $0x63f12611
					button = back
					z = 100}
			endif
		endif
		...
		add_user_control_helper \{text = qs(0xf7723015)
			...
			z = 100}
		case nudge
		add_user_control_helper \{text = qs(0x1c84e3f9)
			button = green
			z = 100}
		add_user_control_helper \{text = qs(0x3fc1c076)
			button = red
			z = 100}
		add_user_control_helper \{text = qs(0x5d45f14b)
			button = yellow
			z = 100}
		default
		if ScreenElementExists \{id = jam_studio_element}
...
script jam_studio_tilt_meter 
	...
	if NOT (<last_tilt> = ($<tilt_var>))
		...
		if ((<chromatic> = 0) || <jam_instrument> = 0)
			switch (<last_tilt>)
				case 0
				legacydoscreenelementmorph id = <pitch_indicator> time = <time> rot_angle = -20
				case 1
				legacydoscreenelementmorph id = <pitch_indicator> time = <time> rot_angle = 20
			endswitch
		else
			switch (<last_tilt>)
				case 0
				legacydoscreenelementmorph id = <pitch_indicator> time = <time> rot_angle = -45
				case 1
				legacydoscreenelementmorph id = <pitch_indicator> time = <time> rot_angle = -15
				case 2
				legacydoscreenelementmorph id = <pitch_indicator> time = <time> rot_angle = 15
				case 3
				legacydoscreenelementmorph id = <pitch_indicator> time = <time> rot_angle = 45
...
script jam_update_curr_note_text 
	...
	switch <jam_instrument>
		...
		case 4
		0xb12c0fe3 = (<note_fret> + <velocity>)
		curr_octave = (<0xb12c0fe3> / 12)
		CastToInteger \{curr_octave}
		formatText TextName = curr_note_text qs(0x78f345f4) s = ($jam_note_text_string_octave) a = <note_text> b = (<curr_octave> + 1)
		case 3
		if (<note_type> = 0)
