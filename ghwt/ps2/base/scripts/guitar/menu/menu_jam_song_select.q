		...
		if (<user_song_count> < ($jam_max_user_songs))
			if (<advanced_record> = 0)
				add_generic_menu_icon_item \{icon = icon_jam_create_new
					text = qs(0x1bca1511)
					pad_choose_script = 0x487fef17
					pad_choose_params = {
						state = uistate_jam_song_wizard
					}
				...
				add_generic_menu_icon_item {
					...
					text = qs(0x1bca1511)
					pad_choose_script = 0x487fef17
					pad_choose_params = {state = uistate_recording {back_to_jam_band = 0 current_instrument = 0 Player = ($jam_current_recording_player)}}
					additional_focus_script = menu_jam_create_song_focus
					additional_unfocus_script = menu_jam_create_song_unfocus
				}
			endif
...
script delete_jam_song \{advanced_record = 0}
	...
	Change memcard_jamsession_ps2_file_name = ($jam_curr_directory_listing [<index>].actual_file_name)
...
script jam_start_song_from_quickplay 
	...
	if (<example_song> = 0)
		Change memcard_jamsession_file_name = <FileName>
		Change memcard_jamsession_ps2_file_name = ($jam_curr_directory_listing [<jam_directory_index>].actual_file_name)
...
script jam_quickplay_get_and_start_song 
	...
	jam_start_song_from_quickplay FileName = <FileName> example_song = <example_song> jam_directory_index = <jam_directory_index>
endscript
