...
script create_jam_menu 
	...
	make_generic_menu \{title = qs(0x67c266fb)
		...
		vmenu_id = jam_menu
		dims = (400.0, 400.0)}
	jam_menu :obj_spawnscript \{jam_recording_check_disconnect}
...
script 0x90c04eda 
	...
	if (($primary_controller) >= 0 && ($primary_controller) < <array_Size>)
		if (<active_controllers> [($primary_controller)] = 1)
			state = connected
		else
			state = disconnected
		endif
