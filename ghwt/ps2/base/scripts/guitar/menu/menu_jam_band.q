...
script menu_jam_band_add_control_helpers \{state = NULL}
	...
	switch <state>
		...
		if GotParam \{instrument}
			if NOT ((<instrument> = 2) || (<instrument> = 3) || (<instrument> = 4))
				add_user_control_helper \{text = qs(0x9feadc54)
					button = back
					z = 100}
			endif
		endif
...
script jam_poll_for_controller_changes 
	active_controllers = [0 0 0 0 0 0 0 0]
	prev_active_controllers = [0 0 0 0 0 0 0 0]
	prev_active_mic = [0 0 0 0 0 0 0 0]
	prev_controller_types = [None None None None None None None None]
	0xac50515d = [-1 -1 -1 -1]
	0x3e868198 = [0 0 0 0 0 0 0 0]
	begin
	active_controllers = [0 0 0 0 0 0 0 0]
	...
	pi = (<player_num> - 1)
	if (<player_num> = 4)
		if ($primary_controller >= 0 && $primary_controller < 8)
			if (<0x3e868198> [$primary_controller] = 0)
				count = ($primary_controller)
			endif
		endif
	endif
	if (<count> = -1)
		if (<0xac50515d> [<pi>] != -1)
			...
			GetArraySize <0x3e868198>
			i = 0
			begin
			if (<active_controllers> [<i>] = 1)
				if (<0x3e868198> [<i>] = 0)
					SetArrayElement ArrayName = 0x3e868198 index = <i> NewValue = <player_num>
					SetArrayElement ArrayName = 0xac50515d index = (<player_num> - 1) NewValue = <i>
					count = <i>
					break
				endif
			endif
	...
	if NOT (<is_active_controller> = (<prev_active_controllers> [<count>]))
		...
		elseif (<is_active_controller> = 0)
			...
			if (<is_active_controller> = 0)
				...
				jam_destroy_player Player = <player_num>
				setplayerinfo <player_num> jam_instrument = -1
				SetArrayElement ArrayName = 0x3e868198 index = <count> NewValue = 0
				SetArrayElement ArrayName = 0xac50515d index = <pi> NewValue = -1
...
script jam_band_create_highway 
	...
	if (<Mod> = 0)
		...
		CreateScreenElement {
			...
			rgba = [80 80 80 255]
			Pos = <bar_pos>
			dims = ((2.0, 0.0) + ((0.0, 1.0) * <jam_band_highway_height>))
...
script jam_create_guitar_type_select_menu 
	...
	switch <select_instrument>
		...
		machine_text = $0x6024e2ce
		case 4
		type_text = qs(0x58ce03d1)
		machine_text = $0xb7c2a351
	endswitch
...
script jam_band_pause \{back_to_jam_band = 1
	...
	if NOT controllerhasdpad controller = <controller>
		<menu_pos> = (2.0, 120.0)
	endif
	new_menu {
	...
	elseif GotParam \{paused_at_inst}
		...
		option1_script = 0x2d7bea19
		option2_text = qs(0x00000000)
		option2_script = 0x2d7bea19
		if NOT controllerhasdpad controller = <controller>
			if ($jam_highway_playing = 0)
				option1_text = qs(0xcdaa4ea8)
				option1_script = 0xcaa42215
				option2_text = qs(0x04018eac)
				option2_script = 0x75ccbb3b
			else
				if ($jam_band_recording = 1)
					option1_text = qs(0xcdaa4ea8)
					option1_script = 0xcaa42215
					...
					option1_text = qs(0x31029e1f)
					option1_script = 0xb5331962
					option2_text = qs(0x9dbd05c7)
					option2_script = 0x2d7bea19
				endif
			endif
		endif
		pause_options = [
			...
			{
				name_text = <option1_text>
				fit_to_dims = (150.0, 25.0)
				pause_script = <option1_script>
				allow_inst = [1 1 1 1 1 1]
				0xd8978923
			}
			{
				name_text = <option2_text>
				fit_to_dims (150.0, 25.0)
				pause_script = <option2_script>
				allow_inst = [1 1 1 1 1 1]
				0xd8978923
			}
			{
				name_text = qs(0x30a2dabf)
				fit_to_dims (150.0, 25.0)
				pause_script = 0x7af8e675
				allow_inst = [1 1 1 1 1 1]
				0xd8978923
			}
			{
	...
	if StructureContains structure = (<pause_options> [<option>]) is_toggle
		if (<jam_instrument> = 3)
			...
			switch <jam_instrument>
				...
				case 2
				machine_text = $0x0d590451
				case 4
				machine_text = $0x4191898e
			endswitch
			if (($is_arpeggiator [<jam_instrument>]) = 1)
	...
	if StructureContains structure = (<pause_options> [<option>]) 0xd8978923
		if controllerhasdpad controller = <controller>
			<show_option> = 0
		endif
	endif
	if StructureContains structure = (<pause_options> [<option>]) 0xa1f46fd7
		if ($jam_highway_playing = 1)
			<show_option> = 0
		endif
	endif
	if StructureContains structure = (<pause_options> [<option>]) 0x3a6694c8
		if ($jam_highway_recording = 1)
			<show_option> = 0
		endif
	endif
...
script line6_pod_effect_change 
	...
	if GotParam \{up}
		...
		if (<jam_instrument> = 0)
			SetArrayElement ArrayName = jam_current_instrument_effects globalarray index = 1 NewValue = <effect>
		endif
		if (<jam_instrument> = 1)
			SetArrayElement ArrayName = jam_current_instrument_effects globalarray index = 0 NewValue = <effect>
		endif
...
script line6_pod_back 
	...
	if (<jam_instrument> = 0)
		SetArrayElement ArrayName = jam_current_instrument_effects globalarray index = 1 NewValue = <previous_effect>
	endif
	if (<jam_instrument> = 1)
		SetArrayElement ArrayName = jam_current_instrument_effects globalarray index = 0 NewValue = <previous_effect>
	endif
	line6_pod_remove <...>
...
script guitar_jam_settings_preview_effect 
	...
	Wait \{1
		gameframe}
	0xa372e023 = $0x355c9929
	Change \{0x355c9929 = $lead_jam_set}
	jam_get_sample_checksum \{fret = 0
		string = 0
		Type = 0
		jam_instrument = 1}
	Change 0x355c9929 = <0xa372e023>
	PlaySound <sample_checksum> pitch = (<pitch_shift> + (($jam_current_tuning) / 10.0)) vol = -6 buss = jammode_leadguitar
...
pause_melody_kit_options = [
	...
	{
		name_text = qs(0x4b898c84)
		fit_to_dims = (130.0, 25.0)
		submenu_script = guitar_jam_change_melody_kit
		submenu_exit_script = guitar_jam_melody_kit_exit
		pakname = 'bit16'
		sample_start = 0xc61ec6e1
	}
	{
...
script guitar_jam_settings_update_click_text 
	...
	if ScreenElementExists id = <option_text_id>
		<option_text_id> :se_setprops text = <click_text> dims = (190.0, 34.0)
	endif
endscript
...
script guitar_jam_settings_update_menu_sounds_text 
	...
	if GotParam \{option_text_id}
		<option_text_id> :SetProps text = <flip_text> dims = (190.0, 34.0)
	endif
endscript
...
script guitar_jam_settings_toggle_lefty 
	...
	if (<lefty_flip> = 0)
		<lefty_flip> = 1
		flip_text = $0xbf3345fc
	else
		<lefty_flip> = 0
		flip_text = $0xd8bcc317
	endif
	if GotParam \{option_text_id}
		<option_text_id> :SetProps text = <flip_text> dims = (190.0, 34.0)
	endif
...
script guitar_jam_settings_preview_note 
	...
	switch <jam_instrument>
		...
		jam_get_sample_checksum \{fret = 0
			...
			Type = 0
			jam_instrument = 0}
		PlaySound <sample_checksum> pitch = (<pitch_shift> + (($jam_current_tuning) / 10.0)) vol_percent = (50.0 * ($0xd0f42bdf / 100.0)) buss = jammode_rhythmguitar
		case 1
		jam_get_sample_checksum \{fret = 0
			string = 0
			Type = 0
			jam_instrument = 1}
		PlaySound <sample_checksum> pitch = (<pitch_shift> + (($jam_current_tuning) / 10.0)) vol_percent = (50.0 * ($0xb483d0a1 / 100.0)) buss = jammode_leadguitar
		case 2
		jam_get_sample_checksum \{fret = 0
			string = 0
			Type = 0
			jam_instrument = 2}
		PlaySound <sample_checksum> pitch = (<pitch_shift> + (($jam_current_tuning) / 10.0)) vol_percent = (100.0 * ($0x1b560918 / 100.0)) buss = jammode_bass
		case 3
		...
		jam_get_single_sample_for_melody_playback \{fret = 0
			...
			Type = 0
			jam_instrument = 4}
		PlaySound <sample_checksum> pitch = (<pitch_shift> + (($jam_current_tuning) / 10.0)) vol_percent = (50.0 * ($0xaf59d0a1 / 100.0)) buss = jammode_vox
	endswitch
	if NOT (<jam_instrument> = 3)
...
script jam_band_pause_save_as_text_select 
	...
	if (<user_song_count> < ($jam_max_user_songs))
		...
		if (<from_adv> = 1)
			0x57501c34 = uistate_recording
		else
			0x57501c34 = uistate_jam_band
		endif
...
script jam_band_pause_save_as \{0x57501c34 = uistate_jam_band}
	Change \{memcard_jamsession_ps2_file_name = ''}
	...
	ui_memcard_save_jam event = menu_back data = {state = <0x57501c34> editing = 1 show_popup = 0}
endscript
...
script jam_band_save_dialog \{dialog = 0}
	...
	create_popup_warning_menu {
		...
		options = [
			...
			{
				func = jam_band_pause_save_as_text_select
...
script jam_create_extra_info \{Player = 1}
	...
	if GotParam \{arpeggiator}
		...
		info1_text = $0x676039aa
		info2_text = qs(0xda851ce1)
		info3_text = $0x7ef73c4a
	...
	if GotParam \{drum_machine}
		...
		info1_text = $0x676039aa
		info2_text = $0x7144d5ea
		info3_text = $0x7ef73c4a
		info4_text = $0x094678ac
	endif
