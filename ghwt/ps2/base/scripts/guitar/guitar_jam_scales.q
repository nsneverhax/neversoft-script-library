...
script jam_scales_audible_scale_preview 
	...
	if (<jam_instrument> = 0)
		...
		Wait \{1
			gameframe}
		PlaySound <final_note_sample> pitch = (<pitch_shift> + (($jam_current_tuning) / 10.0)) vol = -6 buss = jammode_rhythmguitar
	...
	elseif (<jam_instrument> = 1)
		...
		Wait \{1
			gameframe}
		PlaySound <final_note_sample> pitch = (<pitch_shift> + (($jam_current_tuning) / 10.0)) vol = -6 buss = jammode_leadguitar
	...
	elseif (<jam_instrument> = 4)
		...
		Wait \{1
			gameframe}
		PlaySound <final_note_sample> pitch = (<pitch_shift> + (($jam_current_tuning) / 10.0)) vol = -6 buss = jammode_vox
...
script jam_scales_audible_note_preview 
	...
	if (<jam_instrument> = 0)
		...
		Wait \{1
			gameframe}
		PlaySound <sample_checksum> pitch = (<pitch_shift> + (($jam_current_tuning) / 10.0)) vol = -6 buss = jammode_rhythmguitar
		...
		switch (<jam_instrument>)
			...
			case 4
			PlaySound <sample_checksum> pitch = (<pitch_shift> + (($jam_current_tuning) / 10.0)) vol = -6 buss = jammode_vox
