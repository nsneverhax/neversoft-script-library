...
script create_leaderboard_groups_menu \{menu_title = 'LEADERBOARDS'
	...
	elseif (isngc)
		<global_list_single_player> = 0xa135a1cd
		<global_list_multi_player> = 0x7c598c0d
	else
	...
	createscreenelement {
		...
		text = <menu_subtitle>
		just = [center top]
		pos = (($online_general_menu_pos) + (0.0, 105.0))
		z_priority = 4.0
	}
	fit_text_into_menu_item id = <id> max_width = 223
...
script leaderboard_group_add_item {
	...
	setscreenelementprops {
		id = <id>
		pos = (((<menu_width> - <text_width>) / 2.0) * (1.0, 0.0) + (0.0, -2.0))
	}
	padding = 10
	setscreenelementprops {
		id = <cont_id>
	...
	setscreenelementprops {
		id = <id>
		pos = (<bar_pos> + (1.0, 0.0) * <menu_width> - (2.0, 0.0))
	}
	...
	if gotparam \{leaderboard_data}
		...
		if screenelementexists id = <leaderboard_difficulty>
			if NOT isngc
				...
				diff = 'E'
				if ((<leaderboard_data> [<array_count>].data [3]) = 'easy')
					diff = 'E'
				elseif ((<leaderboard_data> [<array_count>].data [3]) = 'medium')
					diff = 'M'
				elseif ((<leaderboard_data> [<array_count>].data [3]) = 'hard')
					diff = 'H'
				elseif ((<leaderboard_data> [<array_count>].data [3]) = 'expert')
					diff = 'Ex'
				endif
				formattext textname = difficulty '(%a)' a = <diff>
		...
		if (<array_count> < 10)
			...
			<leaderboard_entry> :setprops block_events
