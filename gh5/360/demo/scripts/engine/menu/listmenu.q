...
script listmenu_physics_elements 
	requireparams \{[
		...
		all}
	create_2d_spring_system \{desc_id = generic_list_menu
		num_springs = 2
		stiffness = 75
		rest_length = 1}
	if generic_list_menu :desc_resolvealias \{name = alias_skull_add_container
			param = skull_add_id}
	...
	if generic_list_menu :desc_resolvealias \{name = alias_skull_add_container
			param = skull_add_id}
		<skull_add_id> :obj_spawnscript ui_alphaflight
	else
		scriptassert \{'pdetrich'}
	endif
endscript
