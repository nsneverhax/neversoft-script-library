...
script persistent_band_create_band 
	...
	if gotparam \{startup}
		if NOT demobuild
			...
			if (<part> = vocals)
				persistent_band_prevent_female_vocalist savegame = <savegame> index = <i> player = <player>
			endif
			i = (<i> + 1)
			repeat 4
		endif
...
script persistent_band_create_character \{async = 0}
	...
	if demobuild
		part = (($part_priority) [<index>])
		setplayerinfo <player> part = <part>
		removeparameter \{character_id}
	endif
