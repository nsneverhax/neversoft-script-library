		...
		elseif ($competitive_rules = p8_pro_faceoff)
			if (<num_players> > 0)
				if (<num_guitars> <= 2 && <num_drums> <= 1 && <num_vocals> <= 1)
					...
					SendStructure \{callback = band_lobby_update_ticker
						data_to_send = {
							msg_checksum = incorrect_instrument
						}}
					band_lobby_update_ticker \{msg_checksum = incorrect_instrument}
				endif
			endif
		else
...
script select_quit_network_game 
	...
	if ($net_popup_active = 0)
		...
		if GotParam \{from_fail_menu}
			...
			generic_event_choose no_sound state = uistate_online_quit_warning data = {is_popup player_device = <device_num>}
		endif
	endif
endscript
			...
			if playerinfoequals <Player> controller = <controller_index>
				if (<Type> = freeplay)
					if NOT playerinfoequals <Player> freeplay_state = dropped
						return player_num = <Player>
					elseif GotParam \{allow_dropped_player}
						return player_num = <Player>
					endif
				else
					return player_num = <Player>
