0x87cd730b = [
	{
		desc_id = metallicablackpbass
	}
	{
		desc_id = 0xc55d60a4
	}
]
esp_jh_black_truckster = {
	genre = [
		rock
		punk
		`classic	rock`
		goth
		`black	metal`
		`heavy	metal`
		`glam	rock`
		pop
	]
	preset_name = qs(0x4047e65e)
	is_metallica = 1
	cas_guitar_body = {
		desc_id = gtr31_horizon
	}
	cas_guitar_finish = {
		desc_id = gtr33_flamed
	}
	cas_guitar_neck = {
		desc_id = gtr_24fret_fretboard_03
	}
	cas_guitar_head = {
		desc_id = gtr31c_neckhead_02
	}
	cas_guitar_head_finish = {
		desc_id = espcolor
		chosen_materials = {
			material1 = black_1guitar
		}
	}
	cas_guitar_pickguards = {
		desc_id = None
	}
	cas_guitar_bridges = {
		desc_id = gtr31_bridge_fr
	}
	cas_guitar_knobs = {
		desc_id = gtr_32_knobs_b
	}
	cas_guitar_pickups = {
		desc_id = gtr31_pickup_gold
	}
}
cas_preset_metallica_guitars = [
	{
		desc_id = gtr2_esp_truckster_02
		$esp_jh_black_truckster
	}
]
metallicablackpbass = {
	genre = [
		rock
		punk
		`classic	rock`
		goth
		`black	metal`
		`heavy	metal`
		`glam	rock`
		pop
	]
	preset_name = qs(0x4047e65e)
	is_metallica = 1
	cas_bass_body = {
		desc_id = bass17_esp_ec_01
	}
	cas_bass_finish = {
		desc_id = bass17_esp_ec_ld_12
		chosen_materials = {
			material1 = yellow_1
		}
	}
	cas_bass_neck = {
		desc_id = bass_fretboard_24black
	}
	cas_bass_neck_finish = {
		desc_id = bass_fretboard_inlay_01
	}
	cas_bass_head = {
		desc_id = bass14_neckhead_04
	}
	cas_bass_head_finish = {
		desc_id = esp_solidcolor
		chosen_materials = {
			material1 = black_1guitar
		}
	}
	cas_bass_pickguards = {
		desc_id = None
	}
	cas_bass_pickups = {
		desc_id = bass9_pkup_type3_blk_2
		chosen_materials = {
			material1 = grey_2guitar
		}
	}
	cas_bass_bridges = {
		desc_id = bass17_bridge_4str_04
	}
	cas_bass_knobs = {
		desc_id = bass17_esp_knob_10
		chosen_materials = {
			material1 = grey_3guitar
		}
	}
}
cas_preset_metallica_basses = [
	{
		desc_id = bass16_esp_surveyor_02
		$metallicablackpbass
	}
]
cas_preset_guitars = [
	{
		cas_guitar_body = {
			desc_id = gtr9_esp_ax
		}
		cas_guitar_finish = {
			desc_id = gtr9_esp_eclipse_ld_16
			chosen_materials = {
				material1 = navy_3
			}
		}
		cas_guitar_body_detail = {
			desc_id = none_esp
		}
		cas_guitar_body_fade = {
			desc_id = fade_none
		}
		cas_guitar_neck = {
			desc_id = gtr_24fret_fretboard_02
		}
		cas_guitar_head = {
			desc_id = gtr62_neckhead04
		}
		cas_guitar_head_finish = {
			desc_id = espcolor
			chosen_materials = {
				material1 = navy_1
			}
		}
		cas_guitar_head_detail = {
			desc_id = esp_none
		}
		cas_guitar_head_fade = {
			desc_id = head_fade_none
		}
		cas_guitar_pickguards = {
			desc_id = None
		}
		cas_guitar_bridges = {
			desc_id = gtr9_esp_bridge_03
		}
		cas_guitar_knobs = {
			desc_id = gtr9_knob_02
			chosen_materials = {
				material1 = red_3
			}
		}
		cas_guitar_pickups = {
			desc_id = gtr1_esp_emg_pkup_2
		}
		cas_guitar_logo = {
			desc_id = None
		}
	}
	{
		cas_guitar_body = {
			desc_id = gtr62b_esp_jack
		}
		cas_guitar_finish = {
			desc_id = gtr62_esp_alexi_ld_10_color
			chosen_materials = {
				material1 = grey_5guitar
			}
		}
		cas_guitar_body_detail = {
			desc_id = none_esp
		}
		cas_guitar_body_fade = {
			desc_id = fade_none
		}
		cas_guitar_neck = {
			desc_id = gtr_24fret_fretboard_02
		}
		cas_guitar_head = {
			desc_id = gtr62_neckhead03
		}
		cas_guitar_head_finish = {
			desc_id = espcolor
			chosen_materials = {
				material1 = grey_5guitar
			}
		}
		cas_guitar_head_detail = {
			desc_id = esp_none
		}
		cas_guitar_head_fade = {
			desc_id = head_fade_none
		}
		cas_guitar_pickguards = {
			desc_id = None
		}
		cas_guitar_bridges = {
			desc_id = gtr_hardware62a
		}
		cas_guitar_knobs = {
			desc_id = gtr62_knob_type01
		}
		cas_guitar_pickups = {
			desc_id = gtr1_esp_emg_pkup_2
		}
		cas_guitar_logo = {
			desc_id = None
		}
	}
	{
		cas_guitar_body = {
			desc_id = gtr2_esp_truckster_02
		}
		cas_guitar_finish = {
			desc_id = gtr9_esp_eclipse_ld_11
			chosen_materials = {
				material1 = green_2
			}
		}
		cas_guitar_body_detail = {
			desc_id = none_esp
		}
		cas_guitar_body_fade = {
			desc_id = fade_none
		}
		cas_guitar_neck = {
			desc_id = gtr_22fret_fretboard_04
		}
		cas_guitar_head = {
			desc_id = gtr12_neckhead_01
		}
		cas_guitar_head_finish = {
			desc_id = espcolor
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_guitar_head_detail = {
			desc_id = esp_none
		}
		cas_guitar_head_fade = {
			desc_id = head_fade_none
		}
		cas_guitar_pickguards = {
			desc_id = gtr5_pickguard_01
		}
		cas_guitar_bridges = {
			desc_id = gtr4_esp_bridge_02
			chosen_materials = {
				material1 = grey_2guitar
			}
		}
		cas_guitar_knobs = {
			desc_id = gtr4_knob_02
		}
		cas_guitar_pickups = {
			desc_id = gtr3_esp_emg_pkup_2
		}
		cas_guitar_logo = {
			desc_id = None
		}
	}
	{
		cas_guitar_body = {
			desc_id = gtr63c_esp_ex
		}
		cas_guitar_finish = {
			desc_id = gtr63f_esp_ex_ld_12
			chosen_materials = {
				material1 = purple_blue_3
			}
		}
		cas_guitar_body_detail = {
			desc_id = none_esp
		}
		cas_guitar_body_fade = {
			desc_id = fade_none
		}
		cas_guitar_neck = {
			desc_id = gtr_22fret_fretboard_01
		}
		cas_guitar_head = {
			desc_id = gtr12_neckhead_01
		}
		cas_guitar_head_finish = {
			desc_id = espcolor
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_guitar_head_detail = {
			desc_id = esp_none
		}
		cas_guitar_head_fade = {
			desc_id = head_fade_none
		}
		cas_guitar_pickguards = {
			desc_id = gtr63e_esp_pg1
		}
		cas_guitar_pickguard_finish = {
			desc_id = custom_color
			chosen_materials = {
				material1 = grey_4guitar
			}
		}
		cas_guitar_bridges = {
			desc_id = gtr_hardware63
		}
		cas_guitar_knobs = {
			desc_id = gtr63_knob_type5
		}
		cas_guitar_pickups = {
			desc_id = gtr3_esp_emg_pkup_2
		}
		cas_guitar_logo = {
			desc_id = None
		}
	}
	{
		cas_guitar_body = {
			desc_id = gtr64_esp_f
		}
		cas_guitar_finish = {
			desc_id = gtr64_esp_f_ld_04_color
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_guitar_body_detail = {
			desc_id = none_esp
		}
		cas_guitar_body_fade = {
			desc_id = fade_none
		}
		cas_guitar_neck = {
			desc_id = gtr_24fret_fretboard_03
		}
		cas_guitar_head = {
			desc_id = gtr64_neckhead02
		}
		cas_guitar_head_finish = {
			desc_id = gtr9_neckhead_d_01
		}
		cas_guitar_head_detail = {
			desc_id = esp_none
		}
		cas_guitar_head_fade = {
			desc_id = head_fade_none
		}
		cas_guitar_pickguards = {
			desc_id = gtr64_esp_pg2
		}
		cas_guitar_pickguard_finish = {
			desc_id = custom_color
			chosen_materials = {
				material1 = grey_3guitar
			}
		}
		cas_guitar_bridges = {
			desc_id = gtr_hardware63
		}
		cas_guitar_knobs = {
			desc_id = gtr64_knob_type3
		}
		cas_guitar_pickups = {
			desc_id = gtr3_esp_emg_pkup_2
		}
		cas_guitar_logo = {
			desc_id = None
		}
	}
	{
		cas_guitar_body = {
			desc_id = gtr30_h101
		}
		cas_guitar_finish = {
			desc_id = gtr30_ld15
		}
		cas_guitar_body_detail = {
			desc_id = none_esp
		}
		cas_guitar_body_fade = {
			desc_id = fade_none
		}
		cas_guitar_neck = {
			desc_id = gtr_24fret_fretboard_03
		}
		cas_guitar_head = {
			desc_id = gtr30_nekhed_e
		}
		cas_guitar_head_finish = {
			desc_id = gtr4_neckhead_06_ld_11
		}
		cas_guitar_head_detail = {
			desc_id = esp_none
		}
		cas_guitar_head_fade = {
			desc_id = head_fade_none
		}
		cas_guitar_pickguards = {
			desc_id = None
		}
		cas_guitar_bridges = {
			desc_id = gtr30_bridge_d
		}
		cas_guitar_knobs = {
			desc_id = gtr_30_knobs_d
		}
		cas_guitar_pickups = {
			desc_id = gtr30_pickup_gold
		}
		cas_guitar_logo = {
			desc_id = None
		}
	}
	{
		cas_guitar_body = {
			desc_id = gtr30_h101
		}
		cas_guitar_finish = {
			desc_id = gtr30_h101_d_color
			chosen_materials = {
				material1 = purple_blue_1
			}
		}
		cas_guitar_body_detail = {
			desc_id = none_esp
		}
		cas_guitar_body_fade = {
			desc_id = fade_none
		}
		cas_guitar_neck = {
			desc_id = gtr_24fret_fretboard_01
		}
		cas_guitar_head = {
			desc_id = gtr30_neckhead_b
		}
		cas_guitar_head_finish = {
			desc_id = espcolor
			chosen_materials = {
				material1 = purple_blue_1
			}
		}
		cas_guitar_head_detail = {
			desc_id = esp_none
		}
		cas_guitar_head_fade = {
			desc_id = head_fade_none
		}
		cas_guitar_pickguards = {
			desc_id = None
		}
		cas_guitar_bridges = {
			desc_id = gtr30_brdg_strgthru
		}
		cas_guitar_knobs = {
			desc_id = gtr_30_knobs_c
		}
		cas_guitar_pickups = {
			desc_id = gtr30_emg
		}
		cas_guitar_logo = {
			desc_id = None
		}
	}
	{
		cas_guitar_body = {
			desc_id = gtr31_horizon
		}
		cas_guitar_finish = {
			desc_id = gtr33_flamed
		}
		cas_guitar_body_detail = {
			desc_id = none_esp
		}
		cas_guitar_body_fade = {
			desc_id = fade_none
		}
		cas_guitar_neck = {
			desc_id = gtr_24fret_fretboard_03
		}
		cas_guitar_head = {
			desc_id = gtr31c_neckhead_02
		}
		cas_guitar_head_finish = {
			desc_id = espcolor
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_guitar_head_detail = {
			desc_id = esp_none
		}
		cas_guitar_head_fade = {
			desc_id = head_fade_none
		}
		cas_guitar_pickguards = {
			desc_id = None
		}
		cas_guitar_bridges = {
			desc_id = gtr31_bridge_fr
		}
		cas_guitar_knobs = {
			desc_id = gtr_32_knobs_b
		}
		cas_guitar_pickups = {
			desc_id = gtr31_pickup_gold
		}
		cas_guitar_logo = {
			desc_id = None
		}
	}
	{
		cas_guitar_body = {
			desc_id = gtr2_esp_truckster_02
		}
		cas_guitar_finish = {
			desc_id = gtr4_esp_eclipse_d_01_color
			chosen_materials = {
				material1 = red_2
			}
		}
		cas_guitar_body_fade = {
			desc_id = gtr4_esp_eclipse_dtl_03
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_guitar_body_detail = {
			desc_id = gh5_none
		}
		cas_guitar_neck = {
			desc_id = gtr_22fret_fretboard_03
		}
		cas_guitar_neck_finish = {
			desc_id = 22fret_ld86_esp_dots
			chosen_materials = {
				material1 = grey_4guitar
			}
		}
		cas_guitar_head = {
			desc_id = gtr62_neckhead01
		}
		cas_guitar_head_finish = {
			desc_id = espcolor
			chosen_materials = {
				material1 = red_4
			}
		}
		cas_guitar_head_detail = {
			desc_id = esp_none
		}
		cas_guitar_head_fade = {
			desc_id = gtr4_neckhead_dtl_03
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_guitar_pickguards = {
			desc_id = None
		}
		cas_guitar_bridges = {
			desc_id = gtr4_esp_bridge_02
		}
		cas_guitar_knobs = {
			desc_id = gtr4_esp_knob_10_03
		}
		cas_guitar_pickups = {
			desc_id = gtr1_esp_emg_pkup_2
		}
		cas_guitar_logo = {
			desc_id = None
		}
	}
	{
		cas_guitar_body = {
			desc_id = gtr70_sch_ultra
		}
		cas_guitar_finish = {
			desc_id = gtr70_sch_ultra_ld08
		}
		cas_guitar_body_fade = {
			desc_id = fade_none
		}
		cas_guitar_body_detail = {
			desc_id = gh5_none
		}
		cas_guitar_neck = {
			desc_id = gtr_22fret_fretboard_03
		}
		cas_guitar_neck_finish = {
			desc_id = 22fret_ld86_esp_dots
			chosen_materials = {
				material1 = grey_4guitar
			}
		}
		cas_guitar_head = {
			desc_id = gtr70_neckhead_04
		}
		cas_guitar_head_finish = {
			desc_id = espcolor
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_guitar_head_detail = {
			desc_id = esp_none
		}
		cas_guitar_head_fade = {
			desc_id = gtr74_neckhead_01_dtl04
			chosen_materials = {
				material1 = red_2
			}
		}
		cas_guitar_pickguards = {
			desc_id = None
		}
		cas_guitar_bridges = {
			desc_id = gtr70_bridge
		}
		cas_guitar_knobs = {
			desc_id = chromedomes
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_guitar_pickups = {
			desc_id = duncanbodbl
		}
		cas_guitar_logo = {
			desc_id = None
		}
	}
	{
		cas_guitar_body = {
			desc_id = gtr90_dr_daisy
		}
		cas_guitar_finish = {
			desc_id = gtr90_dr_daisy_ld02
			chosen_materials = {
				material1 = red_2
			}
		}
		cas_guitar_body_fade = {
			desc_id = gtr90_dr_daisy_dtl04
			chosen_materials = {
				material1 = red_orange_2
			}
		}
		cas_guitar_body_detail = {
			desc_id = gtr90_dr_daisy_dtl07
			chosen_materials = {
				material1 = grey_5guitar
			}
		}
		cas_guitar_neck = {
			desc_id = gtr_22fret_fretboard_04
		}
		cas_guitar_neck_finish = {
			desc_id = 22fret_ld86_esp_dots
			chosen_materials = {
				material1 = grey_4guitar
			}
		}
		cas_guitar_head = {
			desc_id = gtr90_dr_neckhead01
		}
		cas_guitar_head_finish = {
			desc_id = espcolor
			chosen_materials = {
				material1 = red_2
			}
		}
		cas_guitar_head_detail = {
			desc_id = esp_none
		}
		cas_guitar_head_fade = {
			desc_id = gtr90_neckhead01_dtl04
			chosen_materials = {
				material1 = red_orange_2
			}
		}
		cas_guitar_pickguards = {
			desc_id = gtr90_pg
		}
		cas_guitar_pickguard_finish = {
			desc_id = custom_color_esp
			chosen_materials = {
				material1 = grey_4guitar
			}
		}
		cas_guitar_bridges = {
			desc_id = gtr_bridge_mcswain_l_4daisy
		}
		cas_guitar_knobs = {
			desc_id = chromedomes
			chosen_materials = {
				material1 = grey_4guitar
			}
		}
		cas_guitar_pickups = {
			desc_id = lipstick
		}
		cas_guitar_logo = {
			desc_id = None
		}
	}
	{
		cas_guitar_body = {
			desc_id = gtr81_prs_miramt
		}
		cas_guitar_finish = {
			desc_id = gtr81_prs_mt_ld03
			chosen_materials = {
				material1 = purple_blue_1
			}
		}
		cas_guitar_body_fade = {
			desc_id = gtr81_prs_mt_dtl04
			chosen_materials = {
				material1 = navy_1
			}
		}
		cas_guitar_body_detail = {
			desc_id = gh5_none
		}
		cas_guitar_neck = {
			desc_id = gtr_22fret_fretboard_03
		}
		cas_guitar_neck_finish = {
			desc_id = 22fret_ld86_esp_dots
			chosen_materials = {
				material1 = grey_4guitar
			}
		}
		cas_guitar_head = {
			desc_id = gtr81_neckhead01
		}
		cas_guitar_head_finish = {
			desc_id = espcolor
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_guitar_head_detail = {
			desc_id = esp_none
		}
		cas_guitar_head_fade = {
			desc_id = gtr78_neckhead_dtl04
			chosen_materials = {
				material1 = purple_blue_1
			}
		}
		cas_guitar_pickguards = {
			desc_id = gtr81_pguard
		}
		cas_guitar_pickguard_finish = {
			desc_id = custom_color_esp
			chosen_materials = {
				material1 = grey_4guitar
			}
		}
		cas_guitar_bridges = {
			desc_id = gtr78_straightbridge
		}
		cas_guitar_knobs = {
			desc_id = chromedomes
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_guitar_pickups = {
			desc_id = duncanbodbl
		}
		cas_guitar_logo = {
			desc_id = None
		}
	}
]
cas_preset_basses = [
	{
		cas_bass_body = {
			desc_id = bass16_esp_surveyor_02
		}
		cas_bass_finish = {
			desc_id = bass14_esp_ax_ld_11
			chosen_materials = {
				material1 = green_2guitar
			}
		}
		cas_bass_body_detail = {
			desc_id = none_esp
		}
		cas_bass_body_fade = {
			desc_id = fade_none
		}
		cas_bass_neck = {
			desc_id = bass_fretboard_22a
		}
		cas_bass_neck_finish = {
			desc_id = bass_fretboard_inlay_02
			chosen_materials = {
				material1 = grey_3guitar
			}
		}
		cas_bass_head = {
			desc_id = bass14_neckhead_04
		}
		cas_bass_head_finish = {
			desc_id = esp_solidcolor
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_bass_head_detail = {
			desc_id = none_esp
		}
		cas_bass_head_fade = {
			desc_id = bass_head_fade_none
		}
		cas_bass_pickguards = {
			desc_id = None
		}
		cas_guitar_logo = {
			desc_id = None
		}
	}
	{
		cas_bass_body = {
			desc_id = bass63_ex
		}
		cas_bass_finish = {
			desc_id = bass63_ex_ld_16
			chosen_materials = {
				material1 = red_2
			}
		}
		cas_bass_body_detail = {
			desc_id = none_esp
		}
		cas_bass_body_fade = {
			desc_id = fade_none
		}
		cas_bass_neck = {
			desc_id = bass_fretboard_24d
		}
		cas_bass_head = {
			desc_id = bass63_neckhead
		}
		cas_bass_head_finish = {
			desc_id = bass63_neckhead_ld_11
			chosen_materials = {
				material1 = yellow_orange_2
			}
		}
		cas_bass_head_detail = {
			desc_id = none_esp
		}
		cas_bass_head_fade = {
			desc_id = bass_head_fade_none
		}
		cas_bass_pickguards = {
			desc_id = None
		}
		cas_bass_pickups = {
			desc_id = bass9_pkup_type1_blk_02
			chosen_materials = {
				material1 = grey_2guitar
			}
		}
		cas_bass_bridges = {
			desc_id = bass_bridges_03
		}
		cas_bass_knobs = {
			desc_id = bass63_knob_type4
			chosen_materials = {
				material1 = red_2
			}
		}
		cas_guitar_logo = {
			desc_id = None
		}
	}
	{
		cas_bass_body = {
			desc_id = bass16_esp_surveyor_02
		}
		cas_bass_finish = {
			desc_id = bass14_esp_surveyor_d_color
			chosen_materials = {
				material1 = grey_5guitar
			}
		}
		cas_bass_body_detail = {
			desc_id = none_esp
		}
		cas_bass_body_fade = {
			desc_id = bass14_esp_ax_dtl_05
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_bass_neck = {
			desc_id = bass_fretboard_24d
		}
		cas_bass_neck_finish = {
			desc_id = bass_fretboard_24_inlay_05
		}
		cas_bass_head = {
			desc_id = bass14_neckhead_04
		}
		cas_bass_head_finish = {
			desc_id = esp_solidcolor
			chosen_materials = {
				material1 = grey_5guitar
			}
		}
		cas_bass_head_detail = {
			desc_id = none_esp
		}
		cas_bass_head_fade = {
			desc_id = bass9_neckhead4_dtl_05
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_bass_pickguards = {
			desc_id = None
		}
		cas_bass_pickups = {
			desc_id = bass9_pkup_type3_blk_2
		}
		cas_bass_bridges = {
			desc_id = bass14_bridge_4str_01
		}
		cas_bass_knobs = {
			desc_id = bass14_esp_knob_10_04
		}
		cas_guitar_logo = {
			desc_id = None
		}
	}
	{
		cas_bass_body = {
			desc_id = bass9_esp_ax_01
		}
		cas_bass_finish = {
			desc_id = bass9_esp_ax_d_color
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_bass_body_detail = {
			desc_id = none_esp
		}
		cas_bass_body_fade = {
			desc_id = bass9_esp_ax_dtl_04
			chosen_materials = {
				material1 = grey_2guitar
			}
		}
		cas_bass_neck = {
			desc_id = bass_fretboard_22b
		}
		cas_bass_neck_finish = {
			desc_id = bass_fretboard_inlay_03
		}
		cas_bass_head = {
			desc_id = bass9_neckhead_01
		}
		cas_bass_head_finish = {
			desc_id = esp_solidcolor
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_bass_head_detail = {
			desc_id = none_esp
		}
		cas_bass_head_fade = {
			desc_id = bass_head_fade_none
		}
		cas_bass_pickguards = {
			desc_id = None
		}
		cas_bass_pickups = {
			desc_id = bass9_pkup_type1_blk_02
			chosen_materials = {
				material1 = grey_2guitar
			}
		}
		cas_bass_bridges = {
			desc_id = bass9_bridge_4str_01
		}
		cas_bass_knobs = {
			desc_id = bass9_knob_03
			chosen_materials = {
				material1 = grey_3guitar
			}
		}
		cas_guitar_logo = {
			desc_id = None
		}
	}
	{
		cas_bass_body = {
			desc_id = bass17_esp_ec_01
		}
		cas_bass_finish = {
			desc_id = bass17_esp_ec_ld_16
		}
		cas_bass_body_detail = {
			desc_id = none_esp
		}
		cas_bass_body_fade = {
			desc_id = fade_none
		}
		cas_bass_neck = {
			desc_id = bass_fretboard_24c
		}
		cas_bass_neck_finish = {
			desc_id = bass_fretboard_inlay_03
		}
		cas_bass_head = {
			desc_id = bass14_neckhead_04
		}
		cas_bass_head_finish = {
			desc_id = esp_solidcolor
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_bass_head_detail = {
			desc_id = none_esp
		}
		cas_bass_head_fade = {
			desc_id = bass_head_fade_none
		}
		cas_bass_pickguards = {
			desc_id = None
		}
		cas_bass_pickups = {
			desc_id = bass9_pkup_type3_blk_2
			chosen_materials = {
				material1 = grey_2guitar
			}
		}
		cas_bass_bridges = {
			desc_id = bass17_bridge_4str_04
		}
		cas_bass_knobs = {
			desc_id = bass17_knob_05
			chosen_materials = {
				material1 = grey_3guitar
			}
		}
		cas_guitar_logo = {
			desc_id = None
		}
	}
	{
		cas_bass_body = {
			desc_id = bass17_esp_ec_01
		}
		cas_bass_finish = {
			desc_id = bass17_esp_ec_ld_color
			chosen_materials = {
				material1 = blue_1
			}
		}
		cas_bass_body_detail = {
			desc_id = none_esp
		}
		cas_bass_body_fade = {
			desc_id = fade_none
		}
		cas_bass_neck = {
			desc_id = bass_fretboard_24black
		}
		cas_bass_neck_finish = {
			desc_id = bass_fretboard_inlay_05
		}
		cas_bass_head = {
			desc_id = bass14_neckhead_02
		}
		cas_bass_head_finish = {
			desc_id = esp_solidcolor
			chosen_materials = {
				material1 = purple_blue_1
			}
		}
		cas_bass_head_detail = {
			desc_id = none_esp
		}
		cas_bass_head_fade = {
			desc_id = bass_head_fade_none
		}
		cas_bass_pickguards = {
			desc_id = None
		}
		cas_bass_pickups = {
			desc_id = bass9_pkup_type3_blk_2
			chosen_materials = {
				material1 = grey_2guitar
			}
		}
		cas_bass_bridges = {
			desc_id = bass17_bridge_4str_04
		}
		cas_bass_knobs = {
			desc_id = bass17_knob_03
			chosen_materials = {
				material1 = grey_3guitar
			}
		}
		cas_guitar_logo = {
			desc_id = None
		}
	}
	{
		cas_bass_body = {
			desc_id = bass17_esp_ec_01
		}
		cas_bass_finish = {
			desc_id = bass17_esp_ec_ld_12
			chosen_materials = {
				material1 = yellow_1
			}
		}
		cas_bass_body_detail = {
			desc_id = none_esp
		}
		cas_bass_body_fade = {
			desc_id = fade_none
		}
		cas_bass_neck = {
			desc_id = bass_fretboard_24black
		}
		cas_bass_neck_finish = {
			desc_id = bass_fretboard_inlay_01
		}
		cas_bass_head = {
			desc_id = bass14_neckhead_04
		}
		cas_bass_head_finish = {
			desc_id = esp_solidcolor
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_bass_head_detail = {
			desc_id = none_esp
		}
		cas_bass_head_fade = {
			desc_id = bass_head_fade_none
		}
		cas_bass_pickguards = {
			desc_id = None
		}
		cas_bass_pickups = {
			desc_id = bass9_pkup_type3_blk_2
			chosen_materials = {
				material1 = grey_2guitar
			}
		}
		cas_bass_bridges = {
			desc_id = bass17_bridge_4str_04
		}
		cas_bass_knobs = {
			desc_id = bass17_esp_knob_10
			chosen_materials = {
				material1 = grey_3guitar
			}
		}
		cas_guitar_logo = {
			desc_id = None
		}
	}
	{
		cas_bass_body = {
			desc_id = bass17_esp_ec_01
		}
		cas_bass_finish = {
			desc_id = bass17_esp_ec_ld_11
			chosen_materials = {
				material1 = orange_2
			}
		}
		cas_bass_body_detail = {
			desc_id = bass17_esp_ax_dtl_13
		}
		cas_bass_body_fade = {
			desc_id = bass17_esp_ax_dtl_05
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_bass_neck = {
			desc_id = bass_fretboard_22b
		}
		cas_bass_neck_finish = {
			desc_id = bass_fretboard_inlay_06
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_bass_head = {
			desc_id = bass14_neckhead_03
		}
		cas_bass_head_finish = {
			desc_id = esp_solidcolor
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_bass_head_detail = {
			desc_id = none_gh5
		}
		cas_bass_head_fade = {
			desc_id = bass_head_fade_none
		}
		cas_bass_pickguards = {
			desc_id = None
		}
		cas_bass_pickups = {
			desc_id = bass9_pkup_type1_blk_02
		}
		cas_bass_bridges = {
			desc_id = bass17_bridge_4str_04
		}
		cas_bass_knobs = {
			desc_id = bass17_esp_knob_10
		}
	}
	{
		cas_bass_body = {
			desc_id = bas_ibz_sr505
		}
		cas_bass_finish = {
			desc_id = bs_ibz_sr_ld01
		}
		cas_bass_body_detail = {
			desc_id = bs_ibz_sr_dtl02
		}
		cas_bass_body_fade = {
			desc_id = bass17_esp_ax_dtl_05
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_bass_neck = {
			desc_id = bass_fretboard_22b
		}
		cas_bass_neck_finish = {
			desc_id = bass_fretboard_inlay_06
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_bass_head = {
			desc_id = bas_ibz_sr_head01
		}
		cas_bass_head_finish = {
			desc_id = esp_solidcolor
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_bass_head_detail = {
			desc_id = none_gh5
		}
		cas_bass_head_fade = {
			desc_id = bs_ibz_sr_hd_dtl04
			chosen_materials = {
				material1 = orange_2
			}
		}
		cas_bass_pickguards = {
			desc_id = None
		}
		cas_bass_pickups = {
			desc_id = ibz_type1
		}
		cas_bass_bridges = {
			desc_id = ibztrad
		}
		cas_bass_knobs = {
			desc_id = chromedomes
		}
	}
	{
		cas_bass_body = {
			desc_id = bs_sch_t_body
		}
		cas_bass_finish = {
			desc_id = bs_sch_mt_dtl01
			chosen_materials = {
				material1 = grey_5guitar
			}
		}
		cas_bass_body_detail = {
			desc_id = bs_sch_mt_ld02
			chosen_materials = {
				material1 = red_1
			}
		}
		cas_bass_body_fade = {
			desc_id = bs_sch_mt_ld09
			chosen_materials = {
				material1 = red_1
			}
		}
		cas_bass_neck = {
			desc_id = bass_fretboard_21a
		}
		cas_bass_neck_finish = {
			desc_id = bs_sch22_inl_ld32
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_bass_head = {
			desc_id = bs_sch_t_nhead
		}
		cas_bass_head_finish = {
			desc_id = bass_neckhead_mt
		}
		cas_bass_head_detail = {
			desc_id = none_esp
		}
		cas_bass_head_fade = {
			desc_id = bass_head_fade_none
		}
		cas_bass_pickguards = {
			desc_id = bs_sch_t_pga
		}
		cas_bass_pickups = {
			desc_id = basslines1b
		}
		cas_bass_bridges = {
			desc_id = schbb104
		}
		cas_bass_knobs = {
			desc_id = knurled
		}
	}
	{
		cas_bass_body = {
			desc_id = bass9_esp_ax_01
		}
		cas_bass_finish = {
			desc_id = bass9_esp_ax_d_color
			chosen_materials = {
				material1 = red_1
			}
		}
		cas_bass_body_detail = {
			desc_id = bass9_esp_ax_dtl_19
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_bass_body_fade = {
			desc_id = bass9_esp_ax_dtl_03
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_bass_neck = {
			desc_id = bass_fretboard_24black
		}
		cas_bass_neck_finish = {
			desc_id = neck_inlay_fl_blank
		}
		cas_bass_head = {
			desc_id = bass9_neckhead_01
		}
		cas_bass_head_finish = {
			desc_id = gh5_solidcolor
			chosen_materials = {
				material1 = black_1guitar
			}
		}
		cas_bass_head_detail = {
			desc_id = none_esp
		}
		cas_bass_head_fade = {
			desc_id = bass_head_fade_none
		}
		cas_bass_pickguards = {
			desc_id = None
		}
		cas_bass_pickups = {
			desc_id = bass9_pkup_type1_blk_02
		}
		cas_bass_bridges = {
			desc_id = bass9_bridge_4str_01
		}
		cas_bass_knobs = {
			desc_id = bass9_esp_knob_10_04
		}
	}
]
cas_preset_drums = [
	{
		cas_drums = {
			desc_id = singlebasskit
		}
		cas_drum_finish = {
			desc_id = gh5_shell_ld19
			chosen_materials = {
				material1 = red_1
			}
		}
		cas_drum_detail = {
			desc_id = gh5_skin_ld29
			chosen_materials = {
				material1 = red_1
			}
		}
		cas_drums_sticks_l = {
			desc_id = drm_stick_l
		}
		cas_drums_sticks_r = {
			desc_id = drm_stick_r
		}
	}
	{
		cas_drums = {
			desc_id = singlebasskit
		}
		cas_drum_finish = {
			desc_id = gh5_shell_ld39
			chosen_materials = {
				material1 = red_1
			}
		}
		cas_drum_detail = {
			desc_id = gh5_skin_ld63
		}
		cas_drums_sticks_l = {
			desc_id = drm_stick_l
		}
		cas_drums_sticks_r = {
			desc_id = drm_stick_r
		}
	}
	{
		cas_drums = {
			desc_id = singlebasskit
		}
		cas_drum_finish = {
			desc_id = gh5_shell_ld36
		}
		cas_drum_detail = {
			desc_id = gh5_skin_ld92
		}
		cas_drums_sticks_l = {
			desc_id = drm_stick_l
		}
		cas_drums_sticks_r = {
			desc_id = drm_stick_r
		}
	}
	{
		cas_drums = {
			desc_id = singlebasskit
		}
		cas_drum_finish = {
			desc_id = gh5_shell_ld42
		}
		cas_drum_detail = {
			desc_id = gh5_skin_ld102
		}
		cas_drums_sticks_l = {
			desc_id = drm_stick_l
		}
		cas_drums_sticks_r = {
			desc_id = drm_stick_r
		}
	}
	{
		cas_drums = {
			desc_id = singlebasskit
		}
		cas_drum_finish = {
			desc_id = gh5_shell_ld34
		}
		cas_drum_detail = {
			desc_id = gh5_skin_ld21b
		}
		cas_drums_sticks_l = {
			desc_id = drm_stick_l
		}
		cas_drums_sticks_r = {
			desc_id = drm_stick_r
		}
	}
	{
		cas_drums = {
			desc_id = singlebasskit
		}
		cas_drum_finish = {
			desc_id = gh5_shell_ld17
		}
		cas_drum_detail = {
			desc_id = gh5_skin_ld15
		}
		cas_drums_sticks_l = {
			desc_id = drm_stick_l
		}
		cas_drums_sticks_r = {
			desc_id = drm_stick_r
		}
	}
	{
		cas_drums = {
			desc_id = singlebasskit
		}
		cas_drum_finish = {
			desc_id = gh5_shell_ld16
		}
		cas_drum_detail = {
			desc_id = gh5_skin_ld41
		}
		cas_drums_sticks_l = {
			desc_id = drm_stick_l
		}
		cas_drums_sticks_r = {
			desc_id = drm_stick_r
		}
	}
]
cas_preset_female_vocals = [
	{
		cas_mic = {
			desc_id = mic_standard
		}
		cas_mic_stand = {
			desc_id = mic_stand_female
		}
	}
]
cas_preset_male_vocals = [
	{
		cas_mic = {
			desc_id = mic_standard
		}
		cas_mic_stand = {
			desc_id = mic_stand_female
		}
	}
]
larsu_drumskin_tama = {
	cas_drums = {
		desc_id = lars_drumkit
	}
	cas_drum_finish = {
		desc_id = 0xe2f794a4
		chosen_materials = {
			material1 = red_orange_4
		}
	}
	cas_drum_detail = {
		desc_id = 0x376d3949
	}
}
larsu_drumskin_youngmetalattack = {
	cas_drums = {
		desc_id = lars_drumkit
	}
	cas_drum_finish = {
		desc_id = 0xe2f794a4
		chosen_materials = {
			material1 = red_orange_4
		}
	}
	cas_drum_detail = {
		desc_id = 0x406a09df
	}
}
larsu_drumskin_sanitarium = {
	cas_drums = {
		desc_id = lars_drumkit
	}
	cas_drum_finish = {
		desc_id = 0xe2f794a4
		chosen_materials = {
			material1 = red_orange_4
		}
	}
	cas_drum_detail = {
		desc_id = 0x129be2b8
	}
}
larsu_drumskin_arghmatie = {
	cas_drums = {
		desc_id = lars_drumkit
	}
	cas_drum_finish = {
		desc_id = 0xe2f794a4
		chosen_materials = {
			material1 = red_orange_4
		}
	}
	cas_drum_detail = {
		desc_id = 0x659cd22e
	}
}
larsu_drumskin_zombielars = {
	cas_drums = {
		desc_id = lars_drumkit
	}
	cas_drum_finish = {
		desc_id = 0xe2f794a4
		chosen_materials = {
			material1 = red_orange_4
		}
	}
	cas_drum_detail = {
		desc_id = 0xfc958394
	}
}
larsu_drumskin_kingnothing = {
	cas_drums = {
		desc_id = lars_drumkit
	}
	cas_drum_finish = {
		desc_id = 0xe2f794a4
		chosen_materials = {
			material1 = red_orange_4
		}
	}
	cas_drum_detail = {
		desc_id = 0x8b92b302
	}
}
larsu_drumskin_donington = {
	cas_drums = {
		desc_id = lars_drumkit
	}
	cas_drum_finish = {
		desc_id = 0xe2f794a4
		chosen_materials = {
			material1 = red_orange_4
		}
	}
	cas_drum_detail = {
		desc_id = 0x15f626a1
	}
}
larsu_drumskin_scaryguy = {
	cas_drums = {
		desc_id = lars_drumkit
	}
	cas_drum_finish = {
		desc_id = 0xe2f794a4
		chosen_materials = {
			material1 = red_orange_4
		}
	}
	cas_drum_detail = {
		desc_id = 0x62f11637
	}
}
larsu_drumskin_summer = {
	cas_drums = {
		desc_id = lars_drumkit
	}
	cas_drum_finish = {
		desc_id = 0xe2f794a4
		chosen_materials = {
			material1 = red_orange_4
		}
	}
	cas_drum_detail = {
		desc_id = 0xfbf8478d
	}
}
