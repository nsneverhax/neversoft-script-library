...
script songlist_build 
	...
	if (<mode> = music_store)
		...
		if (<music_store_mode> = download_list)
			current_menu :se_setprops {
				event_handlers = [
					{pad_start generic_event_choose params = {no_sound state = uistate_music_store_purchase data = {is_popup exclusive_device = <exclusive_device> purchase_struct = <ui_struct> go_to_confirm}}}
				]
				replace_handlers
			}
...
script songlist_item_focus \{time = 0.0}
	...
	if NOT (<mode> = music_store)
		...
		if NOT song_is_jamsession song = (<ui_struct>.song_checksum)
			...
			songlist_detail :se_setprops \{songlist_detail_intensity_cont_alpha = 1.0
...
script songlist_check_playlist_after_sd_settles 
	begin
	begin
	if NOT sdcardmanagerfunc \{func = is_busy}
		break
	endif
	WaitOneGameFrame
	repeat
