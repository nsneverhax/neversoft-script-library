...
script display_as_made_famous_by \{rot_angle = -7
		time = 0.25}
	ScriptAssert \{'display_as_made_famous_by: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script destroy_setlist_menu_practice 
	ScriptAssert \{'destroy_setlist_menu_practice: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script setlist_debug_completesong 
	...
	if (<enable_cheat> = 1)
		...
		getplayerinfo \{1
			base_score}
		setplayerinfo 1 score = (<base_score> * 2.8 + 1)
		Progression_SongWon
		if ($game_mode = p1_quickplay || $game_mode = p2_quickplay)
			menu_top_rockers_check_for_new_top_score
		endif
		songpreview = ($current_songlist_song_preview)
		change_tab tab = ($current_tab)
		Change current_songlist_song_preview = <songpreview>
		Change \{disable_menu_sounds = 1}
		begin
		...
		if (<unlocked> = 1 || $is_network_game = 1)
			...
			if (<array_Size> > 0)
				...
				if IsSongAvailable song_checksum = <song_checksum> song = ($g_gh3_setlist.<tier_checksum>.songs [<song_count>]) for_bonus = <for_bonus>
					...
					if NOT ($game_mode = practice || $game_mode = p2_pro_faceoff || $game_mode = p2_roadie_battle)
						if ($game_mode = p1_quickplay || $game_mode = p2_quickplay)
...
script setlist_show_helperbar \{text_option1 = qs(0x2e900a4e)
		text_option2 = qs(0x271e2cfc)
		button_option1 = qs(0xbb0b71dd)
		button_option2 = qs(0x3c936d12)
		spacing = 16}
	clean_up_user_control_helpers
	text_options = [
		qs(0x643800a7)
...
script setlist_return_to_2p_hub 
	ScriptAssert \{'setlist_return_to_2p_hub: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
