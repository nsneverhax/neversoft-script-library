...
script create_ui_physics_test 
	...
	switch $physics_test_type
		...
		ui_event event = menu_change state = uistate_generic_dialog_box data = {options = <options>}
		case 11
		CreateScreenElement \{Type = SpriteElement
			id = bg_angry
			parent = root_window
...
script apply_continuous_force \{Force = (0.0, 0.0)}
	ScriptAssert \{'apply_continuous_force: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script create_2d_spring \{stiffness = 500.0
	...
	if NOT GotParam \{no_visual}
		formatText checksumName = spring_visual 'spring_visual_%i' i = ($spring_num)
		CreateScreenElement {
			Type = SpriteElement
			id = <spring_visual>
			parent = <objID1>
	...
	begin
	GetScreenElementPosition id = <objID1>
	<pos_1> = <screenelementpos>
	<pos_1_x> = (<pos_1>.(1.0, 0.0))
	<pos_1_y> = (<pos_1>.(0.0, 1.0))
	GetScreenElementPosition id = <objid2>
...
script physics_element_face_velocity 
	ScriptAssert \{'physics_element_face_velocity: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script apply_continuous_torque \{Force = (0.0, 0.0)
	...
	ScriptAssert \{'apply_continuous_torque: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript

script ui_rotate_something \{time = 4}
	ScriptAssert \{'ui_rotate_something: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script create_physics_particles \{parent = root_window}
	ScriptAssert \{'create_physics_particles: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
