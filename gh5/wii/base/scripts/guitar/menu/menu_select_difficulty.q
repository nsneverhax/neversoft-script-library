...
script create_select_difficulty_menu_for_practice 
	ScriptAssert \{'create_select_difficulty_menu_for_practice: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script create_select_difficulty_menu \{practice = 0}
	...
	if NOT ($game_mode = p2_pro_faceoff)
		getnumplayersingame
		if ((<num_players> = 2) || ($menu_mp_select_mode_is_friendly_faceoff = 1))
	...
	menu_font = fontgrid_text_a1
	getplayerinfo \{1
		bot_play}
	if ((<bot_play> = 1) || ($is_network_game))
		exclusive_device = ($primary_controller)
	else
	...
	set_focus_color \{Color = red175}
	set_unfocus_color \{Color = white210}
...
script selected_difficulty 
	...
	setplayerinfo 1 difficulty = <diff>
	setplayerinfo 2 difficulty = <diff>
	getplayerinfo \{1
		controller}
	if (<controller> < 4)
		SetArrayElement ArrayName = default_difficulty globalarray index = <controller> NewValue = <diff>
	endif
	if isngc
	...
	elseif ($practice_enabled)
		...
		switch ($game_mode)
			...
			if check_part_against_current_controller can_change_level = <can_change_level> device_num = <device_num>
...
script create_mp_select_difficulty_menu 
	...
	getplayerinfo \{2
		...
		out = device2}
	if ($is_network_game = 1)
		<exclusive_dev> = <device1>
	else
		<exclusive_dev> = <device2>
	endif
	if ($is_network_game = 1)
		no_focus_value = 1
	else
	...
	displaySprite \{parent = sd_container
		...
		rot_angle = 90
		dims = (212.0, 128.0)}
	if ($game_mode = gh4_p2_career || $game_mode = gh4_p2_quickplay)
...
script temp_selected_difficulty \{Player = 1}
	...
	if (($g_sd_p1_ready) && ($g_sd_p2_ready))
		...
		getplayerinfo \{2
			difficulty
			out = difficulty2}
		diff_index_p1 = ($difficulty_list_props.<difficulty>.index)
		diff_index_p2 = ($difficulty_list_props.<difficulty>.index)
		min = <diff_index_p1>
...
script check_part_against_current_controller 
	...
	endswitch
	SpawnScriptLater wrong_instrument_popup params = {device_num = <device_num> can_change_level = <can_change_level> part = <part>}
	return \{FALSE}
endscript

script wrong_instrument_popup 
	destroy_dialog_box
	options = [
		{
			func = try_continue_again params = {can_change_level = <can_change_level> device_num = <device_num>}
	...
	text = $wii_controller_disconnect
	player_display = (<device_num> + 1)
	switch (<part>)
		case guitar
		case bass
	...
	msg_txt_blck = <text>
	create_dialog_box {
		heading_text = $wii_popup_error
		body_text = <msg_txt_blck>
		options = <options>
		no_background
		dlog_event_handlers = [
