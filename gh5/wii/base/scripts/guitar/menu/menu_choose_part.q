...
script select_instrument_scroll \{Dir = down
	...
	if (<Player> = 1)
		getplayerinfo \{1
			ready = 1}
		if ((<ready> = 1) || ($g_si_player1_locked = 1) && (<overridelock> = 0))
			return
		endif
		if ($g_si_player1_index = 0)
		...
		getplayerinfo \{2
			ready = 1}
		if ((<ready> = 1) || ($g_si_player2_locked = 1) && (<overridelock> = 0))
			return
		endif
		if ($g_si_player2_index = 0)
...
script create_ready_icons \{pos1 = (440.0, 120.0)
	...
	displayText parent = <container> text = qs(0x679dee25) Pos = (50.0, 30.0) Scale = (1.25, 0.9) font = fontgrid_text_a1 z = 100 rgba = [223 223 223 255]
...
script change_pos_ready_sign \{Player = 1
		Pos = (0.0, 0.0)}
	ScriptAssert \{'change_pos_ready_sign: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
