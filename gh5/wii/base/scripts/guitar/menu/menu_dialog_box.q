...
script create_dialog_box {
	...
	if NOT GotParam \{no_helpers}
		...
		add_user_control_helper text = qs(0xc18d5e76) button = green z = 1000000 <all_buttons_flag>
		if GotParam \{back_button_script}
			printf 'back_button_script = %s' s = <back_button_script>
			add_user_control_helper text = qs(0xaf4d5dd2) button = red z = 1000000 <all_buttons_flag>
		endif
	endif
...
script dialog_box_setup_physics_elements 
	if (<desc_type_text> = 'dialog_box_wii_music_store' || <desc_type_text> = 'dialog_box_standard' || <desc_type_text> = 'dialog_box_alternate' || <desc_type_text> = 'dialog_box_standard_musicstudio' || <desc_type_text> = 'dialog_box_extended')
	...
	if (<desc_type_text> = 'dialog_box_wii_music_store' || <desc_type_text> = 'dialog_box_standard' || <desc_type_text> = 'dialog_box_standard_musicstudio')
	...
	if (<array_Size> > 2)
		...
		if (<array_Size> = 2)
			...
			elseif (<template> = trc)
				...
				<unfocus_params> = {shrink_scale = 1.4}
				shrink_scale = 1.4
				Shadow = FALSE
			else
				if (<index> = 0)
		...
		if ((GotParam num_focusable_options) && (<num_focusable_options> = 1))
			event_handlers = [
				...
				{unfocus dialog_box_unfocus params = <unfocus_params>}
