
script ui_create_quickplay_choice 
	destroy_loading_screen
	Change \{rich_presence_context = presence_menus}
	Change \{respond_to_signin_changed = 1}
	Change \{respond_to_signin_changed_func = None}
	CreateScreenElement \{parent = root_window
		id = musiccentralmenu_id
		Type = descinterface
		desc = 'mainmenu'
		mainmenu_helper_text_text = qs(0x03ac90f0)}
	mainmenu_setup_physics_elements \{screen_id = musiccentralmenu_id}
	mainmenu_setup_option \{choose_script = quickplay_choice_choose_quickplay
		back_state = uistate_mainmenu
		menu_item_text = qs(0x90cc45b5)}
	mainmenu_setup_option \{choose_script = quickplay_choice_choose_freeplay
		back_state = uistate_mainmenu
		menu_item_text = qs(0x8a778398)}
	set_focus_color
	set_unfocus_color
	add_user_control_helper \{text = qs(0xc18d5e76)
		button = green
		z = 100000}
	add_user_control_helper \{text = qs(0xaf4d5dd2)
		button = red
		z = 100000}
	LaunchEvent Type = focus target = current_menu data = {child_index = <selected_index>}
endscript

script ui_destroy_quickplay_choice 
	clean_up_user_control_helpers
	DestroyScreenElement \{id = musiccentralmenu_id}
endscript

script quickplay_choice_choose_quickplay 
	main_menu_select_band_lobby lobby_mode = quickplay <...>
endscript

script quickplay_choice_choose_freeplay 
	main_menu_select_freeplay
endscript
