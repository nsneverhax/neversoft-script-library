...
script ui_gig_complete_handle_challenge_strip 
	if ScreenElementExists \{id = gig_complete_noncompetitive_id}
		...
		if (<Type> = career && $end_credits = 0)
			...
			<max_challenge_stars> = (<max_challenge_stars> + (($gig_star_rewards).<current_challenge>.gold))
			<max_challenge_stars> = (<max_challenge_stars> + (($gig_star_rewards).<current_challenge>.platinum))
			<max_challenge_stars> = (<max_challenge_stars> + (($gig_star_rewards).<current_challenge>.diamond))
