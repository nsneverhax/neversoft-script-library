...
script ui_online_setup_display_friends_dialog 
	ui_event_wait_for_safe
	destroy_dialog_box
	create_dialog_box \{heading_text = qs(0x54a29d26)
...
script ui_online_setup_import_friends 
	...
	printstruct <...>
	if GotParam \{import}
		printf \{qs(0xbdeb6414)}
		display_exclusive_notification text = ($wii_notification_strings.friends_imported)
		if ((<has_completed_setup> = FALSE) || ($g_ngc_friends_enabled = 0))
			NetSessionFunc \{Obj = friends
				func = enable_friends
				params = {
					enable = 1
				}}
			Change \{g_ngc_friends_enabled = 1}
		endif
	else
		printf \{qs(0x9ed71c79)}
		display_exclusive_notification text = ($wii_notification_strings.friends_removed)
		if ((<has_completed_setup> = FALSE) || ($g_ngc_friends_enabled = 1))
			NetSessionFunc \{Obj = friends
				func = enable_friends
				params = {
					enable = 0
				}}
			Change \{g_ngc_friends_enabled = 0}
		endif
	endif
