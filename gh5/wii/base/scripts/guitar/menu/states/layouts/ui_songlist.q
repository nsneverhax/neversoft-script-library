...
script music_store_load_venue 
	...
	musicstudio_set_ambience \{ambience = ghtunes_focus_ambience
		update_extra_info}
	ScreenFX_UpdateFXInstanceParams \{viewport = bg_viewport
		Name = av_test0
		Type = audiovisualize
		On = 1
		primitives = $jow_viz_pinktunnel}
	Wait \{1
		Second}
endscript
...
script ui_init_songlist \{mode = play_song}
	...
	gamemode_gettype
	if ((<Type> = quickplay) || (<Type> = competitive) || (<Type> = freeplay) || (<rb_mode> = 1))
...
script ui_create_songlist \{mode = play_song
	...
	if NOT (<mode> = music_store || <mode> = leaderboard || <mode> = practice || <mode> = toprockers || <mode> = archive)
