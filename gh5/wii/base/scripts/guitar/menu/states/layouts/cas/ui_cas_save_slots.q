...
script ui_create_cas_save_slots 
	...
	displaySprite \{parent = generic_menu
		tex = gradient_256
		Pos = (640.0, 360.0)
		dims = (1440.0, 720.0)
		just = [
...
script exit_save_instrument \{default_name = qs(0x00000000)
	...
	ui_event event = <event> data = {state = uistate_edit_name is_popup char_limit = 18 default_name = <default_name> title = qs(0x17f49e18) device_num = ($primary_controller) accept_script = save_instrument_accept_name accept_params = {<...>} no_show_history = <no_show_history>}
endscript
...
script apply_cap_design 
	...
	printstruct \{$edit_graphic_layer_infos}
	generic_event_back
endscript
...
script save_cap_design \{default_name = qs(0x00000000)
		event = menu_change}
	RemoveParameter \{text}
	ui_event event = <event> data = {state = uistate_edit_name is_popup char_limit = 18 default_name = <default_name> title = qs(0x17f49e18) device_num = ($primary_controller) accept_script = save_cap_design_accept accept_params = {<...>} no_show_history = <no_show_history>}
endscript
...
script set_save_cap_return 
	...
	globaltag_setarrayelement savegame = <savegame> array_name = <list> index = <index> element = <entry>
	cag_update_photo
	ui_memcard_autosave_replace event = menu_back data = {num_states = 1 savegame = <savegame> requested_autosave = 1}
endscript
