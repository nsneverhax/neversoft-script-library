...
script ui_create_character_hub_spawned 
	...
	if (<bot_play1> = 1)
		...
		if NOT (<bot_original_controller> = -1)
			setplayerinfo 1 controller = <bot_original_controller>
		endif
	endif
	if (<bot_play2> = 1)
		getplayerinfo \{2
			bot_original_controller}
		if NOT (<bot_original_controller> = -1)
			setplayerinfo 2 controller = <bot_original_controller>
		endif
	endif
	if (<bot_play3> = 1)
		getplayerinfo \{3
			bot_original_controller}
		if NOT (<bot_original_controller> = -1)
			setplayerinfo 3 controller = <bot_original_controller>
		endif
	endif
	if (<bot_play4> = 1)
		getplayerinfo \{4
			bot_original_controller}
		if NOT (<bot_original_controller> = -1)
			setplayerinfo 4 controller = <bot_original_controller>
		endif
	endif
	...
	if NOT ($is_network_game)
		AddArrayElement array = <array> element = {pad_start ui_band_mode_continue}
	endif
...
script ui_character_hub_continue 
	...
	if (<num_ready> = 2)
		...
		if (($game_mode = p2_pro_faceoff) || ($game_mode = p2_roadie_battle))
			...
			generic_event_choose \{state = uistate_singleplayer_character_hub}
		endif
	endif
endscript
