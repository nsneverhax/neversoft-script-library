...
script ui_create_singleplayer_character_hub_spawned 
	...
	if init_singleplayer_character_hub pass_to_gigboard = <pass_to_gigboard>
		getfirstplayer
		...
		if NOT profile_can_be_modified id = <character_id>
...
script init_singleplayer_character_hub \{Player = 1}
	...
	if GotParam \{pass_to_gigboard}
		SpawnScriptNow cleanup_singleplayer_hub_return_tutorials params = {Player = <Player> Progression = <Progression>}
		return \{FALSE}
	else
		if ($first_character_hub_enter = 0)
			getplayerinfo <Player> character_id
			if NOT characterprofileexists Name = <character_id> savegame = <savegame>
				stars
				printf \{'eek, this is bad!'}
				stars
				setplayerinfo <Player> character_id = ($failsafe_car_profile)
				setplayerinfo <Player> character_savegame = -1
			endif
			cas_queue_new_character_profile id = <character_id> Player = <Player> savegame = <savegame> hide_old_character = 1
		endif
	endif
