menu_mp_select_mode_font = fontgrid_text_a1
coop_dlc_active = 0
...
script create_mp_select_mode_menu 
	...
	SetScreenElementProps \{id = current_menu
		rot_angle
		spacing_between = 12}
	set_focus_color \{Color = pinkish_red}
	set_unfocus_color \{Color = black}
	if ((($is_network_game) = 1) && (($ranked_match) = notranked))
		Scale = 0.6
	else
		Scale = 0.7
	endif
	...
	GetScreenElementPosition \{id = mp_text}
	<screenelementpos> = <Pos>
	GetScreenElementDims \{id = mp_text}
	...
	CreateScreenElement {
		...
		event_handlers = [
			{focus menu_select_multiplayer_mode_focus}
			{unfocus menu_select_multiplayer_mode_unfocus}
			{pad_choose mp_select_mode_menu_select_mode params = {mode = p2_roadie_battle}}
		]
	}
...
script mp_select_mode_menu_select_mode \{mode = p2_pro_faceoff}
