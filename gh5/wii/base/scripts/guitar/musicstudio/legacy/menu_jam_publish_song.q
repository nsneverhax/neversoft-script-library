...
script create_jam_publish_song_menu \{new_genre = -1}
	...
	if ((<array_Size>) > 0)
		...
		setstructureparam struct_name = local_struct param = layers value = <band_logo>
...
script dwc_checkprofanitycallback 
	broadcastevent Type = check_profanity_result data = {Result = <Result>}
endscript
...
script file_id_test 
	ScriptAssert \{'file_id_test: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script jam_upload_song_failed \{dialog = 0}
	...
	switch <dialog>
		...
		elseif isps3
			upload_dialog = 'GHTunes is disabled because online service is disabled on your PlayStation®Network account due to parental control restrictions'
		elseif isngc
			upload_dialog = $wii_ghtunes_parental
		endif
		case 7
		upload_dialog = qs(0x7cc9a727)
		case 8
		if isxenonorwindx
			upload_dialog = 'No gamer profile currently signed in. Please sign into an Xbox LIVE multiplayer enabled gamer profile to upload to GHTunes.'
		elseif isps3
			upload_dialog = 'You must be signed into the PlayStation®Network to upload to GHTunes.'
		elseif isngc
			upload_dialog = $wii_ghtunes_must_be_signed
		endif
		case 9
		if isxenonorwindx
			upload_dialog = 'You must have an Xbox LIVE Gold Membership to upload a song to GHTunes. Please sign in and try again.'
		elseif isps3
			upload_dialog = 'You must be signed into the PlayStation®Network to upload to GHTunes.'
		elseif isngc
			upload_dialog = qs(0x044a9da2)
		endif
		case 10
