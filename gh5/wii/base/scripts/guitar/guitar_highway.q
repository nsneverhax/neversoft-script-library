button_up_models = [
	{
		green = {
			...
			material_4d_lip = mat_nb_5d_green_rim_pos1
			material_4d_mid = mat_nb_5d_green_cup_pos1
			material_4d_head = mat_nb_5d_green_cap_pos1
			material_4d_head_lit = mat_nb_5d_green_cap_lit_pos1
			material_4d_neck = mat_nb_neck
			material_4d_down = mat_nb_5d_green_cup_dwn_pos1
...
script toggle_debug_highways \{combo = -1}
	ScriptAssert \{'toggle_debug_highways: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script setup_debug_highway \{Player = 1
	...
	if ($game_mode = tutorial)
		<highway_texture> = highway_tutorial
	endif
...
script setup_highway \{Player = 1
	...
	CreateScreenElement {
		...
		Pos = <container_pos>
		alpha = 0
		tags = {curr_x_scale = 1.0 dest_x_scale = 1.0 curr_y_scale = 1.0 dest_y_scale = 1.0 curr_x_pos = (<Pos> [0]) dest_x_pos = (<Pos> [0]) curr_extra_offset_factor = 0 dest_extra_offset_factor = 0 force_update = 0}
	}
	...
	getplayerinfo <Player> highway_streak_texture
	if ($game_mode = tutorial)
		<highway_texture> = highway_tutorial
	endif
	create_highway_material Player = <Player> texture = highway_judy
	...
	formatText checksumName = highlight 'streak_highlightp%p' p = <Player> AddToStringLookup = true
	CreateScreenElement {
		Type = SpriteElement
		id = <highlight>
		parent = <container_id>
		material = <highway_streak_texture>
		Pos = <hpos>
		dims = <hDims>
		blend = add
		just = [left top]
		z_priority = 0.2
		rgba = $hw_color_default
		highway_player = <Player>
		highway_speed = 0.5
	}
	...
	Set2DHighwaySpeed speed = <highway_speed> id = <highlight> Player = <Player>
	...
	gamemode_gettype
	if (<Type> = freeplay)
		<hw_container_id> :obj_spawnscript highway_scale_manager params = {Player = <Player>}
	endif
	if is_roadie_battle_mode
		sidebar_right_ns_meterp1 :se_setprops \{Pos = (-29.0, -235.0)}
		sidebar_right_ns_meterp2 :se_setprops \{Scale = (-0.43, 0.43)
			Pos = (-220.0, -360.0)
			combo_note_leds_container_pos = (-3.0, 5.0)
			hud_meter_combo_multiplier_scale = (-0.65000004, 0.65000004)}
	endif
endscript
...
script destroy_highway 
	...
	formatText checksumName = Name 'streak_highlightp%p' p = <Player>
	if ScreenElementExists id = <Name>
		DestroyScreenElement id = <Name>
	endif
...
script create_highway_prepass_object \{roty = 0}
	ScriptAssert \{'create_highway_prepass_object: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script disable_highway 
	...
	if ScreenElementExists \{id = highway_containerp1}
		SetScreenElementProps \{id = highway_containerp1
			alpha = 0
			time = 0.4}
	endif
	if ScreenElementExists \{id = highway_containerp2}
		SetScreenElementProps \{id = highway_containerp2
			alpha = 0
			time = 0.4}
	endif
	if ScreenElementExists \{id = highway_containerp3}
		SetScreenElementProps \{id = highway_containerp3
			alpha = 0
			time = 0.4}
	endif
	if ScreenElementExists \{id = highway_containerp4}
		SetScreenElementProps \{id = highway_containerp4
			alpha = 0
			time = 0.4}
	endif
	if ScreenElementExists \{id = HUD_2D_Containerp1}
...
script highway_pulse_multiplier_loss \{Player = 1
	...
	if ScreenElementExists id = <container_id>
		if <container_id> :GetSingleTag doing_pulse
			...
			OnExitRun highway_pulse_multiplier_loss_cleanup params = {container_id = <container_id> original_position = <original_position>}
...
script highway_pulse_multiplier_loss_cleanup 
	...
	legacydoscreenelementmorph {id = <container_id> Pos = <original_position> just = [center bottom] time = 0.0}
endscript

script highway_visible 
	ScriptAssert \{'highway_visible: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript

script highway_invisible 
	ScriptAssert \{'highway_invisible: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script get_non_vocalist_player_number 
	...
	ScriptAssert 'No non-vocalist found! %i' i = <Player>
endscript

script player_is_lowest_numbered_guitar_or_bass 
	ScriptAssert \{'player_is_lowest_numbered_guitar_or_bass: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript

script tempo_light_on 
	ScriptAssert \{'tempo_light_on: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script highway_streak_glow \{Enabled = 1}
	if playerinfoequals <Player> part = vocals
		...
		formatText checksumName = left_glow 'Sidebar_NSGLOW_left%p' p = <Player> AddToStringLookup = true
		formatText checksumName = right_glow 'Sidebar_NSGLOW_right%p' p = <Player> AddToStringLookup = true
...
script update_highway_scale \{Player = 1
	...
	extra_bottom_x_scale = ((1.0 + $highway_guitarfp.widthOffsetFactor + <extra_offset_factor>) / (1.0 + $highway_guitarfp.widthOffsetFactor))
	dan_tweak = 1.07
	special_extra_bottom_x_scale = ((1.0 + $highway_guitarfp.widthOffsetFactor + (<extra_offset_factor> * <dan_tweak>)) / (1.0 + $highway_guitarfp.widthOffsetFactor))
	<pos_index> = (<Player> -1)
	...
	formatText checksumName = gem_container 'gem_containerp%p' p = <Player>
	GetScreenElementPosition id = <gem_container>
	X = (<screenelementpos> [0])
	...
	rescalepostable rows = <rows> startY = <startY> height = <height> scaley = <y_scale_factor> pos_index = <pos_index> extrawidthoffsetfactor = <extra_offset_factor> extrabottomxscale = <extra_bottom_x_scale>
	...
	formatText checksumName = highway_sprite 'Highway_2Dp%p' p = <Player>
	SetScreenElementProps id = <highway_sprite> Scale = <hscale> Pos = <hpos>
	formatText checksumName = fretbar_id 'fretbar_containerp%p' p = <Player>
	...
	langle = ($sidebar_angle [(<Player> -1)])
	formatText checksumName = sidebar_left_container 'sidebar_container_leftp%p' p = <Player> AddToStringLookup = true
	SetScreenElementProps id = <sidebar_left_container> Pos = <scaled_lpos> rot_angle = <angle>
	scaled_rpos = ((((0.0 - $sidebar_x [(<Player> -1)]) * <x_scale_factor> * <special_extra_bottom_x_scale>) * (1.0, 0.0)) + ($sidebar_y [(<Player> -1)] * (0.0, 1.0)))
	rangle = (0.0 - <angle>)
	formatText checksumName = sidebar_right_container 'sidebar_container_rightp%p' p = <Player> AddToStringLookup = true
	SetScreenElementProps id = <sidebar_right_container> Pos = <scaled_rpos> rot_angle = (0.0 - <angle>) Scale = <Scale>
	formatText checksumName = sidebar_right_sprite 'sidebar_rightp%p' p = <Player> AddToStringLookup = true
	GetScreenElementProps id = <sidebar_right_sprite>
	sidebar_scale_x = (<Scale> [0])
	sidebar_scale_y = (<Scale> [1])
	...
	switch <num_highways>
		...
		<sidebar_scale_y> = 2.0
		case 2
		<sidebar_scale_y> = 1.9499999
		case 3
		<sidebar_scale_y> = 1.9
		case 4
		<sidebar_scale_y> = 1.8499999
		default
		printf 'Error: Trying to rescale the sidebars for %d highways!' d = <num_highways>
	endswitch
	setpaircomponents Scale X = <sidebar_scale_x> y = <sidebar_scale_y>
	SetScreenElementProps id = <sidebar_right_sprite> Scale = <Scale>
	formatText checksumName = sidebar_left_sprite 'sidebar_leftp%p' p = <Player> AddToStringLookup = true
	GetScreenElementProps id = <sidebar_left_sprite>
	sidebar_scale_x = (<Scale> [0])
	setpaircomponents Scale X = <sidebar_scale_x> y = <sidebar_scale_y>
	SetScreenElementProps id = <sidebar_left_sprite> Scale = <Scale>
	...
	nowbar_scale = (<pcx> * <x_scale_factor> * <special_extra_bottom_x_scale>)
	GetScreenElementPosition id = <gem_container>
	X = (<screenelementpos> [0])
	y = (<highway_playline> - (<highway_playline> * <nowbar_scale>))
	p = (((1.0, 0.0) * <X>) + ((0.0, 1.0) * <y>))
	...
	<scale_pair> = (((1.0, 0.0) * (-1 * (<Scale> [0]))) + ((0.0, 1.0) * (<Scale> [1])))
	<rockmeter_pos> = (((1.0, 0.0) * (-1 * (<rockmeter_pos> [0]))) + ((0.0, 1.0) * (<rockmeter_pos> [1])))
	...
	switch <num_highways>
		...
		<ns_pos_y> = <ns_pos_y>
		case 2
		<ns_pos_x> = (<ns_pos_x> + 10.0)
		<ns_pos_y> = <ns_pos_y>
		case 3
		<ns_pos_x> = (<ns_pos_x> + 2.5)
		<ns_pos_y> = (<ns_pos_y> + 35.0)
		case 4
		<ns_pos_x> = (<ns_pos_x> + 2.5)
		<ns_pos_y> = (<ns_pos_y> + 35.0)
		default
		printf 'Error: Trying to move the note streak meter for %d highways!' d = <num_highways>
	endswitch
	setpaircomponents Pos X = <ns_pos_x> y = <ns_pos_y>
	...
	update_whammy_angles Player = <Player> x_scale = <x_scale_factor> extra_offset_factor = <extra_offset_factor>
endscript

script morph_highway_spawned \{Player = 1
		startscaley = 1.0
		endscaley = 0.5
		startscalex = 1.0
		endscalex = 0.5
	...
	<stepscaley> = ((<endscaley> - <startscaley>) / <numSteps>)
	<stepscalex> = ((<endscalex> - <startscalex>) / <numSteps>)
	stepposx = ((<endposx> - <startposx>) / <numSteps>)
	formatText checksumName = hw_name 'highway_containerp%p' p = <Player>
	begin
	<startscaley> = (<startscaley> + <stepscaley>)
	<startscalex> = (<startscalex> + <stepscalex>)
	<startposx> = (<startposx> + <stepposx>)
	update_highway_scale Player = <Player> y_scale_factor = <startscaley> x_scale_factor = <startscalex>
	GetScreenElementPosition id = <hw_name>
	newpos = (((1.0, 0.0) * <startposx>) + ((0.0, 1.0) * (<screenelementpos> [1])))
	<hw_name> :se_setprops Pos = <newpos>
	Wait <stepframes> gameframes
	repeat (<numSteps> -1)
	update_highway_scale Player = <Player> y_scale_factor = <endscaley> x_scale_factor = <endscalex>
	GetScreenElementPosition id = <hw_name>
	newpos = (((1.0, 0.0) * <startposx>) + ((0.0, 1.0) * (<screenelementpos> [1])))
	<hw_name> :se_setprops Pos = <newpos>
endscript
...
script update_whammy_angles \{Player = 1
	...
	if (<part> = guitar || <part> = bass)
		updatestringangles Player = <Player>
	endif
endscript

script updatestringangles 
	RequireParams \{[
			Player
		]}
	GetArraySize \{$gem_colors}
...
script spawn_morph_highway \{Player = 2}
	formatText checksumName = script_id 'morph_highwayp%p' p = <Player>
	begin
	SpawnScriptNow morph_highway_spawned id = <script_id> params = {
		Player = <Player>
		startscaley = 1.0
		endscaley = 0.8
		startscalex = 1.0
...
script spawn_morph_highway_test_x 
	begin
	SpawnScriptNow \{morph_highway_spawned
		params = {
			Player = 1
			startscaley = 1.0
			endscaley = 1.0
...
script spawn_morph_highway_test_y 
	begin
	SpawnScriptNow \{morph_highway_spawned
		params = {
			Player = 1
...
script update_highway_value 
	gap = (<dest> - <curr>)
	absolutevalue value = <gap>
	abs_gap = <value>
	if (<gap> = 0)
		next = <dest>
		changed = FALSE
	elseif GotParam \{immediate}
		next = <dest>
		changed = true
...
delta_scale = 0.005
delta_pos = 5.5
delta_extra_offset_factor = 0.015
...
script highway_scale_manager \{jamroom_highway = 0}
	...
	begin
	<objID> :GetTags
	update_highway_value dest = <dest_x_scale> curr = <curr_x_scale> max_delta = $delta_scale immediate
	next_x_scale = <next>
	change_x_scale = <changed>
	update_highway_value dest = <dest_y_scale> curr = <curr_y_scale> max_delta = $delta_scale immediate
	next_y_scale = <next>
	change_y_scale = <changed>
	update_highway_value dest = <dest_x_pos> curr = <curr_x_pos> max_delta = $delta_pos immediate
	next_x_pos = <next>
	change_x_pos = <changed>
	update_highway_value dest = <dest_extra_offset_factor> curr = <curr_extra_offset_factor> max_delta = $delta_extra_offset_factor immediate
	next_extra_offset_factor = <next>
	change_extra_offset_factor = <changed>
	if (<change_y_scale> = true || <change_x_scale> = true || <change_extra_offset_factor> = true || <force_update> = 1)
		update_highway_scale Player = <Player> x_scale_factor = <next_x_scale> y_scale_factor = <next_y_scale> extra_offset_factor = <next_extra_offset_factor> jamroom_highway = <jamroom_highway>
		<objID> :SetTags curr_x_scale = <next_x_scale> curr_y_scale = <next_y_scale> curr_extra_offset_factor = <next_extra_offset_factor> force_update = 0
	endif
	if (<change_x_pos> = true)
		GetScreenElementPosition id = <objID>
		next_pos = (((1.0, 0.0) * <next_x_pos>) + ((0.0, 1.0) * (<screenelementpos> [1])))
		<objID> :se_setprops Pos = <next_pos>
		<objID> :SetTags curr_x_pos = <next_x_pos>
