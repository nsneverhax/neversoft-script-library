...
script init_standard_highway_hud 
	...
	if (is_roadie_battle_mode)
		...
		<flip_scale> = (1.0, 1.0)
		<team_name_rgba> = [255 128 128 255]
		<frame_bg_rgba> = [255 128 128 255]
		<team_name_just> = [left , bottom]
		<player_save> = <Player>
		getfirstplayer \{on_screen}
		if (<player_save> = <Player>)
			<flip_scale> = (-1.0, 1.0)
			<team_name_rgba> = [128 128 255 255]
			<frame_bg_rgba> = [128 128 255 255]
			<team_name_just> = [right , bottom]
			<roadie_icon_texture_id> = `rb-wiihud-portrait-p1-temp`
		else
			<roadie_icon_texture_id> = `rb-wiihud-portrait-p2-temp`
		endif
		ui_song_breakdown_get_player_display_name player_index = <player_save>
		CreateScreenElement {
			parent = <highway_hud_root>
			id = <hud_id>
			Type = descinterface
			desc = 'Roadie_Battle_Hud_Icons'
			...
			team_name_text = <player_name_text>
			team_name_rgba = <team_name_rgba>
			team_name_internal_just = <team_name_just>
			frame_bg_rgba = <frame_bg_rgba>
			roadie_icon_texture = <roadie_icon_texture_id>
		}
	endif
endscript
...
script setup_sidebar_sp_meter 
	...
	formatText checksumName = left_sp_id 'sidebar_left_sp_meterp%p' p = <Player> AddToStringLookup = true
	CreateScreenElement {
		Type = descinterface
		desc = 'hud_sidebar_starpowermeter'
		id = <left_sp_id>
	...
	hud_attach_widget_sidebar_starpowermeter Player = <Player> desc_id = <id>
	formatText checksumName = hud_cont_r 'sidebar_container_right_hudp%p' p = <Player> AddToStringLookup = true
	formatText checksumName = right_sp_id 'sidebar_right_sp_meterp%p' p = <Player> AddToStringLookup = true
	<scale_pair> = (((1.0, 0.0) * (-1 * <Scale>)) + ((0.0, 1.0) * <Scale>))
	CreateScreenElement {
		Type = descinterface
...
script kill_highway_hud \{Player = 1}
	ScriptAssert \{'kill_highway_hud: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript

script gethudmessageparentid \{Player = 1}
	ScriptAssert \{'getHUDMessageParentID: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script turn_on_touch_glow_for_drummer_sp \{Player = 1}
	getplayerinfo <Player> part
	if NOT (<part> = drum)
		return
	endif
	...
	if NOT (<lefthanded_gems> = 1)
		color_count = <array_index>
	else
		color_count = ((<array_Size> - 2) - <array_index>)
	endif
	...
	if GotParam \{in}
		if ScreenElementExists id = <touch_glow>
			SpawnScriptNow fade_touch_glow params = {touch_glow = <touch_glow> rgba = ($touch_glow_props [<array_index>].rgba) in}
		endif
	else
		if ScreenElementExists id = <touch_glow>
			SpawnScriptNow fade_touch_glow params = {touch_glow = <touch_glow> rgba = ($touch_glow_props [<array_index>].rgba)}
		endif
	endif
endscript
...
script check_hud_parent \{Player = 1}
	ScriptAssert \{'check_hud_parent: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript

script test_hud_message \{Player = 1}
	ScriptAssert \{'test_hud_message: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
