...
script setup_band_hud \{force_layout = 0}
	...
	if StructureContains \{structure = $g_hud_2d_struct_used
		...
		if (is_roadie_battle_mode)
			if hud_root :desc_resolvealias \{Name = alias_faceoff_meter
					param = faceoff_meter}
				<faceoff_meter> :se_setprops desc = 'HUD_roadie_battle_meter' Pos = {(0.0, -125.0) relative} z_priority = 40
				hud_attach_widget_band_versus_meter parent_id = <faceoff_meter>
			endif
		else
	...
	elseif (<num_vocalists_shown> = 1)
		...
		if (<num_players_shown> = 1)
			...
			Change \{sysnotify_ingame_position = centerleft}
		endif
	else
		Change \{sysnotify_ingame_position = centerleft}
	endif
...
script get_player_by_part 
	ScriptAssert \{'get_player_by_part: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script update_hud_layout 
	...
	if (is_roadie_battle_mode)
		printf \{'ROADIE BATTLE!'}
		<desired_layout> = hud_roadie_battle
	endif
...
script reset_hud 
	ScriptAssert \{'reset_hud: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script update_score_meter_spawned 
	ScriptAssert \{'update_score_meter_spawned: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
