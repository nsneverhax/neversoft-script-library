...
script Skate8_SFX_Request_Background_Area_Change 
	ScriptAssert \{'Skate8_SFX_Request_Background_Area_Change: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script Skate8_SFX_Background_Play \{crossfadetime = 0.7
	...
	if NOT ($SFX_Background_Current_Checksum = $SFX_Background_Requested_Checksum)
		...
		if NOT ($crowd_in_jam_mode_song_state = 1)
			printf \{channel = sfx
				qs(0x95d13fb2)}
			PlaySound <backchecksumname> vol_percent = <newvolumepercent> pan1x = -1 pan1y = -1 pan2x = 1 pan2y = -1 Priority = 105 buss = Crowd_Beds attack_length = <fadeintime> decay_length = 0 release_length = 0 attack_function = <fadeintype> num_loops = -1
		else
			printf \{channel = sfx
				qs(0x3cad13d5)}
		endif
...
script One_Shot_SoundEvent \{waitTime = 15
	...
	if NOT ((z_studio = <pak>) || (z_soundcheck = <pak>) || (z_board_room = <pak>))
