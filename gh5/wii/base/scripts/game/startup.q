...
script network_driver_startup 
	printf \{qs(0xb58f8d76)}
	NetSessionFunc \{func = write_dnas_binary}
	printf \{qs(0xaddf7bad)}
	printf \{qs(0xc4309d38)}
	if loadnetworkdrivers \{viewer_ip = $viewer_ip
			viewer_subnet = $viewer_subnet
			viewer_gateway = $viewer_gateway}
		printf \{qs(0xc3582ca9)}
	else
		printf \{qs(0xf9912577)}
	endif
	printf \{qs(0xaddf7bad)}
endscript

script Set_Outline_Shader_to_Default 
	ScriptAssert \{'Set_Outline_Shader_to_Default: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
