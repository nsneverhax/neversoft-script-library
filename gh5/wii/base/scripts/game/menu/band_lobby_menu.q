		...
		if ($g_leader_player_num > 0)
			...
			if (<managing> = 0 || (<managing> = 1 && <num_players> != 1) || <has_managed> = 1)
				if NOT is_roadie_battle_mode
					add_user_control_helper controller = <controller> text = ($band_lobby_strings.button_players) button = yellow z = 100000
				endif
				if ((band_lobby_live_profile_ingame) || (isngc))
					if ($g_ngc_friends_enabled = 1)
						add_user_control_helper controller = <controller> text = ($band_lobby_strings.button_friends) button = blue z = 100000
					endif
				endif
			endif
			if (<managing> = 0)
...
script band_lobby_update_menu 
	...
	if ScreenElementExists id = ($band_lobby_viewport_menus [<submenu_index>])
		...
		if ($<submenu_data>.is_managed = 1)
			...
			($<submenu_data>.desc_id) :se_setprops lobby_frame_sm_rgba = <rgba_md>
		endif
		band_lobby_get_player_color viewport = <submenu_index>
...
script band_lobby_update_submenu_pad_handlers 
	...
	if NOT freestyle_is_active
		...
		if (<add_shortcuts> = 1)
			if NOT is_roadie_battle_mode
				if ($g_ngc_friends_enabled = 1)
					AddArrayElement array = <array> element = {pad_option band_lobby_submenu_pad_handler_wrapper params = {choose_script = band_lobby_friendslist submenu_index = <viewport_index> params = <standard_params>}}
				endif
				AddArrayElement array = <array> element = {pad_option2 band_lobby_submenu_pad_handler_wrapper params = {choose_script = band_lobby_players submenu_index = <viewport_index> params = <standard_params>}}
			endif
