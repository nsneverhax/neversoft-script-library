...
script band_lobby_continue_check 
	...
	if (<check_type> = net)
		...
		if ((isps3) || (isngc))
			if NOT ((CheckForSignIn controller_index = $primary_controller dont_set_primary) && (islobbyconnected default_lobby))
	...
	if band_lobby_continue_check check_type = <check_type> Player = <Player> submenu_index = <viewport_index> private = <private>
		if checksumequals a = ($g_lobby_state) b = vs
			if is_roadie_battle_mode
				ui_roadie_battle_choose_sides_try_continue
				return
			elseif band_lobby_check_continue_private_competitive
...
script band_lobby_check_controllers 
	...
	if NOT band_lobby_check_single_controller Player = <Player>
		band_lobby_change_focus_submenu_all \{focus_type = unfocus}
		data = {device_num = <controller> is_popup}
		if is_roadie_battle_mode
			data = {<data> from_roadie_battle_choose_sides}
		endif
