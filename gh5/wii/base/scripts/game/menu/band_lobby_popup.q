	...
	if current_band_lobby_popup :desc_resolvealias \{Name = alias_pu_menu
		...
		if ((isps3) || (isngc))
			if isngc
				...
				invite_handler = [
					{pad_option2 band_lobby_friendslist_view_invites}
				]
			endif
			if NOT InNetGame
...
script band_lobby_popup_change_focus 
	...
	if checksumequals a = <focus_type> b = focus
		<objID> :se_setprops {
			...
			focus_highlight_cont_alpha = 1.0
...
script band_lobby_popup_create_submenu 
	...
	if <id> :desc_resolvealias Name = alias_game_rules_vmenu param = menu_id
		...
		endswitch
		num_items = 0
		unfocusable_color = [30 30 30 255]
		<id> :desc_resolvealias Name = alias_band_lobby_item_boot param = item_id
		create_2d_spring_system desc_id = <item_id> num_springs = 2 stiffness = 50 rest_length = 1
		if InNetGame
			kick = 0
		endif
		if (<kick> = 1)
