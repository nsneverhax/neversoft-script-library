...
script freestyle_drum_hit \{drum = 0}
	...
	if (($freestyle_player_data [<Player>].has_highway = true) && (<auto_play_note> = FALSE))
		...
		freestyle_ring_drum_light gem_mask = <gem_mask> player_index = <Player>
		freestyle_stats_drum_note_played <...>
	elseif (($freestyle_player_data [<Player>].input_source = recording) && ($freestyle_player_data [<Player>].author_type = human))
		freestyle_ring_drum_light gem_mask = <gem_mask> player_index = <Player>
	endif
