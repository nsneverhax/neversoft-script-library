...
script create_character_select_menu \{Player = 1}
	...
	if find_profile_by_id id = ($<player_status>.character_id)
		...
		formatText checksumName = 0xb2f2fa1b 'xb360demo_disabled_text_p%p' p = <Player>
		if ScreenElementExists id = <0xb2f2fa1b>
			if StructureContains structure = <profile_struct> 0x708a6d72
				SetScreenElementProps id = <0xb2f2fa1b> alpha = 0
			endif
		endif
	endif
	...
	i = 0
	begin
	get_valid_character_index char_index = ($<cs_jumped> + <i>)
	get_musician_profile_struct index = <index>
	if StructureContains structure = <profile_struct> 0x708a6d72
		0x28fa5dee = {}
	elseif (<i> > 7)
	...
	if (<Player> = 1)
		...
		if ($is_network_game)
			player_name_pos = (<cs_menu_pos> + (675.0, 0.0))
		endif
		player_title_pos = (<cs_menu_pos> + (675.0, -30.0))
		player_title_num_pos = (<cs_menu_pos> + (675.0, -30.0))
	endif
	<icon_id> :GetTags
...
script cs_scroll_up_or_down \{Player = 1
	...
	if ScreenElementExists id = <0xb2f2fa1b>
		...
		if StructureContains structure = <profile_struct> 0x708a6d72
			...
			SetScreenElementProps id = <0xb2f2fa1b> alpha = 1
		endif
		if (<Player> = 1)
			GUITARIST :Hide
		else
...
script 0x4a5fc87f \{char_index = 0}
	get_musician_profile_size
	index = 0
	begin
	get_musician_profile_struct index = <index>
	...
	if NOT StructureContains structure = (<profile_struct>) 0x708a6d72
		valid = 0
	endif
	if NOT (<profile_struct>.Type = GUITARIST)
		valid = 0
...
script 0x441fbd40 
	array_entry = 0
	begin
	if NOT 0x4a5fc87f char_index = <array_entry>
		break
	endif
	<array_entry> = (<array_entry> + 1)
	repeat
