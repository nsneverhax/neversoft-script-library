...
script check_for_star_power 
	0xa7f9f530 player_status = <player_status>
	Change \{p1_star_ready = 0}
	Change \{p2_star_ready = 0}
	do_star = 0
	...
	0x9ab05a43 = 0
	begin
	if NOT 0x6787173f \{test = gamemodeisp2careerorcoop}
		begin
		if 0x6787173f \{test = starpowerusable}
			break
		endif
		Wait \{1
			gameframe}
	...
	if 0x6787173f \{test = starpoweramount}
		...
		if NOT ($ui_flow_manager_state [(<Player> -1)] = online_pause_fs)
			if IsGuitarController controller = ($<player_status>.controller)
				...
				if (<righty> <= $<player_status>.star_tilt_threshold)
					<0x9ab05a43> = (<0x9ab05a43> + 1)
				else
					<0x9ab05a43> = 0
				endif
				if (<0x9ab05a43> >= $star_power_trigger_frames)
					<0xe23255e6> = 1
					<do_star> = 1
					<guitar_used_select> = 0
					Change GlobalName = <0x87381641> NewValue = 1
					<0x9ab05a43> = 0
					if 0x6787173f \{test = gamemodeisp2careerorcoop}
						if (($<player_status>.Player) = 1)
							SpawnScriptNow \{show_coop_raise_axe_for_starpower
								params = {
		...
		if (<do_star> = 1)
			if NOT 0x6787173f \{test = gamemodeisp2careerorcoop}
				if NOT ($boss_battle = 1 && $<player_status>.Player = 2)
					SpawnScriptNow wait_for_inactive params = {player_status = <player_status> guitar_used_select = <guitar_used_select>}
				endif
			endif
		endif
		if ($boss_battle = 1)
			...
			if ($game_mode = p2_battle ||
				...
				if ($game_mode = p2_battle)
					if (($is_network_game) && (($<player_status>.Player) = 1))
						if (($<player_status>.current_num_powerups - 1) >= 0)
