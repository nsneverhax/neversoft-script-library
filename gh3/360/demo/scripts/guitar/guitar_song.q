...
script preload_song \{starttime = 0
	...
	if StructureContains structure = <song_struct> Name = band_playback_volume
		setsoundbussparams {band_Balance = {vol = ((<song_struct>.band_playback_volume) - 2.5)}} time = <fadeintime>
	else
		setsoundbussparams {band_Balance = {vol = -2.5}} time = <fadeinttime>
	endif
	if StructureContains structure = <song_struct> Name = guitar_playback_volume
		setsoundbussparams {Guitar_Balance = {vol = ((<song_struct>.guitar_playback_volume) - 2.5)}} time = <fadeintime>
	else
		setsoundbussparams {Guitar_Balance = {vol = -2.5}} time = <fadeintime>
	endif
