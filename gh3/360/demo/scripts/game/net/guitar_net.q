...
script 0x239941f9 
	printf "TMR - repairing strings %f" f = <flags>
	check_button = 65536
	GetArraySize \{$gem_colors}
	array_count = 0
	begin
...
script online_win_song 
	...
	if (<quit_early>)
		Change \{player2_present = 0}
		ExtendCrc \{hud_destroygroup_window
			'p1'
			out = hud_destroygroup}
...
script send_leader_board_message 
	...
	elseif ($game_mode = p2_career)
		...
		switch ($current_difficulty2)
			case easy
			<0x30928930> = 1
			<0x12cd2ded> = lb_coop_career_easy
			case medium
			<0x30928930> = 2
			<0x12cd2ded> = lb_coop_career_med
			case hard
			<0x30928930> = 3
			<0x12cd2ded> = lb_coop_career_hard
			case expert
			<0x30928930> = 4
			<0x12cd2ded> = lb_coop_career_expert
		endswitch
		get_progression_globals game_mode = ($game_mode)
		summation_career_score tier_global = <tier_global>
		Player_list = [
...
script online_song_end_write_stats \{song_type = single}
	...
	endswitch
	if (<song_type> = coop)
		formatText checksumName = leaderboard_id 'lb_coop_%s' s = ($net_setlist_songs [($net_song_num)]) DontAssertForChecksums
	else
...
script net_get_game_mode_leaderboard_id 
	switch ($game_mode)
		case p2_faceoff
		formatText checksumName = leaderboard_id_p1 'lb_faceoff_%s' s = ($current_difficulty) DontAssertForChecksums
		formatText checksumName = leaderboard_id_p2 'lb_faceoff_%s' s = ($current_difficulty2) DontAssertForChecksums
		case p2_pro_faceoff
		formatText checksumName = leaderboard_id_p1 'lb_pro_faceoff_%s' s = ($current_difficulty) DontAssertForChecksums
		formatText checksumName = leaderboard_id_p2 'lb_pro_faceoff_%s' s = ($current_difficulty2) DontAssertForChecksums
		case p2_battle
		formatText checksumName = leaderboard_id_p1 'lb_battle_%s' s = ($current_difficulty) DontAssertForChecksums
		formatText checksumName = leaderboard_id_p2 'lb_battle_%s' s = ($current_difficulty2) DontAssertForChecksums
	endswitch
...
script CreateJoinRefusedDialog 
	...
	CreateScreenElement {
		...
		event_handlers = [
			...
			{pad_choose DestroyScreenElement params = {id = net_popup_container}}
			{pad_choose DestroyScreenElement params = {id = connection_dialog_scroller}}
		]
...
script ui_print_gamertag Name = "NO GAMERTAG!" start_pos = (235.0, 100.0) Color = ($player1_color) Player = 1 dims = (450.0, 35.0) z = 60 offscreen = 0
	...
	if (<Player> = 2 && <Platform> = Xenon)
		...
		CreateScreenElement {
			...
			Pos = (<Pos> + (<height> * (0.0, 1.0)) + (<tag_width> * (1.0, 0.0)) + (-40.0, 10.0))
		}
		displayText {
...
script set_other_player_present 
	...
	ui_print_gamertag Name = ($opponent_gamertag) start_pos = (1065.0, 50.0) Color = ($player2_color) Player = 2 just = [right top] dims = (450.0, 35.0)
endscript
...
script update_net_character 
	player_status = player2_status
	change_made = 0
	if NOT ($<player_status>.character_id = <char_data>.char_id)
		Change structurename = <player_status> character_id = (<char_data>.char_id)
		<change_made> = 1
	endif
	if NOT ($<player_status>.outfit = <char_data>.outfit)
		Change structurename = <player_status> outfit = (<char_data>.outfit)
		<change_made> = 1
	endif
	if NOT ($<player_status>.style = <char_data>.style)
		Change structurename = <player_status> style = (<char_data>.style)
		<change_made> = 1
	endif
	if NOT ($<player_status>.instrument_id = <char_data>.instrument_id)
		get_musician_instrument_type desc_id = (<char_data>.instrument_id)
		if (<instrument_type> = guitar)
			if NOT (<char_data>.instrument_id = ($player2_selected_guitar))
				Change player2_selected_guitar = (<char_data>.instrument_id)
			endif
		else
			if NOT (<char_data>.instrument_id = ($player2_selected_guitar))
				Change player2_selected_bass = (<char_data>.instrument_id)
			endif
		endif
		Change structurename = <player_status> instrument_id = (<char_data>.instrument_id)
		<change_made> = 1
	endif
	if (<change_made> = 1)
		Change \{generic_select_monitor_p2_changed = 1}
...
script net_dl_content_compatabilty_warning \{parent = root_window
		z = 100
		Pos = (0.0, 0.0)}
	CreateScreenElement {
		Type = ContainerElement
