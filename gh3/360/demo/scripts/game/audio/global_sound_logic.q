...
script GH_Star_Power_Verb_On 
	...
	setsoundbusseffects effect = {effect = echo Name = GuitarEcho1 delay = <beat_duration> DecayRatio = 0.15 Drymix = 1.0 Wetmix = 0.5} time = 0.1
		...
		if ($boss_battle = 1)
			...
			if (($game_mode = p2_faceoff) || ($game_mode = p2_pro_faceoff))
				...
				printf \{channel = sfx
					...
					s = $current_crowd}
				if ($current_crowd > 1)
					transition_sfx_right_side state = <state>
				else
					transition_sfx_left_side state = <state>
				endif
			else
