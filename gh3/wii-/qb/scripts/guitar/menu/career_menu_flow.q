...
script career_autosave_or_setlist 
	...
	if (<autosave> = 1)
		if ($progression_unlock_tier_last_song = 1)
			hack_ps2_set_saveload_successor_checksum \{career_select_venue_fs}
		else
			hack_ps2_set_saveload_successor_checksum \{career_setlist_fs}
		endif
		return \{flow_state = career_autosave_fs}
	else
		if ($progression_unlock_tier_last_song = 1)
