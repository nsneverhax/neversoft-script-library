...
GH3_Career_Songs = {
	...
	tier6 = {
		title = "   The Hottest band on Earth"
		songs = [
			pridenjoy
			blackmagicwoman
			monsters
...
script Progression_RestrictSong \{restricted = 1}
	setlist_prefix = ($<tier_global>.prefix)
	Tier = 1
	begin
	formatText checksumName = tier_checksum 'tier%s' s = <Tier>
	...
	if ($<tier_global>.<tier_checksum>.songs [<array_count>] = <song>)
		formatText checksumName = song_checksum '%p_song%i_tier%s' p = <setlist_prefix> i = (<array_count> + 1) s = <Tier>
		printf "progression_restrictsong: song_checksum = %a restricted = %b" a = <song_checksum> b = <restricted>
		SetGlobalTags <song_checksum> params = {restricted = <restricted>}
		return
	endif
	array_count = (<array_count> + 1)
	repeat <array_Size>
...
script Progression_IsSongRestricted 
	setlist_prefix = ($<tier_global>.prefix)
	Tier = 1
	begin
	formatText checksumName = tier_checksum 'tier%s' s = <Tier>
