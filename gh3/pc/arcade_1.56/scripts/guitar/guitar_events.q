...
script check_first_note_formed 
	...
	begin
	printf "check_first_note_formed: controller = %a primary = %b" a = ($<player_status>.controller) b = $primary_controller
...
script GuitarEvent_SongFailed_Spawned 
	...
	PauseGame
	unpausespawnedscript \{update_audit_timers}
	unpausespawnedscript \{display_time_message}
	unpausespawnedscript \{display_lamp_message}
	unpausespawnedscript \{watch_for_switches}
	unpausespawnedscript \{service_display}
	unpausespawnedscript \{autoplay_input}
	unpausespawnedscript \{display_volume_message}
...
script GuitarEvent_SongWon_Spawned 
	...
	destroy_menu \{menu_id = yourock_text}
	destroy_menu \{menu_id = yourock_text_2}
	destroy_menu \{menu_id = freegame_text}
	tie = FALSE
	text_pos = (640.0, 360.0)
	rock_legend = 0
	fit_dims = (350.0, 0.0)
	...
	if (<freegame> = 1)
		split_text_into_array_elements {
			id = freegame_text
			text = "Free Game!"
			text_pos = (<text_pos> + (0.0, 180.0))
			space_between = (35.0, 0.0)
			flags = {
				rgba = [255 255 255 255]
				Scale = 1.0
				z_priority = 95
