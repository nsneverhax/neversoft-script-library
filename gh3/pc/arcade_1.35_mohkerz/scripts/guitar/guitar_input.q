...
script check_buttons_bot 
	...
	if ($autoplay_mode_enabled)
		miss_percent = $autoplay_miss_percent
		if ($<player_status>.Player = 2)
			miss_percent = $autoplay_miss_percent2
		endif
		GetRandomValue \{Name = random_flip
			integer
			a = 0
			b = 99}
		if (<random_flip> >= 0 && <random_flip> < (0.5 * <miss_percent>))
			Change structurename = <player_status> bot_pattern = 0
			Change structurename = <player_status> bot_strum = 0
		elseif (<random_flip> >= (0.5 * <miss_percent>) && <random_flip> < (1.0 * <miss_percent>))
			Change structurename = <player_status> bot_pattern = 0
			Change structurename = <player_status> bot_strum = 1
		endif
	endif
