...
script create_crowd_models 
	if IsWinPort
		...
		if ((<crowdOption> = 2) && ((<pak> = z_budokan) || (($game_mode = p2_career) || ($game_mode = p2_faceoff) || ($game_mode = p2_pro_faceoff) || ($game_mode = p2_coop))))
			printf \{"Half crowd in multi and budokan"}
			<crowdOption> = 1
		endif
	endif
	if ($disable_crowd = 1)
