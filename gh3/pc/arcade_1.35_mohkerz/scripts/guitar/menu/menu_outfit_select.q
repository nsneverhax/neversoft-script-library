...
script create_select_outfit_menu \{Player = 1}
	...
	if (<Player> = 1)
		...
		if ($is_network_game = 1)
			if ScreenElementExists \{id = vmenu_select_outfit_p2}
	...
	formatText checksumName = players_timer_name 'outfit_watch_timer_p%d' d = <Player>
...
script destroy_select_outfit_menu \{Player = 1}
	...
	formatText checksumName = players_timer_name 'outfit_watch_timer_p%d' d = <Player>
	printf "destroy_select_outfit_menu: killing script %s" s = <players_timer_name>
	KillSpawnedScript Name = <players_timer_name>
endscript

script outfit_watch_timer_p1 
	begin
	...
	Wait \{1
		Second}
	repeat
	printf \{"outfit_watch_timer_p1: exit"}
	if ($os_made_first_selection_bool_p1 = 0)
		SpawnScriptNow \{select_outfit_go_back
			params = {
				Player = 1
			}}
	else
		SpawnScriptNow \{select_outfit_style_choose
			params = {
				Player = 1
			}}
	endif
...
script outfit_watch_timer_p2 
	...
	Wait \{1
		Second}
	repeat
	printf \{"outfit_watch_timer_p2: exit"}
...
script select_outfit_choose \{Player = 1
	...
	printf "select_outfit_choose: enter for player %d" d = <Player>
	if (<Player> = 1)
...
script select_outfit_create_style_menu \{Player = 1}
	...
	if (<Player> = 1)
		...
		if ($is_network_game = 1)
			if ScreenElementExists \{id = vmenu_outfit_style_menu_p2}
	...
	if (<Player> = 1)
		...
		if ScreenElementExists \{id = scrolling_select_outfit_p2}
			LaunchEvent \{Type = unfocus
...
script destroy_select_style_menu \{Player = 1}
	...
	if ($current_num_players = 1)
		...
		if ScreenElementExists \{id = scrolling_outfit_style_menu_p1}
			LaunchEvent \{Type = unfocus
		...
		if (<Player> = 1)
			...
			if ScreenElementExists \{id = scrolling_outfit_style_menu_p2}
				LaunchEvent \{Type = unfocus
...
script select_outfit_style_choose \{Player = 1
		needs_net_message = 1}
	printf "select_outfit_style_choose: player %d" d = <Player>
	if (<Player> = 1)
