...
script ui_flow_manager_respond_to_action \{device_num = 0
		...
		play_sound = 1}
	printf "ui_flow_manager_respond_to_action called heading to %s with play_sound=%d" s = <action> d = <play_sound>
	if ($transitions_locked = 1 || $menu_flow_locked = 1)
		printf \{"ui_flow_manager_respond_to_action: transitions or menu flow locked, exiting"}
		return
	endif
