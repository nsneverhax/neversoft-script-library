...
script create_choose_part_menu 
	...
	if ($is_network_game)
		...
		if ScreenElementExists \{id = si_vmenu_p1}
		...
		if ScreenElementExists \{id = si_vmenu_p2}
			LaunchEvent \{Type = focus
				target = si_vmenu_p2}
		endif
	endif
	...
	SpawnScriptNow \{choose_part_watch_timer}
endscript

script destroy_choose_part_menu 
	printf \{"destroy_choose_part_menu"}
	KillSpawnedScript \{Name = choose_part_watch_timer}
	destroy_gamertags
	destroy_ready_icons
...
script choose_part_watch_timer 
	...
	Wait \{1
		Second}
	repeat
	printf \{"choose_part_watch_timer: p1_index = %d, p2_index = %e"
		d = $g_si_player1_index
		e = $g_si_player2_index}
	if ($p1_ready = 0)
		SpawnScriptNow \{choose_part_menu_select_part
			params = {
				Player = 1
			}}
		Wait \{0.2
...
script choose_part_menu_select_part 
	...
	if (<Player> = 1)
		printf \{"choose_part_menu_select_part: called for player 1 with index = %e"
			e = $g_si_player1_index}
		if ($p1_ready = 0)
			Change \{p1_ready = 1}
		...
		printf \{"choose_part_menu_select_part: called for player 2 with index = %e"
			e = $g_si_player2_index}
		if ($p2_ready = 0)
			Change \{p2_ready = 1}
		...
		if ($p2_ready = 1)
			...
			if ($g_si_player1_index = 0)
				Change \{structurename = player1_status
					part = guitar}
				printf \{"choose_part_menu_select_part: setting p1 to guitar"}
				...
				Change \{structurename = player1_status
					part = rhythm}
				if (<rhythm_track> = 1)
					printf \{"choose_part_menu_select_part: setting p1 to rhythm"}
					Change structurename = player1_status instrument_id = <p1_guitar>
					if find_profile_by_id id = ($player1_status.character_id)
						get_musician_profile_struct index = <index>
						formatText checksumName = default_characterguitartag1 'character_%c_player_1_guitar_tags' c = (<profile_struct>.Name)
					...
					printf \{"choose_part_menu_select_part: setting p1 to bass"}
			...
			if ($g_si_player2_index = 0)
				Change \{structurename = player2_status
					part = guitar}
				printf \{"choose_part_menu_select_part: setting p2 to guitar"}
				...
				Change \{structurename = player2_status
					part = rhythm}
				if (<rhythm_track> = 1)
					printf \{"choose_part_menu_select_part: setting p2 to rhythm"}
					Change structurename = player2_status instrument_id = <p2_guitar>
					if find_profile_by_id id = ($player2_status.character_id)
						get_musician_profile_struct index = <index>
						formatText checksumName = default_characterguitartag2 'character_%c_player_2_guitar_tags' c = (<profile_struct>.Name)
					...
					printf \{"choose_part_menu_select_part: setting p2 to bass"}
...
script select_instrument_scroll \{Dir = down
		overridelock = 0}
	printf "select_instrument_scroll: called for player %d" d = <Player>
	if (<Player> = 1)
