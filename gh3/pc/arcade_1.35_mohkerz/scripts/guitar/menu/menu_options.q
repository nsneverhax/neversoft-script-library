...
script create_options_menu 
	...
	if ($rt_debug_watchdog_info_mode)
		if (<isWDPresent> = FALSE)
			...
			formatText TextName = op_watchdog_status_text "WATCHDOG: dead                         " d = <wd_att_tries> e = <wd_nf_tries> f = <wd_msg_sent_count> g = <wd_msg_recd_count> s = <watchdog_char>
		endif
	else
		if (<isWDPresent> = FALSE)
			formatText TextName = op_watchdog_status_text "WATCHDOG: not found %s" s = <watchdog_char>
		else
			formatText TextName = op_watchdog_status_text "WATCHDOG: dead      " s = <watchdog_char>
		endif
	endif
...
script options_change_index \{move = up}
	printf "options_change_index: index = %d, move = %e" d = ($menu_options_current_index) e = <move>
	...
	index = ($menu_options_current_index)
	printf "options_change_index: index now %d" d = <index>
