...
script create_corrupted_data_menu 
	memcard_cleanup_messages
	if ($MemcardSavingOrLoading = saving)
		text = "Save failed. Player data, high scores, and pricing information will not be retained after power down."
		options = [
			{
		...
		text = "Load failed. Player data, high scores, and pricing information are damaged. Default settings are being applied."
		options = [
			{
...
script create_backup_data_menu 
	memcard_cleanup_messages
	GetPlatform
	switch <Platform>
		case PS3
		...
		case Xenon
		text = "Player data, high scores, and pricing information are damaged. Last successfully saved settings are being applied."
	endswitch
	options = [
		{
			func = memcard_sequence_quit
...
script create_delete_file_menu 
	...
	switch <Platform>
		...
		case Xenon
		create_popup_warning_menu \{title = "CLEARING..."
			textblock = {
				text = "Clearing content."
			}}
	endswitch
endscript
