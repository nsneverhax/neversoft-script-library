...
script lightshow_iterator 
	...
	if NOT (<array_Size> = 0)
		...
		if NOT (<array_Size> = 0)
			...
			printf "BeginProcessBlock: time=%a note=%b length=%c" a = ($<event_array> [<array_entry>] [0]) b = ($<event_array> [<array_entry>] [1]) c = ($<event_array> [<array_entry>] [2])
			if LightShow_BeginProcessBlock {time = ($<event_array> [<array_entry>] [0])
				...
				switch <process_mode>
					...
					LightShow_PassEvent
					case Scr
					printf "lightshow_iterator: eventscr = %d" d = <eventscr>
					printstruct <eventParams>
