...
script preload_song \{starttime = 0
	...
	printf "preload_song: play_audio=%d" d = <play_audio>
	...
	if (<play_audio> = 1)
		printf \{"preload_song: we want audio"}
	...
	printf \{"preload_song: checking all volumes"}
...
script SongUnloadFSB 
	...
	if NOT ($song_fsb_id = -1)
		printf \{"SongUnLoadFSB: pre unloadfsb song_fsb_id = %a"
			a = $song_fsb_id}
		UnLoadFSB \{fsb_index = $song_fsb_id}
		printf \{"SongUnLoadFSB: post unloadfsb"}
		Change \{song_fsb_id = -1}
		Change \{song_fsb_name = 'none'}
	endif
endscript
