...
arcade_not_ok_to_accept_start_switches = 1
arcade_use_mating = 1
arcade_in_game = 0
arcade_in_menu = 0
in_multiplayer_wait_screen = 0
in_continue_screen = 0
coin_handling_enabled = 1
can_have_attract_audio = 0
...
script watch_for_switches 
	...
	if ($test_options_hack_enabled = 1)
		...
		if (<anything_waiting> = 1)
			...
			if ($is_attract_mode = 1 && $is_test_mode = 0 && $is_changing_levels = 0)
				...
				printf \{"switch: %a | test: %b"
					a = $in_switch_test_menu
					b = $is_test_mode}
				if ($in_switch_test_menu = 1)
					SpawnScriptNow \{ui_flow_manager_respond_to_action
						params = {
							action = continue
						}}
				elseif ($is_test_mode = 1)
					LaunchEvent \{Type = pad_choose}
				elseif ($is_changing_levels = 0)
					KillSpawnedScript \{Name = test_in_game_message}
					SpawnScriptNow \{test_in_game_message}
				endif
...
script volume_adjust_message 
	...
	formatText TextName = va_text "VOL ADJ: %p%a" p = <va_plus> a = <adj_val>
