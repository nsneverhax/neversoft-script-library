...
endscript
showing_raise_axe_p1 = 0
showing_raise_axe_p2 = 0

script show_coop_raise_axe_for_starpower 
	if ($<player_status>.star_power_used = 1 ||
			$is_attract_mode = 1 || $Cheat_PerformanceMode = 1 || $playing_song = 0)
		return
	endif
	if ($<player_status>.Player = 1 && $showing_raise_axe_p1 = 1)
		return
	endif
	if ($<player_status>.Player = 2 && $showing_raise_axe_p2 = 1)
		return
	endif
	if ($<player_status>.Player = 1)
		Change \{showing_raise_axe_p1 = 1}
	endif
	if ($<player_status>.Player = 2)
		Change \{showing_raise_axe_p2 = 1}
	endif
	ExtendCrc coop_raise_axe ($<player_status>.text) out = id
	ExtendCrc coop_raise_axe_cont ($<player_status>.text) out = id_cont
	if ($current_num_players = 1)
		original_pos = (($hud_screen_elements [10].Pos) + (335.0, 45.0))
		original_pos_cont = (($hud_screen_elements [10].Pos) + (335.0, 75.0))
	else
		if ($<player_status>.Player = 1)
...
script reset_star_array 
	...
	if ($game_mode = p2_battle ||
		...
		formatText TextName = song_text '%s_%p%d_starbattlemode' s = <song_prefix> p = <part> d = <difficulty_text_nl> AddToStringLookup
		...
		formatText TextName = song_text '%s_%p%d_star' s = <song_prefix> p = <part> d = <difficulty_text_nl> AddToStringLookup
	endif
	printf "reset_star_array: current_song_star_array = %a" a = <song_text>
