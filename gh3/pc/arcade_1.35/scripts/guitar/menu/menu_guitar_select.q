...
script create_select_guitar_menu \{Player = 1
	...
	printf "create_select_guitar_menu: enter for player %d with instrument_type=%s" d = <Player> s = <instrument_type>
	if (<Player> = 1)
		...
		if GetGlobalTags <default_characterguitartag> noassert = 1
			if ((<instrument_type> = bass))
				if (<current_instrument> = guitar)
					...
					printf \{"create_select_guitar_menu: setting instrument type to bass"}
			...
			elseif ((<instrument_type> = guitar))
				if (<current_instrument> = bass)
					...
					printf \{"create_select_guitar_menu: setting instrument type to guitar"}
	...
	if (<Player> = 1)
		...
		if ($is_network_game = 1)
			if ScreenElementExists \{id = vmenu_select_guitar_p2}
	...
	if ($current_num_players = 1)
		...
		printf \{"guitar_select_highlighted_guitar_p1 = %d"
			d = $guitar_select_highlighted_guitar_p1}
	...
	if (ScreenElementExists id = <vparent>)
		formatText checksumName = players_timer_name 'guitar_select_watch_timer_p%d' d = <Player>
		printf "create_character_hub_menu: spawning watch timer %s" s = <players_timer_name>
		SpawnScriptNow <players_timer_name>
		if (ScreenElementExists id = <vparent>)
			printf \{"create_select_guitar_menu: exit"}
			if ScreenElementExists id = <vparent>
...
script select_guitar_build_finish_array \{instrument_type = guitar}
	GetArraySize <gs_available_instruments> globalarray
	printf "select_guitar_build_finish_array: size = %d for type %s" d = <array_Size> s = <instrument_type>
	<our_array_size> = <array_Size>
	get_musician_instrument_size
	<their_array_size> = <array_Size>
	...
	<i> = (<i> + 1)
	repeat <our_array_size>
	GetArraySize <gs_available_finishes> globalarray
	printf "select_guitar_build_finish_array: %d available finishes" d = <array_Size>
	return gs_available_finishes = <gs_available_finishes>
endscript

script destroy_select_guitar_menu \{Player = 1}
	printf \{"destroy_select_guitar_menu: enter"}
	if (<Player> = 1)
	...
	printf \{"destroy_select_guitar_menu: completed destroying menus"}
	formatText checksumName = player_status 'player%i_status' i = <Player>
	...
	printf \{"destroy_select_guitar_menu: killing gs_view_cam"}
	if ($current_num_players = 1)
	...
	printf \{"destroy_select_guitar_menu: done"}
endscript

script guitar_select_watch_timer_p1 
	begin
	...
	if find_profile_by_id id = ($<player_status>.character_id)
		get_musician_profile_struct index = <index>
		formatText checksumName = default_characterguitartag 'character_%c_player_%p_guitar_tags' c = (<profile_struct>.Name) p = <Player>
		GetGlobalTags <default_characterguitartag>
		printf "guitar_select_watch_timer_p1: for player %d, instrument_type = %s" d = <Player> s = <current_instrument>
		<instrument_type> = <current_instrument>
	else
		printf "guitar_select_watch_timer_p1: unable to find profile by id for %s" s = <player_status>
	endif
	if (<instrument_type> = guitar)
		formatText \{checksumName = guitar_select_available_instruments
			'guitar_select_available_guitar_array'}
	...
	printf \{"guitar_select_watch_timer: exit with guitar_select_highlighted_guitar_p1=%d"
		d = $guitar_select_highlighted_guitar_p1}
	if ($gs_made_first_selection_bool_p1 = 0)
		printf \{"guitar_select_watch_timer_p1: going back"}
		SpawnScriptNow select_guitar_go_back params = {Player = <Player>
			gs_available_instruments = <guitar_select_available_instruments>
			instrument_type = <instrument_type>}
	else
		get_instrument_brand_and_finish id = ($<guitar_select_available_instruments> [$guitar_select_highlighted_guitar_p1])
		select_guitar_build_finish_array {brand = <instrument_brand>
			gs_available_instruments = <guitar_select_available_instruments>}
		printf "guitar_select_watch_timer_p1: gs_current_selected_finish = %d (%s)" d = $gs_current_selected_finish s = (<gs_available_finishes> [$gs_current_selected_finish])
		SpawnScriptNow select_guitar_finish_choose params = {id = (<gs_available_finishes> [$gs_current_selected_finish])
			Player = <Player>
			gs_available_instruments = <guitar_select_available_instruments>
			instrument_type = <instrument_type>}
	endif
	printf \{"guitar_select_watch_timer_p1: done, exiting"}
endscript

script guitar_select_watch_timer_p2 
	begin
	check_countdown_timer
	if (<timer_has_expired> = 1)
		break
	...
	Player = 2
	formatText checksumName = player_status 'player%i_status' i = <Player>
	printf \{"guitar_select_watch_timer_p2: player_status = %s"
		s = player_status}
	if find_profile_by_id id = ($<player_status>.character_id)
		get_musician_profile_struct index = <index>
		formatText checksumName = default_characterguitartag 'character_%c_player_%p_guitar_tags' c = (<profile_struct>.Name) p = <Player>
		GetGlobalTags <default_characterguitartag>
		printf "guitar_select_watch_timer_p2: for player %d, instrument_type = %s cur_guitar = %t cur_bass = %u" d = <Player> s = <current_instrument> t = <current_selected_guitar> u = <current_selected_bass>
		<instrument_type> = <current_instrument>
	else
		printf "guitar_select_watch_timer_p2: unable to find profile by id for %s" s = <player_status>
	endif
	if (<instrument_type> = guitar)
		printf \{"guitar_select_watch_timer_p2: instrument is guitar, setting array"}
		formatText \{checksumName = guitar_select_available_instruments
			'guitar_select_available_guitar_array'}
	else
		printf \{"guitar_select_watch_timer_p2: instrument is bass, setting array"}
		formatText \{checksumName = guitar_select_available_instruments
			'guitar_select_available_bass_array'}
	endif
	printf \{"guitar_select_watch_timer_p2: exit with guitar_select_highlighted_guitar_p1=%d"
		d = $guitar_select_highlighted_guitar_p2}
	if ($gs_made_first_selection_bool_p2 = 0)
		printf \{"guitar_select_watch_timer_p2: going back"}
		SpawnScriptNow select_guitar_go_back params = {Player = <Player>
			gs_available_instruments = <guitar_select_available_instruments>
			instrument_type = <instrument_type>}
	else
		...
		select_guitar_build_finish_array {brand = <instrument_brand>
			gs_available_instruments = <guitar_select_available_instruments>}
		printf "guitar_select_watch_timer_p2: gs_current_selected_finish = %d (%s)" d = $gs_current_selected_finish s = (<gs_available_finishes> [$gs_current_selected_finish])
		SpawnScriptNow select_guitar_finish_choose params = {id = (<gs_available_finishes> [$gs_current_selected_finish])
			Player = <Player>
			gs_available_instruments = <guitar_select_available_instruments>
			instrument_type = <instrument_type>}
	endif
	printf \{"guitar_select_watch_timer_p2: done, exiting"}
endscript
guitar_select_camera_changing = 0

script guitar_select_exit_camera_morph 
	printf \{"guitar_select_exit_camera_morph: start"}
	...
	ScreenFX_UpdateFXInstanceParams \{viewport = bg_viewport
		...
		BlurRadius = 6
		BlurResolution = half}
	printf \{"guitar_select_exit_camera_morph: waiting for CCam_WaitMorph"}
	CCam_WaitMorph
	printf \{"guitar_select_exit_camera_morph: done waiting for CCam_WaitMorph"}
	ScreenFX_RemoveFXInstance \{viewport = bg_viewport
		Name = GSdof}
	ScreenFX_RemoveFXInstance \{viewport = bg_viewport
		Name = GSdof2
		time = 2}
	printf \{"guitar_select_exit_camera_morph: dong"}
...
script gs_scroll_up_or_down \{Player = 1
	...
	if (<Player> = 1)
		if ($gs_made_first_selection_bool_p1 = 0)
			...
			if (<Dir> = down)
				if ($gs_selected_brand_p1 < (<avail_size> - 1))
			...
			if (<guitar_changed> = 1)
				...
				if ScreenElementExists \{id = $gs_guitar_brand_ID_p1}
					SetScreenElementProps id = $gs_guitar_brand_ID_p1 text = <instrument_brand>
				endif
				if ScreenElementExists \{id = $gs_guitar_finish_ID_p1}
				...
				if ScreenElementExists \{id = gs_guitar_logo_ID_p1}
		...
		if ($gs_made_first_selection_bool_p2 = 0)
			...
			<avail_size> = <array_Size>
			if (<Dir> = down)
				if ($gs_selected_brand_p2 < (<avail_size> - 1))
					generic_menu_up_or_down_sound params = {<Dir>}
			...
			if (<guitar_changed> = 1)
				...
				get_instrument_brand_and_finish id = <inst_ID>
				if ScreenElementExists \{id = $gs_guitar_brand_ID_p2}
					SetScreenElementProps id = $gs_guitar_brand_ID_p2 text = <instrument_brand>
				endif
				if ScreenElementExists \{id = $gs_guitar_finish_ID_p2}
				...
				get_instrument_logo id = <inst_ID>
				if ScreenElementExists \{id = gs_guitar_logo_ID_p2}
...
script select_guitar_go_back \{Player = 1
		instrument_type = guitar}
	printf "select_guitar_go_back: for player %d, instrument %s" d = <Player> s = <instrument_type>
	if (<Player> = 1)
		printf \{"select_guitar_go_back: guitar_select_old_guitar_p1 = %s"
			s = $guitar_select_old_guitar_p1}
		instrument_id = ($<gs_available_instruments> [($guitar_select_old_guitar_p1)])
		printf "select_guitar_go_back: instrument_id = %s" s = <instrument_id>
		...
		printf \{"select_guitar_go_back: guitar_select_old_guitar_p2 = %s"
			s = $guitar_select_old_guitar_p2}
	...
	if NOT (<instrument_id> = $<player_status>.instrument_id)
		...
		if NOT ($is_network_game)
			if find_profile_by_id id = ($<player_status>.character_id)
				...
				formatText checksumName = default_characterguitartag 'character_%c_player_%p_guitar_tags' c = (<profile_struct>.Name) p = <Player>
				if (<instrument_type> = guitar)
					printf \{"select_guitar_go_back: setting instrument type to guitar"}
					SetGlobalTags <default_characterguitartag> params = {current_instrument = guitar current_selected_guitar = <instrument_id>}
				else
					printf \{"select_guitar_go_back: setting instrument type to bass"}
	...
	if NOT ($is_network_game)
		if find_profile_by_id id = ($<player_status>.character_id)
			get_musician_profile_struct index = <index>
			formatText checksumName = default_characterguitartag 'character_%c_player_%p_guitar_tags' c = (<profile_struct>.Name) p = <Player> AddToStringLookup = true
			printf "select_guitar_go_back: setting instrument type to %s" s = <instrument_type>
...
script select_guitar_choose \{Player = 1
	...
	printf "select_guitar_choose: enter for player %d" d = <Player>
	if ($current_num_players = 1)
		begin
	...
	if (<Player> = 1)
		printf \{"select_guitar_choose: guitar_select_highlighted_guitar_p1=%d"
			d = $guitar_select_highlighted_guitar_p1}
		<inst_ID> = ($<gs_available_instruments> [($guitar_select_highlighted_guitar_p1)])
		printf "select_guitar_choose: inst_id = %d" d = ($<gs_available_instruments> [($guitar_select_highlighted_guitar_p1)])
		Change structurename = player1_status instrument_id = <inst_ID>
		if find_profile_by_id id = ($player1_status.character_id)
			get_musician_profile_struct index = <index>
			formatText checksumName = default_characterguitartag 'character_%c_player_%p_guitar_tags' c = (<profile_struct>.Name) p = <Player>
			if (<instrument_type> = guitar)
				printf \{"select_guitar_choose: setting instrument type to guitar"}
				...
				printf \{"select_guitar_choose: setting instrument type to bass"}
		...
		if ($gs_made_first_selection_bool_p1 = 0)
			...
			printf \{"select_guitar_choose: killing stuff"}
			printf \{"select_guitar_choose: heading out of here"}
		...
		if NOT ($is_network_game)
			if find_profile_by_id id = ($player2_status.character_id)
				get_musician_profile_struct index = <index>
				formatText checksumName = default_characterguitartag 'character_%c_player_%p_guitar_tags' c = (<profile_struct>.Name) p = <Player>
				printf "select_guitar_choose: instrument_type=%s" s = <instrument_type>
				if (<instrument_type> = guitar)
					printf \{"select_guitar_choose: setting instrument type to guitar"}
					SetGlobalTags <default_characterguitartag> params = {current_instrument = guitar current_selected_guitar = <inst_ID>}
				else
...
script get_instrument_name_and_index 
	...
	<index> = (<index> + 1)
	repeat <array_Size>
	get_instrument_name_and_index \{id = Instrument_Pentacle}
...
script get_instrument_brand_and_finish 
	printf "get_instrument_brand_and_finish: called with id=%d" d = <id>
	get_musician_instrument_size
	index = 0
	begin
	get_musician_instrument_struct index = <index>
	printf "get_instrument_brand_and_finish: desc_id=%s" s = (<info_struct>.desc_id)
	if (<id> = (<info_struct>.desc_id))
...
script select_guitar_get_available_index_from_total_index \{index = 0}
	...
	GetArraySize <gs_available_instruments> globalarray
	<avail_size> = <array_Size>
	printf "select_guitar_get_available_index_from_total_index: array_size=%d" d = <array_Size>
	begin
	get_musician_instrument_struct index = <index>
	printf "select_guitar_get_available_index_from_total_index: checking %s against %t" s = ($<gs_available_instruments> [<i>]) t = (<info_struct>.desc_id)
	if (($<gs_available_instruments> [<i>]) = (<info_struct>.desc_id))
		return index = <i>
	endif
	<i> = (<i> + 1)
...
script select_guitar_create_finish_menu \{Player = 1}
	...
	if (<Player> = 1)
		...
		if ($is_network_game = 1)
			if ScreenElementExists \{id = vmenu_guitar_finish_menu_p2}
	...
	if (<Player> = 1)
		if ScreenElementExists \{id = scrolling_select_guitar_p1}
			LaunchEvent \{Type = unfocus
				target = scrolling_select_guitar_p1}
		endif
	else
		if NOT ($is_network_game)
			if ScreenElementExists \{id = scrolling_select_guitar_p2}
				LaunchEvent \{Type = unfocus
	...
	if ($current_num_players = 1)
		...
		printf "select_guitar_create_finish_menu: available finish array_size = %d" d = <array_Size>
		<i> = 0
		begin
...
script select_guitar_go_back_from_finish_menu \{Player = 1
	...
	if ($current_num_players = 1)
		...
		if ScreenElementExists \{id = scrolling_guitar_finish_menu_p1}
			LaunchEvent \{Type = unfocus
		...
		if (<Player> = 1)
			...
			if ScreenElementExists \{id = scrolling_guitar_finish_menu_p2}
				LaunchEvent \{Type = unfocus
...
script select_guitar_finish_highlight \{Player = 1
		index = 0}
	printf "select_guitar_finish_highlight: index=%e, player=%d id=%s" e = <index> d = <Player> s = <id>
	Change gs_current_selected_finish = <index>
	formatText checksumName = player_status 'player%i_status' i = <Player>
...
script select_guitar_finish_choose \{Player = 1
		needs_net_message = 1}
	printf "select_guitar_finish_choose: id=%d" d = <id>
	if (<Player> = 1)
		...
		printf \{"select_guitar_finish_choose: calling select_guitar_choose"}
		...
		printf \{"select_guitar_finish_choose: done"}
	else
		get_instrument_name_and_index id = <id>
		select_guitar_get_available_index_from_total_index {
			gs_available_instruments = <gs_available_instruments>
	...
	printf \{"select_guitar_finish_choose: exit"}
endscript
...
script build_gs_available_guitar_array 
	...
	printf "build_gs_available_guitar_array: 0: total size for instrument %s is now %d" s = <instrument_type> d = <array_Size>
	num_downloads = 0
	...
	GetArraySize <guitar_select_available_instruments> globalarray
	printf "build_gs_available_guitar_array: 1: total size for instrument %s is now %d" s = <instrument_type> d = <array_Size>
	...
	GetArraySize <guitar_select_available_instruments> globalarray
	printf "build_gs_available_guitar_array: 2: total size for instrument %s is now %d" s = <instrument_type> d = <array_Size>
	...
	GetArraySize <guitar_select_available_instruments> globalarray
	printf "build_gs_available_guitar_array: 3: total size for instrument %s is now %d" s = <instrument_type> d = <array_Size>
	...
	printf "build_gs_available_guitar_array: total size for instrument %s is %d" s = <instrument_type> d = $guitar_select_total_num_guitars
	<i> = 0
	GetArraySize <guitar_select_available_instruments> globalarray
	total_size = <array_Size>
	begin
