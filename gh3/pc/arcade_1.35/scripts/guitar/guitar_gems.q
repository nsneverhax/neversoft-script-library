...
script setup_gemarrays 
	...
	if (($<player_status>.part) = rhythm)
		<part> = 'rhythm_'
		printf \{"setup_gemarrays: rhythm_"}
	else
		<part> = ''
		printf \{"setup_gemarrays: blank"}
	endif
	if ($game_mode = p2_career || $game_mode = p2_coop ||
		...
		if StructureContains structure = <song_struct> use_coop_notetracks
			if (($<player_status>.part) = rhythm)
				<part> = 'rhythmcoop_'
				printf \{"setup_gemarrays: rhythmcoop_"}
			else
				<part> = 'guitarcoop_'
				printf \{"setup_gemarrays: guitarcoop_"}
			endif
		endif
	endif
	get_song_prefix song = <song_name>
	temp_difficulty = <difficulty>
	if (<temp_difficulty> = beginner)
		<temp_difficulty> = easy
	endif
	get_difficulty_text_nl difficulty = <temp_difficulty>
...
script calc_health_fudge 
	if ($rt_no_die_early = 1)
		min = (1000.0 * ($current_time) / ($<player_status>.health_invincible_time))
		MathMin a = <min> b = 1.0
		Change rt_no_die_health_fudge = <min>
	else
		Change \{rt_no_die_health_fudge = 1.0}
	endif
endscript
...
script calc_health_invincible_time 
	printf \{"^^^^^^^^^^^^^^^^^^^^^^"}
	printf \{"^^^^^^^^^^^^^^^^^^^^^^"}
	printf \{"Calc Invincible Time"}
	printf \{"^^^^^^^^^^^^^^^^^^^^^^"}
	printf \{"^^^^^^^^^^^^^^^^^^^^^^"}
	if ($rt_no_die_early = 1)
		if ($game_mode = p1_quickplay)
			printf \{"calc_health_invincible_time: p1_quickplay, setting temp_diff to %d"
				d = $current_difficulty}
			temp_diff = $current_difficulty
		else
			if ($current_difficulty = beginner || $current_difficulty2 = beginner)
				temp_diff = beginner
			elseif ($current_difficulty = easy || $current_difficulty2 = easy)
				temp_diff = easy
			elseif ($current_difficulty = medium || $current_difficulty2 = medium)
				temp_diff = medium
			elseif ($current_difficulty = hard || $current_difficulty2 = hard)
				temp_diff = hard
			elseif ($current_difficulty = expert || $current_difficulty2 = expert)
				temp_diff = expert
			endif
		endif
		printf "calc_health_invincible_time: <temp_diff>=%d" d = <temp_diff>
		switch (<temp_diff>)
			case beginner
			Change structurename = <player_status> health_invincible_time = $no_die_early_time_beginner
			printf \{"calc_health_invincible_time: time set to %d"
				d = $no_die_early_time_beginner}
			case easy
			Change structurename = <player_status> health_invincible_time = $no_die_early_time_easy
			printf \{"calc_health_invincible_time: time set to %d"
				d = $no_die_early_time_easy}
			case medium
			Change structurename = <player_status> health_invincible_time = $no_die_early_time_medium
			printf \{"calc_health_invincible_time: time set to %d"
				d = $no_die_early_time_medium}
			case hard
			Change structurename = <player_status> health_invincible_time = $no_die_early_time_hard
			printf \{"calc_health_invincible_time: time set to %d"
				d = $no_die_early_time_hard}
			case expert
			Change structurename = <player_status> health_invincible_time = $no_die_early_time_expert
			printf \{"calc_health_invincible_time: time set to %d"
				d = $no_die_early_time_expert}
		endswitch
	else
		Change structurename = <player_status> health_invincible_time = 0
		Change \{rt_no_die_health_fudge = 1.0}
	endif
endscript
...
script gem_scroller \{Player = 1
	...
	if ((<Player> = 1 && $current_difficulty = beginner) || (<Player> = 2 && $current_difficulty2 = beginner))
		Change structurename = <player_status> check_time_early = (($check_time_early + $check_time_early_beginner_bonus) * $current_speedfactor)
		Change structurename = <player_status> check_time_late = (($check_time_late + $check_time_late_beginner_bonus) * $current_speedfactor)
	else
	...
	if (<do_bot> = 1)
		SpawnScriptLater gem_iterator params = {iterator_text = 'bot' song_name = <song_name> difficulty = <difficulty> part = <part> use_input_array = 'input_array' one_event_per_frame
			...
			Player = <Player> player_status = <player_status> player_text = <player_text>}
		printf channel = log "Spawned bot! player=%a controller=%b" a = <Player> b = (<player_status>.controller)
	endif
	if ($new_net_logic)
		if (<Player> = 2)
			formatText checksumName = bossresponse_array 'bossresponse_array%p' p = <player_text>
	...
	if ((<player_status>.is_local_client) || ($new_net_logic))
		formatText checksumName = cbf_id 'check_buttons_fast_%i' i = <Player>
		SpawnScriptLater check_buttons_fast id = <cbf_id> params = {song_name = <song_name> difficulty = <difficulty>
	...
	printf \{"start_gem_scroller: diff=%d"
		d = $current_difficulty}
...
script flickerdicker_gems 
	begin
	if ($flickerdicker = 0)
		LaunchGemEvent event = flicker_on Player = <Player>
		Change \{flickerdicker = 1}
	else
		LaunchGemEvent event = flicker_off Player = <Player>
		Change \{flickerdicker = 0}
	endif
	Wait \{3
		gameframes}
	repeat
endscript
chuck_gem_fade_alpha = 1.0

script fade_gems 
	begin
	LaunchGemEvent \{event = fade}
	Change chuck_gem_fade_alpha = ($chuck_gem_fade_alpha - 0.1)
	Wait \{2
		gameframes}
	repeat
endscript
...
script win_song 
	...
	song = <gem_array>
	GetArraySize $<song>
	get_maximum_difficulty difficulty1 = ($current_difficulty) difficulty2 = ($current_difficulty2)
	...
	if NOT (<array_Size> = 0)
		if (<arcade_song_fading_enabled>)
			if (<maximum_difficulty> = beginner)
				temp_end_song_time = <arcade_beginner_fade_time>
			elseif (<maximum_difficulty> = easy)
				temp_end_song_time = <arcade_easy_fade_time>
			elseif (<maximum_difficulty> = medium)
				temp_end_song_time = <arcade_medium_fade_time>
			elseif (<maximum_difficulty> = hard)
				temp_end_song_time = <arcade_hard_fade_time>
			else
				temp_end_song_time = <arcade_expert_fade_time>
			endif
			get_song_end_time song = ($current_song)
			printf "song_fade: temp_end_song_time=%d, total_end_time=%e" d = <temp_end_song_time> e = <total_end_time>
			if (<temp_end_song_time> < <total_end_time>)
				end_s = ((<temp_end_song_time> - <starttime>) / 1000.0)
			else
				end_s = ((<total_end_time> - <starttime>) / 1000.0)
			endif
			printf "song_fade: enabled, max diff=%s, end_s=%d" s = <maximum_difficulty> d = <end_s>
		else
			printf \{"song_fade: not enabled"}
			get_song_end_time song = ($current_song)
			end_s = ((<total_end_time> - <starttime>) / 1000.0)
		endif
	...
	if (<arcade_song_fading_enabled>)
		...
		st = (($<player_status>.scroll_time) - $destroy_time)
		printf "win_song: waiting for player %a to shorter fade" a = <Player>
		printf "win_song: waiting %a seconds for shorter fade" a = <st>
		Wait <st> Seconds
		formatText checksumName = cbf_id 'check_buttons_fast_%i' i = <Player>
		KillSpawnedScript id = <cbf_id>
	...
	if ((<wait_on_player> = <Player>) && <arcade_song_fading_enabled>)
		printf \{"win_song: doing shorter fade"}
		GetSoundBussParams \{Guitar_Balance = {
				vol
			}}
		temp_guitar_balance_volume = <vol>
		GetSoundBussParams \{band_Balance = {
				vol
			}}
		temp_band_balance_volume = <vol>
		...
		Wait \{2
			Seconds}
		SpawnScriptNow reset_shorter_fade params = {gbv = <temp_guitar_balance_volume> bbv = <temp_band_balance_volume>}
	endif
...
script reset_shorter_fade 
	Wait \{0.5
		Seconds}
	printf \{"reset_shorter_fade: resetting audio!"}
	SoundBussUnlock \{Guitar_Balance}
	SoundBussUnlock \{band_Balance}
	setsoundbussparams {Guitar_Balance = {vol = <gbv>}} time = 0
	setsoundbussparams {band_Balance = {vol = <bbv>}} time = 0
	SoundBussLock \{Guitar_Balance}
	SoundBussLock \{band_Balance}
endscript
...
script start_gem_scroller \{starttime = 0
		...
		play_audio = 1}
	printf "start_gem_scroller: play_audio=%d is_attract_mode=%e" d = <play_audio> e = $is_attract_mode
	printf \{"start_gem_scroller: diff=%d"
		d = $current_difficulty}
	...
	Change \{boss_play = 0}
	Change \{showing_raise_axe_p1 = 0}
	Change \{showing_raise_axe_p2 = 0}
	...
	if ($player1_status.bot_play = 1)
		...
		printf channel = log "Starting bot for player 1 on controller=%a" a = <controller>
	endif
	...
	preload_song song_name = <song_name> starttime = <song_start_time> play_audio = <play_audio>
	calc_score = true
	...
	GetGlobalTags \{user_options}
	if (<Player> = 1)
		if ($game_mode = p1_quickplay && $active_player = 2)
		...
		printf "start_gem_scroller: player2: lefty_flip_p1 = %a lefty_flip_p2 = %b" a = <lefty_flip_p1> b = <lefty_flip_p2>
...
script kill_gem_scroller \{no_render = 0}
	...
	if ($is_attract_mode = 0)
		if NOT (($game_mode = p2_faceoff) || ($game_mode = p2_pro_faceoff) || ($game_mode = p2_coop) || ($game_mode = p2_battle) || ($game_mode = p2_career))
			if NOT ($original_p1_device = $player2_device)
				Change \{player1_device = $original_p1_device}
			endif
			printf \{"kill_gem_scroller - player1_device now %d [swaptest]"
				d = $player1_device}
		endif
	endif
endscript
...
script restart_gem_scroller \{no_render = 0}
	printf \{"restart_gem_scroller: RESTARTING GEM SCROLLER"}
	mark_unsafe_for_shutdown
	disable_pause
	if GotParam \{replay}
...
script gem_array_stepper 
	...
	begin
	calc_health_fudge song = <song_name> player_status = <player_status>
...
script start_song \{device_num = 0
		practice_intro = 0
		endtime = 999999999}
	printf "start_song: called with device_num = %d [swaptest]" d = <device_num>
	mark_unsafe_for_shutdown
	set_rich_presence_game_mode
	Load_Venue
	Transition_SelectTransition practice_intro = <practice_intro>
	if NOT (($game_mode = p2_faceoff) || ($game_mode = p2_pro_faceoff) || ($game_mode = p2_coop) || ($game_mode = p2_battle) || ($game_mode = p2_career))
		printf \{"start_song: changing devices"}
		Change \{original_p1_device = $player1_device}
		Change player1_device = (<device_num>)
		Change \{structurename = player1_status
			controller = $player1_device}
		Change \{structurename = player2_status
			controller = $player2_device}
		printf \{"start_song: original=%a current=%b"
			a = $original_p1_device
			b = $player1_device}
	else
		printf \{"start_song: NOT changing devices"}
		Change \{structurename = player1_status
			controller = $player1_device}
		Change \{structurename = player2_status
			controller = $player2_device}
