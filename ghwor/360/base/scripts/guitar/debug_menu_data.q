...
debug_selected_intro_1 = 0
debug_selected_intro_2 = 0
...
debug_submenus = [
	{
		...
		items = [
			...
			}
			{
				text = qs(0xd3421a89)
				Type = run_script
				choose_script = debug_menu_add_quickplay_stars
			}
			{
				text = qs(0x8a1119cc)
				Type = option
				value_script = debug_menu_strict_small_pools_get_value
				text_script = debug_menu_strict_small_pools_get_text
				left_script = debug_menu_strict_small_pools_option_toggle
				right_script = debug_menu_strict_small_pools_option_toggle
				option_select = {
					Type = run_script
			...
			{
				multiitem
				build_script = debug_menu_build_song_changelist_menu
				name_script = debug_menu_song_changelist_menu_name_builder
				Type = run_script
				choose_script = debug_menu_song_changelist_menu_choose_script_builder
			}
		]
	}
	{
		...
		items = [
			{
				text = qs(0xcee9e711)
				Type = option
				text_script = debug_menu_friend_feed_count_text_script
				value_script = debug_menu_friend_feed_count_get_value
				left_script = debug_menu_friend_feed_count_left
				right_script = debug_menu_friend_feed_count_right
			}
			{
				text = qs(0x495d69c2)
				Type = option
				text_script = debug_menu_friend_feed_value_type_text_script
				value_script = debug_menu_friend_feed_value_type_get_value
				left_script = debug_menu_friend_feed_value_type_left
				right_script = debug_menu_friend_feed_value_type_right
			}
			{
				text = qs(0x243e7864)
				Type = option
				text_script = debug_menu_friend_feed_destination_text_script
				value_script = debug_menu_friend_feed_destination_get_value
				left_script = debug_menu_friend_feed_destination_left
				right_script = debug_menu_friend_feed_destination_right
			}
			{
				text = qs(0xa16ec938)
				Type = run_script
				choose_script = debug_menu_friend_feed_test_strings_to_tty
			}
			{
				text = qs(0xfbd6b6f6)
				Type = new_menu
				menu_id = 'friend_feed_autotest'
				additional_choose_script = debug_menu_friend_feed_autotest_start_script
			}
			{
				multiitem
				build_script = debug_menu_build_friend_feed_select_menu
				name_script = debug_menu_friend_feed_select_menu_name_builder
				Type = run_script
				choose_script = debug_menu_friend_feed_select_menu_choose_script_builder
			}
		]
	}
	{
		id = 'friend_feed_autotest'
		title = qs(0x4161ac24)
		back_script = debug_menu_friend_feed_autotest_back_script
		items = [
			{
				text = qs(0x8c9523e2)
				Type = run_script
				choose_script = debug_menu_friend_feed_nullscript
			}
			{
				text = qs(0xc2242d2e)
				Type = option
				text_script = debug_menu_friend_feed_autotest_time_text_script
				value_script = debug_menu_friend_feed_autotest_time_get_value
				left_script = debug_menu_friend_feed_autotest_time_left
				right_script = debug_menu_friend_feed_autotest_time_right
			}
		]
	}
	{
	...
	}
	{
		...
		items = [
			...
			}
			{
				text = qs(0x2755bf0f)
				Type = toggle
				Global = quickplay_whammy_rewind_enable
	...
	}
	{
		...
		items = [
			...
			}
			{
				text = qs(0x71751b01)
				Type = toggle
				Global = debug_use_venue_ad_override
				on_script = debug_menu_turn_on_venue_ad_override
				off_script = debug_menu_turn_off_venue_ad_override
			}
			{
				text = qs(0x78e0256e)
				Type = toggle
	...
	}
	{
		...
		items = [
			...
			}
			{
				...
				Type = new_menu
				menu_id = 'autotesting'
				conditional_script = debug_menu_autotest_conditional
			}
			{
				text = qs(0x600ea38c)
				Type = toggle
			...
			}
			{
				text = qs(0xf0b7500e)
				Type = new_menu
				menu_id = 'friend_feed_test'
			}
			{
				text = qs(0x3add0e5c)
				Type = toggle
				Global = g_e3_demo_flow
			}
		]
	}
	{
		...
		items = [
			{
				...
				name_script = debug_menu_play_song_source_name_builder
				Type = new_menu
				menu_id = 'nt_debug_songlist_2'
				menu_params = debug_menu_play_song_params_builder
			}
		]
	}
	{
		id = 'nt_debug_songlist_2'
		title = qs(0x81eb0aa1)
		paged
		items = [
			{
