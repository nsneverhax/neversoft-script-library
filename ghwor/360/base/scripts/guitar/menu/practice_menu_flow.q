...
script practice_restart_song \{generic_params = {
	...
	SpawnScriptNow practice_restart_song_spawned params = {generic_params = <generic_params> stop_rendering = <stop_rendering>}
endscript
...
script practice_update 
	...
	if ($practice_loop_section = 1 && <has_paused> = 0)
		if (<time> > 0 && <song_time_ms> > <time> - 300)
			SpawnScriptNow \{practice_pausesong}
			has_paused = 1
		endif
	endif
	if (<song_time_ms> > ($practice_end_time) - 200)
		if ($practice_loop_section = 1 && $practice_end_time > (<total_end_time> - 100) && <has_paused> = 0)
			SpawnScriptNow \{practice_pausesong}
			has_paused = 1
		endif
...
script practice_choose_change_speed 
	ui_sfx \{menustate = Generic
		uitype = select}
	generic_event_choose state = uistate_practice_select_speed data = {<...>}
endscript
