			...
			if NOT StructureContains structure = (<sections> [<i>]) post_userlayer
				if is_target_in_div part = <part> div_id = <div_id> target = (<sections> [<i>])
					...
					add_car_menu_text_item {
						...
						choose_state_data = {part = <part> title = ((<sections> [<i>]).frontend_desc) section_index = <i> hist_name = ((<sections> [<i>]).frontend_desc) hist_tex = <icon> is_popup camera_list = <camera_list> zoom_camera = <zoom_camera> cam_name = <cam_name>}
					}
				endif
			endif
		endif
