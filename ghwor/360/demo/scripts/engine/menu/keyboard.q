...
script handle_keyboard_input 
	if gotparam \{got_backspace}
		broadcastevent \{type = keyboard_backspace}
	endif
	if gotparam \{got_enter}
		broadcastevent \{type = keyboard_enter}
	endif
	if gotparam \{text}
		broadcastevent type = keyboard_keypress data = {key = <text>}
	endif
	if NOT retailbuild
		if globalexists \{name = profile_pages}
			handle_keyboard_input_profiler <...>
		endif
	endif
endscript

script handle_keyboard_input_profiler 
