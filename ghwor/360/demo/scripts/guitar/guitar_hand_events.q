...
script drumkit_anim_iterator 
	...
	if NOT (<array_size> = 0)
		...
		begin
		getsongglobal name = <event_array> index = <array_entry>
		timemarkerreached_setparams time_offset = <time_offset> time = <element>
		begin
...
script sync_cameras_to_performance \{output_results = 0}
	...
	if NOT songglobalexists name = <camera_array> type = array
		return
	endif
	getsongarraysize <camera_array>
	if (<array_size> = 0)
		printf \{channel = clip
			qs(0xe6cf20ae)}
		return
	...
	if NOT songglobalexists name = <anim_array> type = array
		return
	endif
	getsongarraysize <anim_array>
	if (<array_size> = 0)
		printf \{channel = clip
			qs(0x7d03b8a6)}
		return
	...
	begin
	getsongglobal name = <anim_array> index = <anim_index>
	if ((<element>.scr) = band_playclip)
		next_anim_time = (<element>.time)
	...
	begin
	getsongglobal name = <camera_array> index = <camera_index>
	next_camera_time = <element>
	...
	if (<time_diff> < <threshold>)
		if (<time_diff> = 0)
			...
			setsongglobal name = <camera_array> index = <camera_index> newvalue = <next_anim_time>
			getsongglobal name = <camera_array> index = (<camera_index> + 1)
			decompressnotevalue note_value = <element>
			length = (<length> - <camera_time_difference>)
			compressnotevalue length = <length> note_value = <note_value> velocity = <velocity>
			setsongglobal name = <camera_array> index = (<camera_index> + 1) newvalue = <note_value>
	...
	if (<end_frame> != -1)
		...
		if (<time_diff> < <threshold>)
			if (<time_diff> = 0)
				...
				setsongglobal name = <camera_array> index = <camera_index> newvalue = <end_time>
				getsongglobal name = <camera_array> index = (<camera_index> + 1)
				decompressnotevalue note_value = <element>
				length = (<length> - <camera_time_difference>)
				compressnotevalue length = <length> note_value = <note_value> velocity = <velocity>
				setsongglobal name = <camera_array> globalarray index = (<camera_index> + 1) newvalue = <note_value>
				last_camera_adjusted_index = <camera_index>
			endif
			break
		endif
