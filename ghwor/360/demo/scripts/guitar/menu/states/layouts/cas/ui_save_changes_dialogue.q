...
script exit_save_changes 
	...
	set_charselect_previous_character_id id = ($cas_current_profile)
	get_is_female_from_appearance appearance = <appearance>
	if ($cas_started_from != career_select)
		cas_destroy_all_characters
	endif
	data = {savegame = ($cas_current_savegame) controller = <device_num> requested_autosave = 1}
...
script exit_discard_changes 
	...
	if (($cas_editing_new_character) = true)
		...
		if (($cas_current_savegame) != -1)
			...
			cas_get_player
			if characterprofileexists name = ($g_charselect_previous_character_id) savegame = ($cas_current_savegame)
				character_id = ($g_charselect_previous_character_id)
			else
