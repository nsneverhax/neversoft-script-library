...
script unlock_tracker_career_venue_callback \{chapter_global = !q1768515945}
	if NOT screenelementexists \{id = unlock_tracker}
		return
	endif
	unlock_tracker :gettags
	type_total = (<type_total> + 1)
	formattext checksumname = chapter_unlock '%s' s = (($<chapter_global>).name)
	getsavegamefromcontroller controller = ($primary_controller)
	getglobaltags savegame = <savegame> <chapter_unlock>
	if (<unlocked> = 1)
		unlocked_type_total = (<unlocked_type_total> + 1)
	endif
	unlock_tracker :settags {type_total = <type_total> unlocked_type_total = <unlocked_type_total>}
endscript

script unlock_tracker_career_character_callback \{chapter_global = !q1768515945}
	if NOT screenelementexists \{id = unlock_tracker}
		return
	endif
	unlock_tracker :gettags
	chapter = ($<chapter_global>)
	if structurecontains \{structure = chapter
			boss_chapter}
		return
	endif
	type_total = (<type_total> + 1)
