...
script ghtunes_create_alphasearch_menu 
	...
	switch <search_by>
		case songname
		formattext textname = dialog_text qs(0x5f0690d0) a = ($menu_jam_ghtunes_string)
		case artistname
		formattext textname = dialog_text qs(0x52ff60dd) a = ($menu_jam_ghtunes_string)
		<valid_characters> = [
			$name_entry_upper_characters
			$name_entry_lower_characters
			$name_entry_space_character
			$name_entry_number_characters
			$name_entry_ghtunes_artist_characters
		]
		case fileid
		formattext textname = dialog_text qs(0x6f431f57) a = ($menu_jam_ghtunes_string)
	endswitch
	spawnscriptnow os_keyboard_spawned_script params = {
		controller = ($primary_controller)
		max_length = 12
		dialog_text = <dialog_text>
		accept_script = ghtunes_keyb_input_accept
		accept_params = {search_by = <search_by>}
		cancel_script = ghtunes_keyb_input_cancel
		os_kb_flags = {ps3_alphabet_input}
		valid_characters = <valid_characters>
		string_validation_script = musicstudio_search_is_text_valid
	}
endscript

script ghtunes_keyb_input_accept 
	ghtunes_search {
		search_by = <search_by>
		text = <text>
	}
...
script ghtunes_keyb_input_cancel 
	ghtunes_submenu_back
endscript
...
script jam_ghtunes_leaderboard_select_failed \{dialog = 0}
	...
	switch <dialog>
		case 0
		formattext textname = dialog qs(0xccaae452) a = <leaderboard_name>
	endswitch
	destroy_dialog_box
	create_dialog_box {
			...
			if NOT (<results_array_size> = 0)
				...
				if ((<content_array> [<i>].has_content) = 1)
					...
					if (<i> = 0)
						...
						formattext textname = title_text qs(0xceba88f0) s = (<content_array> [<i>].filename)
						(<children> [<i>]) :setprops title_text = <title_text>
					endif
					if (<last_item> = 0)
...
script jam_ghtunes_failed_dialog \{dialog = 0}
	...
	switch <dialog>
		case 0
		if isxenonorwindx
			dialog_txt = qs(0xdc6bc164)
		elseif isps3
			dialog_txt = qs(0xfd83b555)
		endif
		case 1
		if isxenonorwindx
		...
		case 2
		if isxenonorwindx
			dialog_txt = qs(0x549abd3f)
		elseif isps3
			dialog_txt = qs(0x70c1a812)
		endif
		case 3
		dialog_txt = qs(0xdc6bc164)
		case 4
		if isxenonorwindx
			dialog_txt = qs(0x01bfd1cb)
		elseif isps3
			dialog_txt = qs(0xfd83b555)
		endif
		case 5
		...
		dialog_txt = qs(0xdc6bc164)
		case 8
		dialog_txt = qs(0x576de7ac)
		case 9
		dialog_txt = qs(0xb9fd7c2c)
	endswitch
	destroy_generic_menu
	create_dialog_box {
		heading_text = qs(0xc3b029e6)
