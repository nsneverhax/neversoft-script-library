...
script create_jam_song_select_menu \{curr_tab = 0
	...
	if (<new_ghmix_song> = 1)
		ui_event_remove_params \{param = new_ghmix_song}
		ui_event_wait_for_safe
		jam_song_select_load_song {
			pattern_load = 1
			back_to_jam_band = 0
		...
		if (<user_song_count> < ($jam_max_user_songs))
			if (<advanced_record> = 0)
				add_generic_musicstudio_menu_text_item \{icon = icon_jam_create_new
					text = qs(0x1bca1511)
					pad_choose_script = jam_create_new_song_check
					pad_choose_params = {
						new_state = uistate_musicstudio_jamroom_wizard
						new_state_data = {
							curr_tab = 0
							musicstudio_ghmix = 0
							init_style_select = 1
			...
			elseif (<advanced_record> = 1)
				...
				add_generic_musicstudio_menu_text_item {
					...
					pad_choose_script = jam_create_new_song_check
					pad_choose_params = {
						new_state = uistate_recording
						new_state_data = {back_to_jam_band = 0 current_instrument = 0 player = ($jam_current_recording_player)}
					}
					additional_focus_script = menu_jam_create_song_focus
					additional_unfocus_script = menu_jam_create_song_unfocus
					additional_choose_script = musicstudio_set_ambience
			...
			elseif (<advanced_record> = 2)
				...
				setplayerinfo \{1
					jam_instrument = 0}
				if isxenon
					my_pad_choose_script = jam_create_new_song_check
					my_pad_choose_params = {
						new_state_data = {new_ghmix_song = 1}
					}
				else
					my_pad_choose_script = jam_song_select_load_song
					my_pad_choose_params = {pattern_load = 1 back_to_jam_band = 0 current_instrument = 0 player = ($jam_current_recording_player)}
				endif
				add_generic_musicstudio_menu_text_item {
					icon = icon_jam_create_new
					text = qs(0x1bca1511)
					pad_choose_script = <my_pad_choose_script>
					pad_choose_params = <my_pad_choose_params>
					additional_focus_script = menu_jam_create_song_focus
					additional_unfocus_script = menu_jam_create_song_unfocus
					additional_choose_script = musicstudio_set_ambience
					additional_choose_params = {ambience = create_new_ghmix_choose_ambience}
...
script jam_create_new_song_check 
	if isxenon
		if gotparam \{new_state}
			ui_memcard_check_jam_space {
				event = menu_replace
				state = <new_state>
				data = <new_state_data>
			}
		...
		ui_event_block {
			event = menu_change
			data = {
				state = <new_state> <new_state_data>
			}
		}
	endif
endscript
