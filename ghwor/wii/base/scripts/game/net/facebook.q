...
script facebook_register_callback \{controller = !i1768515945
	...
	if (<Result> = success)
		...
		if (<array_Size> > 0)
			facebook_set_info controller = ($primary_controller) element = account_id value = (<registered_accounts> [0].account_id)
		else
			ScriptAssert \{'facebook_register_callback succeeded but failed to return an account id'}
		endif
	else
...
script facebook_unregister {
	...
	NetSessionFunc {
		...
		params = {
			account_id = (<facebook_info>.account_id)
			controller_index = <controller>
...
script facebook_get_info \{controller = 0}
...
script facebook_check_is_registered_flag \{controller = 0}
	ret = FALSE
	if (($register_account) = 1)
		...
		facebook_get_info
		if NOT is_net_id_zero net_id = (<facebook_info>.account_id)
			<ret> = true
		endif
	endif
	return <ret>
...
script facebook_figured_out_can_post \{controller_index = 0}
...
script facebook_get_registered_accounts 
	printf \{'-- facebook_get_registered_accounts --'}
	NetSessionFunc \{Obj = facebook
		func = get_registered_accounts
		params = {
			callback = facebook_get_registered_accounts_callback
		}}
endscript

script facebook_get_registered_accounts_callback \{registered_accounts = !a1768515945
		Result = !q1768515945
		error_code = None
		dev_error_code = 0}
	printf \{'-- facebook_get_registered_accounts_callback --'}
	GetArraySize (<registered_accounts>)
	i = 0
	array = []
	if (<array_Size> > 0)
