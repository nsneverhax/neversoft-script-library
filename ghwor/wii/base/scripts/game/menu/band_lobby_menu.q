...
script band_lobby_update_button_helpers 
	...
	if GotParam \{menu}
		switch <menu>
			...
			elseif isngc
				add_user_control_helper controller = <controller> text = ($band_lobby_strings.button_display_invtes) button = lb z = 100000
			endif
			...
			if demobuild
				...
				if (<managing> = 0)
					...
					if ininternetmode
						...
						if roadie_battle_lobby_should_have_roadie_button
							add_user_control_helper controller = <controller> text = ($band_lobby_strings.roadie_battle_add_roadie) button = back z = 100000
						endif
						add_user_control_helper controller = <controller> text = ($band_lobby_strings.button_play) button = start z = 100000
					endif
				endif
...
script band_lobby_update_ticker \{msg_checksum = !q1768515945
	...
	printf \{channel = band_lobby
		qs(0x9de6efa4)}
	if inroadiebattlemode
		if (<msg_checksum> != current_state)
			Change \{roadie_battle_band_lobby_popup_pending_via_ticker = true}
			ui_event_wait event = menu_change data = {is_popup state = uistate_band_lobby_popup reason = <msg_checksum>}
		endif
		return
	endif
	if ScreenElementExists \{id = band_lobby_desc_id}
...
script band_lobby_update_submenu_pad_handlers 
	...
	if (<add_shortcuts> = 1)
		...
		if (0 = $<submenu_data>.is_managed)
			if NOT band_lobby_is_anyone_in_forced_flow
				if bl_should_display_challenge_shortcut
	...
	if ($<submenu_data>.is_managed = 0)
		if InNetGame
			...
			if roadie_battle_lobby_should_have_roadie_button
				AddArrayElement array = <array> element = {pad_select band_lobby_submenu_pad_handler_wrapper params = {choose_script = roadie_battle_lobby_goto_roadie_battle submenu_index = <submenu_index> params = <standard_params>}}
			endif
		endif
	endif
...
script band_lobby_update_gamertag 
	...
	if inroadiebattlemode
		if roadie_battle_find_roadie_for_player Player = <Player>
			...
			<roadie_names> = <roadie_team_name>
		else
			<roadie_names> = <gamertag_text>
		endif
		if roadie_battle_lobby_should_scroll_names names = <roadie_names>
			roadie_battle_lobby_create_scrolling_names viewport = (<Player> - 1) names = <roadie_names>
			<gamertag_text> = qs(0x03ac90f0)
		else
			roadie_battle_lobby_destroy_scrolling_names viewport = (<Player> - 1)
			<gamertag_text> = <roadie_names>
		endif
	endif
...
script band_lobby_manage_band_members \{Player = !i1768515945
	...
	if band_lobby_desc_id :desc_resolvealias \{Name = alias_band_viewport_vmenu}
		...
		<resolved_id> :se_setprops {
			exclusive_device = <controller>
			event_handlers = [
				{pad_back band_lobby_band_memebers_back params = {submenu_index = <submenu_index>}}
...
script band_lobby_select_band_member 
	...
	if (<managed_submenu_index> = <band_leader_submenu_index>)
		audio_ui_band_lobby_sfx_logic player_num = ($g_leader_player_num) back = 1
...
script band_lobby_friendslist \{Player = !i1768515945
		submenu_index = !i1768515945}
	printf \{channel = band_lobby
		qs(0xc9b42674)}
	if ($g_career_search != NULL)
	...
	online = 0
	if band_lobby_desc_id :desc_resolvealias \{Name = alias_pu_cont_parent}
		if netstatus_checkforerrors \{net_system = nwc24}
			<nwc24_error> = 1
		elseif ((CheckForSignIn controller_index = <controller> dont_set_primary) && (NetSessionFunc func = is_lobby_available))
			if isxenonorwindx
	...
	if (<can_continue> = 1)
		if NOT NetSessionFunc \{func = can_view_user_content
			...
			band_lobby_friendslist_no_errors {
				Player = <Player>
				submenu_index = <submenu_index>
				controller = <controller>
				online = <online>
...
script band_lobby_friendslist_no_errors \{Player = !i1768515945
		submenu_index = !i1768515945
		controller = !i1768515945
		online = !i1768515945
		resolved_id = !q1768515945}
	universal_help_display_help_check help_context = musicianslist_to_gameplay device_num = <controller>
	if (<show_help_page> = 1)
		band_lobby_change_focus_submenu_all \{focus_type = unfocus}
		ui_create_universal_help {
			next_state_script = band_lobby_friendslist_post_help
			next_state_params = {
				Player = <Player>
				submenu_index = <submenu_index>
				controller = <controller>
		...
		band_lobby_friendslist_post_help {
			Player = <Player>
			submenu_index = <submenu_index>
			controller = <controller>
			online = <online>
...
script band_lobby_friendslist_post_help \{Player = !i1768515945
		submenu_index = !i1768515945
		controller = !i1768515945
		online = !i1768515945
		resolved_id = !q1768515945}
	...
	if ((isXenon) || (<online> = 0))
		band_lobby_really_create_friendslist container = <resolved_id> Player = <Player> controller = <controller> online = <online> submenu_index = <submenu_index>
	else
		band_lobby_pre_create_friendslist container = <resolved_id> Player = <Player> controller = <controller> online = <online> submenu_index = <submenu_index>
	endif
endscript
...
script band_lobby_players \{Player = !i1768515945}
	printf \{channel = band_lobby
		qs(0x05243124)}
	if ($g_career_search != NULL)
		return
