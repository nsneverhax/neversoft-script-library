...
script make_generic_barrel_menu {
	...
	if barrel_menu_desc :desc_resolvealias \{Name = alias_generic_barrel_vmenu}
		...
		<event_handlers> = [
			...
			{pad_scroll_up generic_barrel_menu_pad_up}
			{pad_down generic_barrel_menu_pad_down}
			{pad_scrolldown generic_barrel_menu_pad_down}
		]
...
script generic_barrel_menu_finish \{no_helper_pills = 0
	...
	switch <array_Size>
		...
		if (<max_expansion> >= 3)
			(<children> [0]) :se_getprops dims
			<y_dims> = (((<dims>.(0.0, 1.0)) * 3) + 10)
			barrel_menu_desc :se_setprops generic_barrel_window_dims = ((<x_dims> * (1.0, 0.0)) + (<y_dims> * (0.0, 1.0)))
		else
			(<children> [0]) :se_getprops dims
			<y_dims> = (((<dims>.(0.0, 1.0))) + 10)
			barrel_menu_desc :se_setprops generic_barrel_window_dims = ((<x_dims> * (1.0, 0.0)) + (<y_dims> * (0.0, 1.0)))
		endif
		default
...
script add_generic_barrel_menu_text_item {
	...
	if ScreenElementExists id = <parent>
		CreateScreenElement {
			...
			tags = {
				...
				pointer_focusable = <pointer_focusable>
				focusable = 1
			}
		}
	else
		...
		if ScreenElementExists id = <parent>
			...
			<parent> :SetTags centered = 1
			printf \{qs(0x071efe0e)}
		else
	...
	if GotParam \{pad_circle_script}
		<event_handlers> = [
			{pad_circle <pad_choose_sound>}
			{pad_circle generic_blocking_execute_script params = {pad_script = <pad_circle_script> pad_params = {container_id = <id> <pad_circle_params>}}}
		]
		<id> :se_setprops event_handlers = <event_handlers>
	endif
	if GotParam \{pad_option2_script}
...
script generic_barrel_menu_item_disable \{id = !q1768515945}
	<id> :se_setprops {
		not_focusable
		tags = {focusable = 0}
	}
	<id> :se_setprops menurow_txt_item_rgba = [120 120 120 100]
	<id> :generic_barrel_menu_unpause_item
endscript

script generic_barrel_menu_item_enable \{id = !q1768515945}
	<id> :se_setprops {
		focusable
		tags = {focusable = 1}
	}
	<id> :se_setprops menurow_txt_item_rgba = [216 181 117 255]
	<id> :generic_barrel_menu_unpause_item
endscript
