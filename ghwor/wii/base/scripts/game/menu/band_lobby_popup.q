...
script band_lobby_create_popup_menu \{menu = !q1768515945
	...
	if ScreenElementExists id = <container>
		...
		switch <menu>
			case inviteslist
			if NOT GotParam \{submenu_index}
				ScriptAssert \{qs(0x58ff35ec)}
			endif
			band_lobby_create_popup_menu_inviteslist parent = <container> Player = <Player> submenu_index = <submenu_index>
			back_script = band_lobby_friendslist
			back_params = {Player = <Player> submenu_index = <submenu_index>}
			case friendslist
			if NOT GotParam \{submenu_index}
				ScriptAssert \{qs(0x58ff35ec)}
			endif
			band_lobby_create_popup_menu_friendslist online = <online> parent = <container> Player = <Player> submenu_index = <submenu_index>
			back_script = band_lobby_exit_friendslist
			back_params = {Player = <Player> container = <container>}
			case playerslist
			...
			case dlc_error_msg
			printstruct <...>
			back_script = band_lobby_dlc_error_back
			band_lobby_create_popup_menu_dlc_error parent = <container> Player = <Player> error_text = <error_text>
			case generic_vote
...
script band_lobby_create_popup_menu_dlc_error 
	RequireParams \{[
			parent
			Player
		]
	...
	formatText TextName = error_text qs(0x934ba0f9) e = <error_text>
	create_dialog_box {
		parent = <parent>
		player_device = <controller>
		back_button_script = nullscript
...
script band_lobby_create_popup_menu_friendslist 
	...
	CreateScreenElement {
		...
		z_priority = 20.0
		pu_friends_online_num_text = qs(0x6160dbf3)
		title_text = <title>
		friends_online_alpha = <friend_alpha>
	}
	Change g_current_friends_popup_id = ($g_current_friends_popup_id + 1)
	AssignAlias id = <id> alias = current_band_lobby_popup
	getplayerinfo <Player> gamertag
	<id> :se_setprops pu_gamertag_text = ($<gamertag>)
...
script band_lobby_friendslist_callback_spawned 
	...
	RequireParams \{[
		...
		all}
	if StructureContains \{structure = params
			is_refresh}
		is_refresh = 1
	endif
	if NOT GotParam \{is_refresh}
		if NOT GotParam \{no_wait}
			Wait \{0.5
				Second}
		endif
		formatText checksumName = submenu_data 'band_lobby_submenu_data_%d' d = (<params>.submenu_index)
		Change g_friendlist_refresh_player = ($<submenu_data>.Player)
	endif
	getplayerinfo ($g_friendlist_refresh_player) controller
	if ((GotParam friendlist) && (ScreenElementExists id = band_lobby_popup_menu))
		LaunchEvent \{Type = focus
			target = band_lobby_popup_menu}
		if NOT GotParam \{is_refresh}
			band_lobby_update_button_helpers menu = friendslist Player = ($<submenu_data>.Player) controller = <controller>
		else
			band_lobby_popup_menu :GetSingleTag \{desc_id}
			if ScreenElementExists id = <desc_id>
				<desc_id> :se_setprops pu_spinner_cont_alpha = 1.0
			endif
		endif
			...
			if (<array_Size> > 0)
				...
				if pushfrontendoverridequota \{quota = quota_sprites}
					...
					band_lobby_add_player_ui_to_popup {
						...
						controlling_player = ($g_friendlist_refresh_player)
						desc_id = 'band_lobby_pu_friend_item'
						rich_presence = (<friendlist> [<i>].richpresence)
						rival = (<friendlist> [<i>].rival)
						list_type = friends
						remote_player = 1
						cant_invite_reason = <cant_invite_reason>
					}
					array = []
				...
				if CheckForSignIn controller_index = <controller> dont_set_primary network_platform_only
					readleaderboardbyuidlist request_id = ($g_current_friends_popup_id) callback = band_lobby_rock_record_callback_friends leaderboard_id = lb_rock_record_2 user_id_list = <rock_record_uids>
				else
					LaunchEvent \{Type = focus
						target = band_lobby_popup_menu}
...
script band_lobby_rock_record_callback 
	...
	printstruct <...>
	if ScreenElementExists \{id = band_lobby_popup_menu}
		if NOT ScreenElementExists \{id = invitelist_popup}
			<rows_size> = 0
			if GotParam \{rows}
				GetArraySize <rows>
...
script band_lobby_create_popup_change_profile 
	...
	if (<msg_checksum> = change_profile)
		AddArrayElement array = <array> element = {text = ($band_lobby_strings.continue_guest) func = band_lobby_call_signin_ui_from_popup func_params = {func = showsigninui params = {convert_offline_to_guest}}}
	endif
	if NOT NetSessionFunc func = ismultiplayerallowed controller_index = <controller>
		AddArrayElement array = <array> element = {text = ($band_lobby_strings.continue_guest) func = band_lobby_call_signin_ui_from_popup func_params = {func = showsigninui params = {convert_offline_to_guest}}}
	endif
	AddArrayElement array = <array> element = {text = ($band_lobby_strings.cancel) func = LaunchEvent func_params = {Type = pad_back target = <menu_id>}}
	getplayerinfo <Player> controller
	create_dialog_box {
		parent = <parent>
		player_device = <controller>
		heading_text = ($band_lobby_strings.popup_title_change_profile)
		body_text = ($band_lobby_strings.<msg_checksum>)
		back_button_script = nullscript
		options = <array>
	}
...
script band_lobby_add_player_ui_to_popup \{player_index = !i1768515945
		...
		if GotParam \{friendstate}
			...
			elseif isngc
				if (<rival> = 1)
					switch <friendstate>
						case online
						band_lobby_get_part_icon_parts \{part = rival_online}
						case offline
						band_lobby_get_part_icon_parts \{part = rival_offline}
					endswitch
				else
					band_lobby_get_part_icon_parts part = <friendstate>
				endif
			...
			if (1 = <cant_invite>)
				switch (<cant_invite_reason>)
					case offline
					rich_presence = qs(0x49338c54)
					case already_in_game
					rich_presence = qs(0xbf0fdc23)
					default
	...
	if (<remote_player> = 1)
		...
		if (<rival_op> != None)
			...
			switch <rival_op>
				case add
				<op_script> = band_lobby_send_rival_request
				case remove
				<op_script> = band_lobby_remove_rival
				case withdraw
				<op_script> = band_lobby_withdraw_rival_proposal
			endswitch
			AddArrayElement array = <array> element = {pad_L1 <op_script> params = {net_id = <net_id> Name = <gamertag_text> element_id = <id>}}
		endif
	endif
	if band_lobby_show_compare_achievements {
			list_type = <list_type>
			remote_player = <remote_player>
		}
		AddArrayElement array = <array> element = {
			pad_option2 band_lobby_create_achievements_ui
			params = {
				element_id = <id>
				player_uid = <net_id>
				player_name = <gamertag_text>
				list_type = <list_type>
...
script band_lobby_create_achievements_ui \{element_id = !q1768515945
		player_uid = !a1768515945
		player_name = 0x69696969
		Player = !i1768515945
		list_type = !q1768515945}
	if ScreenElementExists \{id = band_lobby_desc_id}
		if band_lobby_desc_id :desc_resolvealias \{Name = alias_pu_cont_parent}
			band_lobby_exit_friendslist Player = <Player> container = <resolved_id>
		endif
	endif
	data = {
		exit_Script = band_lobby_return_from_achievements_ui
		exit_script_params = {element_id = <element_id> Player = <Player> list_type = <list_type>}
		friends_uid = <player_uid>
		friends_name = <player_name>
	}
	band_lobby_change_focus_submenu_all \{focus_type = unfocus}
	generic_event_choose {
		state = uistate_achievements
		data = <data>
	}
endscript

script band_lobby_return_from_achievements_ui \{element_id = !q1768515945
		Player = !i1768515945
		list_type = !q1768515945}
	band_lobby_kick_leader_out_of_band_manage
	band_lobby_change_focus_submenu_all \{focus_type = focus}
...
script band_lobby_popup_change_focus 
	...
	if checksumequals a = <focus_type> b = focus
		...
		if (<remote_player> = 1)
			...
			if (<rival_op> != None)
				...
				switch <rival_op>
					...
					text = qs(0x463e2ed8)
				endswitch
				add_user_control_helper controller = <controller> text = <text> button = orange z = 100000
			else
		...
		if band_lobby_show_compare_achievements {
			...
			add_user_control_helper controller = <controller> text = qs(0x55b5f54c) button = yellow z = 100000
		else
...
script band_lobby_get_part_icon_parts 
	...
	switch <part>
		...
		case rival_online
		return \{part_props = {
				part_icon_top_texture = rival_online
			}}
		case rival_offline
		return \{part_props = {
				part_icon_top_texture = rival_offline
			}}
		case away
		return \{part_props = {
...
script band_lobby_exit_friendslist 
	RequireParams \{[
			Player
			container
		]
	...
	if NOT NetSessionFunc \{Obj = friends
			func = have_pending_invites}
		if NOT bl_does_have_remote_party_connections
			if band_lobby_are_no_remote_players_in_game
				band_lobby_change_network Player = <Player> network = local
			endif
		endif
	endif
	band_lobby_destroy_popup_menu container = <container>
...
script band_lobby_refresh_friendslist_popup 
	printf \{channel = band_lobby
		qs(0xb724ffe9)}
	if ScreenElementExists \{id = wii_friends_list}
		band_lobby_wait_for_friendslist_safe_refresh
		Change \{g_friendlist_ready_for_refresh = 0}
		if wii_friends_list :desc_resolvealias \{Name = alias_pu_menu
				param = menu_id}
...
script band_lobby_wait_for_friendslist_safe_refresh 
	begin
	if (($g_friendlist_ready_for_refresh) = 1)
		break
	endif
	Wait \{1
		gameframe}
...
script band_lobby_rock_record_callback_friends \{request_id = !i1768515945}
	if (<request_id> = ($g_current_friends_popup_id))
		band_lobby_rock_record_callback <...>
	endif
endscript

script band_lobby_dlc_error_back 
	if ScreenElementExists \{id = band_lobby_popup_menu}
		LaunchEvent \{Type = unfocus
			target = band_lobby_popup_menu}
	endif
...
script band_lobby_send_rival_request \{Name = 0x69696969
		net_id = !a1768515945}
	if is_net_id_zero net_id = <net_id>
		ScriptAssert \{'invalid net id passed to send_rival_request'}
		return
	endif
	StringLength string = <Name>
	if (<str_len> = 0)
		ScriptAssert \{'rival name cannot be blank'}
		return
	endif
	if NOT (NetSessionFunc Obj = friends func = is_in_rivals_list params = {net_id = <net_id>})
		NetSessionFunc Obj = friends func = send_rival_request params = {net_id = <net_id> Name = <Name>}
		user_control_destroy_helper \{button = orange}
		displaynotification \{qs(0xa541139a)
			Type = rival}
	endif
endscript

script band_lobby_remove_rival \{Name = 0x69696969
		net_id = !a1768515945
	...
	band_lobby_confirm_remove_rival_popup \{op_text = qs(0x9f078188)}
	Block \{Type = rival_popup_complete}
	destroy_dialog_box
	band_lobby_update_button_helpers menu = friendslist Player = (<controller> + 1)
	band_lobby_refocus_popup
	if ((<event_data>.confirmed) = true)
		NetSessionFunc Obj = friends func = remove_rival params = {net_id = <net_id> Name = <Name>}
...
script band_lobby_withdraw_rival_proposal \{Name = 0x69696969
		net_id = !a1768515945}
	if is_net_id_zero net_id = <net_id>
		ScriptAssert \{'invalid net id passed to send_rival_request'}
		return
	...
	band_lobby_confirm_remove_rival_popup \{op_text = qs(0x7bbb09da)}
	Block \{Type = rival_popup_complete}
	destroy_dialog_box
	band_lobby_update_button_helpers menu = friendslist Player = (<controller> + 1)
	band_lobby_refocus_popup
	if ((<event_data>.confirmed) = true)
		NetSessionFunc Obj = friends func = withdraw_rival_proposal params = {net_id = <net_id> Name = <Name>}
		if ScreenElementExists id = <element_id>
			DestroyScreenElement id = <element_id>
		endif
	endif
...
script band_lobby_confirm_remove_rival_popup \{heading_text = qs(0xaa163738)
		op_text = 0x69696969}
	pre_string = qs(0x6b6110f5)
	post_string = qs(0x43303387)
	formatText TextName = body_text qs(0x92ebaa6a) a = <pre_string> b = <op_text> c = <post_string>
	options = [
		{
			func = broadcastevent
			func_params = {Type = rival_popup_complete data = {confirmed = true}}
			text = qs(0x58e0a1fb)
		}
		{
			func = broadcastevent
			func_params = {Type = rival_popup_complete data = {confirmed = FALSE}}
			text = qs(0xd2915c27)
		}
	]
	LaunchEvent \{Type = unfocus
...
script band_lobby_get_rival_op \{list_type = None
		net_id = [
			0
			0
		]
		sent_friend_request = invalid}
	rival_op = None
	is_rival = (NetSessionFunc Obj = friends func = is_in_rivals_list params = {net_id = <net_id>})
	is_friend = (NetSessionFunc Obj = friends func = is_in_friends_list params = {net_id = <net_id>})
	...
	elseif (<is_rival>)
		if (<sent_friend_request> = true)
			<rival_op> = withdraw
		else
			<rival_op> = remove
		endif
	endif
	return rival_op = <rival_op>
endscript

script band_lobby_show_compare_achievements \{list_type = !q1768515945
		remote_player = !i1768515945}
	should_show = FALSE
...
script band_lobby_refocus_popup 
	if ScreenElementExists \{id = current_band_lobby_popup}
		if current_band_lobby_popup :desc_resolvealias \{Name = alias_pu_menu
				param = menu_id}
