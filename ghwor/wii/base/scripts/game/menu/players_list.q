	...
	if ((<current_num_players> != <num_players>) || ($players_list_force_refresh = 1))
		...
		if (0 < <num_players>)
			...
			if playerinfoequals <Player> is_local_client = 1
				...
				band_lobby_add_player_ui_to_popup player_index = <Player> parent_id = <objID> controlling_player = <controlling_player> select_actions = <select_actions> list_type = <list_type> remote_player = 1
			endif
