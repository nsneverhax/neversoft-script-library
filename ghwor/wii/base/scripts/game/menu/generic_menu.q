...
script add_generic_menu_text_item {
	...
	if ScreenElementExists id = <parent>
		...
		if <id> :desc_resolvealias Name = alias_alternate_pointer_target param = alternate_pointer_target
			<alternate_pointer_target> :SetTags pointer_focusable = <pointer_focusable>
			<alternate_pointer_target> :SetTags alternate_focus_id = <id>
		else
			<id> :SetTags pointer_focusable = <pointer_focusable>
		endif
	else
	...
	if GotParam \{pad_square_script}
		SetScreenElementProps {
			id = <id>
			event_handlers = [
				{pad_option <pad_choose_sound> params = {menustate = <pad_select_sound_menustate> uitype = <pad_select_sound_uitype>}}
				{pad_option generic_blocking_execute_script params = {pad_script = <pad_square_script> pad_params = {container_id = <id> <pad_square_params>}}}
			]
		}
	endif
	if GotParam \{pad_start_script}
	...
	if GotParam \{not_focusable}
		if GotParam \{header_text}
			rgba = [0 0 0 255] (($g_menu_colors).menu_subhead)
		else
			rgba = (($default_color_scheme).text_color)
		endif
...
script menu_finish controller = ($primary_controller)
	if NOT GotParam \{no_helper_text}
		...
		elseif GotParam \{car_rotate_zoom}
			menu_finish_rotate_zoom <...>
		elseif GotParam \{car_helper_text_cancel_only}
			add_user_control_helper controller = <controller> text = qs(0xf7723015) button = red z = 100000
			menu_finish_rotate_zoom <...>
		else
		endif
