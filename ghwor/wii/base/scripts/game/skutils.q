...
script autolaunch_spawned_and_wait \{Camera = 'none'
	...
	if GotParam \{song}
		playlist_clear
		begin
		playlist_addsong song = <song> version = <version>
		repeat 10
		startnow = 1
	endif
	...
	if GotParam \{competitive_goal}
		...
		Change \{g_net_leader_player_num = 1}
		startnow = 1
	elseif GotParam \{quickplay_challenges}
		playlist_clear
...
script SetPakManCurrentBlock \{block_scripts = 0
	...
	if (<map> = zones)
		...
		formatText TextName = pak_path 'pak/zones/%s_assets.pak' s = <pakname>
		printf 'Unloading level asset pak (%s)' s = <pak_path>
		if ispakloaded <pak_path> Heap = quota_venue
			UnLoadPak <pak_path> Heap = quota_venue
			WaitUnloadPak <pak_path>
		else
			printf \{'Level asset pak wasnt loaded'}
		endif
	endif
	...
	if (<map> = zones && <pak> != None)
		GetPakManCurrentName \{map = zones}
		if (<pakname> != 'unknown')
			formatText TextName = pak_path 'pak/zones/%s_assets.pak' s = <pakname>
			printf 'Loading level asset pak (%s)' s = <pak_path>
			if NOT ispakloaded <pak_path>
				LoadPakAsync async = (1 - <block_scripts>) pak_name = <pak_path> Heap = quota_venue
			endif
		endif
	endif
	return \{true}
...
script Zones_PakMan_Init 
	...
	formatText checksumName = gfx2_array_name '%z_GFX2_NodeArray' z = <zone_string_name>
	...
	formatText checksumName = script_zone_gfx_init '%z_gfx2_init' z = <zone_string_name>
...
script Zones_PakMan_DeInit 
	...
	formatText checksumName = script_zone_gfx_deinit '%z_gfx2_deinit' z = <zone_string_name>
