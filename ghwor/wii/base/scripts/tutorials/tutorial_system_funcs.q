...
tutorial_paused = 0
tutorial_current_pak = 'none'
practice_enabled = 0
training_mode = tutorials
g_tutorial_part_entered_with = guitar
...
script tutorial_system_get_pak \{tutorial_header = 0x69696969
		tutorial_id = 0x69696969
		lesson_id = 0x69696969}
	printf \{channel = tutorial
		qs(0xd1cfdd23)}
	formatText TextName = pakname 'tut_%a_%b' a = <tutorial_id> b = <lesson_id> AddToStringLookup = true
	printf channel = tutorial 'Tutorial will look for pak %a' a = <pakname>
...
script tutorial_hide_venue 
	if ScreenElementExists \{id = tutorial_misc_root}
		DestroyScreenElement \{id = tutorial_misc_root}
	endif
	CreateScreenElement \{parent = root_window
		id = tutorial_misc_root
		Type = ContainerElement
		Pos = (0.0, 0.0)
		z_priority = 0
		alpha = 1}
	CreateScreenElement \{Type = SpriteElement
		parent = tutorial_misc_root
		texture = white
		rgba = [
			0
			0
...
script tutorial_system_start_lesson \{event = menu_replace
	...
	ui_sfx \{menustate = Generic
		uitype = select}
	tutorial_system_get_pak {
		tutorial_header = <tutorial_header>
		tutorial_id = <tutorial_id>
		lesson_id = <lesson_id>
	}
	tutorial_load_pak pakname = <pakname>
...
script tutorial_system_start_page \{event = menu_replace}
	...
	formatText checksumName = state 'UIstate_tutorial_%s' s = (<page_struct>.Type) AddToStringLookup = true
	if GotParam \{autosave}
		...
		tutorial_anti_spam_timer_reset
		ui_event_wait event = <event> data = {state = <state> page_struct = <page_struct> part = <part>}
	endif
endscript

script tutorial_system_page_complete_buttoned 
	if ($tutorial_paused = 1)
		printf \{channel = tutorial
			'Game paused, ignore'}
	else
		if tutorial_anti_spam_timer_okay
			tutorial_system_page_complete <...>
		else
			printf \{channel = tutorial
				'[Skip] ignored, too close to last input/start page'}
		endif
	endif
endscript

script tutorial_system_page_complete \{page_struct = 0x69696969
		from_gameplay = 0}
	printf \{channel = tutorial
		qs(0x05081698)}
...
script tutorial_system_lesson_complete_anim \{anim_text = $wii_tutorial_complete}
	printf \{channel = tutorial
		qs(0xc840d7d0)}
...
script tutorial_system_create_description \{page_struct = 0x69696969
		from_gameplay = 0}
	printf \{channel = tutorial
		qs(0x7530ee46)}
	...
	tutorial_hide_venue
	if (<from_gameplay> = 1)
		<id> :se_setprops ui_visualizer_z_priority = -1
	else
		<id> :se_setprops ui_visualizer_z_priority = 1
	endif
	LaunchEvent \{Type = focus
...
script tutorial_system_create_pausemenu_description 
	...
	generic_menu :se_setprops \{description_text = qs(0x03ac90f0)}
endscript
...
script tutorial_system_setup_event_handlers 
	if ScreenElementExists \{id = tutorial_main}
		SetScreenElementProps {
			...
			event_handlers = [
				{pad_choose tutorial_system_page_complete_buttoned params = {page_struct = <page_struct>}}
			]
			replace_handlers
		}
	endif
...
script tutorial_quit_from_pause_menu 
	StopSoundsByBuss \{tutorial_vo}
...
script tutorial_load_pak \{pakname = 0x69696969}
	formatText TextName = full_pakname 'pak/ui/%a.pak' a = <pakname> AddToStringLookup = true
	if NOT ispakloaded <full_pakname> Heap = quota_misc
		if NOT ($tutorial_current_pak = 'none')
			UnLoadPak ($tutorial_current_pak) Heap = quota_misc
			WaitUnloadPak ($tutorial_current_pak) Block
			Change \{tutorial_current_pak = 'none'}
		endif
		if NOT (<pakname> = 'none' || <pakname> = None)
			printf channel = tutorial 'Tutorial: manually loading pak %a' a = <full_pakname>
			LoadPak <full_pakname> Heap = quota_misc
			Change tutorial_current_pak = <full_pakname>
		else
			Change \{tutorial_current_pak = 'none'}
		endif
	else
		printf channel = tutorial 'Tutorial: desired pak %a already loaded.' a = <full_pakname>
	endif
endscript
g_tutorial_anti_spam_clock = 0

script tutorial_anti_spam_timer_reset 
	GetStartTime
	Change g_tutorial_anti_spam_clock = <starttime>
endscript

script tutorial_anti_spam_timer_okay 
	getelapsedtime \{starttime = $g_tutorial_anti_spam_clock}
	if (<ElapsedTime> > 500)
		tutorial_anti_spam_timer_reset
		return \{true}
	endif
...
script tutorial_verify_ui_pak_loaded 
	if NOT ispakloaded \{'pak/ui/tutorial_shared.pak'}
		SetPakManCurrent \{map = ui_paks
			pakname = 'tutorial_shared'}
		BlockPendingPakManLoads \{map = ui_paks}
	endif
endscript
