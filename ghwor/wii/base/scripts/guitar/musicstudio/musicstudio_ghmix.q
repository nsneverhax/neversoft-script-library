...
script musicstudio_ghmix_init \{editing = 0}
	...
	musicstudio_ghmix_create_highway \{small_view = 1}
...
script musicstudio_skip_forward \{speed_multiplier = 1.0}
	...
	printf channel = musicstudio qs(0x66cd9b84) s = <snap_time>
endscript
...
script musicstudio_paste 
	printf \{'musicstudio_paste'}
	musicstudio_mainobj :musicstudioghmix_paste Player = <Player>
	musicstudio_verify_patterns
	musicstudio_force_skip_to_closest_tick
endscript
...
script musicstudio_select_track 
	...
	if GotParam \{song}
		...
		musicstudio_ghmix_create_highway instrument = <new_instrument> small_view = 1
	endif
...
script musicstudio_show_note_limit_warning 
	...
	if (<Active> = 0)
		formatText TextName = note_limit_warning qs(0x3fe6b649) s = ($musicstudio_max_notes_per_song)
...
script musicstudio_show_track_note_limit_warning 
	musicstudio_recording_get_active
	if (<Active> = 1)
		formatText TextName = note_limit_warning qs(0x27f357d4) s = ($gemarraysize)
		if ScreenElementExists \{id = musicstudio_ghmix}
			musicstudio_ghmix :Obj_SpawnScriptNow musicstudio_show_popup_warning params = {warning = <note_limit_warning>}
		endif
	endif
...
script musicstudio_whammy_disable 
	musicstudio_mainobj :musicstudio_getcurrmainplayer
	<main_player> :musicstudioplayer_enablewhammyrewind enable = FALSE
endscript

script musicstudio_whammy_enable 
	musicstudio_mainobj :musicstudio_getcurrmainplayer
	<main_player> :musicstudioplayer_enablewhammyrewind enable = true
endscript

script whammy_set_rewind 
	...
	if ScreenElementExists \{id = musicstudio_ghmix}
		if musicstudio_ghmix :desc_resolvealias \{Name = alias_ghmix_highway
			...
			if <alias_ghmix_highway> :desc_resolvealias Name = ghmix_main_highway_arrow param = ghmix_main_highway_arrow
				...
				KillSpawnedScript \{Name = whammy_direction_change_arrow_notify}
				SpawnScriptNow \{whammy_direction_change_arrow_notify
					params = {
						Color = [
							255
							0
...
script whammy_set_fast_forward 
	Change \{jam_whammy_direction = 1}
	if ScreenElementExists \{id = musicstudio_ghmix}
		if musicstudio_ghmix :desc_resolvealias \{Name = alias_ghmix_highway
				param = alias_ghmix_highway}
...
script whammy_direction_change_arrow_notify \{Color = [
			255
			255
			255
			255
