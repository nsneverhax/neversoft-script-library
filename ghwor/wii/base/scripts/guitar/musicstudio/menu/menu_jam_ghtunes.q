...
script create_jam_ghtunes_menu 
	...
	if NOT NetSessionFunc \{func = can_view_user_content
		...
		get_last_connection_error_text \{context = Misc}
		jam_ghtunes_failed_dialog override_text = <error_text>
		return
	endif
...
script ghtunes_signin_check_unspawned 
	if netstatus_checkforerrors \{net_system = general}
		get_last_connection_error_text \{net_system = general}
		jam_ghtunes_failed_dialog override_text = <error_text>
		return
	endif
...
script jam_ghtunes_leaderboard_select_failed \{dialog = 0}
	...
	switch <dialog>
		case 0
		formatText TextName = dialog qs(0xce6ea53d) a = <leaderboard_name>
	endswitch
	destroy_dialog_box
	create_dialog_box {
			...
			if NOT (<results_array_size> = 0)
				...
				if ((<content_array> [<i>].has_content) = 1)
					...
					if (<i> = 0)
						...
						formatText TextName = title_text qs(0x7f93edfd) s = (<content_array> [<i>].FileName)
						(<children> [<i>]) :SetProps title_text = <title_text>
					endif
					if (<last_item> = 0)
...
script jam_ghtunes_failed_dialog \{dialog = 0
	...
	switch <dialog>
		case 0
		if isxenonorwindx
			dialog_txt = qs(0xb0240226)
		elseif isps3
			dialog_txt = 'You must be signed in to the PlayStation®Network to browse GH¥Tracks.'
		elseif isngc
		...
		dialog_txt = qs(0xb0240226)
		case 8
		dialog_txt = qs(0x3e3b856a)
		case 9
		dialog_txt = qs(0xb9fd7c2c)
	endswitch
	if (<override_text> != qs(0x03ac90f0))
		<dialog_txt> = <override_text>
	endif
	destroy_generic_menu
	create_dialog_box {
		heading_text = qs(0xd028ceaa)
