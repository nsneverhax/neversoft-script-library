...
script common_loading_transitionend 
	...
	if (($is_network_game = 1 && <Type> = career) || $is_network_game = 0)
		if NOT in_song_breakdown_state
			ui_event \{event = menu_back
				data = {
					state = Uistate_gameplay
				}}
		else
			printf \{'---- Common_Loading_TransitionEnd bypassing gameplay state change ----'}
		endif
...
script transition_play_spawned_exit_run 
	if isngc
		popunsafeforshutdown \{context = transition
			Type = unchecked}
	endif
	if ScriptExists <event>
		<event>
...
script Transition_Play_Spawned \{use_song_time = 1}
	...
	if isngc
		pushunsafeforshutdown \{context = transition
			Type = unchecked}
	endif
	GetPakManCurrentName \{map = zones}
	...
	OnExitRun transition_play_spawned_exit_run params = {event = <event>}
	<time> = 0.0
	if (<use_song_time> = 1)
...
script common_encore_spawned_waiting_then_playing_music_for_ui_screen 
	Wait \{2
		Seconds}
	soundname = nothing
	get_ui_screen_music_file_from_zone
	PlaySound <ui_music_soundname_checksum> vol = -10 buss = binkcutscenes
...
script get_ui_screen_music_file_from_zone 
	GetPakManCurrentName \{map = zones}
	...
	return ui_music_soundname_checksum = <ui_music>
endscript
...
script start_preloaded_finalbandoutro_stream 
	printf \{channel = sfx
		qs(0x12d0a014)}
	if NOT ($celeb_intro_stream_id = None)
		StopSoundEvent \{$g_current_crowd_encore
			fade_time = 2.5
			fade_type = log_slow}
