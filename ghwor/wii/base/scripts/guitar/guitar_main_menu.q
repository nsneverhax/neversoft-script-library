...
script mainmenu_populate_menu_options_global 
	mainmenu_text = {
		Play = {
			...
			helper_text = qs(0xee3bc545)
		}
		create = {
			menu_item_text = qs(0x122e3c10)
			helper_text = qs(0x4e5ce1c5)
		}
		career = {
			menu_item_text = qs(0x0f9f249e)
			helper_text = qs(0x228436ab)
		}
		quickplay = {
			menu_item_text = qs(0x7cdc150a)
			helper_text = qs(0xade4304a)
		}
		training = {
			menu_item_text = qs(0xd093953b)
			helper_text = qs(0x9e9d6ca4)
		}
		competitive = {
			menu_item_text = qs(0x6e63fe63)
			helper_text = qs(0xb58cef98)
		}
		party_play = {
		...
		creator = {
			...
			helper_text = qs(0x24d650ce)
		}
		options = {
			menu_item_text = qs(0x976cf9e7)
			helper_text = qs(0x310daf65)
		}
		award_case = {
			menu_item_text = qs(0xa1ae7e56)
			helper_text = qs(0x3a8c0869)
		}
		hall_of_records = {
			menu_item_text = qs(0x624c597d)
			helper_text = qs(0x584479a7)
		}
		debugmenu = {
		...
		motd = {
			...
			helper_text = qs(0xdbca8ada)
		}
		xbox_party = {
			menu_item_text = qs(0x58e064fa)
			helper_text = qs(0x385b994c)
		}
		audience_party = {
			menu_item_text = qs(0xfa304cae)
			helper_text = qs(0xec2d75ef)
		}
		roadie_battle = {
			menu_item_text = qs(0xa6a53570)
			helper_text = qs(0x7ce8735f)
		}
	}
...
script create_main_menu 
	...
	if ($autolaunch_cas = 1)
		...
		spawnscriptdelayed frames = 20 ui_event_wait params = {event = menu_change data = {state = uistate_character_selection device_num = ($primary_controller) from_main_menu = 1}}
	endif
	if ($autolaunch_jam = 1)
...
script main_menu_select_play 
	SetSpawnInstanceLimits \{Max = 1
		management = ignore_spawn_request}
	if should_show_social_network_setup
	...
	if (<show_help_page> = 1)
		...
		set_primary_controller device_num = <device_num> optional_signin = 1 state = <state>
	endif
endscript
...
script main_menu_select_band_lobby \{state = uistate_band_lobby
	...
	<data> = {<data> device_num = <device_num> from_main_menu = 1}
...
script is_in_music_store 
	if ($wii_entering_music_store = 0)
		return \{FALSE}
	endif
	return \{true}
endscript
...
script main_menu_select_music_store 
	if dlcmanagerfunc \{func = is_ecommerce_okay}
		...
		music_store_handle_error <...> player_device = <device_num> func = main_menu_roadie_battle_destroy_error_dialog params = {dont_remove_helpers}
	endif
endscript
...
script set_primary_controller \{event = menu_change
	...
	if GotParam \{jam}
		...
		generic_event_choose event = <event> state = uistate_signin data = {device_num = <device_num> show_mainmenu_bg = 1 no_hide_glitch = <no_hide_glitch> allow_back = 1 new_state = <state> new_data = {<data> jam = 1} jam = 1 require_signin = <require_signin> require_live = <require_live>}
	else
		if ($skip_signin = 1)
			assign_new_primary_controller device_num = <device_num>
			ui_event event = <event> state = <state> data = <data>
		else
			generic_event_choose event = <event> no_sound state = uistate_signin data = {device_num = <device_num> show_mainmenu_bg = 1 no_hide_glitch = <no_hide_glitch> allow_back = 1 new_state = <state> new_data = <data> require_signin = <require_signin> require_live = <require_live> downloads = <downloads> leaderboards = <leaderboards> musicstore = <musicstore> optional_signin = <optional_signin>}
		endif
	endif
endscript
...
script main_menu_enable_freeplay 
	...
	if NOT user_control_helper_exists \{button = yellow}
		add_user_control_helper \{text = qs(0x8a778398)
			...
			z = 100
			all_buttons
			pad_click_script = main_menu_select_freeplay}
	endif
...
script main_menu_select_roadie_battle 
	...
	set_primary_controller device_num = <device_num> state = uistate_roadie_battle_choose_mode data = <data>
endscript
...
script mainmenu_animate_spawned \{id = !q1768515945
	...
	if ScreenElementExists id = <id>
		if ($mainmenu_dropdown_triggered = 0)
			if ResolveScreenElementID id = {<id> child = 1}
				<parent_id> = <resolved_id>
				if ResolveScreenElementID id = {<parent_id> child = {root_container child = dropdown_1_container}}
					if ScriptIsRunning \{rb_animate_sway}
						KillSpawnedScript \{Name = rb_animate_sway}
					endif
					if ScriptIsRunning \{rb_animate_male_basket}
						KillSpawnedScript \{Name = rb_animate_male_basket}
					endif
					<resolved_id> :se_setprops Pos = (0.0, 0.0) time = 0.15
					<resolved_id> :Obj_SpawnScriptNow rb_destroy_when_completed_animating
				endif
				if ResolveScreenElementID id = {<parent_id> child = {root_container child = dropdown_2_container}}
					if ScriptIsRunning \{rb_animate_female_basket}
						KillSpawnedScript \{Name = rb_animate_female_basket}
					endif
					<female_id> = <resolved_id>
					ResolveScreenElementID id = {<resolved_id> child = lift}
					<female_id> :se_setprops Pos = (50.0, 50.0) time = 0.15
					<resolved_id> :se_setprops rot_angle = 0 time = 0.15
					<female_id> :Obj_SpawnScriptNow rb_destroy_when_completed_animating
				endif
			endif
		endif
		if (<block_events> = 1)
		...
		if ($mainmenu_dropdown_triggered = 0)
			DestroyScreenElement id = <parent_id>
