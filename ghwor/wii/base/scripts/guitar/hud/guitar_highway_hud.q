...
script setup_highway_hud 
	...
	setplayerinfo Player = <Player> highway_hud_root = <highway_hud_root>
	if (<part> = vocals)
...
script get_highway_hud_root_id \{Player = 1
		no_assert = 0}
	getplayerchecksum Player = <Player> checksum = highway_hud_root
	return highway_hud_root = <checksum_value>
endscript
...
script setup_challenge_alert_menu \{Player = !i1768515945}
	...
	if (<part> = vocals)
		...
		<alert_menu> :se_setprops Scale = <Scale> Pos = <offset>
	endif
endscript
...
script setup_sidebar_rockmeter \{Player = 1}
	...
	if NOT inroadiebattlemode
		if ((<Type> = career) || (<Type> = quickplay) || (<Type> = freeplay))
			...
			hud_recreate_rockmeter_judy_hilight_hack desc_id = <id>
...
script get_sp_meter_position \{Player = 1}
	...
	if (<part> = vocals)
		...
		if ScreenElementExists id = <sp_meter>
			<sp_meter> :desc_resolvealias Name = alias_center_cont param = id
			GetScreenElementPosition id = <id> summed
			position = <screenelementpos>
		endif
	endif
	...
	if (<Type> = career)
		if NOT InNetGame
			...
			if (<vocals_difficulty_rating> = 0)
				...
				hide_vocal_combo_meter Player = <Player>
			endif
		endif
	endif
endscript
...
script vocal_highway_hud_init_combo_meter 
	...
	if <id> :desc_resolvealias Name = alias_vocals_health_meter param = vocals_health_meter
		hud_recreate_rockmeter_judy_hilight_hack desc_id = <vocals_health_meter>
	else
		ScriptAssert 'player %p vocal health meter not found' p = <Player>
	endif
	if <id> :desc_resolvealias Name = alias_vocals_star_power_meter param = vocals_star_power_meter
		hud_recreate_vocals_rp_tesla_fx_hack desc_id = <vocals_star_power_meter>
	else
		ScriptAssert 'player %p star power meter not found' p = <Player>
	endif
	meters_id = <id>
	setplayerinfo <Player> combo_meter_id = <id>
