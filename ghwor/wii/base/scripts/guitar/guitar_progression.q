...
script Progression_EndCredits 
	pushunsafeforshutdown \{context = ending
		Type = unchecked}
	playlist_clear
	...
	if (<array_Size> > 0)
		Change \{real_end_credits = 1}
		create_credits_menu
		begin
		printf \{channel = clayton
			'Waiting for credits to finish'}
		Wait \{1
			gameframe}
	...
	popunsafeforshutdown \{context = ending
		Type = unchecked}
endscript
...
script progression_updatedetailedstatsforgig 
	...
	gamemode_gettype
	if (<Type> = career)
		get_song_name song = <current_song>
		current_chapter = ($current_chapter)
		format_career_song_gigtag chapter_name = ($<current_chapter>.Name) song_name = <song_name>
	endif
	getnumplayersingame
	if (<num_players> > 0)
		getfirstplayer
		...
		if inroadiebattlemode
			...
			setstructureparam {
				struct_name = curr_song_stats
				param = roadie_assist
				value = <assist_percentage>
			}
		endif
