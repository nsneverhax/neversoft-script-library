...
script ui_controller_disconnect_update 
	...
	if (<array_Size> > 0)
		...
		if GotParam \{part}
			if ($calibrate_lag_enabled = 1)
				if (($calibrate_prev_parts [<j> - 1].part) = vocals)
					<part> = vocals
				endif
			endif
			...
			if (<part> = vocals)
				...
				if isxenonorwindx
					...
					switch <mic_connected>
						...
						case controller_disconnected
						default
						if iscontrollerpluggedin controller = (<j> - 1)
							if IsGuitarController controller = (<j> - 1) use_raw_type
								formatText TextName = text qs(0x8af6f1c2) t = <text> j = <j>
							elseif isdrumcontroller controller = (<j> - 1) use_raw_type
								formatText TextName = text qs(0x8af6f1c2) t = <text> j = <j>
							else
	...
	if (<array_Size> > 0)
		...
		if GotParam \{menurow_txt_item_text}
			if (<menurow_txt_item_text> = qs(0x182f0173) || <menurow_txt_item_text> = qs(0x9c8bd769))
				...
				generic_barrel_menu_item_disable id = (<children> [0])
			endif
		endif
	endif
endscript
...
script ui_controller_disconnect_enable_continue 
	...
	if (<array_Size> > 0)
		...
		if GotParam \{menurow_txt_item_text}
			if (<menurow_txt_item_text> = qs(0x182f0173) || <menurow_txt_item_text> = qs(0x9c8bd769))
				generic_barrel_menu_item_enable id = (<children> [0])
				LaunchEvent \{Type = unfocus
					target = generic_barrel_vmenu
					data = {
						child_index = 1
...
script ui_controller_disconnect_handle_song_start 
	disable_pause
	gh_pause_game
	ui_controller_disconnect_disable_continue
endscript
