...
script ui_init_award_case 
	pushdisablerendering \{context = going_into_award_case
		Type = unchecked}
...
script ui_create_award_case \{return_from_song = 0
	...
	if NOT GotParam \{song}
		...
		if ((faststructurecontains structure = <ui_struct> Name = song_dlc_xen_index) && ((<ui_struct>.song_dlc_xen_index) > 0))
			formatText checksumName = dlc_checksum 'dlc%s' s = (<ui_struct>.song_dlc_xen_index)
			formatText checksumName = leaderboard_id 'dlc%s_guitar' s = (<ui_struct>.song_dlc_xen_index)
			awardcase :awardcaseuistack_setcurrentdlcsong song = <dlc_checksum>
		endif
		Change current_leaderboard_id = <leaderboard_id>
		Change \{q_award_case_current_goal_name = qp_score_points}
		...
		formatText TextName = awards_text qs(0x414ed9b9) c = <challenge_awards> t = <challenge_awards_total>
	...
	popdisablerendering \{context = going_into_award_case
		Type = unchecked}
endscript

script check_user_parental_settings 
	if isngc
		if NOT NetSessionFunc \{func = can_view_user_content
				params = {
			...
			if netstatus_checkforerrors \{context = Misc}
				show_last_connection_error \{context = Misc}
			endif
		endif
	endif
endscript
...
script ui_destroy_award_case 
	...
	KillSpawnedScript \{Name = award_case_load_song_score_stats_refresh_hack}
			...
			if (<is_song_lb> = 1)
				...
				elseif ($g_award_case_lb_mode = overall)
					if awardcase :awardcaseuistack_hassongscore instrument_index = <instrument_index> player_id = <net_id> goal_name = <goal_name>
						...
						add_super_user_control_helper \{text = qs(0x5979487f)
							button = blue}
					endif
				else
					add_super_user_control_helper \{text = qs(0x5979487f)
...
script award_case_display_error_message \{reason = None
	...
	if inroadiebattlemode
		award_case_show_offline_view \{skip_future_offline_msgs = 1}
		return
	endif
	...
	if ObjectExists \{id = awardcase}
		if NOT awardcase :awardcaseuistack_isofflinemode
			if NOT NetSessionFunc \{func = can_view_user_content
				...
				award_case_show_offline_view
				printf \{channel = award_case
					'Abort displaying offline error dialog because of parental controls'}
				return
			endif
			printf \{channel = award_case
				qs(0x4c4cce1f)}
...
script award_case_load_song_score_stats_refresh_hack 
	SetSpawnInstanceLimits \{Max = 1
		management = kill_oldest}
	if ObjectExists \{id = awardcase}
		Wait \{1
			Seconds}
		SpawnScriptNow award_case_load_song_score_stats params = {instrument_index = <instrument_index> goal_name = <goal_name> inst_config = <inst_config> wait_for_settle = <wait_for_settle>}
	endif
endscript
...
script award_case_load_song_score_stats \{instrument_index = !i1768515945
	...
	SetSpawnInstanceLimits \{Max = 1
		management = kill_oldest}
	if ScriptIsRunning \{award_case_load_song_score_stats_refresh_hack}
		KillSpawnedScript \{Name = award_case_load_song_score_stats_refresh_hack}
	endif
	...
	if ($g_award_case_lb_mode = friends)
		...
		GetArraySize \{display_rows}
		if (<array_Size> = 0)
			SpawnScriptNow award_case_load_song_score_stats_refresh_hack params = {instrument_index = <instrument_index> goal_name = <goal_name> inst_config = <inst_config> wait_for_settle = <wait_for_settle>}
		endif
	...
	if awardcase :awardcaseuistack_getoverallsongscores <...>
		...
		if GotParam \{is_top}
			...
			if ((<row>.index) = <page_trigger_point>)
				...
				if isngc
					SpawnScriptNow \{wii_award_case_halt_while_fetching_rows}
				endif
			endif
...
script award_case_dynamic_rows_update 
	...
	if isngc
		broadcastevent \{Type = wii_leaderboard_fetch_complete}
	endif
endscript
...
script award_case_setup_challenge_menus \{return_from_song = !i1768515945}
	printf \{channel = award_case
		qs(0x0dcce442)}
	if isngc
		get_player_num_from_controller controller_index = ($lb_controller)
	else
		get_local_in_game_player_num_from_controller controller_index = ($lb_controller)
	endif
	if (<player_num> = -1)
...
script display_target_challenge_config_warning \{target_scores = [
	...
	if (<num_players> > 0)
		...
		elseif (<instrument> != Band)
			if (<num_players> > 1 && (<goal_name> = qp_power_score_points || <goal_name> = qp_score_points || <goal_name> = qp_rockout_time || <goal_name> = qp_score_spower || <goal_name> = qp_spower_banked || <goal_name> = qp_spower_time))
				<show_challenge_warning> = 1
				<solo_only_warning> = 1
			elseif (<found_one> = 0)
				<show_challenge_warning> = 1
	...
	elseif (<instrument> = drum && (isrbdrum controller = <controller>) &&
			(<goal_name> = qp_green_notes || <goal_name> = qp_orange_notes || <goal_name> = qp_orange_notes))
	...
	<single_option> = 0
	if (<has_valid_instrument_for_song> = 0 && <can_switch_instrument> = 0)
		<show_challenge_warning> = 1
		<single_option> = 1
		<remove_targeted_challenge> = 1
		<i> = 0
...
script wii_award_case_halt_while_fetching_rows 
	pushblockbuttoneventmappings
	award_case_desc_id :se_setprops \{stats_menu_alpha = 0.0
		instrument_spinner_alpha = 1.0}
	Block \{Type = wii_leaderboard_fetch_complete}
	award_case_desc_id :se_setprops \{stats_menu_alpha = 1.0
		instrument_spinner_alpha = 0.0
		time = 0.15}
