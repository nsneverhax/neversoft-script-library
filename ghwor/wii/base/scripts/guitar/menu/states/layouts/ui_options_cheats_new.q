...
script ui_create_options_cheats_new 
	...
	if cheatsinterface :desc_resolvealias \{Name = alias_key_bars}
		...
		<pad_green> = pad_choose
		<pad_red> = pad_back
		<pad_blue> = pad_option
		<pad_yellow> = pad_option2
		if isps3
...
script ui_options_cheats_new_reset_bars 
	...
	if GetScreenElementChildren \{id = bar_menu}
		...
		<current_key> :se_setprops {
			rgba = [188 210 215 255]
			time = 0.1
		}
		cheatsinterface :se_setprops channel_text = <channel_text>
...
script ui_options_cheats_new_sound_events 
	switch (<Sound>)
		...
		switch (<key>)
			...
			case yellow
			SoundEvent \{event = audio_ui_cheat_select_yellow}
			case blue
		...
		switch (<cheat>)
			case unlocked
			SoundEvent \{event = audio_ui_cheat_positive}
			case already_unlocked
			SoundEvent \{event = audio_ui_cheat_negative}
			case incorrect
			SoundEvent \{event = audio_ui_cheat_negative}
		endswitch
		case shake
