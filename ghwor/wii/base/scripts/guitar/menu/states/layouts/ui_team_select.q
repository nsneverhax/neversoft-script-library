...
script choose_random_band_names 
	randomize
	if InNetGame
		SendStructure callback = net_set_band_names data_to_send = {rand_seed = <rand_seed>}
	endif
...
script team_select_update_player_ui 
	...
	if (<band_request> > 0 && <position> = Normal)
		if (<slide_dir> = left)
			se_setprops \{player_info_pos = (-80.0, 0.0)
...
script team_select_swap_loop 
	...
	if teamswaploop team_0 = <team_0> team_1 = <team_1>
		...
		begin
		if (0 = <loop_count>)
			<new_team_array> = <new_team_0>
			<array> = <team_1>
		else
			<new_team_array> = <new_team_1>
			<array> = <team_0>
		endif
		fastgetarraysize array = <new_team_array>
		if (0 < <array_Size>)
			<new_team_index> = 0
			begin
			RemoveArrayElement array = <array> element = (<new_team_array> [<new_team_index>])
			<new_team_index> = (1 + <new_team_index>)
			repeat <array_Size>
		endif
		if (0 = <loop_count>)
			<team_1> = (<array> + <new_team_1>)
			changeglobalarray global_name = g_team_1 new_value = <team_1>
		else
			<team_0> = (<array> + <new_team_0>)
			changeglobalarray global_name = g_team_0 new_value = <team_0>
		endif
...
script team_select_reorder_teams 
	...
	if (<max_players> > 0)
		...
		if (<num_players> > 0)
			...
			setplayerinfo <restore_player> Player = <restore_player>
			team_select_dump_players
...
script debug_dump_team_globals 
	printf \{qs(0x11f0b140)}
	fastgetarraysize \{array = $g_team_0}
	if (0 < <array_Size>)
		<index> = 0
		begin
		printf qs(0x26eb2bf0) a = ($g_team_0 [<index>])
		<index> = (1 + <index>)
		repeat <array_Size>
	else
		printf \{qs(0x6be2f78f)}
	endif
	printf \{qs(0x08eb8001)}
	fastgetarraysize \{array = $g_team_1}
	if (0 < <array_Size>)
		<index> = 0
		begin
		printf qs(0x26eb2bf0) a = ($g_team_1 [<index>])
		<index> = (1 + <index>)
		repeat <array_Size>
	else
		printf \{qs(0x6be2f78f)}
...
script debug_dump_team_globals_spawned 
	begin
	debug_dump_team_globals
	WaitOneGameFrame
	repeat
endscript
