			...
			if StructureContains structure = (<option_array> [<i>]) icon
				...
				if StructureContains structure = (<option_array> [<i>]) pad_direction
					if (((<option_array> [<i>]).pad_direction) = horiz)
						...
						<pad_left_params> = {part = cas_body group_name = ((<option_array> [<i>]).group_name) rebuild = 1}
						<pad_right_script> = update_bone_deformation
						<pad_right_params> = {part = cas_body group_name = ((<option_array> [<i>]).group_name) rebuild = 1}
				...
				if (<set_up_down> = true)
					...
					<pad_up_params> = {part = cas_body group_name = ((<option_array> [<i>]).group_name) rebuild = 1}
					<pad_down_script> = update_bone_deformation
					<pad_down_params> = {part = cas_body group_name = ((<option_array> [<i>]).group_name) rebuild = 1}
...
script restore_group_defaults 
	...
	if StructureContains structure = ($cas_current_appearance) presets
		if StructureContains structure = (($cas_current_appearance).presets) car_presets_original_face
			<preset_index> = (((($cas_current_appearance).presets).car_presets_original_face).index)
			<face_preset> = (($car_presets_face.<gender>) [<preset_index>])
		endif
	endif
	if (<gender> = male)
	...
	if GotParam \{face_preset}
		default_preset_profile = {<default_preset_profile> <face_preset>}
	endif
	preset_body_bones = {}
...
script update_bone_deformation 
	...
	printf \{'===========UPDATE BONE DEFORMATION STRUCTS======================='}
	printstruct ($cas_current_appearance)
	if GotParam \{rebuild}
		rebuildcurrentcasmodel
	else
		updatecurrentcasmodel buildscript = modelbuilder_build_model buildscriptparams = {kill_objects = <kill_objects>}
	endif
endscript
