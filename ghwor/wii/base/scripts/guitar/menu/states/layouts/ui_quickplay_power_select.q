...
script ui_create_quickplay_power_select \{device_num = !i1768515945
	...
	if qp_power_selection_id :desc_resolvealias \{Name = alias_power_select_menu
		...
		if GetScreenElementChildren id = <power_select_menu>
			<power_select_children> = <children>
			GetArraySize \{power_select_children
				param = power_select_children_size}
			if (<power_select_children_size> > 0)
				<child_idx> = 0
				begin
				...
				if (<power_level> > 0)
					...
					if (<power_level> = 2)
						<level> = 2
						<level_text> = qs(0x5e52a35b)
						<fx_special_alpha> = 1
						<power_name_text> = <power_text2>
					else
						<level> = 1
						<level_text> = qs(0x757ff098)
						<fx_normal_alpha> = 1
					...
					<level_text> = qs(0xacc90def)
					<power_name_text> = qs(0x24a08263)
					<power_desc_text> = qs(0xced0fd2e)
				endif
				(<power_select_children> [<child_idx>]) :desc_applyscene Name = unselected time = 0
				(<power_select_children> [<child_idx>]) :se_setprops {
					event_handlers = [
						{focus qp_power_selection_item_focus params = {rgba = <focus_rgba> objID = (<power_select_children> [<child_idx>])}}
						{unfocus qp_power_selection_item_unfocus params = {rgba = <unfocus_rgba> objID = (<power_select_children> [<child_idx>])}}
				...
				if GetScreenElementChildren id = (<power_select_children> [<child_idx>])
					<element1> = (<children> [0])
					if GetScreenElementChildren id = <element1>
						<element2> = (<children> [1])
						if GetScreenElementChildren id = <element2>
							<element2> :se_setprops position_children = FALSE
							(<children> [2]) :se_setprops Pos = {relative (-20.0, 0.0)}
						endif
					endif
				endif
				<child_idx> = (<child_idx> + 1)
				repeat <power_select_children_size>
			endif
		endif
