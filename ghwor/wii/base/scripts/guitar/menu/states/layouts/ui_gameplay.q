gameplay_defer_end_game = 0
gameplay_defer_end_game_params = {
}
...
script ui_create_gameplay_spawned 
	...
	gamemode_gettype
	<pausing_enabled> = 0
	if NOT ScreenElementExists \{id = you_rock_physics}
		if (<Type> != freeplay)
	...
	if (<Type> = career)
		...
		if ($current_playing_transition = celebintro ||
				...
				$current_playing_transition = finalbattleoutro ||
...
script gameplay_end_game_spawned \{instant = 0}
	...
	Change \{gameplay_defer_end_game = 1}
	Change gameplay_defer_end_game_params = <...>
	if (<instant> = 0)
		Wait \{3
			Seconds}
	endif
