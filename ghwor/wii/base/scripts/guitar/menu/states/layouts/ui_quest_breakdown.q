...
script ui_create_quest_breakdown \{starttime = !i1768515945
	...
	ui_quest_get_giglist_bg hero_name_nl = ($<hero_info>.name_nl) chapter_global = <current_chapter>
	quest_get_char_icon hero_name_nl = ($<hero_info>.name_nl) chapter_name = <current_chapter>
	CreateScreenElement {
		Type = SpriteElement
		parent = root_window
		id = questbreakdownbgid
		texture = <bg_texture>
		Scale = (1.0, 1.0)
		dims = (1280.0, 720.0)
		Pos = (640.0, 360.0)
	...
	if (<do_special_breakdown> = 1)
		...
		stop_at_blue = 0
		questbreakdownid :se_setprops \{Pos = (0.0, 206.0)
			centeredbg_alpha = 0}
		questbreakdownbgid :se_setprops \{alpha = 0}
	endif
	...
	if (<restart> = 1)
		SpawnScriptNow ui_quest_breakdown_wait_and_restart_gem_scroller params = {starttime = <starttime>}
	endif
endscript

script ui_quest_breakdown_wait_and_restart_gem_scroller 
	Wait \{5
		Seconds}
...
script ui_destroy_quest_breakdown 
	if ScreenElementExists \{id = questbreakdownid}
		...
		DestroyScreenElement \{id = questbreakdownbgid}
