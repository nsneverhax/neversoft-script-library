...
script ui_create_tutorial_menu 
	...
	printf \{channel = tutorial
		'Manually loading shared tutorial pak (NOT ui slot pak) into quota_misc'}
	LoadPak \{'pak/ui/tut_shared.pak'
		Heap = quota_misc}
	Change \{g_tut_need_shared_pak = 0}
	make_generic_menu \{title = qs(0xc6510dd1)
		pad_back_script = tutorial_menu_back
		dims = (400.0, 400.0)
...
script ui_destroy_tutorial_menu 
	...
	if ($g_in_tutorial = 0)
		UnLoadPak \{'pak/ui/tut_shared.pak'
			Heap = quota_misc}
		tutorial_load_pak \{pakname = 'none'}
		unlock_all_controllers
	elseif
...
script ui_init_tutorial_menu 
	...
	Change \{g_tut_need_shared_pak = 1}
	net_counter_increment \{counter_name = globalcounter_tutorial}
	hide_crowd_models
	...
	Change g_tutorial_part_entered_with = <old_part>
	getcontrollertype controller = ($primary_controller)
...
script ui_deinit_tutorial_menu 
	...
	tutorial_system_set_part part = ($g_tutorial_part_entered_with)
...
script tutorial_menu_controller_handler \{has_a_mic = !i1768515945}
...
script tutorial_menu_back 
	create_loading_screen
	Wait \{2
		gameframes}
	ui_sfx \{menustate = Generic
		uitype = back}
	generic_event_back \{state = uistate_select_training}
endscript
