...
script ui_create_quest_map_giglist \{chapter_name = !q1768515945
	...
	if ($gameplay_defer_end_game = 1)
		Change \{gameplay_defer_end_game = 0}
		broadcastevent Type = drop_player_end_game data = ($gameplay_defer_end_game_params)
	endif
endscript
...
script ui_quest_map_giglist_choose \{song_name = !q1768515945
	...
	if (<song_name> = $<chapter_name>.ENCORE)
		...
		format_career_song_gigtag chapter_name = ($<chapter_name>.Name) song_name = <song_name>
		GetGlobalTags savegame = ($primary_controller) <song_gigtag> param = stars
		if (<stars> = 0 || $g_e3_demo_flow = 1)
...
script ui_quest_map_giglist_debug_beat \{chosen_song_name = !q1768515945
	...
	get_song_name song = <chosen_song_name>
	...
	if (<completed> = 0)
		...
		get_song_name song = <temp_song_name>
		format_career_song_gigtag chapter_name = <chapter_name> song_name = <song_name>
		GetGlobalTags savegame = ($primary_controller) <song_gigtag> param = stars
