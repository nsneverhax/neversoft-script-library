...
script ui_create_quest_band_select_power_details \{chapter_name = !q1768515945}
	...
	load_pak_for_quest_chapter chapter = <chapter_name>
...
script ui_destroy_quest_band_select_power_details 
	...
	if ScreenElementExists \{id = questbandselectpowerdetailsid}
		questbandselectpowerdetailsid :GetSingleTag \{chapter_name}
		DestroyScreenElement \{id = questbandselectpowerdetailsid}
	endif
	if GotParam \{chapter_name}
		unload_pak_for_quest_chapter chapter = <chapter_name>
	else
		ScriptAssert \{'Failed to get chapter_name tag from QuestBandSelectPowerDetailsID! Likely memory leak.'}
	endif
endscript
