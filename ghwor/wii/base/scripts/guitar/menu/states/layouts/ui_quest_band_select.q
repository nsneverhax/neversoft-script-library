...
script ui_create_quest_band_select \{from_fail_song = 0}
	...
	if console_is_net_career_client
		event_handlers = [
			{pad_choose ui_sfx params = {menustate = setlist uitype = select}}
			{pad_choose net_career_prompt_client_no_continue}
			{pad_back generic_event_back}
			{pad_back unload_pak_for_quest_chapter}
...
script ui_quest_band_select_focus 
	...
	if (<state> = unselected)
		...
		ui_quest_band_select_selected_scene_hack objID = <objID>
	endif
endscript
...
script ui_quest_band_select_continue 
	...
	printf \{channel = clayton
		'ui_quest_band_select_continue'}
	printf \{channel = clayton
		'ui_quest_band_select_continue - 1'}
	clean_up_user_control_helpers
	printf \{channel = clayton
		'ui_quest_band_select_continue - 2'}
	ui_quest_band_select_change_band \{band_num = 1}
	printf \{channel = clayton
		'ui_quest_band_select_continue - 3'}
	<current_chapter> = ($current_chapter)
	playlist_clear
	printf \{channel = clayton
		'ui_quest_band_select_continue - 4'}
	GetArraySize ($<current_chapter>.songs)
	<songs_index> = 0
	begin
	printf channel = clayton 'ui_quest_band_select_continue - 5.%d' d = <songs_index>
	<song> = ($<current_chapter>.songs [<songs_index>])
	playlist_addsong song = <song> insert_at = back
	<songs_index> = (<songs_index> + 1)
	repeat <array_Size>
	printf \{channel = clayton
		'ui_quest_band_select_continue - 5'}
	SpawnScriptNow \{audio_play_loc_vo_manually
		params = {
			stream_name = 'UI_Band_Chosen'
			stream_to_stop = 'UI_Band_Choose'
		}}
	Wait \{3.4
		Seconds}
	printf \{channel = clayton
		'ui_quest_band_select_continue - 6'}
	CreateScreenElement {
		parent = root_window
	...
	printf \{channel = clayton
		'ui_quest_band_select_continue - 7'}
	...
	printf \{channel = clayton
		'ui_quest_band_select_continue - 8'}
	if (<from_fail_song> = 1)
	...
	if InNetGame
		...
		printf \{channel = clayton
			'ui_quest_band_select_continue - 9'}
	endif
endscript
...
script ui_quest_band_select_change_band \{band_num = !i1768515945}
	...
	if (<gender> = male)
		...
		SetArrayElement ArrayName = band_array index = <index> NewValue = (<band_array> [1])
		SetArrayElement ArrayName = band_array index = 1 NewValue = <temp>
		break
	endif
	index = (<index> + 1)
	repeat 4
...
script ui_quest_band_select_set_band2_member \{menu_index = !i1768515945
	...
	if (<selected> = 1)
		...
		ui_quest_band_select_selected_scene_hack objID = <resolved_id>
...
script ui_quest_band_select_selected_scene_hack 
	GetScreenElementChildren id = <objID>
	GetArraySize <children>
	i = 0
	begin
	(<children> [<i>]) :se_setprops Pos = (46.5, 1.0)
	i = (<i> + 1)
	repeat <array_Size>
endscript
