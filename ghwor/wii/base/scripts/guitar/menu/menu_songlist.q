...
script songlist_create \{locked = FALSE
	...
	if (<mode> = archive_entry)
		Change g_saved_songlist_sort_index = ($g_songlist_sort_index)
	endif
	songlist_change_sort_index index = ($g_songlist_sort_indexes.<mode>)
	...
	if NOT (<mode> = archive)
		...
		if (music_store != <mode>)
			<songlist_id> :Obj_SpawnScriptLater attempt_to_add_friend_feed params = {menu = SongList parent_id = SongList songlist_mode = <mode>}
		endif
		if (<mode> = music_store)
			<id> :se_setprops {
				songlist_bg_illo_alpha = 0.0
	...
	if SongList :desc_resolvealias \{Name = alias_songlist_vmenu}
		...
		if NOT (<mode> = music_store || <mode> = archive || <mode> = archive_entry)
	...
	if (<mode> != archive)
		sdcard_event_handler = {sdcard_event wii_sdcard_event_songlist params = {songlist_id = <songlist_id>}}
		AddArrayElement array = <event_handlers> element = <sdcard_event_handler>
	...
	if (<mode> = music_store)
		...
		songlist_component :SetTags \{music_store_mode = hub
			...
			tab_enabled = 0}
	elseif (<mode> = archive)
		<sort_option> = {pad_btn_left songlist_go_to_sort_options}
	...
	songlist_mode = <mode>
	if (<mode> = archive)
		songlist_mode = <archive_source>
	endif
	songlist_component :membfunc_songlist_uistack_initialize
	songlist_component :membfunc_songlist_uistack_setmode mode = <songlist_mode>
	...
	gamemode_gettype
	if NOT (<mode> = music_store || <Type> = freeplay || <mode> = archive)
		if (<mode> = leaderboard)
...
script songlist_create_detail_desc \{mode = !q1768515945}
	...
	if NOT (<mode> = archive)
		if (<mode> = music_store || <mode> = archive_entry)
...
script songlist_go_to_options \{Type = !q1768515945}
	...
	if GotParam \{exclusive_device}
		...
		elseif (<Type> = music_store)
			songlist_music_store_proceed_to_pre_purchase {
				Type = <Type>
				exclusive_device = <exclusive_device>
				savegame_device = <savegame_device>
...
script songlist_callback_have_page_find_data_range 
	...
	if (<mode> = music_store)
		...
		music_store_update_blocks_info
	endif
	...
	generic_scrollbar_wgt_update_thumb_pos id = songlist_scrollbar_wgt Dir = down count = <seek_global_index>
	...
	if ($wii_entering_music_store = 1)
		...
		songlist_detail :se_setprops songlist_detail_wii_points_text = <wii_points_text>
	endif
	if (<content_size> = 1)
		if (<mode> = music_store)
			songlist_component :GetSingleTag \{music_store_mode}
	...
	if (<page_size> = 0)
		...
		if (<mode> = archive)
			...
			songlist_component :SetTags \{requires_build = FALSE}
		endif
		if (<mode> = music_store)
			songlist_component :GetSingleTag \{music_store_mode}
			if (<music_store_mode> = special_offers)
...
script songlist_build 
	...
	if (<mode> = music_store)
		...
		if (<music_store_mode> = download_list)
			current_menu :se_setprops {
				event_handlers = [
					{pad_start generic_event_choose params = {no_sound state = uistate_music_store_terms data = {is_popup exclusive_device = <exclusive_device> purchase_struct = <ui_struct> go_to_confirm}}}
				]
				replace_handlers
			}
...
script songlist_add_header \{header_struct = 0x69696969}
	...
	if (<header_text> = $content_source_ghtunes_artist_string)
		<id> :SetProps {songlist_header_text_scale = (1.2, 1.2)}
	endif
	<id> :SetTags {
		header = 1
...
script songlist_add_item \{ui_struct = 0x69696969}
	...
	if (<mode> = music_store)
		songlist_component :GetSingleTag \{music_store_mode}
		if (<music_store_mode> = special_offers)
			if (<title_text> = qs(0xfa6cade3))
				<title_text> = qs(0x8244b5db)
			endif
	...
	if NOT (<mode> = music_store || <mode> = archive)
		...
		songlist_sd_card_icon_alpha = 0
		if (<ui_struct>.song_on_sd = 1 && <ui_struct>.song_on_wii = 0)
			songlist_sd_card_icon_alpha = 1
		endif
		if NOT (<icon> = NULL)
			<item_id> :se_setprops {
				songlist_source_icon_texture = <icon>
				songlist_sd_card_icon_alpha = <songlist_sd_card_icon_alpha>
			}
		else
	...
	if (<mode> = music_store)
		songlist_music_store_add_item {
			...
			song_purchased = (<ui_struct>.purchased)
			song_checksum = <song>
		}
	endif
		...
		if (<mode> = music_store)
			...
			if (<music_store_mode> = special_offers)
				...
				if ((<ui_struct>.Type) = import)
					title_name = (($downloaded_offerings [<index>]).title_name)
					display_name = (($downloaded_offerings [<index>]).display_name)
					if NOT isnabuild
						if faststructurecontains structure = ($downloaded_offerings [<index>]) Name = title_name_eu
							title_name = (($downloaded_offerings [<index>]).title_name_eu)
						endif
						if faststructurecontains structure = ($downloaded_offerings [<index>]) Name = display_name_eu
							display_name = (($downloaded_offerings [<index>]).display_name_eu)
						endif
					endif
					<pad_choose_script> = menu_special_offers_select_import
					<pad_choose_params> = {offeringid = (($downloaded_offerings [<index>]).offeringid) display_name = <display_name> title_name = <title_name> purchase_struct = <ui_struct> id = <id>}
				elseif ((<ui_struct>.Type) = patch)
					<pad_choose_script> = songlist_go_to_options
					<pad_choose_params> = {Type = music_store song_item_id = <id> purchase_struct = <ui_struct>}
				elseif ((<ui_struct>.Type) = ecard_code)
					<pad_choose_script> = menu_special_offers_select_ecard_code
					<pad_choose_params> = {}
				else
					<pad_choose_script> = nullscript
				endif
	...
	<id> :SetTags pad_choose_script = <pad_choose_script>
	<id> :SetTags pad_choose_params = <pad_choose_params>
	return id = <id>
endscript
...
script songlist_go_to_song_library_tab \{skip_scan = 0}
	...
	songlist_change_sort_index index = ($g_songlist_sort_indexes.<return_mode>)
...
script songlist_post_dlc_scan_pre_refresh 
	...
	current_menu :se_setprops {
		event_handlers = [
			{pad_select generic_blocking_execute_script params = {pad_script = songlist_go_to_music_store_tab}}
...
script songlist_music_store_add_item \{id = !q1768515945
		song_on_wii = !i1768515945
		song_on_sd = !i1768515945
		song_purchased = !i1768515945}
	<wii_icon_alpha> = 0.0
	<sd_card_icon_alpha> = 0.0
	<update_icon_alpha> = 0.0
	<purchased_icon_alpha> = 0.0
	<block_count_alpha> = 1.0
	<download_arrow_alpha> = 1.0
	<block_count_text> = qs(0x652a97a3)
	if (<song_on_wii> = 1)
		<wii_icon_alpha> = 1.0
		<download_arrow_alpha> = 0.0
		<block_count_alpha> = 0.0
	endif
	if (<song_on_sd> = 1)
	...
	if (<song_purchased> = 1)
		<purchased_icon_alpha> = 1.0
	endif
	songlist_component :GetSingleTag \{music_store_mode}
	if (<music_store_mode> = download_list)
		dlcmanagerfunc func = get_content_flags content_name = (<song_checksum>)
		if (<content_flags>.in_cart = 0)
			<download_arrow_alpha> = 0.0
		endif
		marketplacefunc func = get_cart_content_blocks content_name = (<song_checksum>)
		formatText TextName = block_count_text '%s' s = (<blocks>)
	endif
	<id> :se_setprops {
		...
		music_store_money_icon_alpha = <purchased_icon_alpha>
		music_store_download_arrow_alpha = <download_arrow_alpha>
		music_store_block_count_alpha = <block_count_alpha>
		music_store_block_count_text = <block_count_text>
	}
endscript
...
script songlist_music_store_view_pack_songs \{ui_struct = 0x69696969}
	...
	current_menu :se_setprops {
		event_handlers = [
			{<back_button> songlist_music_store_back_to_previous_filterspec params = {
					...
					music_store_mode = ($g_music_store_previous_menu)
	...
	if ($g_music_store_previous_menu != hub)
	...
	if GotParam \{Type}
		...
		elseif (<Type> = pack)
			...
			songlist_component :SetTags \{music_store_mode = packs}
			<title> = qs(0x6d06b07b)
			songlist_change_sort_index \{index = 0}
		else
			array = (<ui_struct>.pack_list)
			...
			if NOT (((<ui_struct>.Type) = import) || (<music_store_mode> = download_list))
				...
				songlist_component :SetTags \{music_store_mode = songs}
			endif
			filterspec = [
				{
					key = song_checksum
			...
			elseif ((<ui_struct>.Type) = import)
				offer_index = (<ui_struct>.offer_index)
				display_name = (($downloaded_offerings [<offer_index>]).display_name)
				if NOT isnabuild
					if faststructurecontains structure = (($downloaded_offerings [<offer_index>])) Name = display_name_eu
						display_name = (($downloaded_offerings [<offer_index>]).display_name_eu)
					endif
				endif
				formatText TextName = title qs(0x478a2386) t = <display_name>
	...
	if GotParam \{new_pivot_checksum}
		songlist_refresh_menus pivot_checksum = <new_pivot_checksum>
	elseif GotParam \{no_pivot}
		songlist_refresh_menus \{no_pivot = 1}
	else
...
script songlist_music_store_back_to_previous_filterspec \{filterspec = !a1768515945
	...
	g_music_store_hub_options_index = <index>
	songlist_back pivot_checksum = <pivot_checksum>
endscript
...
script songlist_item_focus \{time = 0.0
	...
	if (<mode> = music_store)
		songlist_component :GetSingleTag \{music_store_mode}
		if (<music_store_mode> = special_offers)
			if NOT ui_event_exists_in_stack \{Name = 'music_store_terms'}
				<id> :se_setprops {
				...
				songlist_detail :se_setprops \{songlist_detail_albums_alpha = 1.0
					songlist_detail_intensity_cont_alpha = 0.0
					songlist_detail_info_vmenu_alpha = 0.0}
		...
		songlist_detail :se_setprops \{songlist_detail_albums_alpha = 0.0
			songlist_detail_intensity_cont_alpha = 1.0
			songlist_detail_info_vmenu_alpha = 1.0}
	endif
	if song_is_jamsession song = (<ui_struct>.song_checksum)
		formatText TextName = text '%a' a = (<ui_struct>.FileName)
	...
	<id> :GetSingleTag pad_choose_script
	<id> :GetSingleTag pad_choose_params
	user_control_helper_change_click_script button = green pad_click_script = <pad_choose_script> pad_click_params = <pad_choose_params>
	if (<mode> = archive)
		return
	endif
	if NOT (<mode> = music_store)
			...
			if StructureContains structure = ($gh_songlist_props.(<ui_struct>.song_checksum)) Name
				...
				if GlobalExists Name = <challenge_global>
					if StructureContains structure = ($<challenge_global>) descrip
						songlist_detail :se_setprops \{songlist_detail_mini_playlist_text = qs(0xf196a4d0)}
...
script songlist_music_store_item_focus \{morph_time = 0.0
	...
	formatText TextName = duration_text qs(0x81ca0519) m = <minutes> s = <Seconds>
	songlist_component :GetSingleTag \{music_store_mode}
	if faststructurecontains structure = <ui_struct> Name = album
	...
	elseif isngc
		...
		if (<music_store_mode> = download_list)
			...
			marketplacefunc func = get_cart_content_blocks content_name = (<ui_struct>.song_checksum)
			if ((<ui_struct>.song_on_wii = 1) || (<ui_struct>.song_on_sd = 1))
	...
	<money_rgba> = (($g_menu_colors).songlist_focus_orangeish)
	<price_rgba> = (($g_menu_colors).songlist_focus_orangeish)
...
script songlist_item_add_to_playlist \{calculate_duration = 1}
	...
	get_savegame_from_controller controller = <savegame_device>
	if (<mode> != leaderboard)
		if NOT issongselectable song = (<ui_struct>.song_checksum) check_sd_card savegame = <savegame>
			current_menu :se_setprops \{unblock_events}
			ui_sfx \{menustate = setlist
		...
		elseif (<Type> = career)
			if ($g_net_leader_player_num != -1)
				if playerinfoequals ($g_net_leader_player_num) is_local_client = 1
					user_control_helper_change_text \{button = green
						text = qs(0x34ff4910)}
					user_control_helper_change_click_script button = green pad_click_script = songlist_item_remove_from_playlist pad_click_params = {ui_struct = <ui_struct>}
				endif
			endif
		endif
	endif
...
script songlist_allow_continue 
	...
	if NOT user_control_helper_exists \{button = start}
		add_user_control_helper \{text = qs(0x59234f9d)
			button = start
			z = 100000}
	endif
endscript
...
script songlist_continue \{autosave_playlist = 1}
	...
	if (<mode> = leaderboard)
		...
		if GotParam \{no_restricted_check}
			generic_event_replace state = uistate_award_case data = {no_targeting song = <song> device_num = <device_num>}
		else
	...
	switch (<Type>)
		...
		if is_roadie_battle_coop
			songlist_continue_for_quickplay launch_game = FALSE autosave_playlist = <autosave_playlist>
			universal_help_display_help_check help_context = roadiequickplay_to_gameplay device_num = ($primary_controller)
			if (<show_help_page> = 1)
				generic_event_choose {
					state = uistate_universal_help
					band_lobby_continue_sfx
					data = {
						next_state_name = uistate_roadie_challenge_target_select
						next_state_params = {
							transition = first_song
						}
			...
			if (<show_help_page> = 1)
				...
				songlist_continue_for_quickplay launch_game = true autosave_playlist = <autosave_playlist>
			endif
		endif
		case freeplay
		destroy_friend_feed
		...
		if inroadiebattlemode
			universal_help_display_help_check help_context = roadiebattle_to_gameplay device_num = ($primary_controller)
			if (<show_help_page> = 1)
				generic_event_choose {
					state = uistate_universal_help
					band_lobby_continue_sfx
					data = {
						next_state_script = roadie_battle_launch_competitive
						device_num = ($primary_controller)
						help_global_index = <help_index>
					}
				}
			else
				roadie_battle_launch_competitive
			endif
		else
...
script roadie_battle_launch_competitive 
	ui_sfx \{menustate = setlist
		uitype = complete}
	gman_startgamemodegoal
	Change current_level = ($g_band_lobby_current_level)
...
script songlist_continue_for_quickplay \{launch_game = 0x00000018
		autosave_playlist = !i1768515945}
	venue = ($g_band_lobby_current_level)
...
script songlist_scroll_down 
	if ScreenElementExists \{id = SongList}
		...
		songlist_component :membfunc_songlist_uistack_getmaxmenuelements
		SetMenuAutoRepeatTimes ((0.3, 0.0) + (($g_songlist_scroll_speed) * (0.0, 1.0)))
		if (<list_dirty> = true || <requires_build> = true)
			return
		endif
...
script songlist_back 
	...
	elseif (<mode> = music_store)
		...
		if (<visited_hub> = 1)
			...
			if ($g_music_store_previous_menu != hub)
				music_store_hub_get_index_by_mode mode = ($g_music_store_previous_menu)
				Change g_music_store_hub_options_index = <index>
			endif
			if ($g_music_store_previous_menu = albums)
				new_filterspec = [
					{
						key = Type
						value = album
			...
			elseif ($g_music_store_previous_menu = packs)
				new_filterspec = [
					{
						key = Type
						value = pack
				...
				songlist_refresh_menus pivot_checksum = <pivot_checksum>
			elseif ($g_music_store_previous_menu = songs)
				new_filterspec = [
					{
						key = Type
						value = song
			...
			elseif ($g_music_store_previous_menu = special_offers)
				new_filterspec = [
					{
						key = Type
						union_list = [import patch ecard_code]
						comparator = equal
					}
				]
				songlist_change_filterspec filterspec = <new_filterspec>
				songlist_component :SetTags \{music_store_mode = special_offers}
				Change \{g_music_store_previous_menu = hub}
...
script songlist_go_to_sort_options 
	ScriptAssert \{qs(0x521bda8d)}
	songlist_go_to_options \{Type = sort}
endscript
...
script wii_sdcard_event_songlist \{refresh_archive = FALSE}
	...
	if (<mode> = music_store)
		...
		if (<music_store_mode> = download_list)
			...
			if (<base_name> = 'music_store_terms')
	...
	elseif (<mode> = music_store || <mode> = import)
		if ($g_songlist_sd_card_required = 1)
			...
			if sdcardmanagerfunc \{func = get_card_error}
				...
				if NOT ((<base_name> = 'ecard_edit_name') || (<base_name> = 'music_store_terms') || (<base_name> = 'music_store_hub') || (<base_name> = 'import_post_legal'))
					contentmanfunc func = refresh Name = <list_name> no_enum
					LaunchEvent \{Type = unfocus
						target = current_menu}
					songlist_refresh_menus \{no_focus = 1}
				else
					songlist_component :SetTags \{songlist_needs_refresh = 1}
				endif
			else
		...
		songlist_destroy_dialog_box
		SpawnScriptNow \{songlist_update_song_preview}
		...
		if contentmanfunc func = refresh Name = <list_name>
			...
			songlist_component :membfunc_songlist_uistack_getcurrentchecksum
			get_savegame_from_controller controller = ($primary_controller)
			get_ui_song_struct_items song = <current_checksum> savegame = <savegame>
			if (<song_not_found> = true || (<ui_struct>.song_is_dlc = 1 && <ui_struct>.song_on_wii = 0))
				current_menu :SetTags \{tag_selected_index = 0}
				songlist_refresh_menus \{no_pivot = 1}
			else
				songlist_refresh_menus
			endif
		...
		songlist_component :Obj_SpawnScriptNow \{songlist_check_playlist_after_sd_settles}
	endif
endscript
...
script songlist_music_store_proceed_to_pre_purchase 
	...
	if ((<purchase_struct>.Type) = song)
		if ((<purchase_struct>.song_on_wii) = 1)
			songlist_music_store_display_already_owned_dialog ui_struct = <purchase_struct> song_on_wii = 1
			return
		elseif ((<purchase_struct>.song_on_sd) = 1)
			songlist_music_store_display_already_owned_dialog ui_struct = <purchase_struct> song_on_sd = 1
			return
		endif
	endif
	generic_event_choose {
...
script songlist_music_store_display_already_owned_dialog 
	...
	if GotParam \{song_on_wii}
		formatText TextName = body_text qs(0x46c02d93) s = (<ui_struct>.song_title)
		<element> = {
