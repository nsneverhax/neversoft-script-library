...
script create_freeplay_instrument_icons \{joining_player = 1}
	RequireParams \{[
			joining_player
		]
		all}
	...
	freeplay_reset_instrument_icons_positions
	Player = 1
	begin
	if NOT playerinfoequals <Player> part = vocals
		if NOT playerinfoequals <Player> freeplay_state = dropped
...
script freeplay_start_jump_player_in \{Player = !i1768515945
	...
	switch <controller_type>
		case guitar
		SpawnScriptNow create_freeplay_instrument_icons params = {joining_player = <Player>}
...
script freeplay_update_actual_position \{time = 0
	...
	if (<part> = vocals)
		...
		getplayerinfo <Player> combo_meter_id
		GetScreenElementPosition id = <combo_meter_id>
		newpos = (((1.0, 0.0) * <x_pos>) + ((0.0, 1.0) * (<screenelementpos> [1])))
		<combo_meter_id> :se_setprops Pos = <newpos>
	else
		getplayerinfo <Player> highway_position
...
script freeplay_reset_instrument_icons_positions 
	freeplay_get_num_visible_highways
	switch <num_highways>
		case 1
		<newpos> = (-50.0, 150.0)
	...
	if (<part> != vocals)
		if (<freeplay_state> != dropped)
			get_highway_hud_root_id Player = <Player>
			if <highway_hud_root> :desc_resolvealias Name = alias_hud_icon_container param = icon_container
				<icon_container> :se_setprops Pos = <newpos>
			endif
		endif
	endif
...
script freeplay_update_instrument_icons \{Player = !i1768515945}
	getplayerinfo <Player> part
	if (<part> != vocals)
		switch (<part>)
			case guitar
		...
		get_highway_hud_root_id Player = <Player>
		if <highway_hud_root> :desc_resolvealias Name = alias_hud_icon_container param = icon_container
			<icon_container> :se_setprops inst_icon_texture = <icon_texture>
		endif
	endif
endscript
