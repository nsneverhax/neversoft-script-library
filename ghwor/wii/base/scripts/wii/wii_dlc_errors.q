...
script wii_dlc_get_error_text \{error_crc = ec_error_ok
		...
		wii_blocks = 0
		sd_blocks = 0}
	if GotParam \{song_title}
		song_title_upper = <song_title>
	else
		song_title = qs(0xb0a778a5)
		song_title_upper = qs(0x8618db05)
	endif
	if NOT ((<error_crc> = ec_error_ok) || (<error_crc> = ec_error_not_busy))
		...
		if (<show_support_code> = FALSE)
			...
			switch <error_crc>
				...
				case ec_error_eula
				if (isnabuild)
					<error_text> = ($ecommerce_messages.message_204076_no_eula_text_noa)
				else
					<error_text> = ($ecommerce_messages.message_204076_no_eula_text_noe)
				endif
				<fatal> = 1
				case ec_error_fake_eula_for_other_region
				<error_text> = ($ecommerce_messages.message_205625_eula_for_other_region)
				<show_support_code> = true
				<fatal> = 1
				case ec_error_register
				case ec_error_shop_setup
				<error_text> = ($ecommerce_messages.rvlec_message_0003_no_shop_account_text)
				<fatal> = 1
				case ec_error_points_restricted
				<error_text> = ($ecommerce_messages.rvlec_message_0007_pin_entry_text)
				case ec_error_insufficient_funds
				<error_text> = ($ecommerce_messages.rvlec_message_0004_wii_points_insufficient_text)
				...
				case ec_error_sync
				<error_text> = ($ecommerce_messages.rvlec_message_0010_wii_dlc_sync_needed_text)
				<show_support_code> = true
				<fatal> = 1
				case ec_error_cfg_home_space
				case ec_error_cfg_user_space
				<error_text> = ($ecommerce_messages.rvlec_message_0011_connect_insufficient_blocks_text)
				<fatal> = 1
				case ec_error_cfg_home_inode
				case ec_error_cfg_user_inode
				<error_text> = ($ecommerce_messages.rvlec_message_0012_connect_insufficient_inodes_text)
				<fatal> = 1
				case ec_error_user_blocks
				case ec_error_cache_blocks
				if GotParam \{failed_update}
					formatText TextName = error_text ($ecommerce_messages.rvlec_message_0005b_update_insufficient_blocks_text) d = <blocks>
				else
					formatText TextName = error_text ($ecommerce_messages.rvlec_message_0005_nand_insufficient_blocks_text) d = <blocks>
				endif
				case ec_error_update_blocks
				formatText TextName = error_text ($ecommerce_messages.rvlec_message_0005b_update_insufficient_blocks_text) d = <blocks>
				case ec_error_user_inodes
				case ec_error_cache_inodes
				if GotParam \{failed_update}
					<error_text> = ($ecommerce_messages.rvlec_message_0006b_update_insufficient_inodes_text)
				else
					<error_text> = ($ecommerce_messages.rvlec_message_0006_nand_insufficient_inodes_text)
				endif
				case ec_error_update_inodes
				<error_text> = ($ecommerce_messages.rvlec_message_0006b_update_insufficient_inodes_text)
				case ec_error_sd_no_space
				formatText TextName = error_text ($cntsd_messages.rvlcntsd_message_0008b) d = <blocks>
				case ec_error_ecard
				<error_text> = qs(0x29837635)
				case ec_error_ecard_not_redeemable
				<error_text> = qs(0x1b581e88)
				case ec_error_ecard_bad_item_count
				<error_text> = qs(0xf7943a35)
				case ec_error_ecard_already_own
				<error_text> = qs(0x37bca16b)
				case ec_error_partial_purchase_failure
				<error_text> = qs(0x013fcf60)
				<fatal> = 1
				...
				if GotParam \{transfer_type}
					if (<transfer_type> = restore)
						...
						formatText TextName = error_text ($cntsd_messages.rvlcntsd_message_0010) s = <song_title_upper>
					endif
				else
					formatText TextName = error_text ($cntsd_messages.rvlcntsd_message_0010) s = <song_title_upper>
				endif
...
script dlc_error_exit_to_main_menu_part1 
	...
	destroy_dialog_box
	if netstatus_checkforerrors \{context = dlc}
		show_last_connection_error \{continue_script = dlc_error_exit_to_main_menu_part2
			context = dlc}
	else
