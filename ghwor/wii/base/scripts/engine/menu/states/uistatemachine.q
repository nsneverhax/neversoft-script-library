...
script ui_event \{event = !q1768515945
	...
	if (<state> != None)
		...
		if GotParam \{data}
			if StructureContains structure = <data> state
				printf '  state=%s' s = (<data>.state)
			endif
		endif
	endif
	UI :Unpause
...
script ui_additional_create 
	printf \{channel = ui_event
		qs(0x392ab336)}
	formatText checksumName = ui_state 'UIState_%b' b = <base_name>
	wii_notify_state_change ui_state = <ui_state>
...
script ui_event_get_top_base_name 
	ui_event_get_top
