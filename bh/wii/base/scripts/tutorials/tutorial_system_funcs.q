...
script tutorial_system_get_pak 
	printf \{channel = tutorial
		qs(0xa7b571ea)}
	RequireParams \{[
			tutorial_header
		...
		all}
	formatText TextName = pakname 'tut_%a_%b' a = <tutorial_id> b = <lesson_id>
	printf 'Tutorial will look for pak %a' a = <pakname>
...
script tutorial_system_page_complete_buttoned 
	if ($tutorial_paused = 1)
		printf \{'Game paused, ignore'}
	else
		if tutorial_anti_spam_timer_okay
			tutorial_system_page_complete <...>
		else
			printf \{'[Skip] ignored, too close to last input/start page'}
		endif
	endif
endscript
...
script tutorial_system_lesson_complete_anim \{anim_text = $wii_tutorial_complete}
	...
	SpawnScriptNow create_exploding_text id = lesson_anim params = {parent = 'you_rock_physics' text = <anim_text> text_physics = 1 placement = top fit_dims = (1050.0, 120.0)}
