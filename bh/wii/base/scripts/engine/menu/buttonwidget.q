...
backing_uninit_rgba = gh51_blue_lt
0x1dc98f7e = gh51_blue_lt
backing_init_rgba = [
	255
	50
	0
...
script button_widget_add_item \{widget_id = widget
	...
	if NOT GotParam \{no_cas_handlers}
		if GotParam \{only_rotate}
			setup_cas_menu_handlers vmenu_id = <widget_id> camera_list = <camera_list> no_zoom
		elseif GotParam \{no_rotate_zoom}
			setup_cas_menu_handlers vmenu_id = <widget_id> camera_list = <camera_list> no_zoom
		else
	...
	CreateScreenElement {
		...
		generic_menu_smenu_textitem_text_dims = (350.0, 35.0)
...
script focus_button_widget 
	...
	SetScreenElementProps {
		...
		generic_menu_smenu_textitem_text_scale = 1.3
		highlight_btn_alpha = 1.0
		light_bar_alpha = 1
	}
	<widget_id> :GetSingleTag backing_id
	if NOT GotParam \{rotate}
		SetScreenElementProps {
			id = <backing_id>
			rgba = ($0x1dc98f7e)
		}
		...
		k = 0
		begin
		SetScreenElementProps id = {(<children> [<k>]) child = sprite} rgba = ($0x1dc98f7e)
		k = (<k> + 1)
		repeat <array_Size>
	endif
	extendcrcwithstring checksum = <widget_id> string = '_particle'
	<widget_id> :obj_spawnscript 0x0542334f params = {id = <extendedchecksum> Pos = (900.0, -15.0) z = 17.5}
...
script unfocus_button_widget 
	...
	SetScreenElementProps {
		...
		generic_menu_smenu_textitem_text_font = fontgrid_text_a1
		generic_menu_smenu_textitem_text_rgba = (($cas_color_scheme).text_color)
		generic_menu_smenu_textitem_text_scale = 1.0
		highlight_btn_alpha = 0.0
		light_bar_alpha = 0
	}
	<widget_id> :GetSingleTag backing_id
	if NOT GotParam \{rotate}
		...
		k = 0
		begin
		SetScreenElementProps id = {(<children> [<k>]) child = sprite} rgba = ($backing_uninit_rgba)
		k = (<k> + 1)
		repeat <array_Size>
	endif
	extendcrcwithstring checksum = <widget_id> string = '_particle'
...
script up_button_widget 
	...
	SpawnScriptNow car_scroll_widget_sfx params = {<...>}
	if GotParam \{pad_left_script}
...
script rot_widget_add_item \{widget_id = widget
	...
	CreateScreenElement {
		...
		event_handlers = [
			...
			{focus focus_button_widget params = {widget_id = <widget_id> rotate}}
			{unfocus unfocus_button_widget params = {widget_id = <widget_id> rotate}}
		]
		z_priority = <z_priority>
		just = [left , top]
		dims = <dims>
...
script init_rotate_widget 
	...
	k = 0
	begin
	SetScreenElementProps id = {(<children> [<k>]) child = sprite} rgba = ($backing_init_rgba)
	k = (<k> + 1)
	repeat <array_Size>
	SoundEvent event = ($button_widget_choose_sound)
	if GotParam \{init_script}
...
script add_generic_menu_button_icon_item \{On = FALSE}
	...
	<item_container> :desc_resolvealias Name = 0xf78e9a21
	CreateScreenElement {
	...
	if NOT GotParam \{no_backing_toggle}
		SetScreenElementProps {
