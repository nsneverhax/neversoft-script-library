	...
	if (<count> > 0)
		...
		if (($freestyle_disconnected_players [<counter>]) = 1)
			...
			switch (<instrument>)
				...
				case nunchuk
				getwiicontrollertype controller = (<player_display> - 1)
				if (<controller_type> = None)
					formatText TextName = text qs(0x0bf470d1) t = <text> j = <player_display>
				else
					formatText TextName = text qs(0x896d022a) t = <text> j = <player_display>
				endif
			endswitch
		endif
		counter = (<counter> + 1)
