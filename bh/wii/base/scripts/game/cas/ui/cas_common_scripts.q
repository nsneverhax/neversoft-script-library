...
script generic_menu_update_arrows \{menu_id = current_menu}
	ScriptAssert \{'generic_menu_update_arrows: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script setup_cas_menu_handlers controller = ($primary_controller)
	...
	elseif isdrumcontroller controller = <controller>
		...
		if NOT GotParam \{no_zoom}
			if GotParam \{zoom_camera}
				if isrbdrum controller = <controller>
					AddArrayElement array = <event_handlers> element = {pad_L1 cas_pull_back_camera params = {button = sqaure zoom_camera = <zoom_camera>}}
				else
					AddArrayElement array = <event_handlers> element = {pad_L1 cas_pull_back_camera_gh params = {button = sqaure zoom_camera = <zoom_camera>}}
				endif
				event_handlers = <array>
			else
				AddArrayElement array = <event_handlers> element = {pad_L1 cas_pull_back_camera_adaptive params = {button = Square pull_back_distance = <pull_back_distance>}}
				event_handlers = <array>
			endif
		endif
	else
		...
		if NOT GotParam \{NO_ROTATE}
			if GotParam \{camera_list}
				AddArrayElement array = <event_handlers> element = {pad_option cas_change_camera_cut params = {camera_list = <camera_list> left}}
				event_handlers = <array>
			else
...
script focus_car_menu_text_item 
	if ScreenElementExists \{id = generic_menu}
		...
		<id> :obj_spawnscript 0x0542334f params = {id = <extendedchecksum> Pos = (900.0, 0.0) z = 9}
	endif
...
script focus_car_layer_options_menu_text_item 
	if ScreenElementExists \{id = generic_menu}
		...
		extendcrcwithstring checksum = <id> string = '_particle'
		<id> :obj_spawnscript 0x0542334f params = {id = <extendedchecksum> Pos = (650.0, 0.0) z = 9}
	endif
	if GotParam \{additional_focus_script}
		<additional_focus_script> <additional_focus_params>
	endif
