...
script 0xcb0276c2 \{load = true
	...
	if ($ps2_car_part_scripts_loaded = 0)
		if (<load> = true)
			unload_song_anims song_name = <song>
			ps2_load_car_part_script Heap = <Heap>
		endif
	else
		if (<load> = FALSE)
			ps2_unload_car_part_script Heap = <Heap>
			if (<0xc28bbbd1> = true && <song> != NULL)
				load_song_anims song_name = <song>
			endif
		endif
	endif
endscript
...
script ps2_load_car_part_script \{Heap = quota_animations}
	if ($ps2_car_part_scripts_loaded = 0)
...
script 0xce69e479 \{Heap = quota_animations}
	Change \{0x56978d8a = 0}
	0xf8e735a8
	PushAssetContext context = <Heap>
...
script ps2_unload_car_part_script \{Heap = quota_animations}
	Change 0x56978d8a = ($0x56978d8a - 1)
	if ($0x56978d8a = 0)
		PushAssetContext context = <Heap>
		UnLoadPak \{'pak/car_parts/car_parts.pak'}
