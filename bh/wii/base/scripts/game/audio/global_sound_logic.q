...
script Generic_Reverb_Functionality_Script \{NewEchoSettings = $echo_dry
		...
		ReverbFadeTime = 0.5}
	ScriptAssert \{'Generic_Reverb_Functionality_Script: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script GH_Star_Power_Verb_Off 
	printf \{channel = sfx
		qs(0xfa676f80)}
	setsoundbussparams {0x2f65de5f = {vol = (($default_BussSet.0x2f65de5f.vol)) pitch = 0.0} time = 0}
	Change \{0xcccf6f08 = 0}
	StopSoundsByBuss \{0x2f65de5f}
...
script GH3_Battle_Play_Whammy_Pitch_Up_Sound 
	getplayerinfo <other_player> whammy_attack out = num_strums
	player_pan = <other_player>
	if (<player_pan> = 1)
		<pan1x> = -0.76199996
		<pan1y> = 0.6470001
		<pan2x> = -0.44799998
			...
			if NOT ($game_mode = practice)
				if GotParam \{event_type}
					...
					switch <event_type>
						...
						default
						ScriptAssert \{qs(0x3c088a0f)}
					endswitch
				endif
			endif
		else
			ScriptAssert \{qs(0x6f5cebdf)}
		endif
	else
		ScriptAssert \{qs(0x595a9f3d)}
	endif
endscript
...
script GH3_Play_A_Crowd_OneShot_Positive_Based_On_Venue 
	ScriptAssert \{'GH3_Play_A_Crowd_OneShot_Positive_Based_On_Venue: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript

script GH3_Play_A_Crowd_OneShot_Negative_Based_On_Venue 
	ScriptAssert \{'GH3_Play_A_Crowd_OneShot_Negative_Based_On_Venue: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script do_actual_changing_of_looping_sound \{loading_transition = 0
	...
	if (($crowd_in_jam_mode_song_state = 1) || (<pak> = z_studio) || (<pak> = z_tool) || (<pak> = z_credits))
...
script transition_sfx_left_side 
	ScriptAssert \{'transition_sfx_left_side: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript

script transition_sfx_right_side 
	ScriptAssert \{'transition_sfx_right_side: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script menu_music_off 
	...
	KillSpawnedScript \{Name = 0x04319469}
...
script Song_Intro_Kick_SFX_Waiting 
	...
	if NOT ((<Type> = freeplay) || ($in_sing_a_long = true))
		...
		if is_roadie_battle_mode
			SpawnScriptNow \{roadie_battle_play_fretboard_sfx}
		else
...
script GH_SFX_Training_Hammer_On_Lesson_2 
	ScriptAssert \{'GH_SFX_Training_Hammer_On_Lesson_2: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script Tutorial_Mode_Finish_Chord_03 
	ScriptAssert \{'Tutorial_Mode_Finish_Chord_03: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script muting_test_setup_script 
	ScriptAssert \{'muting_test_setup_script: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script stoptesttones 
	ScriptAssert \{'StopTestTones: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script menu_music_se_spawned \{vol = 0}
	...
	SpawnScriptNow 0x04319469 params = {waitTime = <waitTime>}
...
script killmenumusic 
	...
	KillSpawnedScript \{Name = 0x04319469}
	Change \{menu_music_preloading = 0}
	stopsound \{0x0a0af01d}
	stopsound \{0xe95a0e39}
	stopsound \{0xf1ef119c}
	stopsound \{0xa1333d9c}
	stopsound \{0xace212e9}
	stopsound \{0x9bed181a}
	stopsound \{0xb6e36b66}
	stopsound \{0x0d08cad4}
	stopsound \{0xaaf90028}
	stopsound \{0xd89254ef}
	stopsound \{0x2d1bd91c}
	stopsound \{0xe2f38a65}
	stopsound \{0xa8040e72}
	stopsound \{0xa2d792a6}
	stopsound \{0x59927b0c}
	stopsound \{0xd35ad5ab}
	stopsound \{0x1c5f65ff}
endscript
...
script you_rock_script 
	...
	if is_roadie_battle_mode
		...
		if (<faceoff_ratio> > 0)
			...
			SoundEvent \{event = roadie_battle_roadie1_win}
		else
			StopSoundsByBuss \{roadie_battle_p2_mode}
			SoundEvent \{event = roadie_battle_roadie2_win}
		endif
	endif
	Wait \{2
		Seconds}
...
script intro_crowds_fast 
	...
	GetPakManCurrent \{map = zones}
	if (<pak> = z_space)
		SoundEvent \{event = 0x85bc7a63}
		SoundEvent \{event = 0x6cd8b292}
	endif
	if NOT (<Type> = freeplay || $calibrate_lag_enabled = 1)
		SoundEvent \{event = $current_crowd_applause_soundevent}
		SoundEvent \{event = $current_crowd_swell_short_soundevent}
...
script 0xaee90917 
	setsoundbussparams {0x2f65de5f = {vol = (($default_BussSet.0x2f65de5f.vol)) pitch = 0.0} time = 0.1}
	Change \{0xcccf6f08 = 1}
	Wait \{0.1
		Seconds}
	setsoundbussparams {0x2f65de5f = {vol = (($default_BussSet.0x2f65de5f.vol) -30)} time = 3}
endscript
