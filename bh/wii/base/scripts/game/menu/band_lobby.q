...
script band_lobby_change_playerinfo 
	...
	elseif GotParam \{character_id}
		...
		if is_roadie_battle_mode
			0x1606f4d5 Player = ($<submenu_data>.Player)
		endif
		...
		if (<action> = forward)
			...
			if StructureContains structure = ($band_lobby_submenu_states.<menu>) focus_param
				...
				if (<array_Size> > 0)
					...
					repeat <array_Size>
					if (<i> >= <array_Size>)
						printf \{'Did not find match for focus, default to 0'}
						<index> = 0
					endif
					default_index = <index>
				endif
	...
	if NOT InNetGame
		...
		if (<num_players> > 0)
			...
			if (
				...
				SetArrayElement ArrayName = 0xbd950a74 index = (<Player> - 1) NewValue = <party_id>
				...
				if ((<connections> [<i>].is_local) = 0)
					...
					if StructureContains structure = <party_shared_data> <party_member>
						if band_lobby_get_player_num_by_party_id party_id = (<connections> [<i>].party_id)
							if ((<0xbd950a74> [(<player_num> - 1)]) != (<connections> [<i>].party_id))
								...
								SetArrayElement ArrayName = 0xbd950a74 index = (<player_num> - 1) NewValue = (<connections> [<i>].party_id)
							endif
...
script band_lobby_seamless_net_change \{callback_params = {
	...
	printstruct <...>
	if (<network> = local)
		band_lobby_change_network Player = <Player> network = <network> focus = <focus>
	...
	if ((($savegame_async_last_error) = None) || (<is_saving_on_lobby_leave> = 0))
		if ininternetmode
			...
			if GotParam \{scan_dlc_if_online}
				if NOT band_lobby_handle_online_dlc_enum <...> Player = <original_player_var> viewport_index = <original_viewport_index>
					return
				endif
			endif
		endif
