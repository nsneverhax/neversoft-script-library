...
script band_lobby_create_popup_menu \{list_type = Player}
	...
	if ScreenElementExists id = <container>
		...
		switch <menu>
			case inviteslist
			if NOT GotParam \{viewport_index}
				ScriptAssert \{qs(0x2473162e)}
			endif
			band_lobby_create_popup_menu_inviteslist parent = <container> Player = <Player> viewport_index = <viewport_index>
			back_script = band_lobby_friendslist
			back_params = {Player = <Player> viewport_index = <viewport_index>}
			case friendslist
			if NOT GotParam \{viewport_index}
				ScriptAssert \{qs(0x2473162e)}
			endif
			band_lobby_create_popup_menu_friendslist online = <online> parent = <container> Player = <Player> viewport_index = <viewport_index>
			...
			band_lobby_create_popup_menu_leave_lobby {
				parent = <container>
				Player = <Player>
		...
		elseif (isngc)
			band_lobby_popup_menu :Obj_SpawnScriptLater bl_refresh_popup_button_helpers params = {menu = <menu> Player = <Player> controller = <controller>}
		endif
...
script band_lobby_create_popup_menu_dlc_error 
	...
	getplayerinfo <Player> gamertag
	getplayerinfo <Player> controller
	formatText TextName = error_text qs(0x0e23eaeb) e = <error_text>
	create_dialog_box {
		parent = <parent>
		player_device = <controller>
		template = no_options
...
script band_lobby_create_popup_menu_friendslist 
	...
	if current_band_lobby_popup :desc_resolvealias \{Name = alias_pu_menu
		...
		if ((isps3) || (isngc))
			if isngc
				invite_handler = [
					{pad_option2 band_lobby_inviteslist params = {Player = <Player> viewport_index = <viewport_index>}}
				]
			else
				invite_handler = [
	...
	if ((GotParam friendlist) && (ScreenElementExists id = band_lobby_popup_menu))
		...
		if (<array_Size> > 0)
			...
			if CheckForSignIn controller_index = <controller> dont_set_primary network_platform_only
				readleaderboardbyuidlist request_id = ($g_current_friends_popup_id) callback = band_lobby_rock_record_callback_friends leaderboard_id = lb_rock_record user_id_list = <rock_record_uids>
			else
				LaunchEvent \{Type = focus
					target = band_lobby_popup_menu}
								...
								if (<net_id> [0] = <lb_net_id> [0] && <net_id> [1] = <lb_net_id> [1])
									if (<children> [<j>]) :GetSingleTag is_friends_list
										...
										(<children> [<j>]) :se_setprops {
											event_handlers = [
												{pad_option display_rock_record params = {net_id = <lb_net_id> unfocus_id = band_lobby_popup_menu band_lobby_no_anim_in from_lobby Player = <controlling_player>}}
											]
										}
									else
										(<children> [<j>]) :GetSingleTag controlling_player
		...
		if GotParam \{friendstate}
			...
			if (1 = <cant_invite>)
				...
				if (<sentfriendrequest> = true)
					cant_invite = 1
					rich_presence = qs(0xda79f6f2)
				elseif (online != <friendstate>)
					cant_invite = 1
					rich_presence = qs(0xd533b880)
				else
					rich_presence = qs(0xe60e3e4a)
				endif
			endif
		else
...
script band_lobby_popup_menu 
	...
	generic_event_choose \{no_sound = 1
...
script band_lobby_inviteslist_callback 
	printf \{channel = band_lobby
		qs(0x0ba4fcff)}
	if ((GotParam invitelist) && (ScreenElementExists id = band_lobby_popup_menu))
		printf \{qs(0x76ba6584)}
		GetArraySize <invitelist>
		printf qs(0x64b2d0fc) d = <array_Size>
		if (<array_Size> > 0)
			i = 0
			begin
			band_lobby_add_invite_to_popup {
				gamertag_text = (<invitelist> [<i>].Name)
				net_id = (<invitelist> [<i>].id)
			}
			<i> = (<i> + 1)
			repeat <array_Size>
		endif
...
script band_lobby_create_popup_menu_inviteslist 
	printf \{channel = band_lobby
		qs(0xdfe8bbe2)}
	RequireParams \{[
			parent
	...
	CreateScreenElement {
		...
		desc = 'band_lobby_pu_friend'
		z_priority = 20.0
		friends_online_alpha = 0.0
		title_text = qs(0x306229d7)
		tags = {
			Player = <Player>
			viewport_index = <viewport_index>
		}
...
script band_lobby_add_invite_to_popup \{desc_id = 'band_lobby_player_info'}
	RequireParams \{[
			gamertag_text
			net_id
		]
	...
	if (ScreenElementExists id = invitelist_popup)
		formatText checksumName = invite_id '%a%b' a = (<net_id> [0]) b = (<net_id> [1])
		if NOT (ScreenElementExists id = <invite_id>)
			CreateScreenElement {
				id = <invite_id>
				parent = band_lobby_popup_menu
				Type = descinterface
				desc = <desc_id>
				autosizedims = FALSE
				dims = (747.0, 62.5)
			...
			array = [
				{unfocus band_lobby_popup_change_focus params = {focus_type = unfocus}}
				{focus band_lobby_popup_change_focus params = {focus_type = focus}}
				{pad_choose pre_friends_accept_invite params = <accept_params>}
				{pad_option2 band_lobby_remove_invite_from_popup params = {net_id = <net_id>}}
			]
			alpha = 0.0
			band_lobby_get_part_icon_parts \{part = remote}
			<id> :se_setprops {
				gamertag_text = <gamertag_text>
				icon_leader_alpha = <alpha>
				icon_chat_alpha = <alpha>
				difficulty_alpha = <alpha>
				difficulty_text_alpha = <alpha>
				difficulty_alpha = <alpha>
				event_handlers = <array>
				<part_props>
			}
...
script band_lobby_remove_invite_from_popup 
	printf \{channel = band_lobby
		qs(0x18249680)}
	RequireParams \{[
			net_id
...
script band_lobby_rock_record_callback_friends 
