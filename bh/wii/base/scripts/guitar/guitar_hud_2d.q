...
script rock_back_and_forth_star_meter 
	ScriptAssert \{'rock_back_and_forth_star_meter: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script hud_show_note_streak_combo \{Player = 1
	...
	if ScreenElementExists \{id = hud_root}
		...
		if (<num_players> = 1)
			...
			style_script_params = {Player = <Player>}
		endif
		...
		hud_create_message {
			...
			style_script_params = <style_script_params>
		}
		return
	endif
	if ($game_mode = gh4_p2_career || $game_mode = p2_coop)
		<Player> = 1
	endif
	begin
	if (<Player> = 1)
	...
	if NOT ScreenElementExists id = <id>
		DestroyScreenElement id = <player_container>
		return
	endif
...
script hud_show_drumfill_challenge 
	ScriptAssert \{'hud_show_drumfill_challenge: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
