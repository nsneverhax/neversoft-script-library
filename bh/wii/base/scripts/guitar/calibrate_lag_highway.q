...
script calibrate_wait_and_focus 
	...
	Wait \{1
		gameframe}
	if ($lag_calibration_highway_interlock = 0)
		break
	endif
	repeat
...
script calibrate_highway_setup_button_helpers \{z = 10000}
	...
	elseif ($calibrate_highway_state = video)
		...
		if (<controller_type> = drum)
			add_user_control_helper override_parent = <parent> text = qs(0xda9efcde) button = blue
		elseif ((<controller_type> = vocals) || (<controller_type> = controller))
			add_user_control_helper override_parent = <parent> text = qs(0xda9efcde) button = green
		else
...
script calibrate_highway_shutdown 
	...
	UnLoadPak \{'pak/ui/options_calibrate_lag.pak'
		Heap = BottomUpHeap}
	...
	if ($calibrate_highway_state = video)
		...
		if ($calibrate_lag_notes_passed >= <num_gems>)
			if calibrate_lag_calc_avg num_required = (<num_gems> / 2)
				...
				calibrate_fade_and_change_helpers \{state = editing}
				calibrationinterface :se_setprops \{helper_alpha = 0
					time = 0.5}
				calibrationinterface :se_waitprops
				calibrationinterface :se_setprops \{helper_text = qs(0x364cd1a0)}
...
script calibrate_highway_init_ui 
	...
	calibrationinterface :se_setprops {
		...
		videolag_rgba = 0xaad894a8
		audiolag_rgba = 0xaad894a8
	}
	CreateScreenElement {
...
script calibrate_create_menu 
	...
	make_generic_barrel_menu \{Pos = (700.0, 375.0)
...
script calibrate_hit_choose 
	...
	if ((($calibrate_highway_state) != audio) && (($calibrate_highway_state) != editing))
		calibrationinterface :se_setprops \{videolag_rgba = 0xaad894a8
			audiolag_rgba = 0xaad894a8}
	endif
endscript
...
script mock_highway_restart \{fade_helpers = 0
	...
	if ($calibrate_highway_gameplay_state = audio)
		...
		elseif isngc
			Change \{calibrating_helper_text = qs(0xf84211f1)}
		else
