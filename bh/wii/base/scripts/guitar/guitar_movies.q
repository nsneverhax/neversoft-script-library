...
script select_visualizer_bank 
	printf \{'F(): Select_Visualizer_Bank'
		channel = visualizer}
	...
	printstruct <bank_choices> channel = visualizer
	...
	if (<array_Size> > 0)
		...
		if ArrayContains array = ($venues_requiring_visualizer) contains = <pak>
			visualizer_load_bank bank = ($visualizer_current_bank)
		endif
...
script create_avui_fx \{viewport = ui_visualizer_viewport}
	...
	if NOT screenFX_FXInstanceExists viewport = <viewport> Name = avui_uber01
		ScreenFX_AddFXInstance viewport = <viewport> Name = avui_uber01 Type = audiovisualize On = 1 primitives = <primitives> mode = <mode> width = <width> height = <height>
	else
		ScreenFX_UpdateFXInstanceParams viewport = <viewport> Name = avui_uber01 Type = audiovisualize On = 1 primitives = <primitives> mode = <mode> width = <width> height = <height>
	endif
endscript
0x5b59c54a = true

script visualizer_event 
	printf \{qs(0xd1e9253a)
		channel = visualizer}
	if ($0x5b59c54a = FALSE)
		printf \{channel = visualizer
			qs(0xbb278c73)
	...
	if GotParam \{av_presets}
		...
		printf qs(0xc93be3cb) a = <mood> b = $visualizer_current_bank donoresolve channel = visualizer
	endif
endscript
...
script PlayMovieAndWait 
	...
	setmovievi
	if NOT GotParam \{loop_start}
		loop_start = 0
	endif
	if NOT GotParam \{loop_end}
		loop_end = (-1)
	endif
	if GotParam \{Loop}
		PlayMovie {textureSlot = 0
			...
			Loop = true
			<...>}
	elseif NOT GotParam \{highpriority}
		PlayMovie {textureSlot = 0
			TexturePri = 1000
			no_looping
			no_hold
...
script visualizer_get_primitives 
	RequireParams \{[
		...
		all}
	printf \{'F(): visualizer_get_primitives'
		channel = visualizer}
	Profile = ($<preset>)
	if IsArray <Profile>
		primitives = ($<preset>)
...
script create_ui_visualizer_fx 
	RequireParams \{[
		...
		all}
	printf \{'F(): create_ui_visualizer_fx'
		channel = visualizer}
	AddParams ($ui_visualizer_viewport_params)
	enable_ui_visualizer
	visualizer_get_primitives preset = <preset>
	create_avui_fx primitives = <primitives> viewport = ui_visualizer_viewport
endscript
...
script clear_ui_visualizer 
	...
	AddParams ($ui_visualizer_viewport_params)
	if ViewportExists id = <viewport>
		ScreenFX_ClearFXInstances viewport = <viewport> mode = UI
	endif
endscript
