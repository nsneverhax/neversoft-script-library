...
script mpm_load_pak \{async = 0
	...
	if mpm_search_for_map_key maps = <map_array> key = pak value = <pak>
		...
		printf '%p already loaded. Adding handle %h' p = <pak> h = <owner> donotresolve
	else
...
script mpm_is_busy 
	ScriptAssert \{'mpm_is_busy: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script mpm_unload_paks 
	ScriptAssert \{'mpm_unload_paks: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script mpm_flush_all_paks_eachlink 
	if NOT StructureContains structure = $mpm_state <checksum>
		printf 'Mapping %a does not exist' a = <checksum> donotresolve
		return
	endif
