...
script freeplay_update_highway_helpers 
	...
	formatText checksumName = helper_id 'highway_helperp%p' p = <Player>
	if ScreenElementExists id = <helper_id>
		formatText checksumName = 0xff332492 'highway_helper_select_p%p' p = <Player>
		formatText checksumName = scroll_id 'highway_helper_scroll_p%p' p = <Player>
		getplayerinfo <Player> controller
		get_controller_type controller_index = <controller>
		if ((<controller_type> = guitar) || (<controller_type> = bass))
			add_user_control_helper {
				text = qs(0xc18d5e76)
				button = green
				Scale = ($freeplay_guitar_select_pill_scale)
				Pos = ($freeplay_guitar_select_pill_pos)
				pos_anchor = [center , bottom]
				just = [center , top]
				override_parent = <helper_id>
				generate_helper_pill_id = FALSE
				helper_pill_id = <0xff332492>
				guitar
			}
			add_user_control_helper {
				text = qs(0xde41a641)
				button = strumbar
				Scale = ($freeplay_guitar_scroll_pill_scale)
				Pos = ($freeplay_guitar_scroll_pill_pos)
				pos_anchor = [center , bottom]
				just = [center , top]
				override_parent = <helper_id>
				generate_helper_pill_id = FALSE
				helper_pill_id = <scroll_id>
				guitar
			}
		elseif (<controller_type> = drum)
			add_user_control_helper {
				...
				button = drum_green
				override_parent = <helper_id>
				Scale = ($freeplay_drum_select_pill_scale)
				Pos = ($freeplay_drum_select_pill_pos)
				pos_anchor = [center , bottom]
				just = [center , top]
				generate_helper_pill_id = FALSE
				helper_pill_id = <0xff332492>
			...
			add_user_control_helper {
				text = qs(0xde41a641)
				button = analog_updown
				Scale = ($freeplay_drum_scroll_pill_scale)
				Pos = ($freeplay_drum_scroll_pill_pos)
				pos_anchor = [center , bottom]
				just = [center , top]
				override_parent = <helper_id>
				generate_helper_pill_id = FALSE
...
script create_freeplay_instrument_menu 
	...
	idx = (<idx> + 1)
	repeat <array_Size>
	freeplay_set_menu_background_dims menu_parent = <menu_parent> menu_width = <menu_width> Player = <Player>
	0x0814cdfa menu_parent = <menu_parent> Player = <Player>
...
script freeplay_mic_preference_menu_update_state 
	...
	if NOT freeplay_is_any_mic_free
		if NOT controller_has_headset controller = <controller>
			SpawnScriptNow freeplay_drop_player_out params = {Player = <Player> no_update}
			return
		endif
	endif
	repeat
...
script freeplay_pause_menu_songlist_pause 
	RequireParams \{[
		...
		all}
	if ((<pause_type> = edit_playlist) || (<pause_type> = new_playlist) || (<pause_type> = new_song))
...
script freeplay_focus_menu_element \{grayed_out = FALSE
	...
	se_setprops \{0x24cf9fe8 = fontgrid_title_a1}
	if
		obj_spawnscript 0x0542334f params = {Pos = (642.0, 0.0) z = 26 parent = <0xf4eb1fcd> start_scale = (0.05, 0.05) end_scale = (0.1, 0.1) emit_spread = 3 velocity = 1.0}
	else
		ScriptAssert \{'UI_art'}
	endif
endscript
...
script freeplay_set_menu_background_dims 
	...
	if playerinfoequals <Player> part = vocals
		...
		light_box_dims = (<background_width> * (1.0, 0.0) + ($0xba4f7ad9) * (0.0, 1.0))
	endif
	ResolveScreenElementID id = {<menu_parent> child = freeplay_menu_background}
...
script 0x0814cdfa 
	...
	if playerinfoequals <Player> part = vocals
		...
		if ScreenElementExists id = <helper_id>
			<helper_id> :se_setprops Pos = (0.0, 0.0)
		endif
	else
		<resolved_id> :se_setprops Pos = ($0x16085506)
		if ScreenElementExists id = <helper_id>
			<helper_id> :se_setprops Pos = ($0x7bc7f613)
		endif
	endif
endscript
...
script freeplay_create_stats_ui 
	...
	if (<num_active_players> > 0)
		...
		CastToInteger \{score}
		formatText TextName = score_amount_text qs(0x4d4555da) s = <score> usecommas
		getplayerinfo <Player> part
		switch <part>
			case guitar
			<id> :se_setprops score_text = <score_text> score_amount_text = <score_amount_text> alpha = 1 0x946e8950 = 1.0 time = 1
			case bass
			<id> :se_setprops score_text = <score_text> score_amount_text = <score_amount_text> alpha = 1 0x6535f4c3 = 1.0 time = 1
			case drum
			<id> :se_setprops score_text = <score_text> score_amount_text = <score_amount_text> alpha = 1 0x117f0d29 = 1.0 time = 1
			case vocals
			<id> :se_setprops score_text = <score_text> score_amount_text = <score_amount_text> alpha = 1 0x11623112 = 1.0 time = 1
		endswitch
		get_highway_hud_menu_parent Player = <Player>
...
script freeplay_disable_menus \{defer_system_notifications_while_disabled = true
	...
	if NOT ui_event_exists_in_stack \{Name = 'ds_connection_handler'}
		destroy_dialog_box
	endif
endscript
