...
script ui_create_song_summary_details \{for_transition = 0}
	...
	if stats_summary_id :desc_resolvealias \{Name = alias_diamondbling
			param = diamondbling_id}
		<diamondbling_id> :obj_spawnscript 0x317f8786 params = {minwidth = 100 maxwidth = 500 maxtime = 0.5}
	else
		ScriptAssert \{'UI_art'}
	endif
	if stats_summary_id :desc_resolvealias \{Name = alias_diamondbling2
			param = diamondbling_id2}
		<diamondbling_id2> :obj_spawnscript 0x317f8786 params = {minwidth = 100 maxwidth = 500 maxtime = 0.5}
	else
		ScriptAssert \{'UI_art'}
	endif
	...
	if (<non_vocalist> > 0)
		...
		if is_roadie_battle_mode
			0xa40da534 = <array_Size>
			GetArraySize \{$0xf7a6385b}
			<array_Size> = (<array_Size> + <0xa40da534>)
		endif
...
script song_summary_details_create_non_vocals_section_entry 
	...
	if is_roadie_battle_mode
		...
		if (<i> < <array_Size>)
			...
			roadie_stats_array = ($0x9d9be6d5 [(<Player> -1)])
			formatText TextName = 0xc06ddaa4 qs(0x76b3fda7) d = (<roadie_stats_array> [<i>])
			AddParam Name = <field_name> structure_name = my_struct value = <0xc06ddaa4>
