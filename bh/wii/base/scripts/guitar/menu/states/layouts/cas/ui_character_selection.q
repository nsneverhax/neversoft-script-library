...
script ui_create_character_selection_spawned 
	...
	if GotParam \{from_main_menu}
		...
		if NOT GotParam \{gone_into_charselect}
			Change \{charselect_previous_character_id = 0xdbbe2b30}
		endif
	endif
	menu_index = 0
	...
	if (<array_Size> < $max_num_create_a_rockers)
		...
		<selection_item_id> :se_setprops character_selection_item_bio_pos = {(0.0, -35.0) relative}
...
script ui_init_character_selection 
	...
	check_car_unlocks
	Change \{charselect_previous_character_id = None}
	SpawnScriptNow \{anim_paks_set_state
		params = {
			state = FrontEnd
			async = 0
		}}
