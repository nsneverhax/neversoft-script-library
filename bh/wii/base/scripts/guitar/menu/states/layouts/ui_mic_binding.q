...
script ui_create_mic_binding \{from_pause_menu = 0}
	...
	if hud_mic_binding :desc_resolvealias \{Name = 0x10aeb4b2}
	...
	if hud_mic_binding :desc_resolvealias \{Name = 0xa02563d1}
		if ScreenElementExists \{id = hud_mic_binding}
	...
	if hud_mic_binding :desc_resolvealias \{Name = 0x5f448112}
		if ScreenElementExists \{id = hud_mic_binding}
			<resolved_id> :obj_spawnscript 0xa323fa10 params = {0x971c66e7 = 1 0x3af134dc = 5 start_dims = (20.0, 20.0)}
		endif
	else
	...
	if hud_mic_binding :desc_resolvealias \{Name = 0x219b1158}
		if ScreenElementExists \{id = hud_mic_binding}
			<resolved_id> :obj_spawnscript 0xa323fa10 params = {0x971c66e7 = 1 0x3af134dc = 5 start_dims = (20.0, 20.0)}
		endif
	else
...
script mic_binding_setup_status_window 
	...
	if (<mic_type> = headset)
		headset_alpha = 1
		mic_alpha = 0
	elseif (<mic_type> = None || <mic_type> = invalid)
		headset_alpha = 0
		mic_alpha = 0
	else
		headset_alpha = 0
	...
	formatText TextName = playername_text qs(0x0f85ceba) a = <Player>
	getplayerinfo <Player> controller
...
script mic_binding_set_mic_type 
	...
	getplayerinfo <Player> mic_type
	if (<new_mic_type> = mic)
		setplayerinfo <Player> mic_preference = mic
		hud_mic_binding :se_setprops {
		...
		setplayerinfo <Player> mic_preference = headset
		hud_mic_binding :se_setprops {
			main_title_text = ($mic_binding_text.sing_title)
...
script mic_binding_bind_mic_finished 
	...
	if (<mic_type_a> = <mic_type_b>)
		mic_binding_player_flow_confirm Player = <Player>
		return
	endif
	player_a = <Player>
	player_b = -1
...
script mic_binding_player_flow_confirm 
	hud_mic_binding :se_setprops {
		...
		main_text = ($mic_binding_text.confirm_text)
	}
