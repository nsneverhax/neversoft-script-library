...
script ui_calibrate_option_choose 
	...
	endswitch
endscript
vv_calibrate_lag_exiting = 0

script ui_options_calibrate_delayed_exit_flag 
	Change \{vv_calibrate_lag_exiting = 1}
	Wait \{1
		Second}
	Change \{vv_calibrate_lag_exiting = 0}
endscript
...
script 0xbd7f12c5 
	...
	if GotParam \{current_changing}
		...
		elseif (<current_changing> = audio)
			calibrationinterface :se_setprops \{audiolag_rgba = 0xaad894a8}
		endif
	else
		Obj_GetID
		<objID> :se_setprops rgba = 0xaad894a8
	endif
endscript
...
script ui_calibrate_option_focus \{Scale = 1.25}
	...
	0x0cf62187 Scale = <Scale> id = <id> focus
	<id> :se_getparentid
...
endscript
lag_calibration_highway_interlock = 0
...
script choose_calibrate_lag 
	...
	LoadPak \{'pak/ui/options_calibrate_lag.pak'
		Heap = BottomUpHeap}
	...
	Change \{lag_calibration_highway_interlock = 1}
