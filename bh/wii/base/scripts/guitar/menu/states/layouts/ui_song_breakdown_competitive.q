...
script ui_create_song_breakdown_shreddfest 
	...
	if (<array_Size> <= 0)
		if is_roadie_battle_mode
			song_breakdown_roadie_battle_determine_order
		else
	...
	if is_roadie_battle_mode
		...
		if (<p1_score> > <p2_score>)
			<left_texture> = `rb-roadie1-happy1`
			<left_num_uv_frames> = (2.0, 1.0)
			<left_dims> = (256.0, 128.0)
			<right_texture> = `rb-roadie2-upset1`
			<right_num_uv_frames> = (2.0, 2.0)
			<right_dims> = (256.0, 256.0)
		elseif (<p2_score> > <p1_score>)
			<left_texture> = `rb-roadie1-upset2`
			<left_num_uv_frames> = (2.0, 1.0)
			<left_dims> = (256.0, 128.0)
			<right_texture> = `rb-roadie2-happy1`
			<right_num_uv_frames> = (2.0, 2.0)
			<right_dims> = (256.0, 256.0)
		else
			<left_texture> = `rb-roadie1-surprised2`
			<left_num_uv_frames> = (2.0, 1.0)
			<left_dims> = (256.0, 128.0)
			<right_texture> = `rb-roadie2-surprised1`
			<right_num_uv_frames> = (2.0, 1.0)
			<right_dims> = (256.0, 128.0)
		endif
		if ScreenElementExists \{id = left_roadie_portrait}
			DestroyScreenElement \{id = left_roadie_portrait}
		endif
		CreateScreenElement {
			Type = SpriteElement
			parent = song_breakdown_competitive_id
			id = left_roadie_portrait
			texture = <left_texture>
			num_uv_frames = <left_num_uv_frames>
			dims = <left_dims>
			Pos = (490.0, -8.0)
			just = [center , center]
			pos_anchor = [center , center]
		...
		if ScreenElementExists \{id = right_roadie_portrait}
			DestroyScreenElement \{id = right_roadie_portrait}
		endif
		CreateScreenElement {
			Type = SpriteElement
			parent = song_breakdown_competitive_id
			...
			num_uv_frames = <right_num_uv_frames>
			dims = <right_dims>
			Pos = (690.0, -8.0)
			just = [center , center]
			pos_anchor = [center , center]
...
script song_breakdown_faceoff_continue_to_next_screen 
	...
	if (<device_num> = ($primary_controller))
		...
		if is_roadie_battle_mode
			get_savegame_from_player \{Player = 1}
			formatText checksumName = formatted_checksum 'venue_%s' s = ($LevelZones.load_z_vvstudio.Name) AddToStringLookup = true
			GetGlobalTags <formatted_checksum> savegame = <savegame>
			if (<unlocked> != 1)
				progression_career_do_venue_unlock venue_to_unlock = load_z_vvstudio savegame = <savegame>
				ui_event \{event = menu_change
					data = {
						state = uistate_unlock_venue
						load_venue_checksum = load_z_vvstudio
					}}
			else
...
script song_breakdown_roadie_battle_determine_order 
	getplayerinfo \{1
		score}
	<p1> = {band_num = 1 band_result = <score>}
	getplayerinfo \{2
		score}
	<p2> = {band_num = 2 band_result = <score>}
	<is_tie> = 0
	<order> = [{} {}]
	if ((<p1>.band_result) >= (<p2>.band_result))
		SetArrayElement ArrayName = order index = 0 NewValue = <p1>
		SetArrayElement ArrayName = order index = 1 NewValue = <p2>
		if ((<p1>.band_result) = (<p2>.band_result))
			<is_tie> = 1
		endif
	elseif ((<p2>.band_result) > (<p1>.band_result))
		SetArrayElement ArrayName = order index = 0 NewValue = <p2>
		SetArrayElement ArrayName = order index = 1 NewValue = <p1>
	endif
	return {
		order = <order>
		is_tie = <is_tie>
