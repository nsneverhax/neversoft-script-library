...
script music_store_choose_download_location 
	...
	if NOT (((<purchase_struct>.Type) = import) || ((<purchase_struct>.Type) = patch))
		if StructureContains \{struct = 0x67e6e23d
				song_original_artist}
			if ((<purchase_struct>.song_original_artist) = 0)
...
script music_store_get_checkout_stage_text 
	switch (<Stage>)
		...
		formatText TextName = error_text ($cntsd_messages.rvlcntsd_message_0007) s = qs(0x8618db05)
		formatText TextName = stage_text qs(0x17b336eb) s = <error_text>
...
script music_store_animate_progress_bar 
	...
	if marketplacefunc \{func = is_init}
		...
		if NOT ((<Stage> = downloading) || (<Stage> = sd_backup_normal) || (<Stage> = sd_backup_catalog) || (<Stage> = updating))
			<progress> = 0
			<0xcf28cbc0> = 0
			0x4a2f438b :se_setprops \{0x5fe4a9e8 = (100.0, 0.0)}
...
script 0x88353ef1 \{progress = 0.0
		0xcf28cbc0 = 0.0}
	<delta> = (((100.0 / 95.0) * 100.0) * <progress>)
	if (<delta> < <0xcf28cbc0>)
		<0xcf28cbc0> = 0
		0x4a2f438b :se_setprops \{0x5fe4a9e8 = (100.0, 0.0)}
	endif
	if (<delta> < 100.0)
		<0x2c01609d> = ((<delta> - <0xcf28cbc0>) / 0.1)
		CastToInteger \{0x2c01609d}
		<0x4174b259> = (100 - ($0x0c03ad74))
		if (<0x4174b259> > <0x2c01609d>)
			<0x4174b259> = <0x2c01609d>
		endif
		<0xde831533> = (<0x2c01609d> - <0x4174b259>)
		if (<0x4174b259> > 8)
			<0x4174b259> = 8
		endif
		if (<0x4174b259> >= 1)
			0x3bbf5c58 0x4174b259 = <0x4174b259>
			<0xcf28cbc0> = (<0xcf28cbc0> + (<0x4174b259> * 0.1) + (<0xde831533> * 0.1))
		endif
		if (<0xde831533> > 0)
			Change 0x0d0c8856 = (($0x0d0c8856) + (<0xde831533> * 0.1))
		endif
	endif
	0x31030b92
	return 0xcf28cbc0 = <0xcf28cbc0>
endscript

script 0x854acc32 
	begin
	se_setprops \{0x23cef2ee = 360.0
		time = 4.0}
	Wait \{4
		Seconds}
	se_setprops \{0x23cef2ee = 0}
	repeat
endscript

script 0x3bbf5c58 \{0x4174b259 = 0
		parent_id = 0x4a2f438b}
	SetSpawnInstanceLimits \{Max = 100
	...
	if (<side> = left)
		GetRandomValue \{Name = start_x
			...
			a = -405
			b = -395}
		GetRandomValue \{Name = start_y
			integer
			a = -50
			b = 15}
		start_pos = (((1.0, 0.0) * <start_x>) + ((0.0, 1.0) * <start_y>))
	elseif (<side> = right)
		GetRandomValue \{Name = start_x
			integer
			a = 395
	...
	0x4a2f438b :desc_resolvealias \{Name = 0xc7d5220b}
	CreateScreenElement {
		Type = SpriteElement
		parent = <resolved_id>
		texture = highlight_sparkle
		just = [center center]
	...
	<id> :obj_spawnscript 0x6df6a3b1
	Change 0x0c03ad74 = (($0x0c03ad74) + 1)
	repeat <0x4174b259>
endscript

script 0x6df6a3b1 
	0x4a2f438b :se_getprops \{0x5fe4a9e8}
	<0x3f55120d> = (95.0 - (0.7 * <0x5fe4a9e8> [1]))
	GetRandomValue \{Name = 0x4852229b
		integer
		a = -10
		b = 10}
	<0x62af5c31> = (((1.0, 0.0) * <0x4852229b>) + ((0.0, 1.0) * <0x3f55120d>))
	GetRandomValue \{Name = move_time
		float
		a = 0.5
		b = 1.0}
	se_setprops Pos = <0x62af5c31> time = <move_time>
	se_setprops alpha = 0 time = <move_time>
	se_setprops rgba = [236 197 60 255] time = <move_time>
	se_setprops rot_angle = 360 time = <move_time>
	se_waitprops
	Change 0x0d0c8856 = (($0x0d0c8856) + 0.1)
	Change 0x0c03ad74 = (($0x0c03ad74) -1)
	Die
endscript
...
script 0x31030b92 
	0x4a2f438b :se_getprops \{0x5fe4a9e8}
	<0xcad4c7f2> = (<0x5fe4a9e8> [1] + ($0x0d0c8856))
	if (<0xcad4c7f2> > 100)
		<0xcad4c7f2> = 100
	endif
	<new_dims> = (((1.0, 0.0) * <0x5fe4a9e8> [0]) + ((0.0, 1.0) * <0xcad4c7f2>))
	0x4a2f438b :se_setprops {
		0x5fe4a9e8 = <new_dims>
	}
	Change \{0x0d0c8856 = 0.0}
endscript
