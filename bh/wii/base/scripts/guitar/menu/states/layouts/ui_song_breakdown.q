...
script ui_create_song_breakdown 
	...
	if NOT screenFX_FXInstanceExists \{viewport = bg_viewport
		...
		ScreenFX_AddFXInstance {
			...
			Name = venue_DOF
			($dof_endsongblur_tod_manager.screen_FX [0])
		}
	else
...
script ui_create_song_breakdown_normal 
	...
	if song_breakdown_is_single_player_game
		...
		if song_breakdown_noncompetitive_id :desc_resolvealias \{Name = 0xcea3fb70
				param = star_glow_id}
			<star_glow_id> :obj_spawnscript 0xd6ac24bd
		else
			ScriptAssert \{'dschorn1'}
		endif
		...
		if song_breakdown_noncompetitive_id :desc_resolvealias \{Name = alias_diamondbling4
			...
			<diamondbling4_id> :obj_spawnscript 0x976144e6 params = {minwidth = 4 maxwidth = 8 maxtime = 0.5}
		else
			ScriptAssert \{'UI_art'}
		endif
		if song_breakdown_noncompetitive_id :desc_resolvealias \{Name = alias_diamondbling5
				param = diamondbling5_id}
			<diamondbling5_id> :obj_spawnscript 0x976144e6 params = {minwidth = 4 maxwidth = 8 maxtime = 0.5}
		else
			ScriptAssert \{'UI_art'}
		endif
	...
	if (<achieved_gold_stars> = 1)
		if ScreenElementExists \{id = song_breakdown_noncompetitive_id}
			...
			if song_breakdown_noncompetitive_id :desc_resolvealias \{Name = 0x0707c0a0
					param = 0xd8c6cde7}
				<0xd8c6cde7> :obj_spawnscript anim_bling params = {maxradius = 50 minradius = 20 maxtime = 0.8}
			else
				ScriptAssert \{'UI_art'}
			endif
			if song_breakdown_noncompetitive_id :desc_resolvealias \{Name = 0x1e06f5a8
					param = 0xf661e096}
				<0xf661e096> :obj_spawnscript anim_bling params = {maxradius = 50 minradius = 20 maxtime = 0.8}
			else
				ScriptAssert \{'UI_art'}
			endif
		endif
...
script ui_song_breakdown_get_player_display_name 
	RequireParams \{[
		...
		all}
	<Name> = qs(0x00000000)
	if (($is_network_game = 1) || (isps3) || (isngc))
	...
	if (is_roadie_battle_mode)
		roadie_battle_get_roadie_from_player Player = <player_index>
		<roadie_index> = (<roadie> -1)
		<ds_name_text> = ($roadie_infos [<roadie_index>].Name)
		formatText TextName = ds_name_text qs(0x48c6d14c) d = <ds_name_text>
		band_lobby_rockertext Player = <player_index> viewport_index = (<player_index> -1)
		<player_name_text> = (<text_params>.a)
		formatText TextName = player_name_text '%p & %d' p = <player_name_text> d = <ds_name_text>
		formatText TextName = player_name_text qs(0x66680619) p = <player_name_text>
	endif
...
script ui_song_breakdown_add_handlers_for_next_transition_spawned 
	RequireParams \{[
		...
		all}
	printf \{'ui_song_breakdown_add_handlers_for_next_transition_spawned'}
	if isSinglePlayerGame
		array = []
	else
...
script ui_win_song_continue_next_menu 
	...
	switch ($game_mode)
		...
		if ($last_song_played = jamsession)
			...
			ui_memcard_autosave is_popup event = <event> state = <state> data = {all_active_players = true}
		endif
		case career
		case gh4_p1_career
		case gh4_p2_career
...
script getspphraseshitfromdetailedstats \{Player = 1}
	ScriptAssert \{'getSpPhrasesHitFromDetailedStats: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript

script getspphrasestotalfromdetailedstats \{Player = 1}
	ScriptAssert \{'getSpPhrasesTotalFromDetailedStats: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript

script getavgmultiplierfromdetailedstats \{Player = 1}
	ScriptAssert \{'getAvgMultiplierFromDetailedStats: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script song_breakdown_end_game 
	...
	if ((<is_game_over> = 1) && ($net_popup_active = 0))
		...
		ui_event_get_top
		printstruct <...>
		if ((<base_name> = 'online_quit_warning') || (<base_name> = 'song_summary_details'))
			generic_event_back
			ui_event_wait_for_safe
		endif
		switch <drop_reason>
