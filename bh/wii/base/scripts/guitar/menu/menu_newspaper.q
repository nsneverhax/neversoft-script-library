...
script create_newspaper_menu \{for_practice = 0}
	...
	if playerinfoequals \{bot_play = 1}
	...
	if (<total_notes> > 0)
		getplayerinfo \{1
			notes_hit}
		<p1_percent_complete> = (100 * <notes_hit> / <total_notes>)
	endif
	...
	if (<total_notes> > 0)
		getplayerinfo \{2
			notes_hit}
		<p2_percent_complete> = (100 * <notes_hit> / <total_notes>)
	endif
	...
	if NOT ($game_mode = p1_career || $game_mode = p2_career || $game_mode = p2_coop || $game_mode = p1_quickplay || $game_mode = p2_quickplay || $game_mode = p3_quickplay || $game_mode = p4_quickplay || <for_practice> = 1)
		...
		getplayerinfo <the_player> outfit
		getplayerinfo <the_player> style
...
script np_scroll_text_horizontal \{time = 5
	...
	ScriptAssert \{'np_scroll_text_horizontal: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script np_create_text \{Pos = (200.0, 200.0)
	...
	ScriptAssert \{'np_create_text: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script np_create_options_menu \{Pos = (600.0, 300.0)
	...
	if (<for_practice> = 1)
		continue_handlers = [
			...
			{pad_choose generic_event_back params = {state = uistate_songlist no_jamsession}}
		]
		CreateScreenElement {
			Type = TextElement
			parent = current_menu
...
script scale_textblock \{reset_scale = 0}
	ScriptAssert \{'scale_textblock: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript

script np_2p_hilite_sections 
	ScriptAssert \{'np_2p_hilite_sections: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
