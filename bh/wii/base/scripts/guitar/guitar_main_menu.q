...
script create_main_menu_elements 
	...
	mainmenu_setup_option \{choose_script = main_menu_select_vip
		menu_item = motd}
	mainmenu_id :SetTags motd_id = <mainmenu_item_id>
	show_debug_menus = 0
...
script main_menu_select_freestyle 
	...
	if ($enable_saving = 1)
		generic_menu_pad_choose_sound
		menu_transition_stoprender \{die_if_ui_event_running = 1}
		set_primary_controller device_num = <device_num> state = 0x588e3331
	else
		LaunchEvent \{Type = unfocus
			target = current_menu}
...
script mainmenu_freeplay_timer 
	...
	Wait \{1
		Second}
	repeat $g_mainmenu_freeplay_timer
