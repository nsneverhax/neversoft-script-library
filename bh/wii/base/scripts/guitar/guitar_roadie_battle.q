...
script roadie_battle_display_intro_screen 
	...
	<id> :se_getprops
	<0x27e50c4f> = (<dims> [0])
	0x86576c3a = ((<0x30c801cd> + <0x27e50c4f>) / 2.0)
	0x999bcf4f = ((<0x30c801cd> - <0x86576c3a>) * 0.65000004)
	PosX = (<0x999bcf4f> - 5)
	PosY = -260
...
script roadie_battle_get_total_time_sabotaged 
	...
	GetArraySize ($<roadie_history>)
	if (<array_Size> < 1)
		return \{total_time_sabotaged = 0.0
			0xa430a0e9 = 0.0}
	endif
	previous_time = 0
	current_time = 0
	0xa46a3e99 = 0
	0x0778683d = [{count = 0 Duration = 0}]
	i = 0
	begin
	<history_entry> = ($<roadie_history> [<i>])
	...
	if (<diff> != 0)
		<current_time> = (<history_entry>.time)
		<0x15ac2477> = {count = <0xa46a3e99> Duration = (<current_time> - <previous_time>)}
		AddArrayElement array = <0x0778683d> element = <0x15ac2477>
		<0x0778683d> = <array>
		<0xa46a3e99> = (<0xa46a3e99> + <diff>)
		<previous_time> = <current_time>
	endif
	<i> = (<i> + 1)
	repeat <array_Size>
	last_entry = {count = <0xa46a3e99> Duration = ($roadie_battle_previous_song_length - <previous_time>)}
	AddArrayElement array = <0x0778683d> element = <last_entry>
	GetArraySize <0x0778683d>
	if (<array_Size> < 1)
		return \{total_time_sabotaged = 0.0
			0xa430a0e9 = 0.0}
	endif
	total_time_sabotaged = 0.0
	0xa430a0e9 = 0.0
	<i> = 0
	begin
	<0x15ac2477> = (<0x0778683d> [<i>])
	<Duration> = (<0x15ac2477>.Duration)
	<0xa46a3e99> = (<0x15ac2477>.0xa46a3e99)
	if (<0xa46a3e99> > 0)
		<total_time_sabotaged> = (<total_time_sabotaged> + <Duration>)
	endif
	<0xa430a0e9> = (<0xa430a0e9> + (<0xa46a3e99> * <Duration>))
	<i> = (<i> + 1)
	repeat <array_Size>
	<0xa430a0e9> = (<0xa430a0e9> / $roadie_battle_previous_song_length)
	return total_time_sabotaged = <total_time_sabotaged> 0xa430a0e9 = <0xa430a0e9>
endscript

script roadie_battle_generate_formatted_roadie_stats 
	getfirstplayer \{on_screen}
	GetArraySize \{$0xf7a6385b}
	Change \{0x9d9be6d5 = [
		]}
	begin
	if (<Player> <= 0)
	...
	roadie_battle_get_sabotaged_and_repaired roadie = <roadie>
	roadie_battle_get_total_time_sabotaged roadie = <roadie>
	i = 0
	CastToInteger \{sabotaged}
	formatText TextName = stat_string qs(0x73307931) s = <sabotaged>
	SetArrayElement ArrayName = array NewValue = <stat_string> index = 0
	CastToInteger \{repaired}
	formatText TextName = stat_string qs(0x73307931) s = <repaired>
	AddArrayElement array = <array> element = <stat_string>
	format_time ms = <total_time_sabotaged>
	<stat_string> = <formatted_time>
	AddArrayElement array = <array> element = <stat_string>
	formatText TextName = stat_string qs(0x73307931) s = <0xa430a0e9>
	AddArrayElement array = <array> element = <stat_string>
	AddArrayElement array = $0x9d9be6d5 element = <array>
	Change 0x9d9be6d5 = <array>
...
script roadie_battle_amp_animate_highway_soundwaves 
	...
	<id1> :se_setprops Scale = <Scale> alpha = 0.0 time = 1.5
	<id1> :se_waitprops
...
script roadie_battle_in_game_disconnect_handler 
	...
	elseif ui_event_exists_in_stack \{Name = 'pausemenu'}
		if ScreenElementExists \{id = current_menu}
			LaunchEvent \{Type = unfocus
				target = current_menu}
			pause_menu_desc_id :se_setprops \{alpha = 0.0}
		endif
		break
	endif
	Wait \{0.5
	...
	if NOT (<roadie_battle_ai> = 1)
		if NOT (($roadie_infos [<roadie_index>].is_ai) = 1)
			...
			elseif (<array_Size> <= 2)
				...
				if ((<time_since_last_attack> >= 40000) && (<array_Size> > 0))
					RunScriptOnScreenElement {
						id = <hud_text_id>
						roadie_battle_animate_text
						params = {
...
script 0x1606f4d5 
	RequireParams \{[
		...
		all}
	band_lobby_rockertext Player = <Player> viewport_index = (<Player> -1)
	wiidscommunication :sendmessage set_player_name player_number = <Player> player_name = (<text_params>.a)
endscript
