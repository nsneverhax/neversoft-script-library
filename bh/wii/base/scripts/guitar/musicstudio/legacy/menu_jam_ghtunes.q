	...
	if ghtunes_element :desc_resolvealias \{Name = jam_user_content_leaderboard}
		...
		ghtunes_leaderboard_menu :SetProps {exclusive_device = ($primary_controller)
			event_handlers = [{pad_up ghtunes_pad_up}
				...
				{pad_L1 ghtunes_show_terms_of_service params = {parent_menu = ghtunes_leaderboard_menu show_connecting_popup}}
	...
	if ghtunes_element :desc_resolvealias \{Name = alias_sort_scrolling_menu}
		...
		ghtunes_sort_scrolling_menu :SetProps {exclusive_device = ($primary_controller)
			event_handlers = [
				...
				{pad_L1 ghtunes_show_terms_of_service params = {parent_menu = ghtunes_sort_scrolling_menu show_connecting_popup}}
			]
		}
		<index> = 0
		begin
...
script jam_get_primary_user_id 
	ScriptAssert \{'jam_get_primary_user_id: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
endscript
lost_network_message_xenon = 'The gamer profile is not enabled for Xbox LIVE multiplayer game play or the network connection is lost.'
lost_network_message_ps3 = 'You are not signed in to the PlayStation®Network.  You must sign in to continue.'
...
script jam_ghtunes_failed_dialog \{dialog = 0}
	...
	switch <dialog>
		case 0
		if isxenonorwindx
			dialog_txt = 'Unable to connect to GHTunes. Please check your network connection and settings and try again.'
		elseif isps3
			dialog_txt = 'You must be signed in to the PlayStation®Network to browse GHTunes.'
		elseif isngc
			dialog_txt = $wii_ghtunes_signin
		endif
		case 1
		if isxenonorwindx
		...
		elseif isngc
			dialog_txt = $wii_ghtunes_unavailable
		endif
		case 2
		if isxenonorwindx
			dialog_txt = 'You have set your gamer profile to prohibit user created content. You will be unable to access GHTunes. Any songs that you have received through GHTunes will not be accessible.'
		elseif isps3
			dialog_txt = 'Online service is disabled on your PlayStation®Network account due to parental control restrictions.'
		elseif isngc
			dialog_txt = $wii_rvldwc_message_0003
		endif
		case 3
		dialog_txt = qs(0xb9f3b9df)
		case 4
		if isxenonorwindx
			dialog_txt = 'No gamer profile currently signed in. Please sign into an Xbox LIVE multiplayer enabled gamer profile to browse GHTunes.'
		elseif isps3
			dialog_txt = 'You must be signed in to the PlayStation®Network to browse GHTunes.'
		elseif isngc
			dialog_txt = $wii_ghtunes_must_be_signed
		endif
		case 5
		dialog_txt = qs(0xb9f3b9df)
		case 6
		...
		case 7
		dialog_txt = qs(0x9bfb2b7e)
		case 8
		dialog_txt = qs(0xb14f4b49)
		case 9
		dialog_txt = 'You are currently signed into a guest gamer profile. In order to use this feature, you must be signed into Xbox LIVE, and not a guest of a multiplayer enabled gamer profile.'
	endswitch
	destroy_generic_menu
	create_dialog_box {
		template = large_body
...
endscript
legal_ghtunes_num_results = 60
legal_ghtunes_curr_start_at = 0
legal_ghtunes_max_start_at = 999999999
legal_ghtunes_on_last_page = 0

script ghtunes_show_terms_of_service \{start_at = 1
		num_results = 0}
	clean_up_user_control_helpers
	...
	printf \{qs(0x8931adb2)}
	SoundEvent \{event = ghtunes_ui_select}
	if GotParam \{show_connecting_popup}
		if ScreenElementExists \{id = ghtunes_terms_dialog_box}
			DestroyScreenElement \{id = ghtunes_terms_dialog_box}
		endif
	endif
	...
	if GotParam \{show_connecting_popup}
		...
		CreateScreenElement {
			...
			event_handlers = [
				...
				{pad_option2 ghtunes_legal_previous_page params = {parent_menu = <parent_menu>}}
				{pad_option ghtunes_legal_next_page params = {parent_menu = <parent_menu>}}
		...
		printf \{qs(0x629a136d)}
		ghtunes_terms_scroll_reset
	endif
	if ghtunes_terms_dialog_box :desc_resolvealias \{Name = alias_text_menu}
		GetArraySize ($ghtunes_terms_of_service_array)
		Mod a = <array_Size> b = $legal_ghtunes_num_results
		Change legal_ghtunes_max_start_at = (<array_Size> - <Mod>)
		<i> = <start_at>
		if (((<start_at>) + ($legal_ghtunes_num_results)) < (<array_Size> -1))
			<repeat_for> = ($legal_ghtunes_num_results)
		else
			<repeat_for> = (<array_Size> - <start_at>)
		endif
		<destroy_iter> = 0
		begin
		formatText checksumName = block_id 'ghtunes_terms_text_block_element_%c' c = <destroy_iter>
		if ScreenElementExists id = <block_id>
			DestroyScreenElement id = <block_id>
		endif
		<destroy_iter> = (<destroy_iter> + 1)
		repeat (<array_Size>)
		<i> = <start_at>
		begin
		formatText checksumName = block_id 'ghtunes_terms_text_block_element_%c' c = <i>
		if isngc
			CreateScreenElement {
	...
	add_user_control_helper \{text = qs(0xaf4d5dd2)
		button = red
		z = 100000}
	if ((($legal_ghtunes_curr_start_at / $legal_ghtunes_num_results) + 1) > 1)
		add_user_control_helper \{text = qs(0xa42bd569)
			button = yellow
			z = 10000}
	endif
	if (($legal_ghtunes_curr_start_at + <repeat_for>) != <array_Size>)
		add_user_control_helper \{text = qs(0x2931d48b)
			button = blue
			z = 10000}
		Change \{legal_ghtunes_on_last_page = 0}
	else
		Change \{legal_ghtunes_on_last_page = 1}
	endif
endscript

script ghtunes_legal_next_page 
	printstruct <...>
	if (($legal_ghtunes_on_last_page) = 1)
		return
	endif
	if ($legal_ghtunes_curr_start_at = 0)
		Change legal_ghtunes_curr_start_at = ($legal_ghtunes_num_results + 1)
	else
		Change legal_ghtunes_curr_start_at = ($legal_ghtunes_curr_start_at + $legal_ghtunes_num_results)
	endif
	SoundEvent \{event = ghtunes_ui_select}
	ghtunes_show_terms_of_service parent_menu = <parent_menu> start_at = $legal_ghtunes_curr_start_at num_results = $legal_ghtunes_num_results
endscript

script ghtunes_legal_previous_page 
	if NOT ((($legal_ghtunes_curr_start_at / $legal_ghtunes_num_results) + 1) > 1)
		return
	endif
	Change legal_ghtunes_curr_start_at = ($legal_ghtunes_curr_start_at - $legal_ghtunes_num_results)
	if ($legal_ghtunes_curr_start_at < 1)
		Change \{legal_ghtunes_curr_start_at = 1}
		return
	endif
	SoundEvent \{event = ghtunes_ui_back}
	ghtunes_show_terms_of_service parent_menu = <parent_menu> start_at = $legal_ghtunes_curr_start_at num_results = $legal_ghtunes_num_results
...
script ghtunes_terms_scroll_reset 
	if isngc
		if ghtunes_terms_dialog_box :desc_resolvealias \{Name = alias_text_menu}
