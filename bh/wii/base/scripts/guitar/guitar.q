...
script guitar_startup 
	...
	PushMemProfile \{'CAudioVisualizerFXPropsState'}
	visualizermanager_init \{0x132ccf8d}
	...
	ps2_load_car_part_script \{Heap = 0x28b22303}
...
script menu_transition_stoprender \{die_if_ui_event_running = 0}
	if (<die_if_ui_event_running> = 1)
...
script 0x61906aac 
	if ScriptIsRunning \{menu_transition_stoprender}
		spawnscript \{0x7312959a}
	else
		StartRendering \{lock = menu_transition}
	endif
endscript

script 0x7312959a 
	begin
	if NOT ScriptIsRunning \{menu_transition_stoprender}
		break
	endif
	WaitOneGameFrame
	repeat
