...
script GuitarEvent_SongWon_Spawned \{quit_network_game_early = 0}
	gamemode_gettype
	...
	if ($autolaunch_startnow = 1)
		...
		if NOT GotParam \{ENCORE_TRANSITION}
			...
			elseif (<Type> = freeplay)
				spawnscript \{0xed78df40}
				freeplay_next_song
				loading_transition = 1
			endif
			if (<Type> != career && <Type> != freeplay)
				if (<quit_network_game_early> = 0)
					0xe55bed32
				endif
			endif
