...
script create_sp_proto_hud_elements 
	ScriptAssert \{'create_sp_proto_hud_elements: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
...
script create_highway_star_power_effect_old 
	...
	<namec2> :se_setprops Pos = <pos2> Scale = (2.0, 1.0) time = 0.5
	Wait \{0.5
		Seconds}
...
script create_vocals_star_power_effect 
	...
	getplayerinfo <Player> part
	if (<part> != vocals)
		ScriptAssert \{'How did you call create vocals star power on a non-vocals player??'}
		return
	endif
...
script create_highway_star_power_effect 
	...
	if (<num_players> = 1)
		...
		endpos_l = {(120.0, -80.0) relative}
		endpos_r = {(-120.0, -80.0) relative}
		rot_l = -33
		rot_r = 33
	endif
	formatText checksumName = cont 'sidebar_container_leftp%p' p = <Player> AddToStringLookup = true
