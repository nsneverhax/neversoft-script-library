	...
	if StructureContains structure = $gh_songlist_props <song>
		if GotParam \{leaderboard}
			if StructureContains structure = ($gh_songlist_props.<song>) xenindex
				return song_prefix = ($gh_songlist_props.<song>.xenindex)
			endif
		endif
...
script 0xbb662f72 \{song = invalid}
	if StructureContains structure = $gh_songlist_props <song>
		song_genre = ($gh_songlist_props.<song>.genre)
		if StructureContains structure = $song_genre_list <song_genre>
			return song_genre_string = (($song_genre_list.<song_genre>).genre_string)
		else
			return song_genre_string = (($song_genre_list.other).genre_string)
		endif
	endif
	printstruct <...>
	ScriptAssert \{qs(0x0ebc33db)}
...
script 0x711e9aa2 
	0xce22902f song_index = <song_index>
	if (<song_index> = (<min_index> + 1))
		return \{true}
	else
		return \{FALSE}
	endif
...
script get_song_year_text \{song = invalid}
	ScriptAssert \{'get_song_year_text: Removed by remove_scripts.pl.  If you hit this assert, tell someone.'}
endscript
