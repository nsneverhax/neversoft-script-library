...
script transition_selectcelebtransition 
	...
	if (<current_challenge> = 0x41634d49)
		AddParam \{Name = 0xb6ec33b2
			value = 1
			structure_name = NewValue}
	elseif (<current_challenge> = 0xb6ec33b2)
		AddParam \{Name = 0x41634d49
			value = 1
			structure_name = NewValue}
	endif
...
script Transition_Play_Spawned 
	...
	GetSongTimeMs
	time_offset = (0 - <time>)
	if NOT (<Type> = loading || <Type> = onlineloading)
		begin
		...
		<force_wait> = ($transition_force_loading_wait)
		begin
		GetSongTimeMs time_offset = <time_offset>
		if (<transition_time> <= <time>)
			if (<force_wait> = 0 || $transition_force_loading_wait = 0)
				Change \{transition_playing = FALSE}
				break
			else
				reset_song_time starttime = (<transition_time> - <time_offset>)
			endif
		endif
		if (<Type> = loading && <force_wait> = 1)
			if ($transition_force_loading_wait = 0)
...
script transition_setcamera 
	...
	printf qs(0x4f221eff) a = <note>
	if GetNoteMapping section = cameras note = <note>
		clip_get_time_and_frame
		printf channel = clip qs(0x789850a5) a = <time_string>
...
script momentcamera_playanim \{cycle = 0
	...
	elseif GotParam \{start_node}
		...
		if CompositeObjectExists Name = <appended_id>
			...
			printf channel = anim_info qs(0x2aca76c0) a = <appended_id> b = <pak> c = <suffix>
		endif
	endif
	...
	if (($debug_camanimdebug) = 1)
		lock_target_name = 'unknown'
		switch <lock_target>
			case moment_cam_lock_target_01
			lock_target_name = 'moment_01'
			Change \{debug_y_line = 1}
			case moment_cam_lock_target_02
			lock_target_name = 'moment_02'
			Change \{debug_y_line = 2}
			case moment_cam_lock_target_03
			lock_target_name = 'moment_03'
			Change \{debug_y_line = 3}
			case moment_cam_lock_target_04
			lock_target_name = 'moment_04'
			Change \{debug_y_line = 4}
			case musician1_mocap_lock_target_01
			lock_target_name = 'musician1_mocap1'
			Change \{debug_y_line = 5}
			case musician1_mocap_lock_target_02
			lock_target_name = 'musician1_mocap2'
			Change \{debug_y_line = 6}
			case musician2_mocap_lock_target_01
			lock_target_name = 'musician2_mocap1'
			Change \{debug_y_line = 7}
			case musician2_mocap_lock_target_02
			lock_target_name = 'musician2_mocap2'
			Change \{debug_y_line = 8}
			case musician3_mocap_lock_target_01
			lock_target_name = 'musician3_mocap1'
			Change \{debug_y_line = 9}
			case musician3_mocap_lock_target_02
			lock_target_name = 'musician3_mocap2'
			Change \{debug_y_line = 10}
			case musician4_mocap_lock_target_01
			lock_target_name = 'musician4_mocap1'
			Change \{debug_y_line = 11}
			case musician4_mocap_lock_target_02
			lock_target_name = 'musician4_mocap2'
			Change \{debug_y_line = 12}
		endswitch
		printdebugtext X = -8 y = ($debug_y_line) str = <lock_target_name>
		formatText TextName = debug_line_text '%a.ska' a = <anim> DontAssertForChecksums
		printdebugtext X = 10 y = ($debug_y_line) str = <debug_line_text>
	endif
...
script Transitions_ResetZone 
	...
	if checksumequals a = ($<nodearray_checksum> [<array_count>].Name) b = 0xa57aaa9c
		resetvalid = FALSE
	endif
	if (<resetvalid> = true)
