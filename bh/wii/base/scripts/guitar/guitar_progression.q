...
script reset_progression_atoms \{force_update = 1}
	...
	create_player_progression_atoms savegame = <savegame>
...
script create_player_progression_atoms 
	printf \{qs(0x6c6313b9)}
	if NOT GotParam \{savegame}
		ScriptAssert \{'create_player_progression_atoms Error: No savegame defined'}
	endif
	progressioncreatevenueatomsarray savegame = <savegame>
	progressioncreategigsatomsarray savegame = <savegame>
	progressioncreaterewardsatomsarray savegame = <savegame>
...
script 0x29c0da71 
	RequireParams \{[
		...
		all}
	gig_star_rewards = ($gig_star_rewards.0xa3eb0a71)
...
script progression_career_do_reward_unlock 
	...
	switch (<reward_type>)
		...
		printf 'Unlocking preset locked character #%a through rewards system' a = <profile_idx> channel = progression_unlocks
...
script progression_reward_song_stars 
	...
	if (<is_local_client> = 1)
		...
		setupdlcsongtag content_name = <song_checksum> savegame = <savegame>
...
script progression_updatedetailedstatsforgig 
	...
	if (<num_players> > 0)
		...
		SetArrayElement ArrayName = song_stats_backup index = (<Player> - 1) NewValue = <curr_song_stats> globalarray
