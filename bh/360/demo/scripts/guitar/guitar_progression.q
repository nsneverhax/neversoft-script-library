...
script progression_create_venue_atoms_array \{async = 0}
	...
	depends_on = <array>
	<0x2a68313b> = ($venue_progression [<index>])
	<0xa99655f8> = (<0x2a68313b>.venue_to_unlock)
	if NOT structurecontains structure = ($levelzones) <0xa99655f8>
		<0x2a68313b> = {<0x2a68313b> venue_to_unlock = load_z_easterisland}
	endif
	atom = {
		name = <name>
		type = scr
		atom_script = progression_career_do_venue_unlock
		atom_params = {<0x2a68313b> savegame = <savegame>}
...
script unlock_career_finale_video_and_achievement 
	...
	getplayerinfo <player> controller
	getsavegamefromcontroller controller = <controller>
	fire_achievement name = 0x82e27279 controller = <controller>
	getnextplayer player = <player> local
	repeat <num_players>
endscript
