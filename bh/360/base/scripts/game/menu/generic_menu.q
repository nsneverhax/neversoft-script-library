...
script make_generic_menu {
	...
	CreateScreenElement {
		...
		generic_menu_vmenu_spacing_between = <spacing_between>
	}
	if ui_event_exists_in_stack \{name = 'debug'}
		alias_name = alias_generic_menu_vmenu_debug
	else
	...
	if (<desc> = 'generic_menu')
		if ScreenElementExists \{id = generic_menu}
			if generic_menu :Desc_ResolveAlias \{name = alias_glitter}
				if ScreenElementExists id = <resolved_id>
					<resolved_id> :Obj_SpawnScript ui_anim_glitter params = {max_interval = 0.25 Color = [128 128 200 200]}
				endif
			else
...
script add_generic_menu_text_item \{focus_script = focus_generic_menu_text_item
	...
	if GotParam \{not_focusable}
		...
		alpha = 1
		if GotParam \{not_focusable_alpha}
			alpha = <not_focusable_alpha>
		endif
		SetScreenElementProps {
			id = <id>
			not_focusable
...
script focus_generic_menu_text_item 
	if ScreenElementExists \{id = generic_menu}
		<id> :SE_SetProps generic_menu_smenu_textitem_text_rgba = ($UIColorRef_Palette.GH51_purple_dk.rgba)
...
script set_generic_menu_scrollbar_worker \{scrollbar_length = 464
		thumb_y_offset = 0
		menu_id = current_menu}
	<menu_id> :GetTags
	GetScreenElementChildren id = <menu_id>
	GetArraySize <children>
	<float_index> = (1.0 * <tag_selected_index>)
	if (<array_size> > 1)
		scroll_perc = (<float_index> / (<array_size> - 1))
	else
		scroll_perc = 0
	endif
	if (<thumb_y_offset> > <scrollbar_length>)
		softassert \{qs(0x1b489582)}
	endif
	<scrollbar_length> = (<scrollbar_length> - <thumb_y_offset>)
	scroll_pos = (((<scroll_perc> * (<scrollbar_length>)) * (0.0, 1.0)) + (<thumb_y_offset> * (0.0, 1.0)))
