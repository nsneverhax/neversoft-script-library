...
script menu_list_focus 
	if ScreenElementExists id = <id>
		current_list_menu :GetSingleTag \{focus_font}
		if GotParam \{focus_font}
			font = <focus_font>
		else
		...
		<id> :SE_SetProps generic_list_menu_item_text_scale = 1.2
		if GotParam \{price}
			<id> :SE_SetProps generic_list_menu_item_price_alpha = 1.0
		endif
		if GotParam \{editable}
	...
	if generic_list_menu :Desc_ResolveAlias \{name = alias_generic_scrollbar}
...
script menu_list_unfocus 
	if ScreenElementExists id = <id>
		current_list_menu :GetSingleTag \{unfocus_font}
		if GotParam \{unfocus_font}
			font = <unfocus_font>
		else
			font = ($test_menu_font)
		endif
		...
		<id> :SE_SetProps generic_list_menu_item_text_scale = 1.0
	endif
