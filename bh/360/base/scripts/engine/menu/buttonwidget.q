...
backing_uninit_focus_rgba = GH51_blue_lt
backing_init_rgba = [
	255
	50
	0
...
script button_widget_add_item \{widget_id = widget
	...
	CreateScreenElement {
		...
		light_bar_pos = {(10.0, 0.0) relative}
		autoSizeDims = true
	}
	text_id = <id>
	if GotParam \{invert}
...
script focus_button_widget 
	...
	if NOT GotParam \{rotate}
		SetScreenElementProps {
			id = <backing_id>
			rgba = ($backing_uninit_focus_rgba)
		}
	else
		...
		k = 0
		begin
		SetScreenElementProps id = {(<children> [<k>]) child = sprite} rgba = ($backing_uninit_focus_rgba)
		k = (<k> + 1)
		repeat <array_size>
	endif
	ExtendCRCWithString checksum = <widget_id> string = '_particle'
	<widget_id> :Obj_SpawnScript create_ui_highlight_particles params = {id = <ExtendedChecksum> Pos = (900.0, -15.0) z = 17.5}
...
script add_generic_menu_button_icon_item \{on = false}
	...
	<item_container> :Desc_ResolveAlias name = alias_generic_menu_smenu_iconitem_container
	CreateScreenElement {
