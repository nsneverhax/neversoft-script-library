...
script ui_create_jam_ratesong_quickplay 
	ui_create_jam_ratesong
endscript

script ui_destroy_jam_ratesong_quickplay 
	ui_destroy_jam_ratesong
endscript

script ui_init_jam_ratesong_quickplay 
	ui_init_jam_ratesong
endscript

script ui_deinit_jam_ratesong_quickplay 
	ui_deinit_jam_ratesong
endscript
	...
	if (<downloaded> = 1)
		...
		if ((<song_is_rated> = 0) && (<singed_in> = 1))
			...
			if ($game_mode = musicstudio)
				...
				ui_event_wait \{event = menu_replace
					data = {
						state = UIstate_jam_ratesong_quickplay
					}}
			endif
		else
