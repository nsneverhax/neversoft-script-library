...
script add_shape_list_subset_to_menu 
	...
	if GetCAPSectionsArray appearance = $cas_current_appearance part = <part>
		...
		mask = (<curr_section>.mask)
		skip_odd_indices = 0
		if (<curr_section>.desc_id = `Font	A1`)
			<skip_odd_indices> = 1
		endif
		if StructureContains structure = <curr_section> mixed_case_highlight
			current_list_menu :SetTags focus_font = (((<mask>) [0]).font)
			current_list_menu :SetTags unfocus_font = (((<mask>) [0]).font)
		endif
		GetArraySize (<mask>)
		i = 0
		begin
		add_item = 1
		if (<i> <= 52 && <skip_odd_indices> = 1)
			Mod a = <i> b = 2
			if (<Mod> = 1)
				<add_item> = 0
			endif
