...
script get_notetrack_part_text 
	...
	if GotParam \{player_status}
		if ($game_mode = gh4_p2_career || $game_mode = p2_quickplay || $force_coop = 1 ||
...
script load_gempaks \{async = 0}
	...
	if ($cheat_starcolor = 1)
		0xd463f0d7 = 'gold_'
	endif
	formatText TextName = gempak 'pak/gems/%p%i.pak' p = <0xd463f0d7> i = <pak> AddToStringLookup = true
	...
	formatText TextName = fxpak 'pak/gems/%i.pak' i = <pak> AddToStringLookup = true

	LoadPakAsync pak_name = <fxpak> Heap = heap_cas no_vram async = <async>
	Change last_fx_set = <fxpak>
endscript
