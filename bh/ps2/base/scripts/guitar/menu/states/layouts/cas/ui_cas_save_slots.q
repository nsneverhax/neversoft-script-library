...
script ui_create_cas_save_slots 
	...
	if (<0x2d65c078> = 1)
		create_menu_backdrop \{texture = 0x6a4c3519}
	endif
...
script load_guitar_save 
	...
	if StructureContains structure = <guitar_save> 0x4aa07dc7
		setcasappearancepartinstance part = (<instrument_info>.0x4aa07dc7) part_instance = (<guitar_save>.0x4aa07dc7)
	endif
	...
	if StructureContains structure = <guitar_save> 0x5f093fa6
		setcasappearancepartinstance part = (<instrument_info>.0x5f093fa6) part_instance = (<guitar_save>.0x5f093fa6)
	endif
...
script slot_focus_script 
	if ScreenElementExists id = <id>
		<id> :se_setprops {
			slot_text_rgba = [220 220 220 255]
		}
	endif
...
script slot_unfocus_script 
	if ScreenElementExists id = <id>
		<id> :se_setprops {
			slot_text_rgba = [150 150 150 255]
		}
	endif
endscript
...
script exit_save_instrument_worker 
	...
	if StructureContains structure = <instrument_info> body_part
		...
		if (<instrument_info>.body_part = cas_guitar_body)
			...
			if (<guitar_category> != metallica)
				achievements_fixxxer controller = ($primary_controller)
			endif
	...
	if StructureContains structure = <instrument_info> 0x4aa07dc7
		if getcasappearancepartinstance part = (<instrument_info>.0x4aa07dc7)
			entry = {
				<entry>
				0x4aa07dc7 = {desc_id = (<part_instance>.desc_id) chosen_materials = (<part_instance>.chosen_materials)}
			}
		endif
	endif
	...
	if StructureContains structure = <instrument_info> 0x5f093fa6
		if getcasappearancepartinstance part = (<instrument_info>.0x5f093fa6)
			entry = {
				<entry>
				0x5f093fa6 = {desc_id = (<part_instance>.desc_id) chosen_materials = (<part_instance>.chosen_materials)}
			}
		endif
	endif
...
script apply_cap_design 
	...
	globaltag_getarrayelement savegame = <savegame> array_name = <list> index = <index>
	0x4ae4019c data = (<element>.layers) <...>
...
script save_cap_design 
	ui_event event = menu_replace data = {state = uistate_cas_text_entry choose_script = save_cap_design_worker choose_params = {<...>} save_cap no_show_history = <no_show_history>}
endscript
