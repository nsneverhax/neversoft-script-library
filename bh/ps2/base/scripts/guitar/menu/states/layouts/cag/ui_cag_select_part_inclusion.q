...
script ui_create_cag_select_part_inclusion_spawned 
				...
				if is_part_unlocked part = <part> desc_id = ((($<part>) [<i>]).desc_id) savegame = ($cas_current_savegame)
					...
					elseif NOT GotParam \{no_edit}
						if is_part_editable part = <part> desc_id = ((($<part>) [<i>]).desc_id)
							...
							pad_option2_params = {part = <part> color_wheel = <color_wheel> num_icons = <num_icons> icon = <hist_tex> cam_name = <cam_name> camera_list = <camera_list> zoom_camera = <zoom_camera> stance = <stance> index = <i> additional_deinit_script = <additional_deinit_script>}
							show_editable = 1
							editable = {editable}
						endif
					endif
	...
	if GotParam \{0x04d6233f}
		if getcurrentcasobject
			bandmanager_changestance Name = <cas_object> stance = <0x04d6233f> no_wait
		endif
	endif
endscript
...
script cag_select_part_inclusion_decide_action 
	...
	if getcaspartmaterials part = <part>
		ui_event event = menu_replace data = {state = uistate_cas_color_edit part_materials = <part_materials> part = <part> num_states = 1 color_wheel = <color_wheel> cam_name = <cam_name> camera_list = <camera_list> zoom_camera = <zoom_camera> return_stance = <stance> additional_deinit_script = <additional_deinit_script>}
	else
		generic_event_back
	endif
endscript
