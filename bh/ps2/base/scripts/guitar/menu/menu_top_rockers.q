...
script menu_top_rockers_create_paper 
	...
	max_characters = 30
	if (<array_Size> < 20)
		my_tr_page :se_setprops \{0x2100c5fe = true}
		my_tr_page :se_setprops \{0x6f5ffa6e = FALSE}
	else
		my_tr_page :se_setprops \{0x2100c5fe = FALSE}
		my_tr_page :se_setprops \{0x6f5ffa6e = true}
	endif
	...
	my_tr_page :Obj_SpawnScriptNow 0x5711d458 params = {difficulty = <difficulty>}
	...
	if (<bestscore> < <player_score>)
		...
		if (<nowrite_full> = 0)
			...
			if ($current_song != jamsession)
				if StructureContains structure = ($permanent_songlist_props.$track_last_song) double_kick
					if ($permanent_songlist_props.($track_last_song).double_kick = 1 && <part> = drum && <double_kick_drum> = 1)
						SetGlobalTags <songname> params = {tr_double_bass_complete = 1}
					else
						SetGlobalTags <songname> params = {tr_double_bass_complete = 0}
					endif
				else
					SetGlobalTags <songname> params = {tr_double_bass_complete = 0}
				endif
...
script 0x5711d458 
	if desc_resolvealias \{Name = 0xed4639b4
			param = gem_id}
		begin
		0xeec85723 difficulty = <difficulty>
		Wait RandomFloat (0.5, 0.9) Seconds
		(<gem_id>) :obj_spawnscript 0xa74b2441 params = {<pos_range>}
		repeat
	else
		0x93d8d5cf \{qs(0xed7024f4)}
	endif
endscript
...
script 0xa74b2441 
	...
	0x92a1564d parent = <objID> Pos = <0xeb3ff33a> dims = (16.0, 16.0) 0x53924c09 = (40.0, 40.0) end_dims = (20.0, 20.0)
endscript

script 0x76aa8985 
	KillSpawnedScript \{Name = 0x5711d458}
	KillSpawnedScript \{Name = 0xeec85723}
	KillSpawnedScript \{Name = 0xa74b2441}
	0x78b6ea53
