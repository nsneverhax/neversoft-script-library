...
script rock_back_and_forth_star_meter 
	move_up_and_down = 1
	if ($game_mode = gh4_p1_career || $game_mode = p1_quickplay || $game_mode = p2_quickplay || $game_mode = gh4_p2_career)
...
script hud_move_note_scorebar \{Player = 1
	...
	if (($game_mode = gh4_p2_career || $game_mode = p2_quickplay) && (<Player> = 2))
		return
	endif
	...
	if (<in> = 1)
		...
		ExtendCrc HUD2D_note_container <player_text> out = new_container
		if ScreenElementExists id = <new_container>
			if ($game_mode = gh4_p1_career || $game_mode = gh4_p2_career || $game_mode = p1_quickplay || $game_mode = p2_quickplay)
