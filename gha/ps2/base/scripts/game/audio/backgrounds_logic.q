...
script Skate8_SFX_Change_Background 
	...
	testing_a = (($SFX_Background_FadeIn_Volume_Start * ($Backgrounds_Master_Volume / 100.0)))
	if NOT (GotParam Overide_Yes)
		if ($PleaseDOCrossfade = 1)
			...
			PlaySound ($SFX_Background_FadeIn_Checksum) vol = (($SFX_Background_FadeIn_Volume_Start * ($Backgrounds_Master_Volume / 100.0))) num_loops = -1 Priority = 105 buss = Crowd_Beds pan1x = -0.897089 pan1y = 0.1418498 pan2x = 0.88932973 pan2y = 0.15726662
		endif
	endif
	WaitOneGameFrame
	temp_counter = 1
	...
	if IsStreamPlaying ($SFX_Background_FadeIn_Checksum)
		SetSoundParams ($SFX_Background_FadeIn_Checksum) vol = (($SFX_Background_FadeIn_Volume_Current * ($Backgrounds_Master_Volume / 100.0)))
	else
		PlaySound ($SFX_Background_FadeIn_Checksum) vol = (($SFX_Background_FadeIn_Volume_Current * ($Backgrounds_Master_Volume / 100.0))) num_loops = -1 Priority = 105 buss = Crowd_Beds pan1x = -0.897089 pan1y = 0.1418498 pan2x = 0.88932973 pan2y = 0.15726662
	endif
	...
	if IsStreamPlaying ($SFX_Background_FadingOut_Checksum)
		SetSoundParams ($SFX_Background_FadingOut_Checksum) vol = (($SFX_Background_FadeOut_Volume_Current * ($Backgrounds_Master_Volume / 100.0)))
	endif
...
script 0xc9863983 \{vol = 11}
	...
	if NOT ($crowd_unique_id = NULL)
		0x7c757af0 $crowd_unique_id vol = ($0x4d507db2 * $Backgrounds_Master_Volume * 2)
	endif
endscript
