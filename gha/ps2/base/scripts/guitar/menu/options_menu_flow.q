...
options_confirm_band_delete_fs = {
	...
	actions = [
		{
			action = continue
			flow_state = options_delete_band_autosave_fs
			transition_right
		}
		{
			action = go_back
	...
	actions = [
		{
			...
			func = ps2_memcard_set_options_as_successor
			flow_state = ps2_memcard_overwrite_warning_ingame_fs
		}
		{
			action = select_load_game
			func = ps2_memcard_set_options_as_successor
			flow_state = ps2_memcard_load_lose_progress_ingame_fs
		}
		{
			action = go_back
			flow_state = options_select_option_fs
