...
script create_data_settings_menu \{for_loaddevice = 0}
	...
	if (<for_loaddevice> = 1)
		...
		if NOT English
			GetScreenElementDims \{id = ds_savegame}
			fit_text_in_rectangle id = ds_savegame dims = ((230.0, 0.0) + (0.0, 1.0) * <height>) only_if_larger_x = 1
			GetScreenElementDims \{id = ds_loadgame}
			fit_text_in_rectangle id = ds_loadgame dims = ((230.0, 0.0) + (0.0, 1.0) * <height>) only_if_larger_x = 1
		endif
		CreateScreenElement \{Type = ContainerElement
			parent = cds_vmenu
			dims = (200.0, 52.0)
