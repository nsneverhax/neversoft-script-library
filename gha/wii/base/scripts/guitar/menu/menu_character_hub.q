...
script create_character_hub_menu \{Player = 1}
	...
	if ($current_num_players = 2)
		...
		if (<Player> = 1)
			<hub_menu_center_pos> = (370.0, 516.0)
		else
			<hub_menu_center_pos> = (900.0, 516.0)
		endif
		if ($game_mode = p2_quickplay || $game_mode = p2_coop)
...
script character_hub_highlighter \{comi = 1
	...
	elseif (<chh_mp> = 1)
		if (<chh_player> = 1)
			...
			<offset_pos> = (530.0, -5.0)
		endif
		if NOT (<comi>)
			<initial_pos> = (220.0, 486.0)
		else
			<initial_pos> = (220.0, 446.0)
		endif
