	...
	if NOT ($game_mode = p1_career || $game_mode = p2_coop || $game_mode = p1_quickplay || $game_mode = p2_quickplay || <for_practice> = 1)
		...
		if ($player1_status.total_notes > 0)
			...
			displayText {
				...
				parent = newspaper_container
				text = '%'
				Pos = (<p1_stats_pos> + (70.0, 240.0) + <pos_tweak>)
				font = text_a4
				z = 4
		...
		if ($player2_status.total_notes > 0)
			...
			displayText {
				...
				parent = newspaper_container
				text = '%'
				Pos = (<p1_stats_pos> + (210.0, 245.0) + <pos_tweak>)
				font = text_a4
				z = 4
				rgba = <ss_percent_color>
				rot = 50
		...
		displayText {
			...
			parent = newspaper_container
			text = '\\u0NOTES HIT'
			Pos = (<p1_stats_pos> + (3.0, 220.0) + <pos_tweak>)
			Scale = (0.62, 0.7)
			font = text_a3
			z = 4
			rgba = <ss_percent_color>
		...
		fit_text_in_rectangle id = ss_p1_notes_text dims = (155.0, 31.0) Pos = (<p1_stats_pos> + (4.0, 219.0) + <pos_tweak>) start_x_scale = 0.4 start_y_scale = 0.7 only_if_larger_x = 1
		displayText {
			id = ss_p2_notes_text
			parent = newspaper_container
			text = '\\u0NOTES HIT'
			Pos = (<p2_stats_pos> + (-62.0, 220.0) + <pos_tweak>)
			Scale = (0.62, 0.7)
			font = text_a3
			z = 4
			rgba = <ss_percent_color>
		...
		SetScreenElementProps id = <id> font_spacing = 0
		fit_text_in_rectangle id = ss_p2_notes_text dims = (155.0, 31.0) Pos = (<p2_stats_pos> + (-62.0, 220.0) + <pos_tweak>) start_x_scale = 0.4 start_y_scale = 0.7 only_if_larger_x = 1
		if ($game_mode = p2_battle)
			<pos_tweak> = (0.0, -40.0)
		else
			<pos_tweak> = (0.0, 0.0)
