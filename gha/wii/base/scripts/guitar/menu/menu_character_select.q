...
script create_net_game_info_panel \{parent = user_control_container
	...
	fit_text_in_rectangle id = <id> dims = ((205.0, 0.0) + (<height> * (0.0, 1.0))) only_if_larger_x = 1
...
script create_character_select_menu \{Player = 1}
	...
	if (ScreenElementExists id = <char_select_parent_container>)
	...
	if (<i> < $g_cs_num_icons)
		...
		formatText checksumName = icon_id 'cs_icon_p%p_id_%d_back' p = <Player> d = <i>
		displaySprite {
			id = <icon_id>
			parent = <char_select_parent_container>
			tex = white
	...
	if ($current_num_players = 2)
		if (<Player> = 1)
			<hilite_container_pos> = (155.0, 150.0)
		else
			<hilite_container_pos> = (960.0, 150.0)
		endif
	endif
	...
	if (<Player> = 1)
		...
		player_name_pos = (<cs_menu_pos> + (-130.0, -60.0))
		player_title_pos = (<cs_menu_pos> + (-185.0, -45.0))
	else
		player_number = '2'
		if ($is_network_game)
			player_name_pos = (<cs_menu_pos> + (655.0, 0.0))
		endif
		player_name_pos = (<cs_menu_pos> + (605.0, -60.0))
		player_title_pos = (<cs_menu_pos> + (660.0, -45.0))
	endif
	<icon_id> :GetTags
	GetUpperCaseString <char_name>
	if ($current_num_players = 1)
		frame_dims = (280.0, 600.0)
		center_pos = (512.0, 340.0)
		create_ui_frame {
			parent = <char_select_parent_container>
			frame_dims = (<frame_dims> + (0.0, 50.0))
		...
		fit_text_in_rectangle id = <id> only_if_larger_x = 1 dims = (150.0, 24.0)
		CreateScreenElement {
			Type = TextElement
			id = <cs_name_text>
			parent = <char_select_parent_container>
		...
		if ($current_num_players = 1)
			fit_text_in_rectangle id = <cs_name_text> dims = ((290.0, 0.0) + (<height> * (0.0, 1.2))) Pos = (508.0, 200.0)
		else
			fit_text_in_rectangle id = <cs_name_text> dims = ((<width> * (1.0, 0.0)) + (<height> * (0.0, 1.0))) Pos = (508.0, 200.0)
		endif
		CreateScreenElement \{Type = ContainerElement
			parent = root_window
			id = select_guitar_container
			Pos = (510.0, -40.0)}
		get_valid_character_index char_index = ($g_cs_jumped_p1 + $g_character_select_icon_index_p1) Player = <Player>
		get_musician_profile_struct index = <index>
		...
		if ($is_network_game)
			...
			if (<Player> = 1)
				...
				circle_offset = (18.0, 30.0)
				circle_rgba = [93 36 0 255]
			else
				cs_player_color = <purpley>
				circle_offset = (-20.0, 30.0)
			...
			elseif Spanish
				formatText TextName = text 'J%n' n = <Player>
			elseif German
				formatText TextName = text 'S%n' n = <Player>
			elseif Italian
				formatText TextName = text 'G%n' n = <Player>
			else
...
script cs_scroll_up_or_down \{Player = 1
	...
	if (<cs_go_new_page>)
		...
		GetUpperCaseString <char_name>
		SetScreenElementProps id = <cs_name_text> text = <UppercaseString> Scale = 1.2
		GetScreenElementDims id = <cs_name_text>
		if ($current_num_players = 1)
			if (<width> > 200)
				fit_text_in_rectangle id = <cs_name_text> dims = ((270.0, 0.0) + (<height> * (0.0, 1.0))) Pos = (508.0, 200.0)
			else
				fit_text_in_rectangle id = <cs_name_text> dims = ((<width> * (1.0, 0.0)) + (<height> * (0.0, 1.0))) Pos = (508.0, 200.0)
			endif
		else
			fit_text_in_rectangle id = <cs_name_text> dims = ((340.0, 0.0) + (<height> * (0.0, 1.2))) only_if_larger_x = 1
		endif
...
script get_valid_character_index \{char_index = 0
	...
	if StructureContains structure = (<profile_struct>) debug_only
		...
		valid = 0
	elseif StructureContains structure = (<profile_struct>) character_select_ignore

		valid = 0
	elseif NOT (<profile_struct>.Type = GUITARIST)
...
script cs_load_characters \{starting_index = $g_cs_jumped_p1
	...
	if (<starting_index> > 0)
		char_index = 2
		<remaining> = $g_cs_num_icons
		remaining = (<remaining> * 2 - 15)
		begin
		if (<Player> = 1)
			formatText checksumName = icon_id 'cs_icon_p1_id_%d' d = <char_index>
			formatText checksumName = 0xb4989b1f 'cs_icon_p1_id_%d_back' d = <char_index>
		elseif (<Player> = 2)
			formatText checksumName = icon_id 'cs_icon_p2_id_%d' d = <char_index>
			formatText checksumName = 0xb4989b1f 'cs_icon_p2_id_%d_back' d = <char_index>
		endif
		char_index = (<char_index> + 1)
		if ScreenElementExists id = <icon_id>
			DoScreenElementMorph id = <icon_id> alpha = 0
			if ScreenElementExists id = <0xb4989b1f>
				DoScreenElementMorph id = <0xb4989b1f> alpha = 0
			endif
		endif
		repeat <remaining>
	endif
...
script cs_reset_selectables \{Player = 1}
	...
	if (<Player> = 1)
		...
		if ScreenElementExists id = <icon_id>
			DoScreenElementMorph id = <icon_id> alpha = 1
			SetScreenElementProps id = <icon_id> texture = Char_Select_Default
			<icon_id> :SetTags selectable = 0
			formatText checksumName = icon_id 'cs_icon_p1_id_%d_back' d = <i>
			if ScreenElementExists id = <icon_id>
				DoScreenElementMorph id = <icon_id> alpha = 1
			endif
		endif
		...
		if ScreenElementExists id = <icon_id>
			...
			SetScreenElementProps id = <icon_id> texture = Char_Select_Default
			<icon_id> :SetTags selectable = 0
			formatText checksumName = icon_id 'cs_icon_p2_id_%d_back' d = <i>
			if ScreenElementExists id = <icon_id>
				DoScreenElementMorph id = <icon_id> alpha = 1
			endif
		endif
...
script get_random_available_character 
	...
	if get_valid_character_index char_index = <random_guitarist_index> Player = 1
		get_musician_profile_struct index = <index>
		formatText checksumName = character_id '%s' s = (<profile_struct>.Name)
	...
	if get_valid_character_index char_index = <random_guitarist_index> Player = 1
		...
		ScriptAssert \{'failed to find a valid character.  This should not happen'}
	endif
endscript
