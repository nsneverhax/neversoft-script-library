	...
	if ($0xc8462798 = 1)
		if NOT ($current_practice_songpreview = $target_practice_songpreview &&
			...
			if ($shutdown_game_for_signin_change_flag = 0)
				SongUnLoadFSBIfDownloaded
				WaitOneGameFrame
				preload_song song_name = <song> starttime = <starttime> fadeintime = 1.0 0x480dcccb = 1
				WaitOneGameFrame
				waitforpreload_song
				WaitOneGameFrame
				WaitOneGameFrame
...
script menu_choose_practice_section_refresh_entries 
	...
	KillSpawnedScript \{Name = 0x1808c1d4}
	SpawnScriptNow \{0x1808c1d4}
endscript
...
script menu_choose_practice_section_set_times 
	...
	if (<array_Size> > 0)
		...
		Change 0x70f20eb0 = (<start_index> - 1)
