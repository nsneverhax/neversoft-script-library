...
script start_gem_scroller \{starttime = 0
	...
	if isXenon
		...
		if isps3
			...
			if (isngc)
				Change \{default_practice_mode_pitchshift_offset_song = $0x184ba9ab}
				Change \{default_practice_mode_pitchshift_offset_slow = $0x1ea136d7}
				Change \{default_practice_mode_pitchshift_offset_slower = $ngc_practice_mode_pitchshift_offset_slower}
				Change \{default_practice_mode_pitchshift_offset_slowest = $0xbd6db77b}
			else
	...
	LoadPak \{'pak/oogame/oogame.pak'
		Heap = 0xf8899a28}
	if NOT existspakmanmap \{map = 0xcf7e837c}
	...
	if ((isps2) || (isngc))
		...
		if NOT should_play_boss_intro
			...
			if ($current_transition = aerosmith_entrance)
				play_encore = 2
			endif
			if ($current_transition = dmcintro)
				play_encore = 3
			endif
			if NOT ($game_mode = p1_career || $game_mode = p2_career || $game_mode = p2_coop)
				play_encore = 0
			endif
			if (<play_encore> = 1)
				switch <pakname>
					case 'z_nipmuc'
					0xadf69650 = 'OTHER\\ENCORE\\NIPMUC'
					case 'z_maxskc'
					0xadf69650 = 'OTHER\\ENCORE\\MAXSKC'
					case 'z_fenway'
					0xadf69650 = 'OTHER\\ENCORE\\FENWAY'
					case 'z_nine_lives'
					0xadf69650 = 'OTHER\\ENCORE\\NINELIVE'
					case 'z_jpplay'
					0xadf69650 = 'OTHER\\ENCORE\\JPPLAY'
					case 'z_hof'
					0xadf69650 = 'OTHER\\ENCORE\\HOF'
					default
					...
					0xadf69650 = 'OTHER\\ENCORE\\NIPMUC'
				endswitch
			elseif (<play_encore> = 2)
				switch <pakname>
					case 'z_nipmuc'
					0xadf69650 = 'OTHER\\INTRO\\NIPMUCA'
					case 'z_maxskc'
					0xadf69650 = 'OTHER\\INTRO\\MAXSKCA'
					case 'z_fenway'
					0xadf69650 = 'OTHER\\INTRO\\FENWAYA'
					case 'z_nine_lives'
					0xadf69650 = 'OTHER\\INTRO\\NINELIVA'
					case 'z_jpplay'
					0xadf69650 = 'OTHER\\INTRO\\JPPLAYA'
					case 'z_hof'
					0xadf69650 = 'OTHER\\INTRO\\HOFA'
					default
				...
				switch <pakname>
					...
					0xadf69650 = 'OTHER\\INTRO\\NIPMUC'
					case 'z_maxskc'
					0xadf69650 = 'OTHER\\INTRO\\MAXSKC'
					case 'z_fenway'
					0xadf69650 = 'OTHER\\INTRO\\FENWAY'
					case 'z_nine_lives'
					if (<play_encore> = 3)
						0xadf69650 = 'OTHER\\INTRO\\NINELIVD'
					else
						0xadf69650 = 'OTHER\\INTRO\\NINELIVE'
					endif
					case 'z_jpplay'
					0xadf69650 = 'OTHER\\INTRO\\JPPLAY'
					case 'z_hof'
					0xadf69650 = 'OTHER\\INTRO\\HOF'
					default
			...
			0x8c046839
			0x810401fe FileName = <0xadf69650> 0x3dd98e8d = 33075 stereo = true
			Change 0x67e65592 = <unique_id>
			waitforpreload_stream \{Stream = 0x67e65592}
		endif
	endif
...
script gem_array_events 
	...
	SetEventHandler response = 0x84fd71d0 event = <id> Scr = GemStep params = {<...>} group = iterator_group
...
script load_full_guitarist_anims 
	...
	if find_profile_by_id id = ($player1_status.0x06e4ecba)
	...
	if ($current_num_players = 2)
		if find_profile_by_id id = ($player2_status.0x06e4ecba)
			if ($Musician_Profiles [<index>].Type = GUITARIST)
				character_name = ($Musician_Profiles [<index>].Name)
				formatText TextName = pak_id 'pak/models/full_anims/%s_full/%s_Full_anims.pak' s = <character_name>
				LoadPak (<pak_id>) Heap = heap_musician5_anim
				Change 0x0c3759c8 = <pak_id>
			endif
		endif
	else
	endif
...
script unload_full_guitarist_anims 
	if ($0x25ffed3a != None)
		UnLoadPak \{$0x25ffed3a
			Heap = heap_musician1_anim}
		Change \{0x25ffed3a = None}
	endif
	if ($0x0c3759c8 != None)
		UnLoadPak \{$0x0c3759c8
			Heap = heap_musician5_anim}
		Change \{0x0c3759c8 = None}
	endif
...
script load_frontend_guitarist_anims 
	...
	get_pak_filename_using_filename_crc \{pak_crc = $bassist_anim_pak_file
		Type = anim}
	if (<found> = 1)
		LoadPak (<pak_name>) heap_musician5_anim
	endif
endscript
