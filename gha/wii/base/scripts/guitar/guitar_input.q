...
script wait_for_inactive 
	...
	if ((($<player_status>.bot_play) = 0) && (($<player_status>.0x0e6b667f > 0)))
...
script star_power_activate_and_drain 
	...
	Change structurename = <player_status> 0x6aeed5fc = ($<player_status>.0x6aeed5fc + 1)
	if isngc
	...
	if GotParam \{net_whammy_length}
		...
		if GuitarGetAnalogueInfo controller = ($<player_status>.controller)
			...
			if ($<player_status>.bot_play = 1)
				<len> = 0.0
				if ((($<player_status>.0x0e6b667f) > 0) && (($<player_status>.0xd4325102) > 0.0))
					Change structurename = <player_status> 0xd00c25fd = (($<player_status>.0xd00c25fd) + ($<player_status>.0xd4325102))
					sin ($<player_status>.0xd00c25fd)
					<len> = ((<sin> + 1.0) / 2.0)
				endif
...
script reset_whammy_pitchshift 
	...
	0x6e9d2ba7 \{$guitar_player1_unique_id}
endscript
