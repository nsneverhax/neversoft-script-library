...
script Progression_SongWon 
	...
	if ($player1_status.total_notes > 0 && $end_credits = 0)
	...
	if ($game_mode = p1_career)
		...
		if GotParam \{current_tier}
			...
			if (<new_stars> > <stars> && $end_credits = 0)
			...
			if (<new_score> > <score> && $end_credits = 0)
...
script Progression_SetProgressionNodeFlags 
	...
	if StructureContains structure = <song_struct> Name = 0x999aca34
		if (<song_struct>.0x999aca34 = aerosmith)
			ls_3_5 = 1
		endif
	endif
	if ($force_encore_autolaunch = 1)
		ls_encore = 1
	endif
