...
script CameraCuts_StartCallback 
	...
	if StructureContains structure = <camStruct> Bloom
		...
		bloomparam = ($0x82c38096.screen_FX [0])
	endif
	if NOT screenFX_FXInstanceExists \{viewport = bg_viewport
			Name = venue_bloom}
		ScreenFX_AddFXInstance {
	...
	if StructureContains structure = <camStruct> guitarist_bloom
		if (<camStruct>.guitarist_bloom = 'on')
	...
	if StructureContains structure = <camStruct> 0x1507cb02
		if (<camStruct>.0x1507cb02 = 'on')
			rhythm :obj_setbloom \{enable = true}
		else
			rhythm :obj_setbloom \{enable = FALSE}
		endif
	else
		rhythm :obj_setbloom \{enable = FALSE}
	endif
	if StructureContains structure = <camStruct> bassist_bloom
		if (<camStruct>.bassist_bloom = 'on')
	...
	if StructureContains structure = <camStruct> vocalist_bloom
		if (<camStruct>.vocalist_bloom = 'on')
...
Default_Cameras_Good = [
	{
		...
		params = {
			...
			Cam1 = {
				LockTo = World
				Pos = (1.3486809, 3.0811198, 12.710393)
				Quat = (0.010933, -0.98202896, 0.185963)
				FOV = 75.0
			}
			Cam2 = {
				LockTo = World
				Pos = (1.7726451, 3.0854542, 12.683418)
				Quat = (0.014122999, -0.98033494, 0.19147001)
				FOV = 75.0
			}
		}
	}
