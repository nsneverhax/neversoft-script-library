...
script push_bandtags \{mode = p1_career}
	...
	0xd67cc7e5 = 0
	begin
	0x180a59f0 index = <0xd67cc7e5>
	if (($<tier_global>.num_tiers) != 0)
		0xde5f1042 = 0
		begin
		formattext checksumname = tier 'tier%d' d = (<0xde5f1042> + 1)
		getarraysize ($<tier_global>.<tier>.songs)
		if (<array_size> != 0)
			formattext checksumname = tiername '%ptier%i' p = ($<tier_global>.prefix) i = (<0xde5f1042> + 1)
			formattext checksumname = tier_checksum 'tier%s' s = (<0xde5f1042> + 1)
			pushglobaltags <tiername> section = <bandname>
			getarraysize ($<tier_global>.<tier_checksum>.songs)
			0xc92c6969 = 0
			begin
			formattext checksumname = song_checksum '%p_song%i_tier%s' p = ($<tier_global>.prefix) i = (<0xc92c6969> + 1) s = (<0xde5f1042> + 1) addtostringlookup = true
