...
script load_song_anims \{async = 0}
	...
	if (<song_name> = jamsession)
		...
		if song_uses_mocap_anim_pak song = <song_name>
			...
			if LoadPakAsync pak_name = <mocap_anim_pak> Heap = heap_musician_anim no_vram async = <async>
...
script unload_song_anims 
	if ($current_song_qpak = jamsession)
		...
		if NOT ($current_song_qpak_mocap = None)
			0x8dbdaee4 qs(0x3d306452) i = ($current_song_qpak_mocap)
...
script start_gem_scroller \{starttime = 0
	...
	if ($ps2_gameplay_restart_song = 0)
		...
		if ($game_mode = p2_faceoff || $game_mode = p2_pro_faceoff || $boss_battle = 1)
			...
			LoadPak \{'pak/oogame/oogamevs.pak'
				Heap = heap_cas}
		elseif ($game_mode = p2_battle)
			...
			LoadPak \{'pak/oogame/oogamebattle.pak'
				Heap = heap_cas}
		elseif ($game_mode = p2_quickplay || $game_mode = p3_quickplay || $game_mode = p4_quickplay ||
				...
				$game_mode = p4_pro_faceoff || $game_mode = p8_pro_faceoff)
			if ispakloaded \{'pak/oogame/oogame.pak'
					Heap = heap_cas}
				UnLoadPak \{'pak/oogame/oogame.pak'
					Heap = heap_cas}
			...
			LoadPak \{'pak/oogame/oogameband.pak'
				Heap = heap_cas}
		else
			if ispakloaded \{'pak/oogame/oogamebattle.pak'
					Heap = heap_cas}
				UnLoadPak \{'pak/oogame/oogamebattle.pak'
					Heap = heap_cas}
			elseif ispakloaded \{'pak/oogame/oogameband.pak'
