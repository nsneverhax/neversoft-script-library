	...
	if (<total_songs> > 0)
		...
		if (<total_songs> > 0)
			...
			if (<score> = 0 || $game_mode = training)
				...
				GetGlobalTags <songname> param = double_bass_complete
				0x8dbdaee4 qs(0xd8eb2118) d = <songname>
				0x8dbdaee4 qs(0x005ff117) d = <double_bass_complete>
				if (<double_bass_complete> = 1)
					<highest_difficulty_texture> = icon_difficulty_expert_plus
				endif
			endif
		...
		if ($is_network_game = 1)
			...
			if ($practice_enabled)
				...
				switch ($game_mode)
					...
					setlist_menu :se_setprops \{block_events}
					generic_menu_pad_choose_sound
					generic_event_choose state = 0x89d75e9b data = {is_popup from_setlist = 1 ignore_camera can_change_level = <can_change_level>}
					case p2_quickplay
					case p3_quickplay
					case p4_quickplay
					setlist_menu :GetSingleTag \{from_top_rocker}
