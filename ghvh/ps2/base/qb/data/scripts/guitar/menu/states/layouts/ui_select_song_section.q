...
script ui_create_select_song_section 
	...
	songsectiondesc :0x8e6af868 \{child = title
		Pos = (30.0, -226.10841)}
	...
	if songsectiondesc :desc_resolvealias \{Name = alias_menu}
		...
		if (<marker_array_size> > 0)
			...
			if (<part> = vocals)
				...
				elseif LocalizedStringEquals a = <short_ui_string> b = $vocal_marker_hype
...
script ui_select_song_section_set_times \{state = uistate_practice_select_speed}
	...
	if (<state> = uistate_practice_select_speed)
		create_loading_screen
	endif
...
script ui_select_song_section_preview \{extras = 0}
	...
	if (<array_Size> > 0)
		...
		if NOT GotParam \{end_index}
			...
			end_time = (($<song_section_array> [<end_index>]).time)
		endif
		if (<extras> = 1)
			end_time = <total_end_time>
		endif
...
script practice_songpreview_monitor 
	...
	if NOT ($current_practice_songpreview = $target_practice_songpreview &&
		...
		end_song
		playing_song = 0
		if ($shutdown_game_for_signin_change_flag = 0 && $0x4026c71d = 0)
			songunload
			...
			if ($current_practice_songpreview = $target_practice_songpreview &&
					...
					$force_practice_songpreview_restart = 0)
				if GotParam \{extras}
					setsoundbussparams {leadvox_balance = {vol = (($default_BussSet.leadvox_balance.vol) - 10)} time = 0.5}
					setsoundbussparams {drums_balance = {vol = (($default_BussSet.drums_balance.vol) - 10)} time = 0.5}
					setsoundbussparams {leadgtr_balance = {vol = (($default_BussSet.leadgtr_balance.vol) - 10)} time = 0.5}
					setsoundbussparams {bass_balance = {vol = (($default_BussSet.bass_balance.vol) - 10)} time = 0.5}
					setsoundbussparams {rhythmgtr_balance = {vol = (($default_BussSet.rhythmgtr_balance.vol) - 10)} time = 0.5}
					setsoundbussparams {crowd_balance = {vol = (($default_BussSet.crowd_balance.vol) - 3)} time = 0.5}
					setsoundbussparams {crowdsingalong_balance = {vol = (($default_BussSet.crowdsingalong_balance.vol) - 100)} time = 0.5}
					setsoundbussparams {band_Balance = {vol = (($default_BussSet.band_Balance.vol) - 10)} time = 0.5}
				else
					setsoundbussparams {leadvox_balance = {vol = (($default_BussSet.leadvox_balance.vol) - 9)} time = 0.5}
					setsoundbussparams {drums_balance = {vol = (($default_BussSet.drums_balance.vol) - 9)} time = 0.5}
					setsoundbussparams {leadgtr_balance = {vol = (($default_BussSet.leadgtr_balance.vol) - 9)} time = 0.5}
					setsoundbussparams {bass_balance = {vol = (($default_BussSet.bass_balance.vol) - 9)} time = 0.5}
					setsoundbussparams {rhythmgtr_balance = {vol = (($default_BussSet.rhythmgtr_balance.vol) - 9)} time = 0.5}
					setsoundbussparams {crowd_balance = {vol = (($default_BussSet.crowd_balance.vol) - 100)} time = 0.5}
					setsoundbussparams {crowdsingalong_balance = {vol = (($default_BussSet.crowdsingalong_balance.vol) - 100)} time = 0.5}
					setsoundbussparams {band_Balance = {vol = (($default_BussSet.band_Balance.vol) - 9)} time = 0.5}
				endif
				begin_song
				playing_song = 1
				if IsObjectScript
					obj_spawnscript practice_songpreview_fadeinandrepeat params = {adjustedtime = <adjustedtime>}
				else
					SpawnScriptNow practice_songpreview_fadeinandrepeat params = {adjustedtime = <adjustedtime>}
				endif
			else
				skip_wait = 1
			endif
...
script practice_songpreview_fadeinandrepeat \{adjustedtime = -1}
	current_wait_time = 0
	if (<adjustedtime> = -1)
		adjustedtime = $target_practice_songpreview_starttime
	endif
	if (<adjustedtime> < ($target_practice_songpreview_starttime))
		current_wait_time = ((($target_practice_songpreview_starttime) - <adjustedtime>) / 1000.0)
		songsetmastervolume \{vol = -100}
		Wait <current_wait_time> Seconds
