...
script ui_create_pausemenu \{for_practice = 0}
	...
	if ($pause_menu_no_bg = 0 && $guitar_motion_enable_test = 0)
		...
		ui_pausemenu_create_bg title_text = <title_text> extra_z = <extra_z> pad_back_script = <pad_back_script> controller = <player_device>
	else
		make_menu {
			pad_back_script = ui_pausemenu_exit
						...
						if ($end_credits = 0)
							...
							if NOT (<0x3e8a7a4d> = -1)
								add_menu_frontend_item {
									...
									internal_scale = <text_scale>
									choose_state = uistate_pausemenu_change_difficulty
									choose_state_data = {player_device = <player_device> Player = <Player> 0x3e8a7a4d = <0x3e8a7a4d>}
									text_internal_just = [center center]
									additional_focus_script = focus_set_spinners
									additional_focus_params = {menu = pausemenu_bg}
								}
	...
	if ($enable_button_cheats = 1)
		add_menu_frontend_item \{text = qs(0x49a0d144)
...
script pause_menu_set_song_info 
	if ScreenElementExists \{id = current_menu_anchor}
		if ($in_tutorial_mode = 1)
			current_menu_anchor :se_setprops \{song_text = qs(0x00000000)
				by_text = qs(0x00000000)
		...
		current_menu_anchor :se_setprops song_text = <song_title> by_text = <song_artist_text> artist_text = <song_artist>
	endif
endscript
