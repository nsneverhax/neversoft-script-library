...
script ui_create_setlist_lyrics_spawned 
	...
	final_array_text = [qs(0xce69019c)]
	final_array_index = 0
	...
	if (<song_array> = jammode_songs)
		...
		if (<total_songs> > 0)
			...
			if (<total_songs> > 0)
				...
				GetUpperCaseString <song_artist>
				<song_artist> = <UppercaseString>
				formatText TextName = song_text qs(0xadab3351) a = <song_artist> t = <song_title>
				<double_kick_alpha> = 0
				if (<song> != jamsession)
				...
				<id> :se_setprops event_handlers = [{pad_choose ui_setlist_lyrics_choose_song params = {song = <song> selected_index = <index>}}]
				...
				if ($sort_restore_selections = 1)
					...
					if (<quickplay_index> != -1)
						<id> :ui_setlist_lyrics_choose_song selected_index = <index> for_custom_setlist = <for_custom_setlist> song = <song> custom_index = <quickplay_index> no_sound
					endif
				endif
				i = (<i> + 1)
				repeat <total_songs>
	...
	if ResolveScreenElementID id = {setlist_menu child = <selected_index>}
		...
		<resolved_id> :obj_spawnscript ui_setlist_lyrics_focus_song params = {time = 0.0 for_custom_setlist = <for_custom_setlist>}
	endif
...
script ui_setlist_lyrics_choose_song_spawned 
	...
	ui_event_wait state = uistate_song_extras_lyrics data = {selected_index = <selected_index> song = <song> double_kick_alpha = <double_kick_alpha> double_kick_pos = <screenelementpos> from_backstage = 1}
endscript
...
script ui_setlist_lyrics_focus_song \{time = 0.1}
	...
	formatText TextName = album_text qs(0xc8cb5531) s = <album_text>
	se_setprops {
