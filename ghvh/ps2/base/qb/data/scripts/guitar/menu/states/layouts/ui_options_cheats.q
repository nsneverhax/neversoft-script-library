...
script ui_create_options_cheats 
	...
	if GotParam <cheat_unlocked>
		rgba = (($g_menu_colors).cheat_locked_unfocus)
		<text> = qs(0xb8a0b6b7)
		unlocked = 0
		toggle = 1
		...
		<item_container_id> :SetTags spinner_offset = 0
		<item_container_id> :se_getparentid
		<item_container_id> :se_setprops {
...
script ui_options_cheats_back 
	...
	0xe6b61ea6 \{a = $cheat_alwaysslide
		b = $cheat_autokick
		c = $cheat_freedrum}
...
script ui_options_cheat_toggle 
	...
	Obj_GetID
	if <objID> :desc_resolvealias Name = textitem_text param = textitem
		GetScreenElementDims id = <textitem>
		<textitem> :se_setprops dims = ((<height> * (0.0, 1.0) + (1.0, 0.0)))
	endif
	exclude_option option_struct = <option_struct>
	refresh_autokick_cheat
	ui_options_cheats_focus
endscript
...
script ui_options_cheats_focus 
	...
	cheatsinterface :se_setprops scrollthumb_pos = (((((<index> + 1.0) / <array_Size>) * 320.0) * (0.0, 1.0)) + (15.0, 0.0)) time = 0.05
	if GotParam \{unlocked}

		if (<unlocked> = 1)

			se_setprops generic_menu_smenu_textitem_text_rgba = ($menu_focus_color)
		else

			se_setprops generic_menu_smenu_textitem_text_rgba = (($g_menu_colors).cheat_locked_focus)
		endif
	endif
	<objID> :menu_focus_set_highlight menu = cheatsinterface
endscript
...
script ui_options_cheats_unfocus 
	...
	if GotParam \{unlocked}
		if (<unlocked> = 1)
			se_setprops generic_menu_smenu_textitem_text_rgba = ($menu_unfocus_color)
		else
			se_setprops generic_menu_smenu_textitem_text_rgba = (($g_menu_colors).cheat_locked_unfocus)
		endif
	endif
endscript
