...
script practicemode_section 
	...
	begin
	0xe6b61ea6 (($<song_section_array> [<current_section_index>]))
	if (($<song_section_array> [<current_section_index>].time) >= $current_starttime)
		break
	endif
	<current_section_index> = (<current_section_index> + 1)
	repeat <array_Size>
	if (<current_section_index> >= <array_Size>)
		<current_section_index> = (<array_Size> - 1)
	endif
	begin
	...
	if (<time> > $current_endtime)
		practice_sectiontext :legacydomorph \{alpha = 0.0
			time = 0.5}
		break
	elseif (<current_section_index> < ($current_section_array_entry))
		practice_sectiontext :legacydomorph \{alpha = 0.0
			time = 0.5}
		Wait \{0.5
			Second}
