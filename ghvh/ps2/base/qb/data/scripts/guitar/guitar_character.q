...
script initial_drummer_anim 
	if ($current_song = jamsession)
		anim = drum_andy_standardlp_hs_120_01
...
script set_playing_win_lose_anims \{isplaying = FALSE}
	if (<isplaying> = true && <isplaying> != ($playing_win_lose_anims))
...
script play_intro_anims 
	set_playing_win_lose_anims \{isplaying = FALSE}
	if NOT GotParam \{ENCORE}
		if (($current_transition) = fastintro || ($current_transition) = None)
			0x8dbdaee4 qs(0xf2997e9c) s = ($current_transition) donotresolve
			return
		endif
	endif
	band_restartidles
	0x8dbdaee4 \{qs(0xf37b3bf6)}
	0x8dbdaee4 \{qs(0xcebd4098)}
	...
	if CompositeObjectExists Name = <checksum>
		if NOT bandmanager_isplayingsimpleanim Name = <checksum>
			...
			if (<intro_anim> != None)
				...
				0x8dbdaee4 'Playing intro anim %a on %b' a = ($<info_struct>.intro_anim_name) b = <checksum> donotresolve
			endif
		endif
	endif
endscript
