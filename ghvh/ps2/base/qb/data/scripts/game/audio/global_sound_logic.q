	...
	if ($Star_power_verb_is_on = 1)
		...
		if (<part> = guitar)
			...
			if isSinglePlayerGame
				...
				0xec7236b3 Player = <Player> On = true
			else
...
script GH_Star_Power_Verb_Off 
	...
	0xec7236b3 Player = <Player> On = FALSE
...
script GH3_Crowd_Event_Listener 
	...
	if NOT ($crowd_in_jam_mode_song_state = 1)
		...
		if (<pak> != z_studio && <pak> != z_studio2 && <pak> != z_tool && <pak> != z_credits && <pak> != z_icecave)
...
script do_actual_changing_of_looping_sound \{loading_transition = 0
	...
	if (($crowd_in_jam_mode_song_state = 1) || ($crowd_in_front_end_state = 1) || (<pak> = z_studio) || (<pak> = z_studio2) || (<pak> = z_tool) || (<pak> = z_credits) || (<pak> = z_icecave))
...
script band_screen_ui_sound 
	if NOT GotParam \{player_num}
		if GotParam \{up}
			PlaySound \{menu_scroll_up
				vol = -10
				pitch = 1}
		else GotParam \{down}
			PlaySound \{menu_scroll_down
				vol = -6.5
				pitch = 3}
		endif
		return
...
script star_power_deployed_sfx_multiplayer \{Player = 1}
	...
	if NOT issoundplaying \{star_deployed_back}
		...
		if ($star_power_deployed_flag = 1)
			...
			0x8dbdaee4 \{qs(0x10dae63c)}

			if (<highway_position> = left)
				0x8dbdaee4 \{qs(0xd077d5d8)}

				SoundEvent \{event = star_power_deployed_sfx_p1}
				SoundEvent \{event = star_power_deployed_cheer_sfx_p1}
			else
				if (<highway_position> = right)
					0x8dbdaee4 \{qs(0x04fe3bd4)}

					SoundEvent \{event = star_power_deployed_sfx_p2}
					SoundEvent \{event = star_power_deployed_cheer_sfx_p2}
				else
					0x8dbdaee4 \{qs(0x0ea3a05a)}
...
script ghtunes_ui_scroll_pan 
	...
	PlaySound ghtunes_menu_scroll pitch = 4 voll_percent = (<0x354cc7af> -15) volr_percent = (<0xcf43facc> -15)
endscript
...
script menu_guitar_lick_sfx 
	switch ($0xf7bea20b)
		...
		case menu_intro_lick_09
		PlaySound \{menu_intro_lick_09
			vol = -5}
		SpawnScriptNow \{menu_music_on
			params = {
				waitforguitarlick = 1
		...
		case menu_intro_lick_13
		PlaySound \{menu_intro_lick_13
			vol = -5}
		SpawnScriptNow \{menu_music_on
			params = {
				waitforguitarlick = 1
