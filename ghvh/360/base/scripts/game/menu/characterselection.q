...
script character_selection_animate_bio 
	...
	if (<height> > 470)
		time = ((<height> -470.0) * 0.05)
		offset = ((470.0 - <height>) * (0.0, 1.0))
		Wait \{5
			Seconds}
		begin
		if ScreenElementExists id = <id>
			<id> :se_setprops character_selection_bio_pos = <offset> time = <time>
		else
			return
		endif
		Wait (<time> + 5) Seconds
		if ScreenElementExists id = <id>
			<id> :se_setprops character_selection_bio_pos = (0.0, 0.0) time = <time>
		else
			return
		endif
		Wait (<time> + 5) Seconds
