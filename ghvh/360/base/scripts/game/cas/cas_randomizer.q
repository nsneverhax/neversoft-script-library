...
script cas_random_should_include_part 
	if GotParam \{category}
		if (<category> = esp)
			ScriptAssert \{'Random Guitar trying to use ESP category'}
		elseif (<category> = gh)
			if StructureContains structure = ($<part_name> [<i>]) is_esp
				printf channel = bort qs(0x10d44f37) n = ($<part_name> [<i>].desc_id) DontAssertForChecksums channel = random_test
				return \{FALSE}
			endif
			if StructureContains structure = ($<part_name> [<i>]) is_metallica
				printf channel = bort qs(0xe1960479) n = ($<part_name> [<i>].desc_id) DontAssertForChecksums channel = random_test
				return \{FALSE}
			endif
			if StructureContains structure = ($<part_name> [<i>]) is_van_halen
				printf qs(0x99401deb) n = ($<part_name> [<i>].desc_id) DontAssertForChecksums channel = random_test
				return \{FALSE}
			endif
		endif
	endif
...
script cas_merge_in_random_entry 
	...
	cas_filter_random_presets_by_category array = $<chosen_array> category = van_halen entry = <entry>
