...
script create_song_chooser_spinner 
	...
	setlist_popup_z = ($setlist_text_z + 500.0)
	CreateScreenElement {
	...
	if ResolveScreenElementID \{id = [
			...
			param = spinners}
		<spinners> :se_setprops alpha = 0.0
	endif
	if NOT GotParam \{selected_index}
		selected_index = 0
	endif
	net_setlist_spinner :Obj_SpawnScriptLater \{spin_names
		params = {
			selected_index = 0
		}}
endscript
...
script destroy_net_setlist_spinner 
	...
	if ResolveScreenElementID \{id = [
			...
			param = spinners}
		<spinners> :se_setprops alpha = 1.0
	endif
	destroy_spinner
		...
		if ScreenElementExists \{id = setlistinterface}
			...
			if setlistinterface :desc_resolvealias \{Name = alias_stars_earned
				...
				if ResolveScreenElementID id = [{id = <stars_earned>} {local_id = root} {local_id = title}]
					<stars_earned> :se_setprops alpha = 1.0 title_text = <bottom_text> title_font = fontgrid_text_3
					menu_frontend_resize_banner banner_id = <stars_earned> max_title_width = 700 end_cap_extra_width = 50
				endif
			endif
		endif
...
script song_selected_countdown 
	...
	if ScreenElementExists \{id = setlistinterface}
		if setlistinterface :desc_resolvealias \{Name = alias_stars_earned
				param = stars_earned}
			if ResolveScreenElementID id = [{id = <stars_earned>} {local_id = root} {local_id = title}]
				<stars_earned> :se_setprops alpha = 0.0 title_text = qs(0x00000000) title_font = fontgrid_text_3
				i = 3
				begin
				if (<i> < 0)
					break
				...
				<stars_earned> :se_setprops alpha = 1.0 title_text = <msg> title_font = fontgrid_text_3
				if (<i> = 3)
