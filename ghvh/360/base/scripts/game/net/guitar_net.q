...
script do_ps3_signin_state_change 
	...
	elseif CheckForSignIn
		...
		if GotParam \{fail_params}
			ui_event_wait event = menu_replace data = {state = <state> <fail_params> player_device = (<fail_params>.controller)}
		else
			ui_event_wait event = menu_replace data = {state = <state> <params> require_live}
		endif
	endif
