...
script create_settings_menu 
	...
	CreateScreenElement \{Type = TextElement
		parent = settings_vmenu
		id = toggle_showsongframe_menuitem
		font = debug
		Scale = 0.75
		rgba = [
			210
...
script toggle_showsongtime 
	...
	toggle_showsongtime_setprop
endscript
debug_showsongframe = 0

script toggle_showsongframe 
	ui_menu_select_sfx
	if ScreenElementExists \{id = debug_songframe_text}
		DestroyScreenElement \{id = debug_songframe_text}
	endif
	if ($debug_showsongframe = 0)
		Change \{debug_showsongframe = 1}
		CreateScreenElement \{Type = TextElement
			parent = root_window
			id = debug_songframe_text
			font = debug
			text = qs(0x7360a92e)
			Scale = 1.0
			Pos = (50.0, 40.0)
	...
	elseif ($debug_showsongframe = 1)
		Change \{debug_showsongframe = 2}
		CreateScreenElement \{Type = TextElement
			parent = root_window
			id = debug_songframe_text
			font = debug
...
script toggle_showsongframe_setprop 
	if NOT ScreenElementExists \{id = toggle_showsongframe_menuitem}
		return
	endif
	if ($debug_showsongframe = 0)
		toggle_showsongframe_menuitem :se_setprops \{text = qs(0x914d6db8)}
	elseif ($debug_showsongframe = 1)
		toggle_showsongframe_menuitem :se_setprops \{text = qs(0x380638c4)}
	else
		toggle_showsongframe_menuitem :se_setprops \{text = qs(0xde2ff380)}
	endif
endscript
...
script refreshfpsdisplay \{interval = 0.1}
	...
	if ObjectExists \{id = fps_anchor}
		...
		if ($fps_display_hold_red > 0)
			Change fps_display_hold_red = ($fps_display_hold_red - 1)
		elseif (($framerate_value) > $framerate_warning)
			Change \{framerate_warning_color = [
					100
					100
					100
					255
			...
			Change \{fps_display_hold_red = 10}
			if (($framerate_value) < 25)
				Change \{framerate_warning_color = [
						0
						0
						128
		...
		SetScreenElementProps id = fps_text rgba = ($framerate_warning_color)
	else
...
script select_test_song 
	Change player1_device = <device_num>
	Change \{game_mode = p1_quickplay}
	Change \{current_num_players = 1}
	Change \{disable_band = 1}
	create_song_menu version = ($current_song_version) from_gameplay = <from_gameplay>
endscript
