...
script Progression_SongWon 
	...
	if (<Type> = career)
		...
		if GotParam \{current_tier}
			...
			printf qs(0xc1ae53f4) d = <setup_encore>
			if (<setup_encore> = 1)
...
script Progression_SetProgressionNodeFlags 
	...
	ChangeNodeFlag \{ls_perf_openingact
		1}
	ChangeNodeFlag \{ls_perf_vanhalen
		0}
	if StructureContains structure = ($gh_songlist_props.<song>) Band
		if is_current_band_vanhalen Band = ($gh_songlist_props.<song>.Band)
			ChangeNodeFlag \{ls_perf_openingact
				0}
			ChangeNodeFlag \{ls_perf_vanhalen
				1}
...
script progression_set_intro_complete 
	...
	<Player> = 1
	begin
	printf channel = ach qs(0xc0036fd2) p = <Player>
	achievements_intruder Player = <Player>
	<Player> = (<Player> + 1)
	repeat <num_players_shown>
endscript

script progression_check_finalbandintro_complete 
	get_current_band_info
	GetGlobalTags <band_info>
	getplayerinfo \{1
		part}
	...
	switch <part>
		case guitar
		check = <finalbandintro_transition_played_guitar>
		case bass
		check = <finalbandintro_transition_played_bass>
		case drum
		check = <finalbandintro_transition_played_drum>
		case vocals
		check = <finalbandintro_transition_played_vocals>
		case Band
		check = <finalbandintro_transition_played_band>
	endswitch
	printf channel = bort qs(0x11c55426) p = <part>
	if (<check> = 0)
		return \{FALSE}
	endif
	return \{true}
...
script progression_set_finalbandintro_complete 
	get_current_band_info
	GetGlobalTags <band_info>
	getplayerinfo \{1
		part}
	...
	switch <part>
		case guitar
		SetGlobalTags <band_info> params = {finalbandintro_transition_played_guitar = 1}
		case bass
		SetGlobalTags <band_info> params = {finalbandintro_transition_played_bass = 1}
		case drum
		SetGlobalTags <band_info> params = {finalbandintro_transition_played_drum = 1}
		case vocals
		SetGlobalTags <band_info> params = {finalbandintro_transition_played_vocals = 1}
		case Band
		SetGlobalTags <band_info> params = {finalbandintro_transition_played_band = 1}
	endswitch
endscript

script progression_shown_credits_before 
	get_current_band_info
	GetGlobalTags <band_info>
	anydone = (<career_credits_guitar_complete> + <career_credits_drum_complete> + <career_credits_vocals_complete> + <career_credits_bass_complete> + <career_credits_band_complete>)
	if (<anydone> > 0)
		return \{true}
	endif
	printf channel = bort qs(0x86d9a369) d = <anydone>
	return \{FALSE}
endscript
...
script progression_should_show_credits 
	...
	if progression_check_all_songs_complete part = <part>
		...
		begin
		getplayerinfo <Player> double_kick_drum
		if (<hardest_complete> > 15)
			if played_all_expertplus_songs Player = <Player>
				achievements_1984 Player = <Player>
			endif
			achievements_diver_down Player = <Player>
			achievements_fair_warning Player = <Player>
			achievements_women_and_children_first Player = <Player>
			achievements_van_halen_ii Player = <Player>
			achievements_van_halen Player = <Player>
		elseif (<hardest_complete> > 7)
			achievements_fair_warning Player = <Player>
			achievements_women_and_children_first Player = <Player>
			achievements_van_halen_ii Player = <Player>
			achievements_van_halen Player = <Player>
		elseif (<hardest_complete> > 3)
			achievements_women_and_children_first Player = <Player>
			achievements_van_halen_ii Player = <Player>
			achievements_van_halen Player = <Player>
		elseif (<hardest_complete> > 1)
			achievements_van_halen_ii Player = <Player>
			achievements_van_halen Player = <Player>
		elseif
			achievements_van_halen Player = <Player>
		...
		<Player> = (<Player> + 1)
		repeat <num_players_shown>
		printf channel = bort qs(0xc0f22805) a = <career_credits_guitar_complete> b = <career_credits_drum_complete> c = <career_credits_vocals_complete> d = <career_credits_bass_complete> e = <career_credits_band_complete>
		if (<check> = 0)
			return \{true}
		endif
	endif
...
script progression_check_all_songs_complete 
	...
	if (<go_through_songs> = 1)
		...
		GetGlobalTags <song_checksum> params = stars
		if (<stars> > 2)
			if NOT (<lock_this_song> = 1)
				completed_songs = (<completed_songs> + 1)
				lock_this_song = 1
			endif
			<temp_complete> = (<temp_complete> + <hardest_array> [<diff_index>])
		endif
		diff_index = (<diff_index> + 1)
		repeat 5
		diff_index = 0
		begin
		if (<temp_complete> > <hardest_array> [<diff_index>])
			if NOT (<temp_complete> && <hardest_array> [<diff_index>])
				<temp_complete> = (<temp_complete> + <hardest_array> [<diff_index>])
			endif
		endif
		diff_index = (<diff_index> + 1)
		repeat 5
		if (<hardest_complete> > <temp_complete>)
			printf channel = bort qs(0xc7a29ed7) i = <temp_complete> s = <song>
			<hardest_complete> = <temp_complete>
		endif
	...
	if (<total_songs> = <completed_songs>)
		printf channel = bort qs(0xbae4efc3) h = <hardest_complete>
		return true hardest_complete = <hardest_complete>
	else
		printf \{channel = bort
			qs(0x929ad222)}
		return \{FALSE}
