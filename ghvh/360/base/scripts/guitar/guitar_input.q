...
script stop_bot_controller \{Player = 1}
	formatText checksumName = player_status 'player%p_status' p = <Player>
	if (StructureContains structure = $<player_status> bot_original_controller)
		if ($<player_status>.bot_original_controller != -1)
			Change structurename = <player_status> controller = ($<player_status>.bot_original_controller)
			Change structurename = <player_status> bot_original_controller = -1
		endif
	endif
endscript
