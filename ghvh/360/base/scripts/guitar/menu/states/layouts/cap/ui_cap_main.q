...
script ui_create_cap_main_spawned savegame = ($cas_current_savegame)
	...
	if GotParam \{from_band_name_logo}
		generic_menu :Obj_SpawnScriptNow \{cap_main_animate_to_logo}
		ui_event_remove_params \{param = from_band_name_logo}
	endif
	if checksumequals a = <part> b = cas_band_logo
...
script cap_main_animate_to_logo 
	if ScreenElementExists \{id = bandinterface}
		bandinterface :se_setprops \{Pos = {
			...
			time = 0.2}
		bandinterface :se_setprops \{name_letter_container_alpha = 0
			time = 0.2}
		bandinterface :se_setprops \{band_name_line_alpha = 0
			time = 0.2}
		bandinterface :se_setprops \{band_name_info_alpha = 0
			time = 0.2}
	endif
endscript
		...
		if GotParam \{back}
			if ScreenElementExists \{id = bandinterface}
				bandinterface :se_setprops \{Pos = (640.0, 360.0)
					time = 0.2}
				bandinterface :se_setprops \{name_letter_container_alpha = 1
					time = 0.2}
				bandinterface :se_setprops \{band_name_line_alpha = 1
					time = 0.2}
				bandinterface :se_setprops \{band_name_info_alpha = 1
					time = 0.2}
				Wait \{0.2
					Seconds
					ignoreslomo}
...
script cas_load_graphic_data 
	...
	if GotParam \{div_id}
		printf channel = UI qs(0xd59a22e6) d = <div_id> donotresolve
	endif
	...
	if NOT GotParam \{target}
		...
		printf channel = UI qs(0x8ca54ffc) t = <target>
	endif
	...
	printf channel = UI qs(0xff32c09c) s = <section_index>
	if GotParam \{section_index}
		...
		if GotParam \{cap}
			...
			if is_target_in_div target = (<cap> [<i>]) part = <part> div_id = <div_id>
				printf channel = UI qs(0xfba10fd5) i = <i>
