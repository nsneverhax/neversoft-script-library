...
script ui_band_mode_create_menu 
	...
	if GotParam \{descs}
		current_desc = (<descs> [<index>])
		<current_desc> :obj_spawnscript ui_band_mode_highlight_motion
		<current_desc> :obj_spawnscript ui_band_mode_ready_motion
	endif
...
script ui_band_menu_occlude_character 
	...
	if GotParam \{descs}
		...
		current_desc = (<descs> [<index>])
		if GotParam \{revert}
			<current_desc> :se_setprops menu_player_bg_alpha = 1.0 scrollbar_alpha = 0.0 scrolling_menu_dims = (200.0, 200.0)
...
script ui_band_mode_update_menu 
	...
	if GotParam \{descs}
		...
		if ((<menu> = join) || (<menu> = character) || (<menu> = p2_character) || (<menu> = ready) || (<menu> = ready2) || (<menu> = p2_ready))
			if <current_desc> :desc_resolvealias Name = alias_highlight
				<resolved_id> :se_setprops Pos = (0.0, 300.0)
			endif
		endif
	endif
	switch <menu>
	...
	if GotParam \{descs}
		...
		if (<menu> = join)
			...
			<current_desc> :se_setprops reposition_pos = (0.0, 0.0) join_text = qs(0x03ac90f0) time = 0.1
		endif
		if ($is_network_game = 1)
	...
	if GotParam \{given_focus}
		...
		if GotParam \{child_index}
			printf channel = kim 'Focus child %s %d' s = <objID> d = <child_index>
		endif
...
script ui_band_mode_create_menu_lefty 
	Obj_GetID
	GetTags
	if ((<instrument> = drum) || (<instrument> = vocals))
		if ($band_mode_mode = career)
			if NOT progression_check_intro_complete
				Intro = 0
				if ($cheat_altband = 1)
					switch <instrument>
						case guitar
						character_id = eddie_van_halen_alt
						case bass
						character_id = wolf_van_halen_alt
						case drum
						character_id = alex_van_halen_alt
						case vocals
						character_id = david_lee_roth_alt
					endswitch
				else
					switch <instrument>
						case guitar
						character_id = eddie_van_halen
						case bass
						character_id = wolf_van_halen
						case drum
						character_id = alex_van_halen
						case vocals
						character_id = david_lee_roth
					endswitch
				endif
				ui_band_mode_show_character Player = (<index> + 1) character_id = <character_id>
				get_player_num_from_controller controller_index = <controller>
				formatText checksumName = player_status 'player%p_status' p = <player_num>
				...
				band_character_rename index = <index> character_name = <fullname>
			endif
		endif
	endif
	fit_width = `scale	each	line	if	larger`
...
script ui_band_mode_animate_ready 
	RequireParams \{[
		...
		all}
	<current_desc> :se_setprops join_text = qs(0x03ac90f0) reposition_pos = (0.0, 0.0) time = 0.1
	...
	if GotParam \{descs}
		...
		if NOT GotParam \{character_name}
			...
			if GotParam \{Player}
				...
				if ($band_mode_mode = career)
					if NOT progression_check_intro_complete
						if NOT ((<character_id> = eddie_van_halen) || (<character_id> = wolf_van_halen) || (<character_id> = alex_van_halen) || (<character_id> = david_lee_roth))
							Intro = 0
						endif
					endif
				endif
...
script ui_band_mode_create_menu_join 
	...
	join_text = (<buttonchar> + qs(0x93117d97))
...
script ui_band_mode_choose 
	...
	if GotParam \{instrument}
		...
		if NOT GotParam \{Intro}
			if ($is_network_game = 0)
				hacky_fix_band_celebrity_character Player = <player_num> instrument = <instrument>
			endif
		endif
	...
	switch <menu>
		...
		if ($is_network_game = 1)
			...
			if ($band_mode_mode = career)
				if progression_check_intro_complete
					...
					i = 1
					begin
					ui_band_mode_kill_character Player = <i>
					random_band_mode_character Player = <i>
					setplayerinfo <i> character_id = <character_id>
					band_character_rename character_name = qs(0x03ac90f0) index = (<i> - 1)
					i = (<i> + 1)
					repeat 4
		...
		if ($band_mode_mode = career)
			if NOT progression_check_intro_complete
				...
				setplayerinfo <player_num> character_id = <character_id>
				band_character_rename character_name = qs(0x03ac90f0) index = <index>
			endif
		endif
		if ($is_network_game = 1)
			SetTags \{menu = net_local_root}
				...
				if (<index> != -1)
					...
					if NOT (<k> = <index>)
						if ScreenElementExists id = {<curr_id_2> child = <k>}
							SetScreenElementProps id = {<curr_id_2> child = <k>} rgba = [64 64 64 200] font = text_a1 material = NULL not_focusable
						endif
					endif
					k = (<k> + 1)
					repeat 4
		...
		if (<menu> = instrument)
			...
			if (<index> != -1)
				if ScreenElementExists id = {<curr_id_2> child = <index>}
					SetScreenElementProps id = {<curr_id_2> child = <index>} rgba = [64 64 64 200] font = text_a1 material = NULL not_focusable
				endif
			...
			if (<num> = 2 && $band_mode_last_num_ready < 2)
				if ($band_mode_current_leader = -1)
					...
					user_control_helper_get_buttonchar \{button = start
						controller = $band_mode_current_leader}
					formatText font = fontgrid_text_3 TextName = button_text qs(0x72f9ae62) b = <buttonchar>
...
script ui_band_mode_choose_sound_spawned \{ready = 0
	...
	if NOT GotParam \{back}
		if (<ready> = 1)
			...
			switch <instrument>
				case drum
				PlaySound select_sound_small vol = -6 buss = Front_End pan1x = <pan1x> pan1y = <pan1y> pan2x = <pan2x> pan2y = <pan2y>
				case guitar
				PlaySound select_sound_small vol = -6 buss = Front_End pan1x = <pan1x> pan1y = <pan1y> pan2x = <pan2x> pan2y = <pan2y>
				case vocals
				PlaySound select_sound_small vol = -6 buss = Front_End pan1x = <pan1x> pan1y = <pan1y> pan2x = <pan2x> pan2y = <pan2y>
...
script ui_band_mode_choose_sound_ready 
	...
	if NOT GotParam \{back}
		switch <instrument>
			case drum
			PlaySound drum_select_affirmation_01 vol = 3 buss = Front_End pan1x = <pan1x> pan1y = <pan1y> pan2x = <pan2x> pan2y = <pan2y>
			case guitar
			PlaySound guitar_select_affirmation_1 vol = 0 buss = Front_End pan1x = <pan1x> pan1y = <pan1y> pan2x = <pan2x> pan2y = <pan2y>
			case vocals
			PlaySound mic_select_affirmation_01 vol = -2 buss = Front_End pan1x = <pan1x> pan1y = <pan1y> pan2x = <pan2x> pan2y = <pan2y>
			case bass
			PlaySound bass_select_affirmation_02 vol = 3 buss = Front_End pan1x = <pan1x> pan1y = <pan1y> pan2x = <pan2x> pan2y = <pan2y>
			default
...
script update_network_ticker 
	...
	switch <msg_checksum>
		...
		user_control_helper_get_buttonchar \{button = start
			controller = $band_mode_current_leader}
		formatText font = fontgrid_text_3 TextName = msg qs(0x72f9ae62) b = <buttonchar>
		case join
		msg_type = horiz
		append = 1
		if (isXenon)
...
script hacky_fix_band_celebrity_character 
	...
	if (<is_local_client> = 1)
		...
		get_controller_part controller = <controller>
		if NOT fix_disallowed_character_choice character_id = <character_id> savegame = <savegame> part = <instrument>
