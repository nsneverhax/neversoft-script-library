...
script wait_for_songpreview_monitor 
	finalprintf \{qs(0x4957bab2)}
	ResetTimer
	<loadscreen_counter> = 0
	begin
	if ($setlist_songpreview_changing = 0)
		break
	else
		if NOT ScriptIsRunning \{setlist_songpreview_monitor}
			finalprintf \{qs(0xca4de0d6)}
			SoftAssert \{qs(0x2b2b3e17)}
			Change \{setlist_songpreview_changing = 0}
			break
		endif
	endif
	if ($is_changing_levels = 1)
		if (<loadscreen_counter> < 1000)
			ResetTimer
		endif
		<loadscreen_counter> = (<loadscreen_counter> + 1)
	endif
	if TimeGreaterThan \{5.0}
		finalprintf \{qs(0xe160b315)}
		SoftAssert \{qs(0x946581ee)}
		Change \{setlist_songpreview_changing = 0}
		break
	endif
	Wait \{1
...
script setlist_songpreview_monitor_cleanup 
	finalprintf \{qs(0xcd825ecd)}
	finalprintf \{qs(0x9e9fdb96)}
	Change \{setlist_songpreview_changing = 0}
endscript
...
script setlist_songpreview_monitor 
	...
	SetSpawnInstanceLimits \{Max = 1
		management = ignore_spawn_request
		checkwholecallstack}
	finalprintf \{qs(0x25b489d5)}
	wait_for_songpreview_monitor
	OnExitRun \{setlist_songpreview_monitor_cleanup}
	songstoppreview
	begin
	if NOT ($current_setlist_songpreview = $target_setlist_songpreview)
		finalprintf \{qs(0x63e81f46)}
		...
		if ($target_setlist_songpreview != <song> || $target_setlist_songpreview = None)
			...
			if songcheckifdownloaded FileName = <song_prefix>
				if NOT Downloads_OpenContentFolder content_index = <content_index> close_if_script_killed
					DownloadContentLost
					return \{FALSE}
				endif
			endif
...
script IsSongAvailable \{for_bonus = 0}
	...
	if StructureContains structure = ($gh_songlist_props.<song>) guitar_only
		if (($game_mode = p2_coop) || ($game_mode = p4_pro_faceoff) || ($game_mode = p6_pro_faceoff_no_mics) || ($game_mode = p8_pro_faceoff))
			printf channel = bort qs(0xf4595faf) g = ($game_mode) s = <song>
			return \{FALSE}
		endif
		<override> = 0
		...
		if ($is_network_game)
			...
			if StructureContains structure = <allowed> guitar
				<num_guitars> = (<num_guitars> + 1)
				if ((<Type> = training) || ($game_mode = p1_quickplay))
					getplayerinfo <player_idx> part
					if (<part> = bass)
						<num_guitars> = (<num_guitars> -1)
					endif
		...
		repeat <num_players>
		if (<Type> = quickplay || <Type> = training)
			if (<override> = 0)
				if (<num_guitars> > 1 || <num_drums> > 1)
					return \{FALSE}
				endif
				if (($gh_songlist_props.<song>.guitar_only != drums) && ((<num_guitars> < 1) || (<num_drums> > 0)))
					return \{FALSE}
				endif
				if ((<num_guitars> + <num_drums>) != <num_players>)
					return \{FALSE}
				endif
				printf channel = bort qs(0xb718e22a) n = <num_guitars> p = <num_players> s = <song> g = ($game_mode)
			endif
		endif
		if (($game_mode = p2_faceoff) || ($game_mode = p2_pro_faceoff) || ($game_mode = p2_battle))
			if NOT (<num_guitars> = <num_players> || <num_drums> = <num_players>)
				return \{FALSE}
			endif
			if (($gh_songlist_props.<song>.guitar_only != drums) && (<num_guitars> < <num_players>))
				return \{FALSE}
			endif
			printf channel = bort qs(0x06ac4fcb) n = <num_guitars> p = <num_players> s = <song> g = ($game_mode)
		endif
		printf channel = bort qs(0xdc9ac966) n = <num_guitars> d = <num_drums> p = <num_players> s = <song> t = <Type> g = ($game_mode)
	endif
	...
	if ($is_network_game = 1)
		...
		if (<saved_in_globaltags> = 1)
			...
			if ((<Type> = quickplay) && (<cooperative> = true))
				look_at_player = 1
				begin
				getplayerinfo <look_at_player> controller
				getsavegamefromcontroller controller = <controller>
				GetGlobalTags <song> savegame = <savegame> param = unlocked
				if (<unlocked> = 1)
					return \{true}
				endif
				look_at_player = (<look_at_player> + 1)
				repeat <num_players>
			else
