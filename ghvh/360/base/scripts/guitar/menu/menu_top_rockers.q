...
script create_top_rockers_menu \{for_options = 0}
	...
	if NOT (<new_score> = -1)
		...
		add_user_control_helper text = qs(0xb73cb78f) button = green <all_button_params> z = 10000
		add_user_control_helper text = qs(0x2a08d9a6) button = yellow <all_button_params> z = 10000
		add_user_control_helper text = qs(0x1c2cd639) button = blue <all_button_params> z = 10000
	else
		...
		if (<nowrite_full> = 0)
			...
			if ($current_song != jamsession)
				if ($permanent_songlist_props.($current_song).double_kick = 1 && <part> = drum && <double_kick_drum> = 1)
					printf \{channel = bort
						qs(0xa2253970)}
					SetGlobalTags <songname> params = {ach_double_bass_complete = 1}
				endif
			endif
		endif
		newbestscore = 1
