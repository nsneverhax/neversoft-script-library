...
debug_movies = 0
drawikid = 0
useprefixedchannels = 0
disabledprintfchannels = [
	kim
	havok
	jimp
	trchen
...
script disablechannel 
	requireparams \{[
			channel
		]
		all}
	if arraycontains array = $disabledprintfchannels contains = <channel>
		return
	endif
	array = $disabledprintfchannels
	addarrayelement array = <array> element = <channel>
	change disabledprintfchannels = <array>
endscript

script enablechannel 
	requireparams \{[
			channel
		]
		all}
	if arraycontains array = $disabledprintfchannels contains = <channel>
		array = $disabledprintfchannels
		removearrayelement array = <array> element = <channel>
		change disabledprintfchannels = <array>
	endif
endscript

script disableallchannels 
	getarraysize \{$knownchannellist}
	printf <array_size>
	if (<array_size> > 0)
		index = 0
		begin
		<temp> = ($knownchannellist [<index>])
		disablechannel channel = <temp>
		index = (<index> + 1)
		repeat <array_size>
	endif
endscript

script enableallchannels 
	getarraysize \{$knownchannellist}
	if (<array_size> > 0)
		index = 0
		begin
		<temp> = ($knownchannellist [<index>])
		enablechannel channel = <temp>
		index = (<index> + 1)
		repeat <array_size>
	endif
endscript
