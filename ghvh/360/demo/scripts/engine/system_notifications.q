...
script sysnotify_wait_until_safe 
	...
	if gotparam \{semaphore}
		if NOT gotparam \{override}
			if ($<semaphore> = 1)
				<should_wait> = 1
				printf 'WAITING FOR %a' a = <semaphore>
			endif
		else
			if ($<override> != 1)
				if ($<semaphore> = 1)
...
script sys_fade_overlay_on \{alpha = 0.5}
...
script sysnotify_handle_signin_change 
	...
	change \{signin_change_happening = 0}
endscript
sysnotify_block_invite = 0
sysnotify_signin_override = 0

script block_invites 
	change \{sysnotify_block_invite = 1}
endscript

script unblock_invites 
	change \{sysnotify_block_invite = 0}
endscript

script set_signin_override 
	change \{sysnotify_signin_override = 1}
endscript

script clear_signin_override 
	change \{sysnotify_signin_override = 0}
endscript
...
script wait_for_sysnotify_unpause \{unpause_flag = wait_for_sysnotify_unpause_flag}
	change globalname = <unpause_flag> newvalue = 0
	printf \{qs(0x7e1c2c48)
		channel = sysnotify}
	begin
	printf qs(0xf5e8c2d0) i = ($paused_for_hardware) channel = sysnotify
	if (($<unpause_flag> = 1) && ($paused_for_hardware = 0))
		break
	endif
	wait \{1
		gameframe}
