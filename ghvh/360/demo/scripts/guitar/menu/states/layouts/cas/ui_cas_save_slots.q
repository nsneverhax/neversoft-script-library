...
script slots_add_item \{focus_script = slot_focus_script
	...
	generic_menu_set_total_length id = <id> parent = current_menu
endscript
...
script exit_save_instrument_worker 
	...
	if (<instrument_info>.desc_id = guitar)
		printf \{channel = bort
			qs(0x9b2f8b68)}
		achievements_fender_bender controller = ($primary_controller)
	elseif (<instrument_info>.desc_id = drum)
		printf \{channel = bort
			qs(0xff5a0677)}
		achievements_bang_your_gong controller = ($primary_controller)
	else
		printf \{channel = bort
			qs(0x3be9a859)}
	endif
...
script save_cap_design_worker 
	...
	if ((gotparam div_id) && (<part> = cas_body))
		if (<div_id> = body || <div_id> = `right arm` || <div_id> = `left arm`)
			achievements_ouch controller = ($primary_controller)
		endif
	elseif (<part> = cas_band_logo && ($achievements_modified_logo) = 1)
		achievements_eye_of_the_beholder controller = ($primary_controller)
	endif
endscript
