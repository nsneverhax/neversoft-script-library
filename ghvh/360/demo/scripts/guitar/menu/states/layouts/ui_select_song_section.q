...
script ui_select_song_section_anim_in 
endscript

script ui_select_song_section_anim_out 
...
script practice_songpreview_monitor \{extras = 0}
	...
	if NOT ($current_practice_songpreview = $target_practice_songpreview &&
		...
		if (<playing_song> = 1)
			fadetime = 0.25
			setsoundbussparams {leadvox_balance = {vol = (($default_bussset.leadvox_balance.vol) - 100)} time = <fadetime>}
			setsoundbussparams {drums_balance = {vol = (($default_bussset.drums_balance.vol) - 100)} time = <fadetime>}
			setsoundbussparams {leadgtr_balance = {vol = (($default_bussset.leadgtr_balance.vol) - 100)} time = <fadetime>}
			setsoundbussparams {bass_balance = {vol = (($default_bussset.bass_balance.vol) - 100)} time = <fadetime>}
			setsoundbussparams {rhythmgtr_balance = {vol = (($default_bussset.rhythmgtr_balance.vol) - 100)} time = <fadetime>}
			setsoundbussparams {band_balance = {vol = (($default_bussset.band_balance.vol) - 100)} time = <fadetime>}
			setsoundbussparams {crowdsingalong_balance = {vol = (($default_bussset.crowdsingalong_balance.vol) - 100)} time = <fadetime>}
			if (<extras> != 1)
				setsoundbussparams {crowd_balance = {vol = (($default_bussset.crowd_balance.vol) - 100)} time = <fadetime>}
			endif
			wait <fadetime> second
		endif
		...
		if ($shutdown_game_for_signin_change_flag = 0)
			...
			if ($current_practice_songpreview = $target_practice_songpreview &&
					$current_practice_songpreview_starttime = $target_practice_songpreview_starttime &&
					$force_practice_songpreview_restart = 0)
				practice_preview_volume_adjust = 4
				setsoundbussparams {leadvox_balance = {vol = (($default_bussset.leadvox_balance.vol) - <practice_preview_volume_adjust>)} time = 1}
				setsoundbussparams {drums_balance = {vol = (($default_bussset.drums_balance.vol) - <practice_preview_volume_adjust>)} time = 1}
				setsoundbussparams {leadgtr_balance = {vol = (($default_bussset.leadgtr_balance.vol) - <practice_preview_volume_adjust>)} time = 1}
				setsoundbussparams {bass_balance = {vol = (($default_bussset.bass_balance.vol) - <practice_preview_volume_adjust>)} time = 1}
				setsoundbussparams {rhythmgtr_balance = {vol = (($default_bussset.rhythmgtr_balance.vol) - <practice_preview_volume_adjust>)} time = 1}
				setsoundbussparams {band_balance = {vol = (($default_bussset.band_balance.vol) - <practice_preview_volume_adjust>)} time = 1}
