...
script friendslist_item_change_focus 
	obj_getid
	if resolvescreenelementid id = [{id = <objid>} {local_id = text}]
		objid = <resolved_id>
	endif
	if gotparam \{focus}
		<objid> :se_setprops rgba = ($menu_focus_color)
	...
	elseif gotparam \{unfocus}
		<objid> :se_setprops rgba = ($menu_unfocus_color)
		if current_menu_anchor :desc_resolvealias \{name = alias_spinner}
...
script invite_to_game 
	requireparams \{[
		...
		all}
	invite_title = qs(0x81ce73a2)
	base_msg = qs(0x05e4f546)
	switch (<instrument>)
		case guitar
