...
jumbotron_viewport_params = {
	...
	textureasset = `tex\jumbotronscreen_d.dds`
...
script create_jumbotron 
	...
	if NOT isassetloaded name = <texdict>
		printf \{channel = movie
			'Zone texdict not found for jumbotron'}
		popassetcontext
		return
	endif
	if NOT istextureindictionary dictionary = <texdict> texture = <textureasset>
		printf channel = movie 'Nowhere to put Jumbotron in %s' s = <pakname>
		popassetcontext
		return
	endif
	popassetcontext
	...
	if (($current_song = eruption) || ($current_song = cathedral) || ($current_song = spanishfly))
		formattext \{checksumname = cameraparams
			'solos_cameras_closeups'}
	endif
...
script grab_magazine_viewport \{select_index = -1}
	...
	printf channel = movie qs(0x547efd22) i = <index>
	if (<select_index> != -1)
		index = <select_index>
		printf channel = movie qs(0xf294df10) i = <index>
	endif
	addparams ($magazine_picture_props [<index>])
	getarraysize <anims>
	getrandomvalue name = rand_index integer a = 0 b = (<array_size> - 1)
	anim = (<anims> [<rand_index>])
	printf channel = movie qs(0xcb15e5e4) i = <anim>
	hide_band
...
script create_bandname_viewport 
	...
	printf channel = movie qs(0x35322bfb) s = <band_name_text_string>
	createscreenelement {
	...
	setsearchallassetcontexts
	if NOT isassetloaded name = <texdict>
		printf channel = movie 'texdict %s not found for bandname' s = <texdict>
		valid = 0
	endif
	if NOT istextureindictionary dictionary = <texdict> texture = <textureasset>
		printf channel = movie 'No texture to put Band Name in %s with %t' s = <pakname> t = <texdict>
		valid = 0
	endif
	if (<valid> = 1)
		printf \{channel = movie
...
script destroy_bandname_viewport 
	...
	printf channel = movie 'Trying to delete %i' i = <id>
	if isassetloaded name = <texdict>
		if istextureindictionary dictionary = <texdict> texture = <textureasset>
			printf channel = movie 'Succeeded deleting %i' i = <id>
			destroyviewporttextureoverride id = <id>
		else
			printf channel = movie '%i - not in texture dictionary' i = <id>
		endif
	else
		printf channel = movie '%i - Asset not loaded' i = <id>
	endif
	index = (<index> + 1)
	repeat <array_size>
	setsearchallassetcontexts \{off}
...
script playmovieandwait 
	...
	printf channel = movie qs(0x23871deb) s = <movie>
	...
	repeat
	if NOT gotparam \{noblack}
		printf channel = movie qs(0x27e8073d) s = <movie>
		fadetoblack \{off
			time = 0}
	endif
...
solos_cameras_closeups = [
	{
		name = guitarist_closeup_jumbo01
	...
	{
		name = guitarist_closeup_jumbo02
		type = closeup
		controlscript = cameracuts_handcam
		params = {
			lockto = guitarist
			pos = (1.6842451, 0.738124, 2.382368)
			quat = (-0.029290998, -0.918691, -0.117583)
			fov = 14.0
			lookat = guitarist
			lookatbone = bone_head
			screenoffset = (0.15910502, -0.32644203)
			distfromlookat = 2.9254112
