...
script create_no_save_found_menu 
	...
	switch <platform>
		case ps3
		if ($memcardjamordefault = jam)
			<textblock> = {text = qs(0x195958e8)}
		else
			<textblock> = {text = qs(0xbf5190c0)}
		endif
		create_popup_warning_menu {
			player_device = ($memcardcontroller)
			textblock = <textblock>
		...
		case xenon
		if ($memcardjamordefault = jam)
			<textblock> = {text = qs(0xc26e135f)}
		else
			<textblock> = {text = qs(0x23ef99a9)}
		endif
		create_popup_warning_menu {
			player_device = ($memcardcontroller)
			title = qs(0x423b9e4e)
...
script create_out_of_space_menu \{message_type = `default`}
	...
	switch <platform>
		...
		formattext textname = message qs(0xc63de245) d = <spacerequired>
		create_popup_warning_menu {
			textblock = {
				text = <message>
				pos = (640.0, 390.0)
