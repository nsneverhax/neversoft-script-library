...
net_helper_text_strings = {
	find_a_game_text = qs(0xd84d29e8)
	...
	switch (<difficulty>)
		...
		if (<controller_type> = drum)
			...
			if (<double_kick_drum> = 1)
				difficulty_icon = setlist_b_db_icon
			else
				difficulty_icon = icon_difficulty_expert
			endif
		else
	...
	if gotparam \{focus}
		...
		if current_menu_anchor :desc_resolvealias \{name = alias_highlight}
			...
			wait \{2
				gameframe}
			getscreenelementdims id = (<objid>)
			scale = (((1.0, 0.0) * (((<width> + 130.0) / 256.0))) + (0.0, 1.5))
			getscreenelementposition id = (<objid>) summed
			highlight :se_setprops pos = {absolute (<screenelementpos> + ((45.0, 0.0) + <post_game_nudge>))} scale = <scale> rot_angle = 0
			highlight :se_waitprops
			turn_on_highlight
			getscreenelementposition id = (<objid>) summed
			highlight :se_setprops pos = {absolute (<screenelementpos> + ((45.0, 10.0) + <post_game_nudge>))} time = 0.05
			highlight :se_waitprops
			getscreenelementposition id = (<objid>) summed
			highlight :se_setprops pos = {absolute (<screenelementpos> + ((45.0, 0.0) + <post_game_nudge>))} scale = <scale> time = 0.05
			highlight :se_waitprops
		endif
	elseif gotparam \{unfocus}
		adjust_highlight_z \{z_pri = 2}
...
script dlc_warning_message 
	if isps3
		message_text = qs(0x588b147a)
	else
		message_text = qs(0x23c4a3bb)
	endif
...
script set_net_ui_to_finished_searching 
	...
	if screenelementexists \{id = onlinelobbyinterface}
		...
		if (<index> = 5)
			...
			onlinelobbyinterface :obj_spawnscriptnow \{dlc_warning_message}
