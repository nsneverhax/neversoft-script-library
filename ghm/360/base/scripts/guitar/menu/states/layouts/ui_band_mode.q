...
script ui_band_mode_update_menu 
	...
	if GotParam \{descs}
		...
		if ((<menu> = join) || (<menu> = character) || (<menu> = p2_character) || (<menu> = ready) || (<menu> = ready2) || (<menu> = p2_ready))
			...
			<resolved_id> :se_setprops name_arrow_up_scale = 0.8
			<resolved_id> :se_setprops name_arrow_down_scale = 0.8
		endif
	endif
	switch <menu>
		case instrument
		...
		if ($band_mode_mode = career)
			if NOT progression_check_intro_complete
				...
				if ($cheat_zombband = 1)
					switch <instrument>
						case guitar
						character_id = kirk_zomb_2
						case bass
						character_id = rob_zomb_2
						case drum
						character_id = larsulrich_zmb2
						case vocals
						character_id = james_zomb_2
					endswitch
				elseif ($cheat_altband = 1)
					switch <instrument>
						case guitar
						character_id = kirk_alt_2
						case bass
						character_id = rob_alt_2
						case drum
						character_id = larsulrich_al2
						case vocals
						character_id = james_alt_2
					endswitch
				else
					switch <instrument>
						case guitar
						...
						character_id = rob_2
						case drum
						character_id = larsulrich_2
						case vocals
						character_id = james_2
					endswitch
				endif
				...
				if (<index> != -1)
					...
					if NOT (<k> = <index>)
						if ScreenElementExists id = {<curr_id_2> child = <k>}
							SetScreenElementProps id = {<curr_id_2> child = <k>} rgba = [64 64 64 200] font = fontgrid_text_a8 material = NULL not_focusable
						endif
					endif
					k = (<k> + 1)
					repeat 4
		...
		if (<menu> = instrument)
			...
			if (<index> != -1)
				if ScreenElementExists id = {<curr_id_2> child = <index>}
					SetScreenElementProps id = {<curr_id_2> child = <index>} rgba = [64 64 64 200] font = fontgrid_text_a8 material = NULL not_focusable
				endif
...
script ui_band_mode_choose_sound_ready 
	...
	if NOT GotParam \{back}
		switch <instrument>
			case drum
			PlaySound drum_select_affirmation_01 vol = 7 buss = Front_End pan1x = <pan1x> pan1y = <pan1y> pan2x = <pan2x> pan2y = <pan2y>
			case guitar
			PlaySound guitar_select_affirmation_1 vol = 3 buss = Front_End pan1x = <pan1x> pan1y = <pan1y> pan2x = <pan2x> pan2y = <pan2y>
			case vocals
			PlaySound mic_select_affirmation_01 vol = 15 buss = Front_End pan1x = <pan1x> pan1y = <pan1y> pan2x = <pan2x> pan2y = <pan2y>
			case bass
			PlaySound bass_select_affirmation_02 vol = 7 buss = Front_End pan1x = <pan1x> pan1y = <pan1y> pan2x = <pan2x> pan2y = <pan2y>
			default
