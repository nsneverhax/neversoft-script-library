	...
	if GotParam \{flame_id}
		if ScreenElementExists id = <flame_id>
			if GotParam \{notestreak}
				...
				if GotParam \{0xe4c0e190}
					CreateScreenElement {
						Type = SpriteElement
						parent = <flame_container>
						id = ($0x1f4c1314 [<highway_num>])
						dims = (55.0, 64.0)
						Scale = (1.0, 1.5)
						Pos = (146.0, 210.0)
						material = 0x84135bbc
			...
			if GotParam \{0x325e947d}
				if ScreenElementExists id = ($0x1f4c1314 [<highway_num>])
					DestroyScreenElement id = ($0x1f4c1314 [<highway_num>])
				endif
				if ScreenElementExists id = ($0x02cb6cf3 [<highway_num>])
					DestroyScreenElement id = ($0x02cb6cf3 [<highway_num>])
				endif
			endif
			SetScreenElementProps {
				id = <flame_id>
