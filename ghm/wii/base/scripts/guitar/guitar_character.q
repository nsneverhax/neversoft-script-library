...
script guitarist_start_anims 
	...
	if CompositeObjectExists Name = <Name>
		...
		<Name> :hero_play_finger_anim anim = ($<finger_anims>.green)
...
script initial_drummer_anim 
	...
	elseif is_current_band_metallica
...
script crowd_play_adjusting_random_anims \{anim = Idle
	...
	if (<array_Size> = 0)
		...
		GetRandomValue Name = newindex integer a = 0 b = (<array_Size> - 1)
		anim_name = (<anims> [<newindex>])
		if (debug_animdebug = 'crowd')
			if (<objID> = crowd1)
				printdebugtext \{X = 2
					y = 3
					str = 'Crowd1 Anim: '}
				printdebugtext X = 32 y = 3 str = <anim_name>
			endif
			if (<objID> = crowd2)
				printdebugtext \{X = 2
					y = 4
					str = 'Crowd2 Anim: '}
				printdebugtext X = 32 y = 4 str = <anim_name>
			endif
			if (<objID> = crowd3)
				printdebugtext \{X = 2
					y = 5
					str = 'Crowd3 Anim: '}
				printdebugtext X = 32 y = 5 str = <anim_name>
			endif
			if (<objID> = crowd4)
				printdebugtext \{X = 2
					y = 6
					str = 'Crowd4 Anim: '}
				printdebugtext X = 32 y = 6 str = <anim_name>
			endif
		endif
...
script get_start_node_id \{character = qs(0x5c2c7d0c)}
	...
	if ($game_mode = p2_faceoff || $game_mode = p2_pro_faceoff)
		switch (<member>)
			case GUITARIST
			character = qs(0x7a3d353c)
			case guitarist2
			character = qs(0x511066ff)
			case BASSIST
			character = qs(0x511066ff)
			case vocalist
			character = qs(0x7a3d353c)
			...
			character = qs(0x511066ff)
			case drummer
			if ($player1_status.part = drum)
				character = qs(0x7a3d353c)
			else
				character = qs(0x3a7aa4dc)
			endif
			case drummer2
			character = qs(0x511066ff)
			default
			printf \{qs(0x52f0543c)}
			character = qs(0x2e920c41)
		endswitch
	else
		switch (<member>)
			case GUITARIST
			character = qs(0x5c2c7d0c)
			case BASSIST
			character = qs(0x65a86eeb)
			case vocalist
			character = qs(0xfd6733b2)
			case vocalist2
			character = qs(0x3bfc2030)
			case drummer
			character = qs(0x3a7aa4dc)
			case drummer2
			character = qs(0x0207c521)
			default
			printf \{qs(0x52f0543c)}
			character = qs(0x2e920c41)
		endswitch
...
script load_frontend_anim_paks 
	...
	mpm_unload_pak pak = frontend_perm_anims async = <async>
	mpm_unload_pak pak = frontend_anims_guit1 async = <async>
	mpm_unload_pak pak = frontend_anims_guit2 async = <async>
	mpm_unload_pak pak = frontend_anims_sing async = <async>
	mpm_unload_pak pak = frontend_anims_drum async = <async>
endscript
...
script play_win_anims 
	...
	bandmanager_turnoffallarmanims
	if NOT ($game_mode = p2_faceoff || $game_mode = p2_pro_faceoff || $game_mode = p2_battle || $boss_battle = 1 || $0x25d71b9d = 1)
		ForEachIn \{$band_character_names
			do = play_win_anims_foreach}
		return
	endif
	if ($boss_battle = 1 || $0x25d71b9d = 1)
