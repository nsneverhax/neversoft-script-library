...
script gig_posters_choose_next 
	...
	if ((($is_network_game = 1) && (IsHost)) || ($is_network_game = 0))
		...
		if ($menu_gp_stage = 1)
			gigboardsetlistinterfaceb :se_setprops \{alpha = 0}
				...
				if (<cash> >= <paytoplay_price>)
					...
					create_popup_warning_menu {
						...
						options = [
							...
							{
								func = gigboard_cancel_paytoplay
								text = qs(0xf7723015)
							}
						]
					}
...
script create_gig_poster_venue_select progression_flag = ($current_progression_flag)
	...
	if NOT StructureContains structure = ($LevelZones.<level_checksum>) debug_only
		...
		if (<add_venue> = 1)
			...
			printf '($GigBoard_Setlist_Props.%s.highlight_rgba) = ' s = <level_poster>
			printstruct ($gigboard_setlist_props.<level_poster>.highlight_rgba)
			CreateScreenElement {
				Type = TextElement
				parent = <item_container>
			...
			printf '($GigBoard_Setlist_Props.%s.songname_rgba) = ' s = <level_poster>
			printstruct ($gigboard_setlist_props.<level_poster>.songname_rgba)
...
script setup_gigboard_poster 
	...
	if (<completed> = 1 || <completed> = 2)
		...
		printf qs(0x6556d4b2) d = <poster_gig_num>
...
script debug_complete_gig_cheat 
	...
	SetGlobalTags <gig_name> params = {encore_unlocked = 1 boss_unlocked = 1 started = 1 gig_progress = 10} all_active_players = 1
...
script refresh_gig_band_money_display 
	...
	if ($current_progression_flag = career_band && $is_network_game = 0)
		getsavegamefromcontroller controller = ($band_mode_current_leader)
		if IsGuitarController controller = ($band_mode_current_leader)
			<0x1af589a4> = 0
		elseif isdrumcontroller controller = ($band_mode_current_leader)
			<0x1af589a4> = 0
		endif
	else
		getsavegamefromcontroller controller = ($primary_controller)
	...
	<0x7c5bff83> = qs(0x1f745aaf)
	<no_button> = qs(0x066f6bee)
	if (<0x1af589a4> = 1)
		<0x7c5bff83> = qs(0x7b189fab)
		<no_button> = qs(0x6203aeea)
	endif
	if ScreenElementExists \{id = gig_band_money_id}
		SetScreenElementProps {
			...
			green_button_text = <0x7c5bff83>
			red_button_text = <no_button>
		}
	endif
endscript
