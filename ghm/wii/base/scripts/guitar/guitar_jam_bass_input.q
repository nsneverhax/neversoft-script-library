...
script jam_input_bass_hopo spawn_id = jam_input_spawns controller = ($player2_status.controller)
	...
	if NOT (<final_note_sample> = NULL)
		...
		<sustain> = 1
		jam_input_bass_play_note <...>
		SpawnScriptNow jam_input_stop_sound_bass id = <spawn_id> params = {select_player = <select_player> controller = <controller> 0x7351719c = <hold_pattern>}
	endif
endscript
...
script jam_input_bass_tilt 
	...
	begin
	if NOT 0x56423850 chromatic = <chromatic> Player = <Player> controller = <controller> bass
		return
	endif
	Wait \{1
		gameframe}
...
script jam_input_bass_play_note \{sustain = 0
	...
	if (<chord_type> = 0)
		...
		switch <note_type>
			...
			case 1
			PlaySound <final_note_sample> pitch = (<pitch_shift> + (($jam_current_tuning) / 10.0)) vol = 5 buss = jammode_bass attack_time = 0.055 attack_function = flat_middle release_function = linear release_time = 10.0 release_length = 0.02 slot = 6 0xc765acc7 = 1
			case 2
			PlaySound <final_note_sample> pitch = (<pitch_shift> + (($jam_current_tuning) / 10.0)) vol = 5 buss = jammode_bass attack_time = 0.07 attack_function = flat_middle release_function = linear release_time = 10.0 release_length = 0.02 slot = 6 0xc765acc7 = 2
			case 3
			PlaySound <final_note_sample> pitch = (<pitch_shift> + (($jam_current_tuning) / 10.0)) vol = 5 buss = jammode_bass release_function = linear release_time = 10.0 release_length = 0.02 slot = 6
		endswitch
	else
		...
		stopsound unique_id = ($jam_input_current_bass) fade_type = linear fade_time = 0.05
		PlaySound <final_note_sample> pitch = (<pitch_shift> + (($jam_current_tuning) / 10.0)) vol = 2 buss = jammode_bass send_vol2 = -24 pan1y = 1.0
	endif
...
script 0x8049a7c7 
	...
	Change jam_current_bass_kit = <bass_kit>
	Change bass_kit_mode = <bass_kit>
	if (<bass_kit> > 0)
		Change 0x8eb83d8c = ($pause_bass_kit_options [<bass_kit>].pakname)
		Change bass_kit_sample = ($pause_bass_kit_options [<bass_kit>].sample_start)
	else
		Change \{0x8eb83d8c = 'rev_bass1'}
		Change \{bass_kit_sample = deepsh101}
	endif
