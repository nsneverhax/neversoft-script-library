...
script ui_create_cap_layers_list 
	...
	if GotParam \{new_layer_selected}
		...
		SetScreenElementProps \{id = create_cap_layers_list_vmenu
			internal_just = [
				0.0
				-1.0
			]}
		...
		bx_refreshmenuitemsindexes \{vmenu_id = create_cap_layers_list_vmenu}
		default_index = 0
		if NOT ((GotParam focus_item) || (GotParam paste_item))
			if GotParam \{deleted_index}
				if ((<deleted_index> + 1) < <num_items>)
					<deleted_index> = (<deleted_index> + 1)
				endif
				default_index = <deleted_index>
			endif
		endif
		bx_genericmenu_setupscrollbar num_items = <num_items> default_index = <default_index>
...
script move_layer 
	...
	ui_event \{event = menu_refresh}
	SpawnScriptLater bx_genericmenu_updatescrollbar params = {bx_scrollbar_index = <layer_target_index>}
...
script layers_list_set_focus 
	printscriptinfo \{'layers_list_set_focus'}
	if ScreenElementExists \{id = layers_list_focus}
		DestroyScreenElement \{id = layers_list_focus}
	endif
	CreateScreenElement {
...
script cap_copy_layer_handler_change_return 
	...
	Wait \{1
		gameframe}
	if GetScreenElementChildren \{id = create_cap_layers_list_vmenu}
		GetArraySize <children>
		i = 0
		begin
		if isscreenelementinfocus id = (<children> [<i>])
			LaunchEvent Type = focus target = (<children> [<i>])
		endif
		i = (<i> + 1)
		repeat <array_Size>
	endif
...
script cap_decrement_target 
	...
	bx_genericmenu_updatescrollbar bx_scrollbar_index = (<target_index> -1)
	if ScreenElementExists \{id = generic_menu}
		if generic_menu :desc_resolvealias \{Name = alias_generic_menu_smenu
				param = generic_smenu}
