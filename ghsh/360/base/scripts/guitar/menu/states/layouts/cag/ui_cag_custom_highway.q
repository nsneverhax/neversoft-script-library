...
script ui_create_cag_custom_highway 
	...
	i = (<i> + 1)
	repeat <array_Size>
	if generic_menu :desc_resolvealias \{Name = alias_generic_menu_small_scrollbar}
...
script focus_change_highway 
	...
	if ScreenElementExists \{id = highway_menu_anchor}
		...
		if GotParam \{total_length}
			...
			scroll_len = 180
			scroll_pos = (((<scroll_perc> * 340) * (0.0, 1.0)) + (-25.0, -258.0))
			generic_menu :se_setprops generic_menu_small_scrollbar_thumb_pos = <scroll_pos>
		endif
	endif
endscript
