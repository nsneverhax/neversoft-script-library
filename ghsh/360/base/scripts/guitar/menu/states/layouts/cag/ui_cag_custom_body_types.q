...
script ui_create_cag_custom_body_types 
	...
	GetArraySize ($<part>)
	i = 0
	printf channel = body_part qs(0xaba52ce9) p = ($<part>)
	begin
	if cas_item_is_visible part = <part> part_index = <i>
		if is_part_unlocked part = <part> desc_id = ((($<part>) [<i>]).desc_id) savegame = ($cas_current_savegame)
			...
			if GotParam \{is_prs}
				if (<is_prs> = 1)
					if NOT StructureContains structure = (($<part>) [<i>]) is_prs
						printf qs(0x99420b35) n = <i> channel = filter_test
						<add_item> = 0
					elseif (((($<part>) [<i>]).is_prs) = 0)
						printf qs(0x939e1f09) n = <i> channel = filter_test
						<add_item> = 0
					endif
				else
					if StructureContains structure = (($<part>) [<i>]) is_prs
						if (((($<part>) [<i>]).is_prs) = 1)
							printf qs(0x8a852e48) n = <i> channel = filter_test
							<add_item> = 0
						endif
					endif
				endif
				...
				if StructureContains structure = (($<part>) [<i>]) is_prs
					if (((($<part>) [<i>]).is_prs) = 1)
						printf qs(0x32a91aa8) n = <i> channel = filter_test
						<add_item> = 0
					endif
				endif
			endif
			if GotParam \{is_sch}
				if (<is_sch> = 1)
					if NOT StructureContains structure = (($<part>) [<i>]) is_sch
						printf qs(0x36bd2b2f) n = <i> channel = filter_test
						<add_item> = 0
					elseif (((($<part>) [<i>]).is_sch) = 0)
						printf qs(0xa1a967e7) n = <i> channel = filter_test
						<add_item> = 0
					endif
				else
					if StructureContains structure = (($<part>) [<i>]) is_sch
						if (((($<part>) [<i>]).is_sch) = 1)
							printf qs(0xb8b256a6) n = <i> channel = filter_test
							<add_item> = 0
						endif
					endif
				endif
				...
				if StructureContains structure = (($<part>) [<i>]) is_sch
					if (((($<part>) [<i>]).is_sch) = 1)
						printf qs(0x5218032f) n = <i> channel = filter_test
						<add_item> = 0
					endif
				endif
			endif
