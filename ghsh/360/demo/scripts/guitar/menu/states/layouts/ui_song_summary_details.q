...
script ui_details_add_compiled_results_to_table 
	...
	if (<array_size> > 0)
		...
		createscreenelement {
			...
			percent_p1_rgba = (($g_menu_colors).menu_sober_green)
			percent_p2_rgba = (($g_menu_colors).menu_sober_green)
			percent_p3_rgba = (($g_menu_colors).menu_sober_green)
			percent_p4_rgba = (($g_menu_colors).menu_sober_green)
		}
		change song_summary_details_row_count = (($song_summary_details_row_count) + 1)
...
script ui_song_details_shift 
	...
	if gotparam \{down}
		if ((<screenelementpos>.(0.0, 1.0)) <= (<height> * -1 + 400))
			return
		endif
	...
	max = ((<total> - <fit>) + 1)
	dx = (290.0 / <max>)
	pos = ((480.0, -10.0) + ((0.0, 1.0) * (<current> * <dx>)))
