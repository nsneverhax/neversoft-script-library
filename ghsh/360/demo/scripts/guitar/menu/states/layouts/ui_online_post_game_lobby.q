...
script ui_create_online_post_game_lobby 
	...
	if onlinelobbyinterface :desc_resolvealias \{name = alias_player_slot_highlight}
...
script create_player_scores_postgame_container 
	change \{network_game_disable_back = 0}
...
script song_being_selected 
	...
	if screenelementexists \{id = onlinelobbyinterface}
		...
		if (<index> = 6)
			change \{network_game_disable_back = 1}
		endif
		if (<index> = 5)
...
script postgame_lobby_quit 
	obj_getid
	<objid> :obj_spawnscriptnow postgame_lobby_quit_spawned params = {<...>}
endscript

script postgame_lobby_quit_spawned 
	setspawninstancelimits \{max = 1
		management = ignore_spawn_request}
	if ($network_game_disable_back = 1)
		return
	endif
