...
script Skate8_SFX_Background_Play \{crossfadetime = 0.7
	...
	if NOT ($SFX_Background_Current_Checksum = $SFX_Background_Requested_Checksum)
		...
		PlaySound $SFX_Background_Requested_Checksum vol_percent = <newvolumepercent> Priority = 105 buss = Crowd_Beds attack_time = <crossfadetime> attack_function = log_fast num_loops = -1
	else
		printf \{channel = sfx
			qs(0x2e8957c6)}
		if issoundplaying \{$SFX_Background_Requested_Checksum}
			SetSoundParams $SFX_Background_Requested_Checksum vol_percent = <newvolumepercent> time = <crossfadetime>
		else
			printf \{channel = sfx
				qs(0x30987070)
				s = $SFX_Background_Requested_Checksum}
...
script backgrounds_shared_logic \{new_fade_time = 3}
	stopsound $SFX_Background_Current_Checksum fade_time = <new_fade_time> fade_type = log_slow
endscript
