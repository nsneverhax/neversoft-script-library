...
script 0xaa6ed678 
	...
	if NOT ScreenElementExists \{id = setlistinterface}
		printf \{qs(0x5e998509)}
		break
	endif
	if ($net_song_countdown = 1)
		printf \{qs(0x03d264ad)}
		break
	endif
	formatText TextName = bottom_text qs(0x3ce75fd4) s = <Name> d = <0xc76918d3>
	...
	if NOT ScreenElementExists \{id = popup_warning_container}
		if (<0xc76918d3> < 1)
			...
			printf \{qs(0x170cced9)}
			if ScreenElementExists \{id = setlist_menu}
				setlist_menu :GetSingleTag \{last_focused_song}
				SpawnScriptNow ui_setlist_continue params = {device_num = <controller> song = <last_focused_song>}
			endif
			break
		endif
		<0xc76918d3> = (<0xc76918d3> - 1)
	endif
	Wait \{1
		Second}
	repeat
...
script create_net_ui \{parent_element = net_setlist_container}
	...
	if ($online_song_choice_id = <net_obj_id>)
		...
		<Name> = $<gamertag>
		0x8dbdaee4 qs(0xb742e6f3) i = (<index> - 1)
		break
	endif
	<index> = (<index> + 1)
	repeat <num_players>
	if (<index> <= 8)
		...
		if ScreenElementExists \{id = setlistinterface}
			if (<is_local_client> = 1)
				...
				setlistinterface :se_setprops number_of_stars_text = <bottom_text>
				spawnscript 0xaa6ed678 params = {Name = <Name>}
			else
...
script spin_names 
	...
	if (<i> = 0)
		...
		SoundEvent \{event = ghmix_select}
		<objID> :se_setprops net_spinner_text_color = [206 151 51 255]
	endif
	Wait \{0.1
		Seconds}
	if (<i> = 0)
