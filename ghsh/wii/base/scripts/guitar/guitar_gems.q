...
button_models = [
	{
		...
		white = {
			...
			battle_star_material = mat_open_note_battle
			battle_star_hammer_material = mat_open_note_battle
...
script calc_health_invincible_time 
	...
	time_easy_rhythm = (<total_end_time> * $health_invincible_time_percentage_easy_rhythm / 100.0)
	...
	if ($game_mode = p1_quickplay || $game_mode = p1_career || $game_mode = p2_battle)
		if NOT ($battle_do_or_die)
			Player = ($<player_status>.Player)
			getplayerinfo <Player> difficulty
			switch <difficulty>
				case easy_rhythm
				health_invincible_time = <time_easy_rhythm>
				case easy
...
script gem_scroller \{Player = 1
	...
	if (<Player> = 1)
		...
		addplayerserverfretbariterator {fretbar_id = fretbarevent id = <player_status> song_name = <song_name> difficulty = <difficulty>
			...
			Player = <Player> player_status = <player_status> player_text = <player_text>}
		addplayerserverfretbariterator {fretbar_id = 0xc6b4231d id = <player_status> song_name = <song_name> difficulty = <difficulty>
			time_offset = (<audio_offset> -50.0) fretbar_function = 0x7a0c4b61 skipleadin = 0
			Player = <Player> player_status = <player_status> player_text = <player_text>}
...
script unload_gempaks 
	...
	if ($last_fx_set != None)
		...
		UnLoadPak <fxpak> Heap = heap_cas async = 0
	...
	if ($last_star_set != None)
		...
		formatText \{TextName = starpak
			...
			i = $last_star_set
			AddToStringLookup = true}
		UnLoadPak <starpak> Heap = heap_cas async = 0
...
script load_gempaks \{async = 0}
	...
	if ($game_mode = p2_battle)
		formatText TextName = gempak '%i_battle.pak' i = <pak> AddToStringLookup = true
	else
...
script load_song_anims \{async = 0}
	...
	if (<song_name> = jamsession)
		...
		get_song_prefix song = <song_name>
		if song_uses_mocap_anim_pak song = <song_name>
			guitar_character_loops_set_state \{state = loop_paks_disabled}
			formatText TextName = mocap_anim_pak 'pak/anims/songs/%i%s_mocap.pak' i = <song_prefix> s = <song_perf_ext> AddToStringLookup = true
			if LoadPakAsync pak_name = <mocap_anim_pak> Heap = heap_musician_anim no_vram async = 1
				0x8dbdaee4 qs(0xcfc9bae8) i = <mocap_anim_pak>
			else
				ScriptAssert \{qs(0x636d24e0)}
			endif
			Change current_song_qpak_mocap = <mocap_anim_pak>
		else
		...
		if NOT LoadPakAsync pak_name = <animqpak> Heap = heap_song no_vram async = 1
			DownloadContentLost
			return
		endif
	endif
...
script unload_song_anims 
	...
	if ($current_song_qpak = jamsession)
		...
		UnLoadPak <songqpak>
		WaitUnloadPak <songqpak>
		if NOT ($current_song_qpak_mocap = None)
			printf qs(0x3d306452) i = ($current_song_qpak_mocap)
			UnLoadPak \{$current_song_qpak_mocap}
			WaitUnloadPak \{$current_song_qpak_mocap}
			Change \{current_song_qpak_mocap = None}
			guitar_character_loops_set_state \{state = loop_paks_enabled}
		endif
	endif
endscript
		...
		if (<song_name> = jamsession)
			...
			if (<download> = 1)
				...
				if NOT (GotParam noanims)
					formatText TextName = 0x2df8cebc '%i_anims.pak' i = <song_prefix> AddToStringLookup = true
					printf qs(0x0761096c) i = <0x2df8cebc>
					if NOT LoadPakAsync pak_name = <0x2df8cebc> Heap = heap_song no_vram async = <async>
						DownloadContentLost
						return
					endif
				endif
				Downloads_LoadLanguageContent stem = <song_prefix> async = <async>
			else
...
script unload_songqpak 
	if NOT ($current_song_qpak = None)
		if ($current_song_qpak = jamsession)
			...
			if (<download> = 1)
				formatText TextName = songqpak '%i%s_song.pak' i = <song_prefix> s = <song_perf_ext> AddToStringLookup = true
				formatText TextName = 0x2df8cebc '%i_anims.pak' i = <song_prefix>
				printf qs(0x8c60e3ce) i = <0x2df8cebc>
				UnLoadPak <0x2df8cebc>
...
script start_gem_scroller \{starttime = 0
	...
	unload_gempaks
	<0x0347f72c> = ''
	if ($game_mode = p2_faceoff || $game_mode = p2_pro_faceoff || $game_mode = p4_pro_faceoff || $boss_battle = 1)
		<0x0347f72c> = 'pak/oogame/oogamevs.pak'
	elseif ($game_mode = p2_battle)
		<0x0347f72c> = 'pak/oogame/oogamebattle.pak'
	...
	if (<song_name> = jamsession)
		...
		0xd7221b77 drum_kit = ($drum_kits [<drum_kit>].string_id) percussion_kit = ($drum_kits [<drum_kit>].percussion_string_id) async = 0
		if GotParam \{melody_kit}
		...
		GetArraySize \{$drum_kits}
		if ((<drum_kit_index> >= <array_Size>) || (<drum_kit_index> < 0))
			<drum_kit_index> = 0
		endif
		Change jam_current_drum_kit = <drum_kit_index>
		loaddrumkitall drum_kit = <drum_kit_type> percussion_kit = ($drum_kits [<drum_kit_index>].percussion_string_id)
	endif
	...
	if 0x2e51d96c
		printstruct \{player1_status}
		printstruct <...>
		0x6efdb753
		0x6797e24c <...>
	endif
	if NOT (<song_name> = jamsession)
		preload_song song_name = <song_name> starttime = <starttime> 0x922dc314 = <0x922dc314>
		printf \{qs(0x1157f54a)}
	...
	if (<loading_transition> = 0)
		...
		yield
		if ((<training_mode> = 0) && ($game_mode != training))
			create_crowd_models
		endif
	endif
	...
	if (gamemode_isbandbattle)
		if NOT ($<player_status>.is_local_client = 1)
			player_on_screen = 0
		endif
	endif
	...
	if ((<calc_score> = true) && (<player_on_screen> = 1))
		calc_songscoreinfo player_status = <player_status> Player = <Player>
	endif
	Player = (<Player> + 1)
	repeat $current_num_players
	yield
...
script wait_and_show_boss_helper_after_intro 
	...
	Transition_Wait
	kill_gem_scroller
	ui_memcard_autosave event = menu_replace state = uistate_battle_helper data = {boss = ($current_song)}
	disable_pause
...
script kill_gem_scroller \{no_render = 0
	...
	if NOT (<Type> = restart)
		...
		KillSpawnedScript \{Name = 0x7a0c4b61}
...
script restart_gem_scroller \{no_render = 0
	...
	mark_unsafe_for_shutdown
	disable_pause
	Change currently_playing_song = <song_name>
	yield
	if (<loading_transition> = 0)
		PauseGame
	endif
...
script load_perm_ingame_anims 
	...
	mpm_unload_pak pak = perm_anims_in_game async = <async>
endscript

script Load_Venue \{block_scripts = 0
		force_venue_reload = 0}
	load_perm_ingame_anims async = (<block_scripts> - 1)
	if ((($current_level) = load_z_forum) || (($current_level) = load_z_meadowlands))
		disablevenueculling
	else
		enablevenueculling
	endif
	...
	if (($game_mode = p8_pro_faceoff) || ($game_mode = p6_pro_faceoff_no_mics) || ($game_mode = p4_pro_faceoff))
		switch ($current_level)
			case load_z_donnington
			Change \{current_level = 0x349d4bd7}
			case load_z_forum
			Change \{current_level = 0x7cf3d4ef}
			case load_z_justice
			Change \{current_level = 0x349d4bd7}
			case load_z_meadowlands
			Change \{current_level = 0x61d09ce6}
			case load_z_mop
			Change \{current_level = 0x59ab4ae1}
			case load_z_stone
			Change \{current_level = 0x59ab4ae1}
			case load_z_tushino
			Change \{current_level = 0x7cf3d4ef}
	...
	if GotParam \{pak}
		...
		if (<pak> = (($LevelZones.$current_level).zone))
			...
			0x776e192b
			printf \{qs(0xfdca610d)}
			destroy_crowd_models
			DestroyViewportTextureOverride viewportid = <viewport_override_id>
			0xfd9d9776
			SetPakManCurrentBlock \{map = zones
				pak = None}
			printf qs(0xc3d16928) p = (($LevelZones.$current_level).zone)
		endif
	else
...
script start_song \{device_num = 69
	...
	if ($metallica_worstcase_screenfx = 1)
		TOD_Proxim_Update_LightFX \{fxParam = $0x4d3dcd82
	...
	if (($current_transition = ENCORE) || ($current_transition = forceintro))
...
script restart_song \{practice_intro = 0
	...
	if (<loading_transition> = 1)
		...
		yield \{'band_builder_get_cpu_singer_id'}
		band_builder_switch_character new_character_id = <singer_id> part = vocals cpu_only async = 1
		yield \{'band_builder_switch_character'}
		...
		Wait \{4
			gameframes
			ignoreslomo}
		if NOT sd_is_song_loaded song = ($current_song)
			sd_unload_song
		endif
	...
	if (<loading_transition> = 0 &&
			<practice_intro> = 0)
		if NOT ($current_transition = forceintro)
		...
		elseif ($debug_encore)
			Change \{current_transition = restartencore}
		elseif NOT (($current_transition = restartencore) || ($current_transition = forceintro))
			Change \{current_transition = fastintro}
		endif
	endif
	if (($current_transition = ENCORE) || ($current_transition = forceintro))
	...
	UnPauseGame
	yield
	restart_gem_scroller song_name = ($current_song) difficulty = ($player1_status.difficulty) difficulty2 = ($player2_status.difficulty) starttime = ($current_starttime) practice_intro = <practice_intro> endtime = <endtime> loading_transition = <loading_transition> 0x922dc314 = <0x922dc314>
	Change \{0xdd4f68fd = 0}
endscript
...
script load_frontend_guitarist_anims 
	return
	if get_pak_filename_using_filename_crc \{pak_crc = $guitarist_anim_pak_file
			Type = anim}
		LoadPak (<pak_name>) heap_musician_anim
	endif
	if get_pak_filename_using_filename_crc \{pak_crc = $bassist_anim_pak_file
			Type = anim}
		LoadPak (<pak_name>) heap_musician_anim
	endif
endscript
...
script unload_frontend_guitarist_anims \{async = 0}
	...
	if get_pak_filename_using_filename_crc pak_crc = <pak_crc> Type = anim
