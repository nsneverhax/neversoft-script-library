...
script create_new_generic_popup \{Priority = 11
	...
	if GotParam \{options}
		GetArraySize <options>
		if (<array_Size> > 0)
			bx_createhighlightstars \{parent = generic_popupelement
				focuswaittime = 2
				stars_id = popup_stars}
		endif
	endif
	generic_popupelement :Obj_SpawnScriptNow \{bg_swap}
	if (0x551b214c)
		generic_popupelement :se_setprops \{0x175c6262 = 1.0}
	endif
	getfocusid
	...
	if GotParam \{options}
		GetArraySize <options>
		if (<array_Size> > 0)
			create_popup_warning_menu_options <...> popup_desc = <desc>
		endif
	endif
	...
	if (<popup_type> = loading_window)
		generic_popupelement :se_setprops \{popupbody_text = qs(0xf59ac77b)}
...
script start_new_loading_process \{time = 0.3333}
	...
	if ($<wait_var> = 0)
		...
		formatText TextName = dot_text qs(0x73307931) s = <dot_text>
...
script create_phrase_option 
	...
	formatText TextName = row_text qs(0x73307931) s = (($<phrase_array>) [<phrase_index>])
	printstruct <...>
	CreateScreenElement {
