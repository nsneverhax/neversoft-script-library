...
calibrate_lag_menu_line_pos = (311.0, 339.0)
calibrate_lag_menu_circle_dims = (96.0, 96.0)
calibrate_lag_menu_circle_velocity = 300
calibrate_lag_menu_circle_inital_pos = (311.0, -167.0)
...
script create_calibrate_lag_dialog_menu 
	...
	create_popup_warning_menu {
		...
		options = [
			{
				func = {0x51bb3caf}
				func_params = {func = ui_event event = menu_back data = {state = uistate_options_calibrate_lag}}
				text = qs(0x182f0173)
			}
		]
	}
...
script menu_calibrate_lag_create_background \{z = 700}
	...
	current_menu_anchor :se_setprops \{calibrate_lag_option_calibrate_dims = (0.0, 50.0)
		calibrate_lag_option_reset_dims = (0.0, 50.0)
		event_handlers = [
			{
				controller_acquired
				0x2ac9f004
...
script menu_calibrate_lag_setup_vmenu 
	...
	(<children> [2]) :se_setprops not_focusable
	event_handlers = [
	...
	(<children> [4]) :se_setprops not_focusable
	event_handlers = [
...
script menu_calibrate_lag_create_circles 
	...
	<circle_id> :se_setprops Pos = (<circle_pos>)
	time_requirement = (<steps> * (1.0 / <fps>))
	<circle_id> :SetTags time_requirement = <time_requirement>
...
script do_calibration_update 
	...
	num_circles_gone = 0
	0x3abc93bb = 1
	update_difference = (<delta_time> * <0x3abc93bb>)
	begin
	formatText checksumName = circle_id 'circle%d' d = <circle_index>
	<circle_id> :GetTags
	<existence> = (<existence> + <update_difference>)
...
script menu_calibrate_lag_manual_choose 
	SetScreenElementProps \{id = cl_vmenu
		block_events}
	cl_arrow_color = (($g_menu_colors).menu_calibrate_arrow_focus)
	if GotParam \{audio}
...
script menu_calibrate_lag_manual_back 
	SetScreenElementProps \{id = cl_vmenu
		unblock_events}
	DestroyScreenElement \{id = cl_manual_adjust_handler}
	cl_arrow_color = (($g_menu_colors).menu_calibrate_arrow_unfocus)
	if GotParam \{audio}
...
script 0x51bb3caf 
	fadetoblack \{On
		...
		alpha = 1
		z_priority = 100011}
	<func> event = <event> data = <data>
endscript

script 0x2ac9f004 
	if NOT GotParam \{device_num}
		device_num = ($calibrate_lag_most_recent_controller)
		if (($calibrate_lag_section = video) || ($calibrate_lag_section = audio))
			clean_up_user_control_helpers
		endif
	...
	elseif ($calibrate_lag_section = audio)
		...
		elseif isdrumcontroller controller = <device_num>
			...
			current_menu_anchor :se_setprops \{calibrate_lag_bottom_text_text = qs(0x98f94473)}
			add_user_control_helper \{text = qs(0xa5d9d07e)
				button = green
				z = 10000}
		endif
