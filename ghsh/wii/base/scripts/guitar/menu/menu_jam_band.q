...
script create_jam_band_menu \{editing = 0
	...
	rgba_highlight = [200 200 200 255]
	set_focus_color rgba = <rgba_highlight>
	set_unfocus_color rgba = ($unselected_default_color)
	...
	menu_music_off
	if NOT ScreenElementExists \{id = jam_band_container}
	...
	setplayerinfo \{4
		jam_instrument = -1}
	jam_setup_song editing = <editing> tutorial = <tutorial> 0x31d73a91 = <0x31d73a91>
	...
	if (<tutorial> = 0)
		if (<editing> = 1)
			...
			formatText TextName = song_name_text <song>
			<song> = <0xa6c41f6f>
		endif
		CreateScreenElement {
			Type = TextElement
...
script menu_jam_band_add_control_helpers \{state = NULL}
	...
	switch <state>
		...
		if ScreenElementExists \{id = jam_studio_element}
			...
			add_user_control_helper \{text = qs(0xb6671f3e)
				button = yellow
				z = 100}
			add_user_control_helper \{text = qs(0x712c0792)
				button = blue
				z = 100}
...
script jam_poll_for_controller_changes 
	...
	if is_regular_controller controller = <count>
		if NOT jam_controller_can_use_mic controller = <count>
	...
	if NOT (<is_active_controller> = (<prev_active_controllers> [<count>]))
		if (<is_active_controller> = 1)
			...
			if NOT ($game_mode = training)
				if is_regular_controller controller = <count>
					if jam_controller_can_use_mic controller = <count>
						0x9e5cc35e = 1
					endif
				endif
			endif
...
script jam_setup_song \{advanced_record = 0
	...
	if (<advanced_record> = 0)
		jam_band_create_highway song = <song> 0x31d73a91 = <0x31d73a91>
	endif
endscript

script jam_band_create_highway \{0x31d73a91 = FALSE}
...
script jam_create_select_instrument_menu \{Player = 0}
	...
	if ((<inst_allow> < 0 || <count> = <inst_allow>) && <show_mic> != 1)
		...
		CreateScreenElement {
			...
			event_handlers = [
				...
				{pad_choose <inst_select_func> params = {Player = <Player> player_cont = <player_cont> select_instrument = <count> wrap = <wrap> 0x530ce5c5 = <0x530ce5c5>}}
			]
			Shadow
			shadow_offs = (3.0, 3.0)
	...
	if ScreenElementExists id = <inst_select_element>
		if (<morph> = 1)
			<inst_select_element> :se_setprops rot_angle = -0.5 Pos = (-110.0, 75.0) time = 0.1
			<inst_select_element> :se_waitprops
			if ScreenElementExists id = <inst_select_element>
				<inst_select_element> :se_setprops rot_angle = 0.3 Pos = (-110.0, 67.0) time = 0.05
				<inst_select_element> :se_waitprops
			endif
			if ScreenElementExists id = <inst_select_element>
				<inst_select_element> :se_setprops rot_angle = 0.0 Pos = (-110.0, 70.0) time = 0.05
				<inst_select_element> :se_waitprops
			endif
		else
			<inst_select_element> :se_setprops rot_angle = 0.0 Pos = (-110.0, 70.0)
		endif
	endif
...
script jam_create_drum_type_select_menu 
	...
	if ScreenElementExists id = <drum_select_element>
		RunScriptOnScreenElement id = <drum_select_element> legacydomorph params = {Pos = (-110.0, 70.0) time = 0.1}
	endif
...
script jam_create_guitar_type_select_menu 
	...
	switch <select_instrument>
		...
		machine_text = qs(0xe5f8e2bb)
		case 4
		type_text = qs(0x58ce03d1)
		machine_text = qs(0x88396708)
	endswitch
	if GotParam \{wrap}
		if (<wrap> = 1)
			0xb6d98342 = wrap
		else
			0xb6d98342 = `scale	each	line	if	larger`
		endif
	else
		0xb6d98342 = `scale	each	line	if	larger`
	endif
	...
	if (GotParam wrap)
		if (<wrap> = 1)
			fit_text_in_rectangle id = <id> dims = (180.0, 90.0)
		else
	...
	if ((GotParam wrap) || (GotParam 0x530ce5c5))
		if (<wrap> = 1)
			fit_text_in_rectangle id = <id> dims = (180.0, 90.0)
		elseif (<0x530ce5c5> = 1)
			fit_text_in_rectangle id = <id> dims = (180.0, 60.0)
		else
	...
	if ScreenElementExists id = <guitar_select_element>
		RunScriptOnScreenElement id = <guitar_select_element> legacydomorph params = {Pos = (-110.0, 70.0) time = 0.1}
	endif
	AssignAlias id = <resolved_id> alias = current_menu
	LaunchEvent \{target = current_menu
		Type = focus}
	...
	if NOT (<jam_instrument> = <last_inst>)
		...
		if (<jam_instrument> >= 0)
			...
			if ScreenElementExists id = <inst_select_text>
				...
				<inst_select_text> :se_setprops rgba = [112 94 62 255]
			endif
		else
...
script jam_player_select_instrument \{Player = 0
	...
	if ScreenElementExists id = <inst_select_element>
		RunScriptOnScreenElement id = <inst_select_element> legacydomorph params = {Pos = (-110.0, -400.0) time = 0.1}
	endif
	jam_destroy_select_instrument_menu Player = <Player>
	jam_destroy_select_drum_type_menu Player = <Player>
	jam_destroy_select_guitar_type_menu Player = <Player>
...
script jam_create_effect_info_box 
	...
	if (<jam_instrument> < 2)
		...
		formatText TextName = effect_text qs(0x73307931) s = <effect_name>
	...
	elseif (<jam_instrument> = 2)
		if ($bass_kit_mode = 0)
			...
			formatText TextName = bass_kit_text qs(0x73307931) s = <bass_kit_name>
	...
	elseif (<jam_instrument> = 3)
		...
		formatText TextName = effect_text qs(0x73307931) s = <kit_name>
		<player_info_element> :SetProps extra_info_text = <effect_text>
	...
	elseif (<jam_instrument> = 4)
		...
		formatText TextName = melody_kit_text qs(0x73307931) s = <melody_kit_name>
...
script jam_setup_fretboard 
	...
	if ($g_in_tutorial = 0)
		RunScriptOnScreenElement id = <base_id> legacydomorph params = {Pos = (0.0, 250.0) time = 0.1}
	else
		RunScriptOnScreenElement id = <base_id> legacydomorph params = {Pos = (0.0, 300.0) time = 0.1}
	endif
...
script jam_fretboard_add_note 
	...
	z_priority = 28
	formatText checksumName = gems_id 'jam_now_bar_%s' s = <Player>
...
script jam_band_pause \{back_to_jam_band = 1
	...
	if GotParam \{adv_record}
		<scrolling_options> :se_setprops Scale = (1.15, 1.0)
	endif
	if GotParam \{adv_record}
		pause_options = [
			{
	...
	getplayerinfo <select_player> jam_instrument
	text_params = {Type = TextElement font = fontgrid_text_a3 just = [center center] Scale = 1 rgba = ($unselected_default_color)}
	...
	if (<show_option> = 1)
		...
		option_rgba = ($unselected_default_color)
		if (<primary_only> = 1)
			<option_rgba> = [50 50 50 255]
		endif
		CreateScreenElement {
			Type = TextBlockElement
			parent = <option_id>
...
script jam_band_pause_submenu 
	...
	if StructureContains structure = (<options_array> [0]) section_heading
		...
		if NOT GotParam \{no_preview}
			...
			CreateScreenElement {
				...
				helper_description_rgba = (($g_menu_colors).menu_title)
	...
	getplayerinfo <select_player> controller
	0x202003aa = (150.0, 280.0)
	if (<added_heading> = 1)
		0x202003aa = (150.0, 320.0)
	endif
	new_menu {
	...
	option_dims = (100.0, 34.0)
	StringLength string = ((<options_array> [<option>]).name_text)
	if (<str_len> >= 14)
		option_dims = (100.0, 44.0)
	endif
	if (<show_option> = 1)
		...
		if NOT ((<option> < 3) && (<added_heading> = 1))
			option_pos = (0.0, 0.0)
...
script line6_pod 
	...
	<pad_back_handler> = {pad_back line6_pod_back params = {<...>}}
	if GotParam \{nobackeventhandler}
		pad_back_handler = {pad_back nullscript}
	endif
	0x10768c50 = {pad_start line6_pod_remove params = {<...>}}
	if ($game_mode = training)
		<0x10768c50> = {pad_start nullscript}
	endif
	event_handlers = [{<pad_back_handler>}
		{pad_down line6_pod_effect_change params = {down <...>}}
		{pad_up line6_pod_effect_change params = {up <...>}}
	...
	if NOT ($jam_advanced_record = 1)
		<line6_pod_id> :se_setprops Scale = 1.05 Pos = <0xeccffde9> time = 0.1
		<line6_pod_id> :se_waitprops
		<line6_pod_id> :se_setprops Scale = 0.95 Pos = <0xeccffde9> time = 0.05
		<line6_pod_id> :se_waitprops
		<line6_pod_id> :se_setprops Scale = 1.0 Pos = <0xeccffde9> time = 0.05
		<line6_pod_id> :se_waitprops
	endif
	LaunchEvent target = <line6_pod_id> Type = focus
	SetArrayElement ArrayName = 0xfb2e7f1a globalarray index = <device_num> NewValue = 1
endscript
...
script line6_pod_remove 
	if NOT (($0xfb2e7f1a [<device_num>]) = 1)
		return
	endif
	SpawnScriptNow 0x74bc20cd params = <...>
endscript
...
script 0x74bc20cd 
	...
	SetArrayElement ArrayName = 0xfb2e7f1a globalarray index = <device_num> NewValue = 0
	ui_menu_select_sfx
...
script line6_pod_effect_change 
	...
	if ((<jam_instrument> = 1) || (<jam_instrument> = 0))
		stopsound \{$jam_settings_effect_lead_sample
			fade_time = 0.1
			fade_type = linear}
		Wait \{0.15
	...
	guitar_jam_effects_toggle_active jam_instrument = <jam_instrument> force_state = On
	line6_pod_update_effect select_player = <select_player> line6_pod_id = <line6_pod_id>
	SpawnScriptNow guitar_jam_settings_preview_effect params = {jam_instrument = <jam_instrument> effect = <effect>}
	SetArrayElement ArrayName = 0xfb2e7f1a globalarray index = <device_num> NewValue = 1
endscript
...
script guitar_jam_settings_preview_effect 
	if ((<jam_instrument> = 0) || (<jam_instrument> = 1))
		stopsound \{$jam_settings_effect_lead_sample
			fade_time = 0.1
			fade_type = linear}
		Wait \{1
		...
		formatText TextName = 0x78427a75 '%a_preview' a = (<effects_array> [<effect>].ps2_prefix)
		formatText checksumName = sample_checksum '%a_preview' a = (<effects_array> [<effect>].ps2_prefix)
		printf qs(0xf70615f3) a = <0x78427a75>
		PlaySound <sample_checksum> vol = 0 buss = jammode_leadguitar
...
script guitar_jam_change_drum_kit 
	...
	if ($jam_advanced_record = 0)
		formatText checksumName = player_info_element 'player_info_element_%a' a = <Player>
		formatText TextName = extra_info_text qs(0x73307931) s = (($pause_drum_kit_options) [<option_index>].name_text)
		<respawn> = 1
		if ScreenElementExists id = <player_info_element>
...
script guitar_jam_change_melody_kit 
	...
	if ($jam_advanced_record = 0)
		<respawn> = 1
		formatText checksumName = player_info_element 'player_info_element_%a' a = <Player>
		formatText TextName = extra_info_text qs(0x73307931) s = (($pause_melody_kit_options) [<option_index>].name_text)
		if ScreenElementExists id = <player_info_element>
			<player_info_element> :se_setprops extra_info_text = <extra_info_text>
		endif
	endif
	...
	if ($jam_advanced_record = 0)
		...
		if ScreenElementExists id = <player_info_element>
			if (<option_index> = 0)
				...
				formatText TextName = extra_info_text qs(0x73307931) s = (($pause_bass_kit_options) [<option_index>].name_text)
				<player_info_element> :se_setprops extra_info_text = <extra_info_text>
				RunScriptOnScreenElement id = <player_info_element> legacydomorph params = {Pos = (3.0, 445.0) time = 0.1}
			endif
		endif
...
script jam_bass_kit_preview 
	...
	Change jam_input_current_bass = <uniqueID>
	Wait \{1
		Second}
	stopsound unique_id = ($jam_input_current_bass) fade_type = linear fade_time = 0.05
	LaunchEvent Type = focus target = <option_id>
...
script guitar_jam_settings_update_pan 
	...
	switch <jam_instrument>
		case 0
		Change jam_rhythm_pan = <new_pan>
		case 1
		Change jam_lead_pan = <new_pan>
		case 2
		Change 0xc35f919c = <new_pan>
		case 4
...
script guitar_jam_settings_bpm 
	...
	create_jam_settings_arrows Player = <Player> parent = <option_id> pos_x = 86
...
script jam_band_pause_save 
	...
	if (<from_adv> = 0)
		...
		ui_memcard_save_jam event = menu_back data = {state = uistate_recording editing = 1 current_instrument = ($jam_current_track) ghmix_persistant_settings = <ghmix_persistant_settings> 0x6d221ff1}
	endif
endscript
...
script jam_band_pause_save_as_text_select 
	...
	if (<user_song_count> < ($jam_max_user_songs))
		...
		if ($jam_band_new_song = 1)
			start_text = qs(0xbf9c7a91)
		else
			start_text = ($jam_selected_song)
		endif
		ui_event event = menu_change data = {state = uistate_jam_publish_title_entry text = <start_text> choose_script = jam_band_pause_save_as choose_params = {0x57501c34 = <0x57501c34>}}
	else
...
script jam_band_pause_save_as \{0x57501c34 = uistate_jam_band}
	Change \{memcard_jamsession_actual_file_name = ''}
	printf \{qs(0x5ffc6b2f)}
	...
	ui_memcard_save_jam event = menu_back data = {state = <0x57501c34> editing = 1 show_popup = 0 0x6d221ff1}
endscript
...
script jam_band_save_dialog_cancel 
	destroy_popup_warning_menu
	if (<from_adv> = 1)
		ui_event event = menu_refresh data = {editing = 1 0x31d73a91 = true current_instrument = ($jam_current_track) ghmix_persistant_settings = <ghmix_persistant_settings>}
	else
...
script jam_band_quit_dialog 
	...
	if ($enable_saving = 1)
		...
		jam_band_pause_quit vmenu_id = <vmenu_id>
	endif
	dim_save_option_for_guest <...> popup_warning_child_index = 0
	add_user_control_helper \{text = qs(0xaf4d5dd2)
		button = red
...
script 0xb5331962 
	jam_band_remove_pause player_pause = <player_pause> scrolling_options = <scrolling_options> event_cont = <event_cont> select_player = <select_player> respawn_input = 1 device_num = <device_num> controller = <controller>
	guitar_jam_simplerecops_play select_player = <select_player>
	guitar_jam_simplerecops_record select_player = <select_player>
endscript
...
script 0x75ccbb3b 
	SpawnScriptNow guitar_jam_simplerecops_skipfb params = {Dir = right select_player = <select_player> direction_override = R2}
endscript

script 0x7af8e675 
	SpawnScriptNow guitar_jam_simplerecops_skipfb params = {Dir = left select_player = <select_player> direction_override = R2}
endscript
