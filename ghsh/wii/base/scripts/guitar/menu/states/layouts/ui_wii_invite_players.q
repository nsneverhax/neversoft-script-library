...
script ui_create_wii_invite_players \{online = 1
	...
	if (<min_players> = <max_players>)
		formatText TextName = players qs(0x76b3fda7) d = <max_players>
	else
		formatText TextName = players qs(0xdf52962a) d = <min_players> e = <max_players>
	endif
	<invite_props> = {
		<invite_props>
	...
	if (<selected> = 1)
		...
		if StructureContains structure = <tags> ($invite_instruments [<j>].checksum)
			...
			if (<index> = <i>)
				...
				if ((<checksum> = guitar1) || (<checksum> = guitar2))
					if (<num_guitars> = 1)
						<checksum> = guitar1
					elseif (<num_guitars> = 2)
						<checksum> = guitar2
					endif
					updatestructelement {
						struct = ($invite_instruments [<j>])
						element = checksum
						value = <checksum>
					}
				endif
