...
script create_jam_song_wizard_menu 
	...
	jam_song_wizard :obj_spawnscript \{jam_recording_check_disconnect}
...
script jam_setup_new_song 
	...
	if NOT GotParam \{no_drum}
		Change \{jam_current_drum_kit = 0}
		loaddrumkitall drum_kit = ($drum_kits [0].string_id) percussion_kit = ($drum_kits [0].percussion_string_id) async = 0
	else
		unloaddrumkitall
	endif
...
script new_song_sub_menu_button_focus 
	...
	switch <parent_menu_index>
		...
		if NOT (<index> = 0)
			...
			drum_kit_index = ($jam_auto_drum_loops_a [(<index> - 1)].drum_kit_index)
...
script new_song_create_choose 
	LaunchEvent \{Type = unfocus
		target = current_menu}
	jam_song_wizard :Obj_KillSpawnedScript \{Name = jam_recording_check_disconnect}
	...
	if NOT (<drums_index> = 0)
		drum_kit_index = ($jam_auto_drum_loops_a [(<drums_index> - 1)].drum_kit_index)
		GetArraySize \{$drum_kits}
		if ((<drum_kit_index> >= <array_Size>) || (<drum_kit_index> < 0))
			<drum_kit_index> = 0
		endif
...
script jam_copy_drum_loop_to_track \{bpm = 120}
	...
	last_fretbar_time = ($editable_fretbars [(($editable_fretbars_size) - 17)])
	index = 0
	num_loops = 0
	begin
	...
	jam_input_drum_sound velocity = <note_velocity> hold_pattern = <hold_pattern> time = <note_time> percussion = 0 auto
	<index> = (<index> + 1)
	if (<index> >= <array_Size>)
...
script jam_auto_bass_play_note 
	...
	Change jam_input_current_bass = <sample_checksum>
endscript
