	...
	if getcasappearancepart part = <part>
		...
		if GotParam \{finishable}
			add_generic_menu_text_item {
				...
				choose_state_data = {part = (<finishable>) body_part = <part> text = qs(0x48b206fc) cam_anim = guitarbody container_id = <container_id> is_popup hist_tex = icon_cag_finishes camera_list = ['cag_custom_body' 'cag_custom_body_R' 'cag_custom_body_B' 'cag_custom_body_L'] zoom_camera = 'cag_custom_body_zoom'}
			}
		endif
