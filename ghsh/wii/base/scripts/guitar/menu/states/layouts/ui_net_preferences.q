...
script ui_create_net_preferences 
	...
	if netprefinterface :desc_resolvealias \{Name = alias_online_lobby_scrollbar
			param = scrollbar_id}
		netprefinterface :SetTags scrollbar_id = <scrollbar_id>
		netprefinterface :GetSingleTag \{menu_items}
		scroll_increment = (168 / (<menu_items> -1))
		<scrollbar_id> :SetTags scroll_increment = <scroll_increment>
		<scrollbar_id> :se_setprops scrollbar_thumb_pos = (0.0, 0.0)
	endif
	create_submenu_fade submenu_z = (<z_priority> - 10) fade_id = preferences_fade parent = netprefinterface
...
script create_net_preferences_menu \{container_id = netprefinterface}
	if netprefinterface :desc_resolvealias \{Name = alias_popup_vmenu}
		...
		if NOT GotParam \{post_game_lobby}
			...
			if display_character_logic savegame = <savegame> profile_struct = <profile_struct> part = <part> 0x2f8e5ba4
...
script create_net_preferences_submenu \{selected_index = 0
	...
	netprefinterface :se_setprops \{alpha = 0.01}
	Obj_GetID
	<objID> :se_getprops
	...
	if prefssubmenuinterface :desc_resolvealias \{Name = alias_online_lobby_scrollbar
			param = submenu_scrollbar_id}
		prefssubmenuinterface :SetTags scrollbar_id = <submenu_scrollbar_id>
	endif
	prefssubmenuinterface :se_setprops title_text = <Name>
	create_submenu_fade submenu_z = (<z_priority> - 10) fade_id = preferences_submenu_fade parent = prefssubmenuinterface
	if prefssubmenuinterface :desc_resolvealias \{Name = alias_popup_vmenu
		...
		GetArraySize <items>
		if (<array_Size> <= 4)
			<submenu_scrollbar_id> :Die
			prefssubmenuinterface :se_setprops \{h_padding_scale = 0}
		endif
		draw_square_bg_param = {}
		if GotParam \{draw_square_bg}
			draw_square_bg_param = {draw_square_bg}
		endif
		if (<array_Size> > 0)
			index = 0
			begin
		...
		if ScreenElementExists id = <scrollbar_id>
			scroll_increment = (168 / (<menu_items> -1))
			<scrollbar_id> :SetTags scroll_increment = <scroll_increment>
			<scrollbar_id> :se_setprops scrollbar_thumb_pos = ((0.0, 1.0) * (<scroll_increment> * <selected_index>))
		endif
...
script destroy_net_preferences_submenu 
	...
	highlightstars :se_setprops \{alpha = 0}
	netprefinterface :se_setprops \{alpha = 1.0}
	netprefinterface :GetTags
...
script net_preferences_submenu_up 
	if ScreenElementExists \{id = prefssubmenuinterface}
		...
		if (<menu_index> = 0)
			...
			if ScreenElementExists id = <scrollbar_id>
				<scrollbar_id> :se_setprops scrollbar_thumb_pos = (0.0, 168.0)
			endif
		else
...
script online_submenu_focus 
	...
	if <objID> :desc_resolvealias Name = alias_submenu_text
		<resolved_id> :se_setprops rgba = (($g_menu_colors).menu_title)
...
script online_submenu_unfocus 
	Obj_GetID
	if <objID> :desc_resolvealias Name = alias_submenu_text
		<resolved_id> :se_setprops rgba = (($g_menu_colors).menu_gold)
	endif
endscript
...
script add_net_preferences_item \{vmenu_id = net_preferences_vmenu
	...
	submenu_item_id = <id>
	if GotParam \{draw_square_bg}
		<submenu_item_id> :se_setprops square_bg_alpha = 1.0
	endif
	if GotParam \{pic}
	...
	if <submenu_item_id> :desc_resolvealias Name = alias_submenu_text
		bx_resizetextfield {
			id = <resolved_id>
			dims = (370.0, 48.0)
