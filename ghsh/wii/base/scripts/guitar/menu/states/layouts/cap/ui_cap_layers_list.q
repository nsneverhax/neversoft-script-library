...
script add_current_layers_to_menu 
	...
	formatText TextName = text qs(0xadd8c9bd) i = <num_layers> j = ($max_cap_layers)
	CreateScreenElement {
...
script layers_list_set_focus 
	...
	layers_list_focus :obj_spawnscript \{highlight_motion
		params = {
			menu_id = layers_list_focus
			no_flip
		}}
endscript
...
script cap_change_copy_handler 
	...
	if NOT ScreenElementExists \{id = copy_element}
		...
		copy_indicator :obj_spawnscript \{highlight_motion
			params = {
				menu_id = copy_indicator
				alias_name = alias_bolts_left
				no_flip
			}}
		copy_indicator :obj_spawnscript \{highlight_motion
			params = {
				menu_id = copy_indicator
				alias_name = alias_bolts_right
				no_flip
			}}
	endif
endscript
