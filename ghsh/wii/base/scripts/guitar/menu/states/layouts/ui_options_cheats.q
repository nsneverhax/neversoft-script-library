...
script bx_resizetextforcheats 
	if (<width> > 715)
		<id> :se_setprops {
			generic_menu_smenu_textitem_text_dims = (((0.0, 1.0) * <height>) + (715.0, 0.0))
...
script cheats_load 
	...
	if GotParam <cheat_unlocked>
		if ((<...>.<cheat_unlocked>) > 0)
			formatText checksumName = 0x1bef3365 'cheat_index%a' a = <i>
			if GotParam <0x1bef3365>
				cheat_index = (<...>.<0x1bef3365>)
				cheat_set_globals globals = ($guitar_hero_cheats [<i>].options [<cheat_index>].globals)
			endif
		endif
	endif
	<i> = (<i> + 1)
