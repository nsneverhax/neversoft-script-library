...
script 0x05924e09 \{extras = 0
		0xc7b45f0a = 0
		fade = 0.0}
	if (<extras> = 1)
		setsoundbussparams {crowdsingalong_balance = {vol = (($default_BussSet.crowdsingalong_balance.vol) + <0xc7b45f0a>)} time = <fade>}
	else
		setsoundbussparams {leadvox_balance = {vol = ($default_BussSet.leadvox_balance.vol + <0xc7b45f0a>)}}
		setsoundbussparams {drums_balance = {vol = ($default_BussSet.drums_balance.vol + <0xc7b45f0a>)}}
		setsoundbussparams {leadgtr_balance = {vol = ($default_BussSet.leadgtr_balance.vol + <0xc7b45f0a>)}}
		setsoundbussparams {bass_balance = {vol = ($default_BussSet.bass_balance.vol + <0xc7b45f0a>)}}
		setsoundbussparams {rhythmgtr_balance = {vol = ($default_BussSet.rhythmgtr_balance.vol + <0xc7b45f0a>)}}
		setsoundbussparams {crowd_balance = {vol = ($default_BussSet.crowd_balance.vol + <0xc7b45f0a>)}}
		setsoundbussparams {crowdsingalong_balance = {vol = ($default_BussSet.crowdsingalong_balance.vol + <0xc7b45f0a>)}}
		setsoundbussparams {band_Balance = {vol = ($default_BussSet.band_Balance.vol + <0xc7b45f0a>)}}
	endif
endscript
...
script ui_destroy_select_song_section 
	...
	setsoundbussparams {user_crowdsingalong = {vol = (($default_BussSet.user_crowdsingalong.vol) - 9)}}
	setsoundbussparams {User_Band = {vol = (($default_BussSet.User_Band.vol) - 9)}}
	setsoundbussparams {crowdsingalong_balance = {vol = (($default_BussSet.crowdsingalong_balance.vol) - 9)}}
...
script practice_songpreview_monitor 
	...
	if NOT ($current_practice_songpreview = $target_practice_songpreview &&
		...
		KillSpawnedScript \{Name = practice_songpreview_fadeinandrepeat}
		if (<playing_song> = 1)
			0x05924e09 0xc7b45f0a = -100 fade = 0.5 extras = <extras>
			Wait \{0.5
				Second}
		endif
		if (0x21ce05e1 = 1)
		...
		if ($shutdown_game_for_signin_change_flag = 0)
			...
			if ($current_practice_songpreview = $target_practice_songpreview &&
				...
				printf qs(0xd7b674ac) d = <adjustedtime>
				if ($0x9ce45dd2 = 1)
					if (<playing_song> = 0)
						begin_song
						<playing_song> = 1
...
script 0x748e04b5 
	0x2fac7537 = 0.0
	0xd37985e8 = (<0x0ffc8edd> - <0xa238a3ea>)
	songsetmastervolume vol = -20 time = <0xc7b45f0a>
	return
	begin
	0xc7b45f0a = ((<0xd37985e8> * <0x2fac7537>) + <0xa238a3ea>)
	if (<0xc7b45f0a> < 0.0)
		if (<0xc7b45f0a> < <0x0ffc8edd>)
			<0xc7b45f0a> = <0x0ffc8edd>
		endif
	else
		if (<0xc7b45f0a> > <0x0ffc8edd>)
			<0xc7b45f0a> = <0x0ffc8edd>
	...
	0x05924e09 0xc7b45f0a = <0xc7b45f0a>
	if (<0x2fac7537> >= <fade_time>)
		break
	endif
	0x2fac7537 = (<0x2fac7537> + 0.06)
	Wait \{0.06
		Seconds}
	repeat
endscript
...
script practice_songpreview_fadeinandrepeat 
	...
	current_wait_time = (<current_wait_time> - ($0x1187513b))
