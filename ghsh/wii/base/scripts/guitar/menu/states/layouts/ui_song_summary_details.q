...
script ui_create_song_summary_details \{for_transition = 0}
	...
	if (<part> = vocals)
		...
		if (<vocal_phrase_max_qual> > 0)
			get_percent_notes_hit player_index = <i>
	...
	if NOT (<vocalist> = 0)
		...
		if (<array_Size> > 0)
			...
			if LocalizedStringEquals a = (<vocal_sections> [<i>]) b = $vocal_marker_freeform
				freeform_rgba = [20 , 20 , 20 , 150]
				<verse_rgba> = <freeform_rgba>
			endif
			if LocalizedStringEquals a = (<vocal_sections> [<i>]) b = $vocal_marker_hype
				hype_rgba = [20 , 20 , 20 , 150]
				<verse_rgba> = <hype_rgba>
			endif
			add_row = 1
...
script my_color_interp \{val = 100
	...
	if (<val> = 100)
		Color = (($g_menu_colors).menu_sober_green)
...
script ui_details_add_compiled_results_to_table 
	...
	if (<array_Size> > 0)
		...
		if StructureContains structure = <temp_struct> player1_cash
			formatText TextName = Foo qs(0xcb2f9062) d = (<temp_struct>.player1_cash)
		...
		if StructureContains structure = <temp_struct> player2_cash
			formatText TextName = Foo qs(0xcb2f9062) d = (<temp_struct>.player2_cash)
		...
		if StructureContains structure = <temp_struct> player3_cash
			formatText TextName = Foo qs(0xcb2f9062) d = (<temp_struct>.player3_cash)
		...
		if StructureContains structure = <temp_struct> player4_cash
			formatText TextName = Foo qs(0xcb2f9062) d = (<temp_struct>.player4_cash)
...
script song_summary_details_add_band_reward 
	if NOT (<Band> = FALSE)
		...
		formatText TextName = reward_name qs(0x73307931) s = <label>
		formatText TextName = cash_value qs(0xcb2f9062) d = <cash>
		CreateScreenElement {
			parent = list_menu
			Type = descinterface
			autosizedims = true
...
script ui_song_details_shift 
	...
	Max = ((<total> - <fit>) + 1)
	dx = (274.0 / <Max>)
	printf qs(0x5985e879) c = <current> d = <dx>
	factor = (<current> * <dx>)
	Pos = (<factor> * (0.0, 1.0))
	Pos = ((480.0, -15.0) + <Pos>)
