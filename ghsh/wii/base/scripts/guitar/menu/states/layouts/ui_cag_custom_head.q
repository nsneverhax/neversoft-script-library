	...
	if getcasappearancepart part = <part>
		...
		if GotParam \{finishable}
			add_generic_menu_text_item {
				text = qs(0xcc83e9d0)
				choose_state = uistate_cag_select_part_inclusion
				choose_state_data = {part = (<finishable>) body_part = <part> text = qs(0x48b206fc) camera_list = ['cag_custom_head' 'cag_custom_head_R' 'cag_custom_head_B' 'cag_custom_head_L'] container_id = <container_id> is_popup hist_tex = icon_cag_head_finishes}
			}
		endif
