...
script create_jam_publish_song_menu \{new_genre = -1}
	...
	if NOT ($0x27707361 = jammode)
		0xc1c4e84c \{mode = jammode
			0xd607e2e6 = 1}
	endif
	NetSessionFunc \{func = stats_init}
	NetSessionFunc \{func = 0xe8b07545}
	...
	if (<playback_track1> >= 0)
		formatText TextName = guitar_playback_text qs(0x55a34b9d) s = ($jam_tracks [<playback_track1>].name_text)
	else
		formatText \{TextName = guitar_playback_text
			qs(0xa532fe7c)}
	endif
	if (<playback_track2> >= 0)
		formatText TextName = bass_playback_text qs(0xaa52ed23) s = ($jam_tracks [<playback_track2>].name_text)
	else
	...
	local_struct = (($editable_jam_album_cover) [0])
	setstructureparam struct_name = local_struct param = layers value = ($default_album_cover [0].layers)
	SetArrayElement ArrayName = editable_jam_album_cover globalarray index = 0 NewValue = <local_struct>
...
script jam_upload_song_choose_script 
	...
	if NOT isacceptablestring string = <newfilename>
		if ((NetSessionFunc func = iscableunplugged) || (<0x4ed04773> = 1))
			destroy_popup_warning_menu
			jam_upload_song_failed \{dialog = 5}
			return
		endif
...
script jam_upload_song_failed \{dialog = 0}
	...
	switch <dialog>
		...
		case 1
		upload_dialog = qs(0x2cd95f66)
		case 2
		formatText TextName = text qs(0x2e070a9c) s = ($ghtunes_minimum_note_count)
		upload_dialog = <text>
		case 3
		minimum_seconds = ($ghtunes_minimum_song_time / 1000)
		CastToInteger \{minimum_seconds}
		formatText TextName = text qs(0x85fdb4df) s = <minimum_seconds>
		upload_dialog = <text>
		case 4
		upload_dialog = qs(0x5f174ec8)
		case 5
		upload_dialog = qs(0x11e85bf4)
		case 6
		upload_dialog = qs(0x4afb3531)
		case 7
		upload_dialog = qs(0x7cc9a727)
		case 8
		if isXenon
			upload_dialog = qs(0x30cd102b)
		elseif isps3
			upload_dialog = qs(0x065896a0)
		endif
		case 9
		if isXenon
			upload_dialog = qs(0x20c7094e)
		elseif isps3
			upload_dialog = qs(0x065896a0)
		endif
		case 10
		...
		case 14
		upload_dialog = qs(0x14b6d9ee)
		case 15
		upload_dialog = qs(0xd9b88597)
		case 16
		upload_dialog = qs(0x11e85bf4)
		case 17
...
script jam_upload_check_dialog \{dialog = 0}
	...
	switch <dialog>
		...
		formatText \{TextName = title
			qs(0xe6cd2b61)}
		formatText TextName = upload_dialog qs(0x8bcd1dfa) s = (<slot> + 1)
		func = jam_upload_song_to_slot
		func_params = {<...> replace}
		case 1
		formatText \{TextName = title
			qs(0x9d4a3853)}
		formatText TextName = upload_dialog qs(0x192cbf4e) s = (<slot> + 1)
