...
script create_jam_ghtunes_menu 
	...
	unload_songqpak
	NetSessionFunc \{func = 0x2b9357ce}
...
script ghtunes_create_alphasearch_menu 
	...
	text_entry_add_item {
		...
		pad_choose_script = ghtunes_string_search
		pad_choose_params = {}
		pad_back_script = ghtunes_submenu_back
...
script ghtunes_string_search 
	...
	formatText TextName = title qs(0x7a4216d6) a = ($ghtunes_leaderboard [7].name_text) b = <text>
...
script ghtunes_signin_check_unspawned 
	...
	if NOT IsLoggedIn
		jam_ghtunes_failed_dialog \{dialog = 10}
		return \{FALSE}
	endif
	return \{true}
endscript
...
script ghtunes_pad_choose 
	...
	if (<event_data>.failed = 0)
		...
		if (<event_data>.failed = 2)
			jam_ghtunes_download_failed_dialog dialog = 7 start_at = ($jam_ghtunes_curr_start_at) board_index = <current_leaderboard_index>
		else
...
script jam_ghtunes_failed_dialog \{dialog = 0}
	...
	switch <dialog>
		...
		dialog_txt = qs(0x11e85bf4)
		case 8
		dialog_txt = qs(0x1f8c9ef7)
		case 9
		dialog_txt = qs(0xb9fd7c2c)
		case 10
		dialog_txt = qs(0xf828602c)
	endswitch
	create_popup_warning_menu {
		title = qs(0x39d32810)
		textblock = {
...
script destroy_jam_ghtunes_menu 
	...
	if ScriptIsRunning \{jam_recording_check_disconnect}
