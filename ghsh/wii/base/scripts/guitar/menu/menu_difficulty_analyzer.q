...
script difficulty_analyzer_show \{song_name = beastandtheharlot}
...
script calc_difficulty_from_all_song_sections \{song_array = None
	...
	printf \{qs(0x5b33168a)}
	if (<output_section_scores> = 1)
		get_song_struct song = <song_checksum>
		get_song_title song = <song_checksum>
		printf {channel = ($diffchannels [<instrument_num>]) qs(0x65eacc04) a = ($difficulty_list_props.<difficulty>.text) b = <song_title> c = (<array_index> + 1) d = ($<marker_array> [<array_index>].marker) e = <diff_score>}
	endif
	printf \{qs(0xc20f15e8)}
	if (<for_graph> = 1)
...
script output_diff_scores 
	...
	if NOT (<song_checksum> = jamsession)
		...
		get_song_struct song = <song_checksum>
		get_song_title song = <song_checksum>
		printf {channel = ($diffchannels [<instrument_num>]) qs(0x8599ca0c) s = ($difficulty_list_props.($difficulty_list [<difficulty>]).text) a = <song_title> X = 0 d = <diff_score>}
		get_bpm_from_song loaded_fretbar = <loaded_fretbar>
		if NOT (<avg> = 0)
			printf {channel = diffanalyzersectiontempo qs(0x6496e5db) d = ($difficulty_list_props.($difficulty_list [<difficulty>]).text) s = <song_title> a = <low> b = <high> c = <avg>}
		endif
	endif
