...
script bossbattle_trigger_on \{Player = 1
	...
	if (($<other_player_status>.Player = 1) && (($<other_player_status>.part = bass) || ($<other_player_status>.part = guitar)))
...
script update_battle_death_meter 
	...
	if ScreenElementExists id = <death_meter_marker>
		...
		if (<meterStep> > 0)
			legacydoscreenelementmorph id = <death_meter_marker> Pos = ((14.5, 100.0) - ((0.0, 1.0) * <pos_update>)) time = <meterStep>
		endif
	else
		break
	endif
