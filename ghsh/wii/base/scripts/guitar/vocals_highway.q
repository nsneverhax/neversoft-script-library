...
vocals_scrolling_fade_end = 350.0
...
script vocals_animate_fists 
	...
	formatText checksumName = fist_tex 'VH_Crowd_Noise_%i' i = <i>
	<objID> :se_setprops texture = <fist_tex>
	Wait ($vocal_hand_anim_frame_delay) gameframes
	repeat
endscript
...
script vocals_create_freeform_section \{Type = melodic}
	...
	if (<Type> != melodic)
		RunScriptOnScreenElement id = <id> vocals_animate_fists
	else
		CreateScreenElement {
			parent = <container_id>
			Type = SpriteElement
...
script vocals_star_power_changed 
	Obj_GetID
	if (<enable> = 1)
		vocals_paint_star_power_on Player = <Player> parent = <objID>
	else
		vocals_paint_star_power_off Player = <Player> parent = <objID>
	endif
endscript
