...
script 0xf3c2fea9 
	RequireParams \{[
		...
		all}
	if ismicrophonepluggedin number = <controller>
		return \{true}
	else
		return \{FALSE}
	endif
...
script vocals_ingame_change_mic_volume 
	...
	printf channel = sfx 'vocals_ingame_change_mic_volume - setting volume %d' d = <Volume>
	ui_options_audio_update_mic_volume vol = <Volume>
	ui_options_audio_get_buss_volume Volume = <Volume>
	SoundBussUnlock \{0xcb23ff19}
	setsoundbussparams {options_vox = {vol = <vol>} time = <time>}
	SoundBussLock \{0xcb23ff19}
	if (<Change> > 0)
		<rgba> = [96 240 96 255]
	else
		<rgba> = [240 96 96 255]
