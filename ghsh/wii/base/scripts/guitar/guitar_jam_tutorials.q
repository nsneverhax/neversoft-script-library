...
script jam_play_tutorial_topic 
	...
	if (<ghmix_tut> = 0)
		if (<index> = 5)
			generic_event_choose \{state = 0x759a65f0
				data = {
					editing = 0
					tutorial = 1
					song = None
	...
	0xc1c4e84c mode = tutorials tutorial = (<tutorial_topic>.lesson_tag)
...
script jam_tutorial_effects_show_menu \{time = 2}
	...
	GetScreenElementChildren \{id = vmenu_options_1}
	GetArraySize <children>
	if (<array_Size> > 0)
		LaunchEvent Type = focus target = (<children> [0])
	endif
...
script jam_tutorial_recording 
	...
	jam_tutorial_show_instrument_ui \{instrument = lead}
	jam_tutorial_add_arrow \{Pos = (338.0, 550.0)
		life = 3
		Scale = 2
		rot = 0}
...
script jam_tutorial_ghtunes 
	Wait \{1
		Second}
	training_show_title \{title = qs(0x122ea38c)}
	Wait \{1
		Second}
	...
	Wait \{1
		Second}
	training_show_title \{title = qs(0x105434a6)}
	Wait \{1
		Second}
	...
	create_jam_song_select_popup \{0xbe7e37bd = 1}
	LaunchEvent \{Type = unfocus
		target = pu_warning_vmenu}
	if GetScreenElementChildren \{id = pu_warning_vmenu}
		SetScreenElementProps id = (<children> [0]) rgba = (($g_menu_colors).menu_title)
	endif
	training_play_sound \{Sound = 'Tut_RS_GHTunes_04_BAS'}
	Wait \{5
		Seconds}
	jam_tutorial_add_arrow \{z = 10000
		Pos = (860.0, 555.0)
		life = 2
		rot = 90}
	Wait \{2
	...
	LaunchEvent \{Type = unfocus
		target = current_menu}
	jam_tutorial_add_arrow \{Pos = (581.0, 228.0)
		life = 2
		rot = 90}
	Wait \{3
		Seconds}
	jam_tutorial_add_arrow \{Pos = (591.0, 276.0)
		life = 2
		rot = 90}
	jam_tutorial_add_arrow \{Pos = (591.0, 330.0)
		life = 2
		rot = 90}
	Wait \{4
		Seconds}
	jam_tutorial_add_arrow \{Pos = (500.0, 430.0)
		life = 2
		rot = 90}
	Wait \{2
		Seconds}
	jam_tutorial_add_arrow \{Pos = (600.0, 530.0)
		life = 2
		rot = 90}
	Wait \{2
		Seconds}
...
script create_faked_jam_publish_song_menu 
	...
	bx_refreshmenuitemsindexes \{vmenu_id = create_publish_song_menu}
...
script jam_tutorial_add_arrow \{Pos = (450.0, 175.0)
	...
	if GotParam \{instrument}
		switch <instrument>
			case rhythm
			<Pos> = (400.0, 220.0) <rot> = 90
			case lead
			<Pos> = (400.0, 260.0) <rot> = 90
			case bass
			<Pos> = (400.0, 300.0) <rot> = 90
			case drum
			case drums
			<Pos> = (400.0, 340.0) <rot> = 90
			case melody
			<Pos> = (400.0, 378.0) <rot> = 90
		endswitch
	endif
	training_add_arrow <...>
endscript
