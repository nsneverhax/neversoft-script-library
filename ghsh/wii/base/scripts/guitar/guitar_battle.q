		...
		if (($<send_to_player>.battle_text_count) > 1)
			...
			if ((($<send_to_player>.battle_text_count) - <count>) > 1)
				...
				legacydoscreenelementmorph {
					...
					printf channel = attacks qs(0x22ee76e7)
					just = [center top]
					time = 0.3
				}
				<count> = (<count> + 1)
	...
	if (($<send_to_player>.battle_text_count) > 0)
		...
		if (<count> < <remove_text>)
			...
			legacydoscreenelementmorph {
				id = <battle_text_to_move>
				Pos = (<screenelementpos> - (0.0, 30.0) + (<x_offset> * (1.0, 0.0)))
				printf channel = attacks qs(0x09c32524)
				just = [center top]
				time = 0.3
			}
			<count> = (<count> + 1)
...
script battle_lightning 
	...
	if ($<other_player_status>.Player = 1)
		...
		spawnscript \{ghm_battlemode_ridethelightning_attack_p1}
		spawnscript gh_battlemode_player1_rtl params = {player_text = <player_text> Player = <Player>} id = battlemode
	else
		Change battle_flicker_difficulty_p2 = <flicker_ammount>
		spawnscript \{ghm_battlemode_ridethelightning_attack_p2}
		spawnscript gh_battlemode_player2_rtl params = {player_text = <player_text> Player = <Player>} id = battlemode
	endif
	GetSongTimeMs
	CastToInteger \{time}
	if ($<other_player_status>.shake_notes > -1)
		...
		repeat
		if ($<other_player_status>.Player = 1)
			ghm_battlemode_ridethelightning_release_p1
			DestroyScreenElement \{id = rtl_p1_lighning}
		else
			ghm_battlemode_ridethelightning_release_p2
...
script battle_lefty_notes 
	...
	if NOT (<time> > $<other_player_status>.lefty_notes)
		...
		if (<time> > (<end_time> - 500))
			animate_lefty_flip other_player_status = <other_player_status> player_text = <player_text> unflip
			Change structurename = <other_player_status> lefthanded_button_ups = (1 + $<other_player_status>.lefthanded_button_ups * -1)
...
script battle_whammy_attack 
	...
	if ($<other_player_status>.Player = 1)
		spawnscript \{ghm_battlemode_trappedunderice_attack_p1}
		spawnscript gh_battlemode_player1_tui_whammy_start params = {player_text = <player_text> Player = <Player>} id = battlemode
	else
		spawnscript \{ghm_battlemode_trappedunderice_attack_p2}
		spawnscript gh_battlemode_player2_tui_whammy_start params = {player_text = <player_text> Player = <Player>} id = battlemode
	endif
	if ($<other_player_status>.whammy_attack < 1)
		...
		GuitarEvent_BattleAttackFinished <...>
		if ($<other_player_status>.Player = 1)
			spawnscript \{ghm_battlemode_trappedunderice_release_p1}
			spawnscript gh_battlemode_player1_tui_whammy_end params = {player_text = <player_text> Player = <Player>} id = battlemode
		else
			spawnscript \{ghm_battlemode_trappedunderice_release_p2}
			spawnscript gh_battlemode_player2_tui_whammy_end params = {player_text = <player_text> Player = <Player>} id = battlemode
		endif
	else
...
script animate_lefty_flip 
	...
	formatText checksumName = name_lip '%s_lip%p' s = (($button_up_models [(<Player> -1)]).<Color>.name_string) p = ($<other_player_status>.text) AddToStringLookup = true
	formatText checksumName = name_mid '%s_mid%p' s = (($button_up_models [(<Player> -1)]).<Color>.name_string) p = ($<other_player_status>.text) AddToStringLookup = true
	formatText checksumName = name_neck '%s_neck%p' s = (($button_up_models [(<Player> -1)]).<Color>.name_string) p = ($<other_player_status>.text) AddToStringLookup = true
	formatText checksumName = name_head '%s_head%p' s = (($button_up_models [(<Player> -1)]).<Color>.name_string) p = ($<other_player_status>.text) AddToStringLookup = true
