...
script training_create_and_hide_headers \{Type = standard
		0xe8465039 = 0}
	training_create_lesson_and_task_headers Type = <Type> 0xe8465039 = <0xe8465039>
	...
	switch <Type>
		...
		if (<0xe8465039> = 1)
			if lessonheader :desc_resolvealias \{Name = alias_tutorial_body_vmenu}
				AssignAlias id = <resolved_id> alias = 0x68ec26e8
				0x68ec26e8 :se_setprops \{dims = (600.0, 245.0)}
			endif
			lesson_header_frame_height = 260
			center_pos = (645.0, 210.0)
		else
			lesson_header_frame_height = 290
			center_pos = (645.0, 220.0)
		endif
		lesson_header_frame_width = 640
		case battle
		CreateScreenElement \{parent = training_header_container
			id = lessonheader
			Type = descinterface
			desc = 'tutorial_header_battle'}
		lesson_header_frame_width = 525
		lesson_header_frame_height = 270
		center_pos = (643.0, 210.0)
		case ghmix
		CreateScreenElement \{parent = training_header_container
			id = lessonheader
			Type = descinterface
...
script tutorial_restart 
	printf \{'tutorial_restart'}
	OnExitRun \{0x8609c665}
...
script tutorial_setup_band \{players = 1}
	...
	if ($current_num_players = 4 || $current_num_players = 8)
		...
		Change structurename = <player_status> highway_texture = highway_axel
		<i> = (<i> + 1)
		repeat 4
	endif
	<i> = 1
...
script training_display_notes_hit \{notes_required = 8}
	if (<notes_hit> < <notes_required>)
		formatText TextName = hit_text qs(0xda06b2ab) h = (<notes_required> - <notes_hit>)
	else
...
script training_generic_lesson_complete 
	printf \{'training_generic_lesson_complete'}
