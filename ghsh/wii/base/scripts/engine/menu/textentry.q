...
script text_entry_add_item {
	...
	if GotParam \{text}
		...
		refresh_text_on_screen char_index_array = ($<index_array>) current_index = 0 caps_lock_state = 0 text_entry = <text_entry>
	endif
...
script 0x06f7c75d 
	printstruct <...>
	printscriptinfo \{'confirm_friend_num'}
	<text_entry> :GetTags
	if NOT 0x33831619 text_entry = <text_entry>
		generic_event_choose no_sound state = uistate_generic_alert_popup data = {pad_choose_script = return_reinit_text_entry pad_choose_params = {text_entry = <text_entry>} is_popup title = qs(0x20e8e33e) text = <validity_message>}
		SoundEvent \{event = Menu_Warning_SFX}
	else
		get_string_from_array array = <char_index_array> index = (<current_index> + 1)
		StringRemoveTrailingWhitespace string = <text>
		printf qs(0x96a21320) d = <new_string>
...
script initiate_text_entry 
	...
	if checksumequals a = <text_case> b = mixed
		<0x84f956aa> = 1
	elseif checksumequals a = <text_case> b = title
		<0x84f956aa> = 1
	endif
	if (<0x84f956aa> = 1)
...
script 0x33831619 
	...
	printf qs(0xb5c7dd9b) d = <str_len>
	printf qs(0x314c5c49) d = <new_string>
	if NOT (<str_len> = 12)
		return \{FALSE}
	else
		return \{true}
...
script kill_carats_in_string 
	RequireParams \{[
		...
		all}
	printf 'Incoming string -%a-' a = <text>
	StringToCharArray string = <text>
	GetArraySize <char_array>
	<src> = 0
	...
	if NOT (<this_element> = qs(0xc08b018f))
		<out_text> = (<out_text> + <this_element>)
	endif
	<src> = (<src> + 1)
	repeat <array_Size>
	printf 'Outgoing string -%a-' a = <out_text>
