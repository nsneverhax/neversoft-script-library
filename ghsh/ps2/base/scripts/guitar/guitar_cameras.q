...
script cameracuts_iterator 
	...
	if NOT (<array_Size> = 0)
		...
		if NOT (<array_Size> = 0)
			...
			if NOT ($game_mode = training || $game_mode = tutorial || $game_mode = practice || $game_mode = p2_faceoff || $game_mode = p2_pro_faceoff || $game_mode = p2_battle || $boss_battle = 1)
				if GetNoteMapping section = cameras note = <note>
					if (($0x8b7966da = 0 || $0x1bbd2941 = 0) && (<note_data>.Scr = CameraCuts_SetArrayPrefix))
...
default_cameras_finalbandoutro = [
	{
		...
		params = {
			...
			Cam1 = {
				LockTo = World
				Pos = (-0.515621, 2.225872, 8.691475)
				Quat = (0.006617, 0.9922649, -0.101027004)
				FOV = 72.0
			}
			Cam2 = {
				LockTo = World
				Pos = (-1.8334188, 4.107831, 17.742096)
				Quat = (0.006607, 0.99214095, -0.101791)
				FOV = 72.0
			}
		}
		DOF = $DOF_Medium01_TOD_Manager
