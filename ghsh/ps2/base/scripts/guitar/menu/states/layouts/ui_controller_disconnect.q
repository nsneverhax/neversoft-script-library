...
script 0xcbdb5862 
	...
	if ($g_in_tutorial = 1)
		...
		<func_params> = {func_params = {0x6d221ff1}}
	endif
	continue_enabled = 1
	...
	if (<array_Size> > 0)
		...
		if ($in_tutorial_mode = 1)
			...
			getplayerinfo \{1
				controller}
			if (<part> = vocals &&
					($current_training_script = training_vocals_tutorial_script ||
						$current_training_script = training_band_tutorial_script))
				has_mic = 0
...
script ui_controller_disconnect_continue 
	...
	if ($g_in_tutorial = 1)
		...
		if GotParam \{0x6d221ff1}
			SpawnScriptLater \{tutorial_restart
				params = {
					0x6d221ff1
				}}
		else
