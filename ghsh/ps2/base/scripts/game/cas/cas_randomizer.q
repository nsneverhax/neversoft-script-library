...
script cas_set_random_appearance 
	...
	if (<is_female> = 1)
		...
		cas_set_random_appearance_part part_struct = {part = cas_male_facial_hair} genre = <genre>
		cas_set_random_color_part part_struct = {part = cas_male_facial_hair} genre = <genre>
	endif
	if GotParam \{cpu_band}
...
script cas_random_should_include_part 
	if GotParam \{category}
		if (<category> = prs)
			if NOT StructureContains structure = ($<part_name> [<i>]) is_prs
				return \{FALSE}
			endif
			if StructureContains structure = ($<part_name> [<i>]) is_sch
				return \{FALSE}
			endif
		elseif (<category> = sch)
			if NOT StructureContains structure = ($<part_name> [<i>]) is_sch
				return \{FALSE}
			endif
			if StructureContains structure = ($<part_name> [<i>]) is_prs
				return \{FALSE}
			endif
		elseif (<category> = gh)
			if StructureContains structure = ($<part_name> [<i>]) is_sch
...
script cas_filter_random_presets_by_category 
	...
	if (<array_Size> > 0)
		...
		if (<entry>.array = cas_preset_guitars)
			if (<category> = prs)
				if NOT StructureContains structure = (<desc_id>) is_prs
					<okay> = 0
				endif
			elseif (<category> = sch)
				if NOT StructureContains structure = (<desc_id>) is_sch
					<okay> = 0
				endif
			else
				if StructureContains structure = (<desc_id>) is_prs
					<okay> = 0
				endif
				if StructureContains structure = (<desc_id>) is_sch
					<okay> = 0
				endif
			endif
		endif
...
script generate_random_instrument 
	...
	cas_stop_loading_anim \{player_index = 0}
endscript
